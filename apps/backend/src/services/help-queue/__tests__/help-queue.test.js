...
describe('HelpQueueService', () => {
    ...
    describe('initialization', () => {
        it('should initialize with database schema', async () => {
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('CREATE TABLE IF NOT EXISTS help_requests'));
        });
    ...
    describe('createRequest', () => {
        it('should create a help request', async () => {
            ...
            const request = await service.createRequest('user-1', codeSnippet, 'Why does this return 1?', 'normal');
        ...
        it('should calculate correct SLA for urgent requests', async () => {
            ...
            const now = new Date();
            const request = await service.createRequest('user-1', codeSnippet, 'Help!', 'urgent');
        ...
        it('should calculate correct SLA for normal requests', async () => {
            ...
            const codeSnippet = { content: 'test', language: 'js' };
            const now = new Date();
            const request = await service.createRequest('user-1', codeSnippet, 'Question', 'normal');
        ...
        it('should set tags on request', async () => {
            mockClient.query.mockResolvedValueOnce({ rows: [] });
            const codeSnippet = { content: 'test', language: 'js' };
            const request = await service.createRequest('user-1', codeSnippet, 'Question', 'normal', ['react', 'hooks']);
    ...
    describe('enrichQuestion', () => {
        it('should enrich question with AI insights', async () => {
            mockClient.query.mockResolvedValueOnce({ rows: [] });
            await service.enrichQuestion('req-1', 'Why does this React hook cause infinite loops? It appears to be a common pattern issue.');
    ...
    describe('assignToExpert', () => {
        ...
        it('should reject assignment if expert at capacity', async () => {
            ...
            await expect(service.assignToExpert('req-1', 'expert-1')).rejects.toThrow('at max capacity');
        });
        it('should handle missing expert', async () => {
            mockClient.query.mockResolvedValueOnce({ rows: [] }); // BEGIN
            mockClient.query.mockResolvedValueOnce({ rows: [] }); // No expert found
            mockClient.query.mockResolvedValueOnce({ rows: [] }); // ROLLBACK
            await expect(service.assignToExpert('req-1', 'unknown-expert')).rejects.toThrow('not found');
        });
    });
    describe('respondToRequest', () => {
        ...
        it('should include code proposal if provided', async () => {
            mockClient.query.mockResolvedValueOnce({ rows: [] });
            await service.respondToRequest('req-1', 'expert-1', 'Here is a fix:', 'const memoized = useCallback(() => {...}, [deps])');
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO help_responses'), expect.arrayContaining(['const memoized = useCallback(() => {...}, [deps])']));
        });
    });
...
//# sourceMappingURL=help-queue.test.js.map