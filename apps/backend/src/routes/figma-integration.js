...
export function initializeFigmaIntegrationRoutes(pool, auditService) {
    const router = Router();
    ...
    router.get('/css-variables', async (req, res) => {
        try {
            ...
            const variables = await figmaService.getCSSVariables(scope);
            res.json({
                success: true,
                scope,
                variables,
    ...
    router.post('/token-mapping', async (req, res) => {
        try {
            ...
            const mapping = await figmaService.mapTokenToCSSVariable(tokenId, tokenName, cssVariableName, cssValue);
            res.json({
                success: true,
                mapping,
            });
    ...
    router.get('/comments/:fileId', async (req, res) => {
        try {
            ...
            const comments = await figmaService.getFigmaComments(fileId, frameId);
            res.json({
                success: true,
                fileId,
                comments,
    ...
    router.post('/sync-from-figma/:fileId', async (req, res) => {
        try {
            const { fileId } = req.params;
            const { frameId } = req.query;
            const count = await figmaService.syncCommentsFromFigma(fileId, frameId);
            res.json({
                success: true,
                synced: count,
            });
