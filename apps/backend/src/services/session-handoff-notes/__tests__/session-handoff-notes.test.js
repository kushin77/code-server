...
describe('SessionHandOffNotesService', () => {
    ...
    it('marks a note posted', async () => {
        ...
        expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE session_handoff_notes SET posted = TRUE'), expect.any(Array));
    });
    it('acknowledges a session', async () => {
        mockClient.query.mockResolvedValueOnce({ rowCount: 1 });
        await service.acknowledgeSession('session-1', 'user-1');
        expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO session_handoff_acknowledgements'), expect.any(Array));
    });
...
//# sourceMappingURL=session-handoff-notes.test.js.map