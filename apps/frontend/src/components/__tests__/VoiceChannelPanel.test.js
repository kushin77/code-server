...
vi.mock('../../utils/voiceChannel', () => ({
    createVoiceSession: (...args) => createVoiceSession(...args),
    fetchVoiceStats: (...args) => fetchVoiceStats(...args),
    fetchWorkspaceVoiceSessions: (...args) => fetchWorkspaceVoiceSessions(...args),
    joinVoiceSession: (...args) => joinVoiceSession(...args),
    leaveVoiceSession: (...args) => leaveVoiceSession(...args),
}));
vi.mock('../../utils/richPresence', () => ({
    fetchTeamRichPresence: (...args) => fetchTeamRichPresence(...args),
    upsertRichPresence: (...args) => upsertRichPresence(...args),
}));
afterEach(() => {
    cleanup();
...
describe('VoiceChannelPanel', () => {
    it('loads voice stats and allows session creation and join', async () => {
        ...
        render(<VoiceChannelPanel workspaceId="portal-main" workspaceLabel="Portal main" teamId="team-main" currentUserId="alice" currentDisplayName="Alice Chen" authToken="token"/>);
        ...
        await waitFor(() => {
            expect(upsertRichPresence).toHaveBeenCalledWith('team-main', 'alice', expect.objectContaining({
                status: 'dnd',
...
//# sourceMappingURL=VoiceChannelPanel.test.js.map