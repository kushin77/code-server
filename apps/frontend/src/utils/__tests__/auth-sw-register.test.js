...
import { registerAuthServiceWorker, sendMessageToSW, getServiceWorkerHealth, forceServiceWorkerUpdate, unregisterAuthServiceWorker, } from '../auth-sw-register';
describe('auth-sw-register', () => {
    let mockNavigatorSW;
    let mockSwRegistration;
    beforeEach(() => {
        // Reset DOM
    ...
    describe('registerAuthServiceWorker', () => {
        ...
        it('should setup message handlers after registration', async () => {
            ...
            expect(mockNavigatorSW.addEventListener).toHaveBeenCalledWith('message', expect.any(Function));
        });
        it('should handle registration failure gracefully', async () => {
            mockNavigatorSW.register = vi.fn(() => Promise.reject(new Error('Registration failed')));
            const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => { });
            await registerAuthServiceWorker();
            expect(consoleSpy).toHaveBeenCalledWith('[auth-sw-register] Failed to register Service Worker:', expect.any(Error));
            const health = getServiceWorkerHealth();
    ...
    describe('sendMessageToSW', () => {
        ...
        it('should warn if SW controller not active', () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith('[auth-sw-register] SW not active, message not sent');
            consoleSpy.mockRestore();
        });
        it('should handle postMessage errors', () => {
            ...
            const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => { });
            sendMessageToSW({ type: 'TEST' });
            expect(consoleSpy).toHaveBeenCalledWith('[auth-sw-register] Failed to send message to SW:', expect.any(Error));
            consoleSpy.mockRestore();
        });
    });
    ...
    describe('forceServiceWorkerUpdate', () => {
        ...
        it('should skip if SW not registered', async () => {
            ...
            mockNavigatorSW.register = vi.fn(() => Promise.reject(new Error('Failed')));
            await registerAuthServiceWorker();
            await forceServiceWorkerUpdate();
    ...
    describe('unregisterAuthServiceWorker', () => {
        ...
        it('should handle unregister failure', async () => {
            mockSwRegistration.unregister = vi.fn(() => Promise.reject(new Error('Unregister failed')));
            const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => { });
            await unregisterAuthServiceWorker();
            expect(consoleSpy).toHaveBeenCalledWith('[auth-sw-register] Failed to unregister SW:', expect.any(Error));
            consoleSpy.mockRestore();
        });
    });
    ...
    describe('initialization', () => {
        ...
        it('should handle errors during initialization gracefully', async () => {
            mockNavigatorSW.register = vi.fn(() => Promise.reject(new Error('Init failed')));
            const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => { });
            await registerAuthServiceWorker();
...
//# sourceMappingURL=auth-sw-register.test.js.map