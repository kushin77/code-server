...
class RelevanceRanker {
     ...
     */
    rankResults(queryVector, queryText, candidates, weights = {}) {
        const mergedWeights = { ...this.defaultWeights, ...weights };
        // Score each candidate
        const scored = candidates.map((candidate) => {
            const semanticScore = SimilarityScorer_1.SimilarityScorer.cosineSimilarity(queryVector, candidate.vector);
            const syntacticScore = SimilarityScorer_1.SimilarityScorer.jaroWinklerSimilarity(queryText, candidate.text.substring(0, Math.min(100, candidate.text.length)));
            const tokenScore = SimilarityScorer_1.SimilarityScorer.tokenOverlapSimilarity(queryText, candidate.text);
            // Calculate composite score
            const relevanceScore = semanticScore * mergedWeights.semanticSimilarity +
     ...
     */
    filterByThreshold(results, minScore = 0.3) {
        return results.filter((result) => result.relevanceScore >= minScore);
    }
    /**
     * Get top N results
     */
    getTopResults(results, topN = 10) {
        return results.slice(0, topN);
    }
    /**
     * Combine multiple ranking strategies (ensemble)
     */
    ensembleRank(queryVector, queryText, candidates, strategies) {
     ...
     */
    boostResults(results, boostRules) {
     ...
     */
    diversifyResults(results, maxResult = 10, similarityThreshold = 0.85) {
        ...
        for (const result of results) {
            ...
            for (const selectedResult of selected) {
                const similarity = SimilarityScorer_1.SimilarityScorer.cosineSimilarity(result.vector, selectedResult.vector);
     ...
     */
    explainScore(result, queryVector, queryText, weights = {}) {
        const mergedWeights = { ...this.defaultWeights, ...weights };
        const semanticScore = SimilarityScorer_1.SimilarityScorer.cosineSimilarity(queryVector, result.vector);
        const syntacticScore = SimilarityScorer_1.SimilarityScorer.jaroWinklerSimilarity(queryText, result.text.substring(0, Math.min(100, result.text.length)));
        const tokenScore = SimilarityScorer_1.SimilarityScorer.tokenOverlapSimilarity(queryText, result.text);
        const components = {
...
//# sourceMappingURL=RelevanceRanker.js.map