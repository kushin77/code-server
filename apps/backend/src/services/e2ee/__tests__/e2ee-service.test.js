...
describe('E2EE Service', () => {
    ...
    describe('Session Key Management', () => {
        it('should create Megolm session key for room', async () => {
            const sessionKey = await e2eeService.createSessionKey('room-123', 'user-alice', 'megolm');
        ...
        it('should increment message index for forward secrecy', async () => {
            const sessionKey = await e2eeService.createSessionKey('room-789', 'user-alice', 'megolm');
            expect(sessionKey.messageIndex).toBe(0);
        });
    });
    describe('Message Encryption', () => {
        ...
        it('should encrypt message', async () => {
            ...
            const encryptedMsg = await e2eeService.encryptMessage('user-alice', 'device-001', 'room-123', content);
    ...
    describe('Device Verification', () => {
        it('should verify device with trusted fingerprint', async () => {
            const trustedFingerprints = new Set(['abc123def456']);
            const verified = await e2eeService.verifyDevice('user-alice', 'device-001', 'abc123def456', trustedFingerprints);
            expect(verified).toBe(true);
        });
        it('should reject untrusted device', async () => {
            const trustedFingerprints = new Set(['abc123def456']);
            const verified = await e2eeService.verifyDevice('user-alice', 'device-002', 'xyz789uvw', trustedFingerprints);
            expect(verified).toBe(false);
        });
        it('should emit device-verified event', async () => {
            const trustedFingerprints = new Set(['trusted-fp']);
            return new Promise((resolve) => {
    ...
    describe('Message Verification', () => {
        ...
        beforeEach(async () => {
            ...
            const trustedFingerprints = new Set(['trusted']);
    ...
    describe('Integration', () => {
        it('should handle complete encryption workflow', async () => {
            ...
            // 3. Verify device
            const verified = await e2eeService.verifyDevice('user-alice', 'device-001', 'trusted-fp', new Set(['trusted-fp']));
            expect(verified).toBe(true);
            // 4. Encrypt message
            const content = {
                type: 'text',
...
//# sourceMappingURL=e2ee-service.test.js.map