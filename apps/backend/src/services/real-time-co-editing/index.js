...
export class RealTimeCoEditingEngine extends EventEmitter {
     ...
     */
    joinSession(docId, clientId, userId, workspaceId, initialStateVector) {
     ...
     */
    sync(docId, clientId, remoteVector) {
     ...
     */
    updatePresence(docId, userId, clientId, position, selection) {
        const presenceMap = this.presence.get(docId);
        if (!presenceMap) {
            return;
        }
     ...
     */
    compactIfNeeded(docId) {
        const ops = this.pendingOps.get(docId) || [];
        if (ops.length >= this.config.compactionThreshold) {
            const snapshot = this.compactionService.compactDocument(docId, ops.map(op => ({
    ...
    detectConflict(op) {
        ...
        const conflicts = pendingOps.filter(pending => pending.clientId !== op.clientId &&
