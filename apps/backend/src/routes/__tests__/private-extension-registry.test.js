...
describe('private extension registry routes', () => {
    ...
    function createService() {
        ...
        fs.writeFileSync(approvedManifestPath, JSON.stringify({
        ...
        fs.writeFileSync(blockedManifestPath, JSON.stringify({
            policy_version: '2026.04.22',
            policy_date: '2026-04-22',
            blocked: [
                {
    ...
    it('audits a blocked validation decision', async () => {
        ...
        expect(mockAuditService.emit.mock.calls.some(([event]) => event && typeof event === 'object' && 'action' in event
            ? event.userId === 'ci-bot' &&
                event.action === 'deny' &&
                event.resourceType === 'config' &&
                event.resource === 'publisher.blocked-helper' &&
                event.fileAction === 'read'
            : false)).toBe(true);
    });
});
//# sourceMappingURL=private-extension-registry.test.js.map