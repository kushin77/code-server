...
describe('Phase 11: Advanced Resilience & HA/DR', () => {
    ...
    describe('CircuitBreaker: Error Handling', () => {
        it('should preserve error details from wrapped function', async () => {
            ...
            await expect(breaker.execute(() => Promise.reject(customError))).rejects.toThrow('Custom error message');
        });
        it('should handle synchronous errors', async () => {
            const breaker = createCircuitBreaker('test', { failureThreshold: 5, resetTimeout: 30000 });
            await expect(breaker.execute(() => { throw new Error('Sync error'); })).rejects.toThrow('Sync error');
        });
    ...
    describe('ChaosEngineer: Test Lifecycle', () => {
        it('should create chaos test with correct configuration', () => {
            ...
            const test = engineer.startChaosTest('Latency Spike', 'latency', ['api-service', 'database'], 60000, 0.8);
        ...
        it('should complete chaos test after duration', () => {
            const engineer = createChaosEngineer();
            const test = engineer.startChaosTest('Quick Test', 'latency', ['service'], 10000, 0.5);
    ...
    describe('ChaosEngineer: Failure Scenarios', () => {
        it('should handle latency scenario', () => {
            const engineer = createChaosEngineer();
            const test = engineer.startChaosTest('Latency Test', 'latency', ['service'], 5000, 0.8);
            expect(test.scenario).toBe('latency');
            expect(test.expectedBehavior).toContain('latency');
        });
        it('should handle failure scenario', () => {
            const engineer = createChaosEngineer();
            const test = engineer.startChaosTest('Failure Test', 'failure', ['service'], 5000, 0.8);
        ...
        it('should handle partial partition scenario', () => {
            const engineer = createChaosEngineer();
            const test = engineer.startChaosTest('Partition Test', 'partial-partition', ['service'], 5000, 0.5);
            expect(test.scenario).toBe('partial-partition');
        });
        it('should handle cascading failure scenario', () => {
            const engineer = createChaosEngineer();
            const test = engineer.startChaosTest('Cascading Test', 'cascading-failure', ['service-1', 'service-2', 'service-3'], 10000, 0.9);
    ...
    describe('ChaosEngineer: Metrics & Recovery', () => {
        it('should track test metrics', () => {
            const engineer = createChaosEngineer();
            const test = engineer.startChaosTest('Metric Test', 'latency', ['service'], 5000, 0.5);
        ...
        it('should track request metrics', () => {
            const engineer = createChaosEngineer();
            const test = engineer.startChaosTest('Request Test', 'latency', ['service'], 5000, 0.5);
        ...
        it('should record recovery time', () => {
            const engineer = createChaosEngineer();
            const test = engineer.startChaosTest('Recovery Test', 'failure', ['service'], 5000, 0.3);
        ...
        it('should calculate data loss', () => {
            const engineer = createChaosEngineer();
            const test = engineer.startChaosTest('Data Loss Test', 'failure', ['database'], 5000, 0.8);
            expect(test.metrics.dataLoss).toBeGreaterThanOrEqual(0);
        });
    });
    describe('ChaosEngineer: Service Registration', () => {
        ...
        it('should handle multiple service registrations', () => {
            ...
            const test = engineer.startChaosTest('Multi-service Test', 'failure', ['service-1', 'service-2', 'service-3'], 5000, 0.5);
            expect(test.targetServices.length).toBe(3);
        });
        it('should handle unregistered services gracefully', () => {
            const engineer = createChaosEngineer();
            const test = engineer.startChaosTest('Unregistered Service Test', 'failure', ['unregistered-service'], 5000, 0.5);
    ...
    describe('ResiliencePhase11Agent: Failover Management', () => {
        it('should create failover manager', () => {
            const agent = createResilienceAgent();
            const manager = agent.createFailoverManager('database', {
                strategy: 'active-passive',
                healthCheckInterval: 5000,
                failureThreshold: 3,
                replicationDelay: 100,
        ...
        it('should track primary replica', () => {
            const agent = createResilienceAgent();
            agent.createFailoverManager('database', {
                strategy: 'active-passive',
                healthCheckInterval: 5000,
                failureThreshold: 1,
                replicationDelay: 100,
    ...
    describe('ResiliencePhase11Agent: Chaos Engineering', () => {
        it('should start chaos test', () => {
            const agent = createResilienceAgent();
            const test = agent.startChaosTest('Latency Injection', 'latency', ['api-service'], 30000, 0.5);
        ...
        it('should validate chaos test safety limits', () => {
            ...
            const test = agent.startChaosTest('Safe Test', 'latency', ['service'], 30000, 0.8 // 80% intensity
            );
    ...
    describe('HA/DR Integration: Complete Resilience Stack', () => {
        it('should orchestrate circuit breaker + failover + chaos', async () => {
            ...
            // Setup failover
            agent.createFailoverManager('api-service', {
                strategy: 'active-passive',
                healthCheckInterval: 5000,
                failureThreshold: 2,
                replicationDelay: 100,
            ...
            // Start chaos test
            const test = agent.startChaosTest('Full Stack Test', 'failure', ['api-service'], 10000, 0.3);
            expect(test).toBeDefined();
            const status = agent.getResilienceStatus();
            expect(status.systemHealthScore).toBeGreaterThanOrEqual(0);
        });
        ...
        it('should recover from cascading failures', async () => {
            ...
            agent.startChaosTest('Cascading Failure', 'cascading-failure', services, 10000, 0.6);
            jest.advanceTimersByTime(11000);
            const status = agent.getResilienceStatus();
    ...
    describe('HA/DR Integration: Disaster Recovery', () => {
        ...
        it('should enable cross-region failover', () => {
            ...
            agent.createFailoverManager('primary-db', {
                strategy: 'active-passive',
                healthCheckInterval: 5000,
                failureThreshold: 2,
                replicationDelay: 100,
    ...
    describe('HA/DR Integration: Performance Under Failure', () => {
        it('should maintain <100ms latency during failover', async () => {
            const agent = createResilienceAgent();
            agent.createFailoverManager('api', {
                strategy: 'active-passive',
                healthCheckInterval: 5000,
                failureThreshold: 1,
                replicationDelay: 100,
        ...
        it('should handle traffic shift during failover', () => {
            const agent = createResilienceAgent();
            agent.createFailoverManager('load-balanced-service', {
                strategy: 'active-passive',
                healthCheckInterval: 5000,
                failureThreshold: 2,
                replicationDelay: 100,
    ...
    describe('Performance: Scalability', () => {
        ...
        it('should manage 20 failover managers', () => {
            const agent = createResilienceAgent();
            for (let i = 0; i < 20; i++) {
                agent.createFailoverManager(`service-${i}`, {
                    strategy: 'active-passive',
                    healthCheckInterval: 5000,
                    failureThreshold: 3,
                    replicationDelay: 100,
        ...
        it('should run 10 concurrent chaos tests', () => {
            const agent = createResilienceAgent();
            for (let i = 0; i < 10; i++) {
                agent.startChaosTest(`Chaos Test ${i}`, 'latency', [`service-${i}`], 10000, 0.5);
            }
            const status = agent.getResilienceStatus();
            expect(status.chaosTestsRunning).toBeGreaterThanOrEqual(0);
        });
...
function createCircuitBreaker(name, config) {
    return {
        ...
        execute: jest.fn(async function (fn) {
...
function createFailoverManager(serviceName, config, primaryReplica) {
    return {
        serviceName,
        config,
        primaryReplica,
        replicas: new Map(),
        failoverHistory: [],
        registerReplica: jest.fn(function (replicaId, isHealthy = true) {
            this.replicas.set(replicaId, { replicaId, isHealthy, lastHeartbeat: Date.now(), consecutiveFailures: 0, latency: 0, capacity: 100 });
        }),
        updateReplicaHealth: jest.fn(function (replicaId, isHealthy, latency, capacity) {
        ...
        manualFailover: jest.fn(function (targetReplicaId, reason) {
            const oldPrimary = this.primaryReplica;
            this.primaryReplica = targetReplicaId;
            this.failoverHistory.push({ timestamp: Date.now(), trigger: 'manual', fromReplica: oldPrimary, toReplica: targetReplicaId, reason });
            return true;
        }),
...
function createChaosEngineer() {
    return {
        ...
        registerService: jest.fn(function (serviceName, simulator) {
            this.failureSimulators.set(serviceName, simulator);
        }),
        startChaosTest: jest.fn(function (name, scenario, targetServices, duration, intensity) {
            const test = {
                id: `chaos-${Date.now()}`,
                name,
                scenario,
        ...
        completeChaosTest: jest.fn(function (testId) {
...
function createResilienceAgent() {
    return {
        ...
        createCircuitBreaker: jest.fn(function (config) {
            const breaker = createCircuitBreaker(config.name, config);
            this.circuitBreakers.set(config.name, breaker);
            return breaker;
        }),
        executeProtected: jest.fn(async function (serviceName, fn) {
...
//# sourceMappingURL=phase11.test.js.map