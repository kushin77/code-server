 ...
 * Get all replica IDs
 */
export function getReplicaIds() {
    return FEDERATION_CONFIG.regions.map(region => region.tags.replica_id);
}
/**
 * Get region by replica ID
 */
export function getRegionByReplicaId(replicaId) {
    return FEDERATION_CONFIG.regions.find(r => r.tags.replica_id === replicaId);
}
/**
 * Calculate inter-region latency estimates (ms)
 * Based on geographic distance and typical network latency
 */
export function getEstimatedLatency(fromRegion, toRegion) {
    const latencies = {
        'us-west': {
            'us-west': 1,
            'eu-west': 150,
...
export function validateFederationConfig() {
    ...
    if (FEDERATION_CONFIG.regions.length < 3) {
        errors.push(`Minimum 3 regions required, found ${FEDERATION_CONFIG.regions.length}`);
    }
    ...
    const replicaIds = FEDERATION_CONFIG.regions.map(r => r.tags.replica_id);
    ...
    for (let i = 0; i < cidrs.length; i++) {
        for (let j = i + 1; j < cidrs.length; j++) {
            ...
            if (cidrs[i].split('.')[1] === cidrs[j].split('.')[1]) {
                errors.push(`CIDR overlap detected: ${cidrs[i]} and ${cidrs[j]}`);
            }
        }
    }
...
//# sourceMappingURL=FederationConfig.js.map