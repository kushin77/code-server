...
describe('RecordingService', () => {
    ...
    it('should start recording', async () => {
        const rec = await service.startRecording('user1', 'user1@example.com', 'workspace1', 'session1');
    ...
    it('should emit recording-started event', () => {
        return new Promise((resolve) => {
            ...
            service.startRecording('user1', 'user1@example.com', 'workspace1', 'session1');
        });
    });
    it('should assign unique recording IDs', async () => {
        const rec1 = await service.startRecording('user1', 'user1@example.com', 'workspace1', 'session1');
        await new Promise((resolve) => setTimeout(resolve, 1));
        const rec2 = await service.startRecording('user1', 'user1@example.com', 'workspace1', 'session2');
        expect(rec1.id).not.toBe(rec2.id);
    });
    // ==================== Stop Recording Tests ====================
    it('should stop recording', async () => {
        const rec = await service.startRecording('user1', 'user1@example.com', 'workspace1', 'session1');
        const stopped = await service.stopRecording('user1', 'user1@example.com', rec.id);
    ...
    it('should emit recording-stopped event', () => {
        return new Promise((resolve) => {
            service
                .startRecording('user1', 'user1@example.com', 'workspace1', 'session1')
                .then((rec) => {
    ...
    it('should record event', async () => {
        const rec = await service.startRecording('user1', 'user1@example.com', 'workspace1', 'session1');
        const event = await service.recordEvent(rec.id, 'file-change', 'user1', 'workspace1', 'session1', { path: 'src/index.ts', added: 10, removed: 5 });
    ...
    it('should emit event-recorded event', () => {
        return new Promise((resolve) => {
            service
                ...
                service.recordEvent(rec.id, 'file-change', 'user1', 'workspace1', 'session1', {});
            });
        });
    });
    it('should increment event count', async () => {
        const rec = await service.startRecording('user1', 'user1@example.com', 'workspace1', 'session1');
        for (let i = 0; i < 5; i++) {
            await service.recordEvent(rec.id, 'file-change', 'user1', 'workspace1', 'session1', {});
        }
    ...
    it('should track file changes', async () => {
        const rec = await service.startRecording('user1', 'user1@example.com', 'workspace1', 'session1');
        const eventTypes = ['file-change', 'file-create', 'file-delete'];
        for (const eventType of eventTypes) {
            await service.recordEvent(rec.id, eventType, 'user1', 'workspace1', 'session1', {});
        }
        const updated = await service.getRecording(rec.id);
    ...
    it('should track terminal output', async () => {
        const rec = await service.startRecording('user1', 'user1@example.com', 'workspace1', 'session1');
        const eventTypes = ['terminal-output', 'terminal-input'];
        for (const eventType of eventTypes) {
            await service.recordEvent(rec.id, eventType, 'user1', 'workspace1', 'session1', {});
        }
        const updated = await service.getRecording(rec.id);
    ...
    it('should track chat messages', async () => {
        const rec = await service.startRecording('user1', 'user1@example.com', 'workspace1', 'session1');
        for (let i = 0; i < 3; i++) {
            await service.recordEvent(rec.id, 'chat-message', 'user1', 'workspace1', 'session1', { message: 'test' });
        }
        const updated = await service.getRecording(rec.id);
    ...
    it('should track debug events', async () => {
        const rec = await service.startRecording('user1', 'user1@example.com', 'workspace1', 'session1');
        const eventTypes = ['debug-breakpoint', 'debug-step'];
        for (const eventType of eventTypes) {
            await service.recordEvent(rec.id, eventType, 'user1', 'workspace1', 'session1', {});
        }
        const updated = await service.getRecording(rec.id);
    ...
    it('should seek to event', async () => {
        const rec = await service.startRecording('user1', 'user1@example.com', 'workspace1', 'session1');
        for (let i = 0; i < 5; i++) {
            await service.recordEvent(rec.id, 'file-change', 'user1', 'workspace1', 'session1', { index: i });
        }
        await service.startPlayback({
            recordingId: rec.id,
            userId: 'user1',
    ...
    it('should create shareable link', async () => {
        const rec = await service.startRecording('user1', 'user1@example.com', 'workspace1', 'session1');
        const link = await service.createShareableLink('user1', 'user1@example.com', rec.id);
    ...
    it('should list recordings for user', async () => {
        for (let i = 0; i < 3; i++) {
            await service.startRecording('user1', 'user1@example.com', 'workspace1', `session${i}`);
        }
        const list = await service.listRecordings('user1');
        expect(list.length).toBe(3);
    });
    it('should query recordings', async () => {
        await service.startRecording('user1', 'user1@example.com', 'workspace1', 'session1');
        const result = await service.queryRecordings({ userId: 'user1' });
        expect(result.total).toBeGreaterThan(0);
    });
    it('should filter by workspace', async () => {
        for (let i = 0; i < 2; i++) {
            await service.startRecording('user1', 'user1@example.com', `workspace${i}`, 'session1');
        }
    ...
    it('should log audit entry for recording start', async () => {
        await service.startRecording('user1', 'user1@example.com', 'workspace1', 'session1', '192.168.1.1', 'Mozilla/5.0');
    ...
    it('should track recordings by user', async () => {
        for (let i = 1; i <= 2; i++) {
            await service.startRecording(`user${i}`, `user${i}@example.com`, 'workspace1', 'session1');
        }
        const stats = await service.getStatistics();
    ...
    it('should throw error if not initialized', async () => {
        const newService = new RecordingService();
        await expect(newService.startRecording('user1', 'user1@example.com', 'ws1', 'session1')).rejects.toThrow();
    });
...
//# sourceMappingURL=recording-service.test.js.map