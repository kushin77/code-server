...
Options:
  --action ACTION         drift-report|snapshot-restore-test
...
for p in /home/coder/workspace /home/coder/.local/share/code-server/User /home/coder/.local/share/code-server/extensions; do
  if [ -e "\$p" ]; then
    ...
    sig=$(find "\$p" -type f -printf "%P|%s|%T@\\n" 2>/dev/null | sort | sha256sum | awk "{print \$1}")
...
run_snapshot_restore_test() {
  ...
  run_primary_cmd "set -euo pipefail;
if ! docker inspect code-server >/dev/null 2>&1; then
...
docker exec code-server sh -lc '
set -eu
archive=/tmp/code-server-tierA-${ts}.tgz
tar -czf "\$archive" -C /home/coder workspace .local/share/code-server/User .local/share/code-server/extensions
';
...
main() {
  ...
  case "$ACTION" in
    drift-report|snapshot-restore-test)
      ;;
    *)
      log_fatal "Invalid --action '${ACTION}'. Allowed: drift-report|snapshot-restore-test"
      ;;
  esac
