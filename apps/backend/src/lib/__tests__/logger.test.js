...
describe('StructuredLogger', () => {
    ...
    describe('PII scrubbing', () => {
        it('redacts password fields', () => {
            const entries = captureStdout(() => logger.info('login', { password: 'super-secret' }));
            expect(entries[0].password).toBe('[REDACTED]');
        });
        it('redacts token fields', () => {
            const entries = captureStdout(() => logger.info('request', { token: 'abc123' }));
            expect(entries[0].token).toBe('[REDACTED]');
        });
        it('redacts nested secret fields', () => {
            const entries = captureStdout(() => logger.info('config', { db: { password: 'db-pass' } }));
            expect(entries[0].db?.password).toBe('[REDACTED]');
        });
        it('redacts bearer tokens in string values', () => {
            const entries = captureStdout(() => logger.info('auth', { header: 'Bearer eyJhbGc.eyJzdWI.SflKx' }));
            expect(entries[0].header).toBe('Bearer [REDACTED]');
        });
        it('redacts email addresses', () => {
            const entries = captureStdout(() => logger.info('user', { contact: 'user@example.com' }));
            expect(entries[0].contact).toBe('[EMAIL-REDACTED]');
        });
        it('preserves non-PII fields', () => {
            const entries = captureStdout(() => logger.info('request', { method: 'GET', statusCode: 200 }));
            expect(entries[0].method).toBe('GET');
            expect(entries[0].statusCode).toBe(200);
        });
        it('does not modify message text (message is not a field name)', () => {
            const entries = captureStdout(() => logger.info('token validated', {}));
    ...
    describe('error logging', () => {
        it('includes error name and message', () => {
            ...
            const errorField = entries[0].error;
    ...
    describe('request logging', () => {
        it('logs HTTP request with method, path, status', () => {
            const entries = captureStdout(() => logger.request('GET', '/api/health', 200, 42));
            const http = entries[0].http;
        ...
        it('logs 4xx at warn level', () => {
            const entries = captureStderr(() => logger.request('GET', '/api/missing', 404, 10));
            expect(entries[0].level).toBe('warn');
        });
        it('logs 5xx at error level', () => {
            const entries = captureStderr(() => logger.request('POST', '/api/fail', 500, 100));
            expect(entries[0].level).toBe('error');
        });
    });
});
