...
export var ChaosExperimentType;
(function (ChaosExperimentType) {
    ChaosExperimentType["REGION_FAILURE"] = "REGION_FAILURE";
    ChaosExperimentType["NETWORK_LATENCY"] = "NETWORK_LATENCY";
    ChaosExperimentType["PACKET_LOSS"] = "PACKET_LOSS";
    ChaosExperimentType["CONNECTION_POOL_EXHAUSTION"] = "CONNECTION_POOL_EXHAUSTION";
    ChaosExperimentType["SLOW_RESPONSE"] = "SLOW_RESPONSE";
    ChaosExperimentType["CASCADING_FAILURE"] = "CASCADING_FAILURE";
})(ChaosExperimentType || (ChaosExperimentType = {}));
export class ChaosTestEngine extends EventEmitter {
        ...
        try {
            ...
            switch (experiment.type) {
                ...
                case ChaosExperimentType.CONNECTION_POOL_EXHAUSTION:
                    result = await this.injectConnectionPoolExhaustion(experiment, startTime);
                    break;
     ...
     */
    async injectRegionFailure(experiment, startTime) {
        ...
        const findings = [];
        if (failoverDetected) {
            findings.push(`Failover detected in ${failoverLatency}ms (within acceptable SLA)`);
        }
        else {
            findings.push('WARNING: Failover not detected - requests may have been lost');
        }
        ...
        return {
            ...
            systemBehavior: {
                ...
                requestsSucceeded,
                requestsFailed,
                successRate: (requestsSucceeded / (requestsSucceeded + requestsFailed)) * 100,
            },
            recoveryMetrics: {
     ...
     */
    async injectNetworkLatency(experiment, startTime) {
        ...
        for (let i = 0; i < requestCount; i++) {
            const requestStartTime = Date.now();
            // Simulate latency
            const simulatedLatency = 50 + additionalLatency + Math.random() * 50;
            try {
        ...
        const findings = [];
        findings.push(`Average latency increased by ${Math.round(additionalLatency)}ms`);
        if (avgLatency > 200) {
            findings.push(`WARNING: Average latency ${Math.round(avgLatency)}ms exceeds SLA`);
        }
        const endTime = new Date();
        return {
            experimentId: experiment.id,
     ...
     * Inject packet loss
     */
    async injectPacketLoss(experiment, startTime) {
        ...
        const findings = [];
        findings.push(`Simulated ${Math.round(packetLossRate)}% packet loss rate`);
        if (packetLossRate > 10) {
            findings.push('ERROR: Packet loss rate exceeds acceptable threshold (>10%)');
        }
        const endTime = new Date();
        return {
            experimentId: experiment.id,
     ...
     */
    async injectConnectionPoolExhaustion(experiment, startTime) {
     ...
     */
    async injectSlowResponse(experiment, startTime) {
     ...
     */
    async injectCascadingFailure(experiment, startTime) {
     ...
     */
    async runExperimentSuite(experiments) {
        const results = [];
...
//# sourceMappingURL=ChaosTestEngine.js.map