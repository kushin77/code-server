 ...
 */
export const DEFAULT_SLO_CONFIG = {
    sloTargetMs: 100,
...
export class SLOTrackingEngine {
     ...
     */
    calculateMetrics(sinceMs) {
        const cutoffTime = sinceMs || Date.now() - this.config.retentionMs;
        const relevantMetrics = Array.from(this.metrics.values()).filter((m) => m.endTime >= cutoffTime);
    ...
    getWindowMetrics(startMs, endMs) {
        const windowMetrics = Array.from(this.metrics.values()).filter((m) => m.endTime >= startMs && m.endTime <= endMs);
        const windowBreaches = this.breaches.filter((b) => b.timestamp >= startMs && b.timestamp <= endMs);
     ...
     */
    getRecentBreaches(limit = 20, sinceMs) {
     ...
     */
    getLastMinutesMetrics(minutes = 60) {
        const now = Date.now();
    ...
    updateSessionStats(sessionId, sloMet, latencyMs) {
    ...
    startCleanupInterval() {
        ...
        this.cleanupInterval = setInterval(() => {
            ...
            const activeSessions = new Set(Array.from(this.metrics.values()).map((m) => m.sessionId));
