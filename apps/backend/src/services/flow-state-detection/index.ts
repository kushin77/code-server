...
export type FlowState = 'flow' | 'distracted' | 'available';
export type PingPriority = 'low' | 'normal' | 'high';
...
export class FlowStateDetectionService extends EventEmitter {
  private logger = getLogger('FlowStateDetectionService');
  private pool: Pool;
  ...
  private determineState(wordsPerMinute: number, switchCount: number, durationMinutes: number): { state: FlowState; confidence: number; reason: string } {
  ...
  ): Promise<FlowActivityRecord> {
    const client = await this.pool.connect();
    try {
      await client.query(
        `INSERT INTO flow_activity_log (user_id, words_per_minute, switch_count, duration_minutes, app_name) VALUES ($1, $2, $3, $4, $5)`,
        [userId, wordsPerMinute, switchCount, durationMinutes, appName]
      );
      ...
      const record: FlowActivityRecord = {
        userId,
        wordsPerMinute,
        switchCount,
        durationMinutes,
  ...
  async detectFlowState(userId: string): Promise<FlowStateSnapshot> {
    const client = await this.pool.connect();
    try {
      ...
      const detected: FlowStateSnapshot = {
  ...
  async queuePing(userId: string, message: string, priority: PingPriority = 'normal'): Promise<FlowPing> {
    const client = await this.pool.connect();
    try {
      await client.query(
        `INSERT INTO flow_ping_queue (user_id, message, priority) VALUES ($1, $2, $3)`,
  ...
  async getQueuedPings(userId: string): Promise<FlowPing[]> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `SELECT user_id, message, priority, queued_at, delivered_at FROM flow_ping_queue WHERE user_id = $1 AND delivered_at IS NULL ORDER BY queued_at ASC`,
        [userId]
      );
  ...
  async completeFlowSession(userId: string): Promise<{ delivered: number; state: FlowState }> {
    const client = await this.pool.connect();
    try {
      const queued = await client.query(
        `SELECT id FROM flow_ping_queue WHERE user_id = $1 AND delivered_at IS NULL`,
        [userId]
      );

      await client.query(
        `UPDATE flow_ping_queue SET delivered_at = CURRENT_TIMESTAMP WHERE user_id = $1 AND delivered_at IS NULL`,
        [userId]
      );
  ...
  async getFlowState(userId: string): Promise<FlowStateSnapshot | null> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `SELECT user_id, state, confidence, reason, updated_at FROM flow_state_snapshots WHERE user_id = $1`,
        [userId]
      );
  ...
  async getFlowHistory(userId: string, limit: number = 20): Promise<FlowActivityRecord[]> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `
...
export async function initializeFlowStateDetectionRoutes(service: FlowStateDetectionService) {
