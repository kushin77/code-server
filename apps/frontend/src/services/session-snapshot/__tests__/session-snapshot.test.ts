 ...
 */
class MockSessionSnapshotStorage implements Partial<SessionSnapshotStorage> {
  ...
  async listSnapshots(
    ...
    const snapshots: SnapshotMetadata[] = paged.map((snap) => ({
      id: snap.id,
  ...
  async getStats() {
    ...
    return {
      ...
      workspaceStats: Array.from(workspaceMap.entries()).map(
...
class MockSessionSnapshotService extends SessionSnapshotService {
  constructor(private mockStorage: MockSessionSnapshotStorage) {
    super();
  }
  ...
  async initialize(workspaceId: string): Promise<void> {
    ...
    (this as any).currentWorkspaceId = workspaceId;
    (this as any).isInitialized = true;
  }
}

describe('Session Snapshot Service', () => {
  let service: MockSessionSnapshotService;
  let storage: MockSessionSnapshotStorage;
  ...
  describe('Snapshot Capture', () => {
    ...
    it('should track capture metrics', async () => {
      ...
      expect(metrics!.captureEndMs).toBeGreaterThanOrEqual(
        metrics!.captureStartMs
      );
  ...
  describe('Integration Tests', () => {
    ...
    it('should support concurrent operations', async () => {
      const promises = Array.from({ length: 5 }, (_, i) =>
        service.captureSnapshot(`Concurrent ${i}`)
      );
