    ...
    return async (req, res, next) => {
        try {
            ...
            if (!userRoles || userRoles.length === 0) {
                logger.warn(`User ${req.user.email} has no assigned roles`, { userId: req.user.sub });
                audit?.emit({
                    userId: req.user.sub,
            ...
            if (!hasRequiredRole) {
                logger.warn(`User ${req.user.email} unauthorized for roles ${allowedRoles.join(', ')}`, {
                    userId: req.user.sub,
                    userRoles,
            ...
            logger.debug(`User ${req.user.email} authorized with roles: ${userRoles.join(', ')}`, { userId: req.user.sub });
            audit?.emit({
                userId: req.user.sub,
                userEmail: req.user.email,
                method: req.method,
 ...
 *   }
 */
export function RequireRoleDecorator(...roles) {
    return function (target, propertyKey, descriptor) {
        ...
        descriptor.value = async function (req, res, next) {
            try {
                ...
                if (!hasRequiredRole) {
                    logger.warn(`User ${req.user.email} unauthorized for decorator roles ${roles.join(', ')}`);
                    return res.status(403).json({
                        error: 'Forbidden',
