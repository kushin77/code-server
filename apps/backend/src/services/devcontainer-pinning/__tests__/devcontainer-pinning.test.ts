...
import {
  DevcontainerPinningService,
  getDevcontainerPinningService,
} from '../pinning-service.js';
import {
  DevcontainerProvisioningService,
  getDevcontainerProvisioningService,
 ...
 */
const MOCK_DEVCONTAINER: PinnedDevcontainer = {
...
const MOCK_PINNED_DEVCONTAINER: PinnedDevcontainer = {
  ...MOCK_DEVCONTAINER,
...
describe('Devcontainer Hash Pinning', () => {
  let pinningService: DevcontainerPinningService;
  let provisioningService: DevcontainerProvisioningService;
  ...
  describe('Pinning Service', () => {
    ...
    it('should scan devcontainer.json', async () => {
      const result = await pinningService.scanDevcontainer(
        'ws-test',
    ...
    it('should detect unpinned elements', async () => {
      ...
      expect(result.unpinnedElements.features).toContain(
        'ghcr.io/devcontainers/features/git'
      );
    });

    it('should pin hashes in devcontainer.json', async () => {
      const pinned = await pinningService.pinHashes(
        'ws-test',
        MOCK_DEVCONTAINER
      );
    ...
    it('should verify reproducibility', async () => {
      const verification = await pinningService.verifyReproducibility(
        MOCK_PINNED_DEVCONTAINER,
        {
          image: MOCK_PINNED_DEVCONTAINER.imageDigest!,
          'feature-ghcr.io/devcontainers/features/git':
            MOCK_PINNED_DEVCONTAINER.featureHashes![
              'ghcr.io/devcontainers/features/git'
            ].hash,
    ...
    it('should calculate pinning statistics', async () => {
      // Scan first
      await pinningService.scanDevcontainer(
        'ws-test-stats',
        '.devcontainer/devcontainer.json',
        MOCK_DEVCONTAINER
      );
  ...
  describe('Provisioning Service', () => {
    ...
    it('should handle provisioning failures', async () => {
      ...
      const invalidConfig: PinnedDevcontainer = {}; // Invalid: no image
    ...
    it('should get provisioning result', async () => {
      ...
      const provResult = await provisioningService.provision(
        request,
        MOCK_PINNED_DEVCONTAINER
      );
  ...
  describe('Integration', () => {
    it('should support full workflow: scan → pin → provision → verify', async () => {
      ...
      // Scan
      const scanResult = await pinningService.scanDevcontainer(
        workspaceId,
        '.devcontainer/devcontainer.json',
        MOCK_DEVCONTAINER
      );
      ...
      // Pin
      const pinnedConfig = await pinningService.pinHashes(
        workspaceId,
        MOCK_DEVCONTAINER
      );
    ...
    it('should emit events', async () => {
      return new Promise<void>(async (resolve) => {
        ...
        await pinningService.pinHashes(
          'ws-emit-test',
          MOCK_DEVCONTAINER
        );
      });
