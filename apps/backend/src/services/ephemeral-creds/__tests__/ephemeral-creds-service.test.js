...
describe('Ephemeral Credentials Service', () => {
    ...
    describe('Credential Requests', () => {
        it('should request database credential', async () => {
            const request = await service.requestCredential('session-123', 'user-alice', 'database', 'app_db', 3600);
    ...
    describe('Credential Fulfillment', () => {
        ...
        it('should emit credential-generated event', async () => {
            return new Promise((resolve) => {
                ...
                const req = service.requestCredential('session-999', 'user-dave', 'database', 'db_test', 3600);
    ...
    describe('Session Credential Revocation', () => {
        beforeEach(async () => {
            const req1 = await service.requestCredential('session-multi', 'user-alice', 'database', 'db1', 3600);
            const req2 = await service.requestCredential('session-multi', 'user-alice', 'cloud', 'gcp', 3600);
    ...
    describe('Statistics', () => {
        beforeEach(async () => {
            for (let i = 0; i < 3; i++) {
                const req = await service.requestCredential('session-stats', 'user-alice', 'database', `db${i}`, 3600);
    ...
    describe('Deny Request', () => {
        it('should deny credential request', async () => {
            const req = await service.requestCredential('session-deny', 'user-alice', 'database', 'db', 3600);
        ...
        it('should emit request-denied event', async () => {
            const req = await service.requestCredential('session-deny2', 'user-bob', 'cloud', 'aws', 3600);
            return new Promise((resolve) => {
    ...
    describe('Integration', () => {
        it('should handle complete credential workflow', async () => {
            ...
            const request = await service.requestCredential('session-workflow', 'user-alice', 'database', 'app_db', 3600);
            expect(request.status).toBe('pending');
            // 2. Fulfill request
            const credential = await service.fulfillCredential(request.id);
            expect(credential.status).toBe('ready');
        ...
        it('should track session bundle', async () => {
            const svc = new EphemeralCredsService();
            await svc.initialize();
            const req = await svc.requestCredential('session-bundle', 'user-alice', 'database', 'db', 3600);
...
//# sourceMappingURL=ephemeral-creds-service.test.js.map