...
import { ProjectType, WorkspaceProfile } from "./detection-engine"
...
export interface PromptResult {
  ...
  customizationApplied?: {
    extensionsToInstall?: string[] // Subset if user deselected some
    skipSettings?: boolean
    skipTasks?: boolean
    skipDebugConfigs?: boolean
  }
}
...
export class UserPromptService extends EventEmitter {
  ...
  private detectedType: ProjectType
   ...
   */
  generateWalkthroughMetadata(): Record<string, any> {
   ...
   */
  private generateProfileDescription(): string {
...
`
   ...
   */
  private generateExtensionsDescription(): string {
  ...
  generateApprovalPrompt(): {
    ...
    buttons: Array<{
      label: string
      action: "approve" | "decline" | "customize"
    }>
  } {
    const typeLabel = this.getProjectTypeLabel(this.detectedType)
    const confidenceLabel = this.getConfidenceLabel(this.confidence)
...
Your existing settings will be preserved.`,
   ...
   */
  processUserResponse(action: "approve" | "decline" | "customize", customizations?: Record<string, boolean>): PromptResult {
    if (action === "decline") {
      return {
        approved: false,
