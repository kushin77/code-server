...
const SLO_ID_NAMESPACE = '8d3d349b-1d7d-4dbf-8f93-3f4a8d26b4a1';

const buildSloId = (metricType: SLOMetric['metricType']): string =>
  uuidv5(metricType, SLO_ID_NAMESPACE);

type IncidentCorrelationLogPayload = Record<string, unknown> | Error | undefined;

class IncidentCorrelationLogger {
  ...
  info(payloadOrMessage?: IncidentCorrelationLogPayload | string, message?: string): void {
    this.log('info', payloadOrMessage, message);
  }

  warn(payloadOrMessage?: IncidentCorrelationLogPayload | string, message?: string): void {
    this.log('warn', payloadOrMessage, message);
  }

  error(payloadOrMessage?: IncidentCorrelationLogPayload | string, message?: string): void {
    this.log('error', payloadOrMessage, message);
  }

  debug(payloadOrMessage?: IncidentCorrelationLogPayload | string, message?: string): void {
    this.log('debug', payloadOrMessage, message);
  }

  private log(
    level: 'info' | 'warn' | 'error' | 'debug',
    payloadOrMessage?: IncidentCorrelationLogPayload | string,
    message?: string
  ): void {
    if (typeof payloadOrMessage === 'string' || payloadOrMessage === undefined) {
      this.logger.log({ level, message: payloadOrMessage ?? message ?? '' });
      return;
    }
    ...
    if (payloadOrMessage instanceof Error) {
      this.logger.log({
        level,
        message: message ?? payloadOrMessage.message,
        error: {
          message: payloadOrMessage.message,
          name: payloadOrMessage.name,
          stack: payloadOrMessage.stack,
        },
      });
      return;
    }
...
export class IncidentCorrelationEngine {
  private logger: IncidentCorrelationLogger;
  private pool: Pool;
  ...
  constructor(
    dbPool: Pool,
    prometheusUrl: string = process.env.PROMETHEUS_URL || 'http://localhost:9090',
    ...
    this.logger = new IncidentCorrelationLogger();
    this.pool = dbPool;
  ...
  private async queryPrometheusInstantValue(query: string): Promise<number | null> {
    try {
      const response = await axios.get<PrometheusQueryResult>(`${this.prometheusUrl}/api/v1/query`, {
        params: { query },
      });
      ...
      this.logger.error({ error, query }, 'Error querying Prometheus');
      return null;
    }
  }
  ...
  private async fetchSLOMetricsFromPrometheus(): Promise<SLOMetric[]> {
    try {
      ...
      const latencyValue = await this.queryPrometheusInstantValue(
        'histogram_quantile(0.99, rate(collaboration_message_latency_ms[5m]))'
      );

      if (latencyValue !== null) {
        metrics.push({
          sloId: buildSloId('latency'),
          metricType: 'latency',
          threshold: 500,
      ...
      const syncFailureRate = await this.queryPrometheusInstantValue(
        'rate(collaboration_sync_failures[5m]) / rate(collaboration_sync_attempts[5m])'
      );

      if (syncFailureRate !== null) {
        metrics.push({
          sloId: buildSloId('sync_failure_rate'),
