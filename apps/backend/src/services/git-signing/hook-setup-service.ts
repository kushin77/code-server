...
import { GitsignConfig, PreCommitHookConfig } from './types.js';
...
export class GitHookSetupService extends EventEmitter {
  ...
  private hooks = new Map<string, PreCommitHookConfig>();
  ...
  async installHook(
    ...
    try {
      ...
      const hookConfig: PreCommitHookConfig = {
        ...
        environment: {
          ...
          SIGSTORE_OIDC_CLIENT_ID:
            process.env.SIGSTORE_OIDC_CLIENT_ID || 'sigstore',
        },
        createdAt: Date.now(),
        updatedAt: Date.now(),
      };
   ...
   */
  async removeHook(hookPath: string): Promise<void> {
    try {
   ...
   */
  async verifyHook(hookPath: string): Promise<boolean> {
    try {
      const content = await fs.readFile(hookPath, 'utf-8');
   ...
   */
  getHook(hookPath: string): PreCommitHookConfig | undefined {
   ...
   */
  private generateHookScript(config: GitsignConfig): string {
 ...
 */
let hookSetupInstance: GitHookSetupService | null = null;

/**
 * Get global hook setup service instance
 */
export async function getGitHookSetupService(): Promise<GitHookSetupService> {
