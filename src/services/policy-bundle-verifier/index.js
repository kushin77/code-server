...
import { FailSafeMode, } from "./schema";
/**
...
export class PolicyBundleVerifier {
     ...
     */
    async verifySignature(bundle, errors) {
    ...
    validateBundleStructure(bundle, errors) {
        ...
        for (const field of requiredFields) {
            if (!(field in bundle) || bundle[field] === undefined) {
                errors.push({
                    code: "MISSING_FIELD",
    ...
    validateIdentityAssertion(identity, errors) {
        ...
        for (const field of requiredFields) {
            if (!(field in identity) || identity[field] === undefined) {
                errors.push({
     ...
     */
    validateIssuerAndAudience(bundle, errors) {
     ...
     */
    async validateRevocation(bundle, errors, warnings) {
     ...
     */
    cacheBundle(bundle, result, durationSeconds = 300) {
     ...
     */
    getFailSafeContext(cachedBundle) {
     ...
     */
    reconstructSignedMessage(bundle) {
     ...
     */
    async fetchPublicKeyFromJWKS(issuer) {
 ...
 */
export function createPolicyBundleVerifier(options = {}) {
    const fullOptions = {
