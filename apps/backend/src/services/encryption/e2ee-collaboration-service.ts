...
export class E2EECollaborationService extends EventEmitter {
  private logger = getLogger("E2EECollaboration")
  private sessions = new Map<string, E2EESession>()
  private messageCache = new Map<string, CollaborationMessage[]>()
  private keyCache = new Map<string, Buffer>()
  private keyBackupConfig: KeyBackupConfig

  constructor(backupConfig?: KeyBackupConfig) {
    super()
   ...
   */
  initializeSession(userId: string, workspaceId: string): {
    success: boolean
    sessionId: string
    publicKey: string
    ephemeralPublicKey: string
   ...
   */
  async backupKeysToVault(): Promise<{
    success: boolean
    backupId: string
    keysBackedUp: number
  }> {
   ...
   */
  async restoreKeysFromVault(backupId: string): Promise<{
    success: boolean
    keysRestored: number
  }> {
    try {
   ...
   */
  verifyForwardSecrecy(sessionId: string, messageId: string): {
    forwardSecure: boolean
    ratchetRotated: boolean
  } {
    const ratchetKey = this.keyCache.get(`${sessionId}-ratchet-${messageId}`)
   ...
   * Get session status
   */
  getSessionStatus(sessionId: string): E2EESession | null {
...
// Singleton instance
let instance: E2EECollaborationService | null = null

export function getE2EEService(config?: KeyBackupConfig): E2EECollaborationService {
  if (!instance) {
