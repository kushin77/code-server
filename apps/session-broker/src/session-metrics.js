export const createSessionBrokerTelemetryState = () => ({
...
const escapeLabelValue = (value) => value.replace(/\\/g, '\\\\').replace(/\n/g, '\\n').replace(/"/g, '\\"');
...
const formatMetricLine = (name, labels, value) => {
    const formattedLabels = formatLabels(labels);
    return `${name}${formattedLabels} ${Number.isInteger(value) ? value : value.toFixed(6)}`;
};
export const renderSessionBrokerPrometheusMetrics = (snapshot) => {
    const lines = [];
