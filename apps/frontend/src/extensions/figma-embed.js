...
const Panel = ({ children, className = '' }) => (<div className={`border rounded-lg shadow ${className}`}>{children}</div>);
const PanelHeader = ({ children, className = '' }) => (<div className={`bg-gray-100 border-b p-4 font-semibold ${className}`}>{children}</div>);
const PanelBody = ({ children, className = '' }) => (<div className={`p-4 ${className}`}>{children}</div>);
const TextInput = (props) => <input type="text" className="border rounded px-3 py-2 w-full" {...props}/>;
const Spinner = () => <div className="inline-block animate-spin">⟳</div>;
...
const Badge = ({ children }) => (<span className="bg-gray-200 text-gray-900 px-2 py-1 rounded text-sm">{children}</span>);
...
const Button = ({ onClick, disabled = false, children, className = '' }) => (<button onClick={onClick} disabled={disabled} className={`bg-blue-500 text-white px-4 py-2 rounded disabled:opacity-50 ${className}`}>
    {children}
  </button>);
/**
 ...
 */
const fetchFileNodes = useCallback(async (fileKey) => {
    if (!apiClient)
        return;
    try {
        setLoading(true);
...
const extractNodes = (node, nodes = []) => {
    ...
    if (nodeObj.type === 'FRAME' ||
        ...
        nodes.push({
            ...
            absoluteBoundingBox: nodeObj.absoluteBoundingBox,
            visible: nodeObj.visible !== false,
        });
    }
    // Recursively process children
    if (nodeObj.children && Array.isArray(nodeObj.children)) {
        nodeObj.children.forEach((child) => extractNodes(child, nodes));
    }
    return nodes;
};
/**
 ...
 */
const fetchFileComments = useCallback(async (fileKey) => {
    if (!apiClient)
        return;
    try {
...
const handleAddComment = useCallback(async () => {
    if (!apiClient || !selectedFile || !commentText.trim())
        return;
    try {
        setLoading(true);
...
return (<Box p={3}>
      <Panel>
        ...
        <PanelBody>
          {error && (<Alert variant="error" onClose={() => setError(null)}>
              {error}
            </Alert>)}
          ...
          {!selectedFile ? (<Box>
              ...
              {files.length === 0 ? (<p>No Figma files found.</p>) : (files.map((file) => (<Box key={file.key} p={2} mb={2} style={{
                border: '1px solid #e1e4e8',
                borderRadius: '6px',
                cursor: 'pointer',
                transition: 'background 0.2s',
            }} onClick={() => fetchFileNodes(file.key)}>
                    {file.thumbnailUrl && (<img src={file.thumbnailUrl} alt={file.name} style={{ width: '100%', marginBottom: '8px', borderRadius: '4px' }}/>)}
              ...
              <Box mb={3}>
                ...
                {nodes.length === 0 ? (<p>No components or frames found.</p>) : (nodes.map((node) => (<Box key={node.id} p={2} mb={1} style={{
              ...
              <Box mb={3}>
                ...
                <Box mb={2} p={2} style={{ background: '#f6f8fa', borderRadius: '4px' }}>
                  <TextInput placeholder="Add a comment..." value={commentText} onChange={(e) => setCommentText(e.target.value)} disabled={loading} block mb={1}/>
                  <Button size="small" onClick={handleAddComment} disabled={!commentText.trim() || loading}>
                    {loading ? 'Posting...' : 'Post Comment'}
                  </Button>
                </Box>

                {/* Comments List */}
                {comments.map((comment) => (<Box key={comment.id} p={2} mb={1} style={{ border: '1px solid #e1e4e8', borderRadius: '4px' }}>
                    <Flex mb={1} alignItems="center" gap={1}>
                      <img src={comment.user.imgUrl} alt={comment.user.handle} style={{ width: '24px', height: '24px', borderRadius: '50%' }}/>
...
//# sourceMappingURL=figma-embed.js.map