 ...
 */
router.post('/record-sync', (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/window', (req: Request, res: Response) => {
  try {
    ...
    const startMs = parseInt(start as string);
    const endMs = parseInt(end as string);
  ...
  try {
    ...
    res.json({
      ...
      sessions: sessions.sort((a, b) =>
        b.sloCompliancePercent - a.sloCompliancePercent
      ),
      timestamp: Date.now(),
    });
  } catch (error) {
 ...
 */
router.get('/breaches', (req: Request, res: Response) => {
  try {
    const service = getSLOTrackingService();
 ...
 */
router.get('/last-minutes', (req: Request, res: Response) => {
  try {
