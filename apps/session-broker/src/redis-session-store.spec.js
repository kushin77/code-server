...
const createMockRedisClient = () => {
    ...
    return {
        ...
        exists: vi.fn(async (key) => (data.has(key) ? 1 : 0)),
        keys: vi.fn(async (pattern) => {
            const regex = new RegExp(pattern.replace(/\*/g, '.*'));
            return Array.from(data.keys()).filter((k) => regex.test(k));
        }),
        mget: vi.fn(async (keys) => keys.map((k) => data.get(k) || null)),
        mset: vi.fn(async (keyValues) => {
...
//# sourceMappingURL=redis-session-store.spec.js.map