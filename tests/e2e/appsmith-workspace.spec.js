...
test.describe('Appsmith Login & Workspace Access - E2E Tests', () => {
    ...
    test.describe('Happy Path - Basic Appsmith Access', () => {
        ...
        test('user can create new application', async ({ authenticatedPage }) => {
            ...
            const createButton = authenticatedPage.locator('button:has-text("Create")').or(authenticatedPage.locator('[data-testid="create-app-button"]'));
        ...
        test('user can edit application canvas', async ({ authenticatedPage }) => {
            ...
            const canvas = authenticatedPage.locator('iframe[title="iframe"]').or(authenticatedPage.locator('.t--canvas'));
        ...
        test('user can add widgets to canvas', async ({ authenticatedPage }) => {
            ...
            await authenticatedPage.waitForLoadState('networkidle');
            // Open widget library
            const widgetLibraryBtn = authenticatedPage.locator('[data-testid="entity-explorer"]').or(authenticatedPage.locator('.t--widget-library-toggle'));
        ...
        test('user can save application', async ({ authenticatedPage }) => {
            await authenticatedPage.goto(`${APPSMITH_URL}/editor`);
            // Find save button
            const saveButton = authenticatedPage.locator('button:has-text("Save")').or(authenticatedPage.locator('[data-testid="save-button"]'));
    ...
    test.describe('Happy Path - Workspace Operations', () => {
        test('user can view workspace info', async ({ authenticatedPage }) => {
            ...
            const workspaceInfo = authenticatedPage.locator('[data-testid="workspace-info"]').or(authenticatedPage.locator('.t--workspace-settings'));
            expect(workspaceInfo).toBeDefined();
        });
        test('user can create new workspace', async ({ authenticatedPage }) => {
            await authenticatedPage.goto(APPSMITH_URL);
            // Find create workspace option
            const createWorkspaceBtn = authenticatedPage.locator('text=Create Workspace').or(authenticatedPage.locator('[data-testid="create-workspace"]'));
        ...
        test('user can switch between workspaces', async ({ authenticatedPage }) => {
            ...
            const workspaceSwitcher = authenticatedPage.locator('[data-testid="workspace-switcher"]').or(authenticatedPage.locator('.t--workspace-dropdown'));
        ...
        test('user can access workspace settings', async ({ authenticatedPage }) => {
            ...
            const settingsBtn = authenticatedPage.locator('[data-testid="workspace-settings-btn"]').or(authenticatedPage.locator('text=Settings'));
    ...
    test.describe('Edge Cases', () => {
        ...
        test('special characters in app name handled', async ({ authenticatedPage }) => {
            ...
            if (await createButton.isVisible()) {
                ...
                const nameInput = authenticatedPage.locator('input[placeholder*="name"]').or(authenticatedPage.locator('[data-testid="app-name-input"]'));
    ...
    test.describe('Integration with Portal', () => {
        test('can navigate back to portal from Appsmith', async ({ authenticatedPage }) => {
            ...
            const backButton = authenticatedPage.locator('[data-testid="back-to-portal"]').or(authenticatedPage.locator('text=Back'));
...
//# sourceMappingURL=appsmith-workspace.spec.js.map