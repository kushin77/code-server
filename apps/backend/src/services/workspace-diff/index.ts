...
export interface WorkspaceDiffFile {
  status: 'added' | 'modified' | 'deleted' | 'renamed' | 'copied' | 'updated' | 'unknown';
  path: string;
  previousPath?: string;
}
...
export interface WorkspaceDiffSnapshot extends WorkspaceDiff {
  id: string;
}
...
export class WorkspaceDiffService extends EventEmitter {
  private logger = getLogger('WorkspaceDiffService');
  private pool: Pool;
  ...
  async captureWorkspaceDiff(userId: string, repoPath: string): Promise<WorkspaceDiffSnapshot> {
  ...
  generateDiff(userId: string, repoPath: string, baseRef: string, headRef: string): WorkspaceDiff {
  ...
  async saveSnapshot(diff: WorkspaceDiff): Promise<WorkspaceDiffSnapshot> {
    const client = await this.pool.connect();
    try {
  ...
  async getLatestSnapshot(query: WorkspaceDiffQuery): Promise<WorkspaceDiffSnapshot | null> {
    const client = await this.pool.connect();
    try {
  ...
  async listSnapshots(query: WorkspaceDiffQuery): Promise<WorkspaceDiffSnapshot[]> {
    const client = await this.pool.connect();
    try {
  ...
  async getWorkspaceDiffSinceLastSeen(userId: string, repoPath: string): Promise<WorkspaceDiffSnapshot> {
    return this.captureWorkspaceDiff(userId, repoPath);
  }

  private readChangedFiles(repoPath: string, baseRef: string, headRef: string): WorkspaceDiffFile[] {
    ...
    return output.split('\n').filter(Boolean).map(line => {
      ...
      const statusMap: Record<string, WorkspaceDiffFile['status']> = {
        A: 'added',
        M: 'modified',
        D: 'deleted',
        U: 'updated'
  ...
  private summarizeChanges(changedFiles: WorkspaceDiffFile[]): string {
...
export async function initializeWorkspaceDiffRoutes(service: WorkspaceDiffService) {
  ...
  router.post('/api/workspace-diff/refresh/:userId', async (req, res) => {
    try {
      const repoPath = req.body.repoPath as string;
