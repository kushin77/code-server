...
import { SymbolDiscussionsService } from '../index';
// Mock the logger
vi.mock('../../../lib/logger', () => ({
    getLogger: vi.fn(() => ({
        info: vi.fn(),
...
describe('SymbolDiscussionsService', () => {
    ...
    describe('initialization', () => {
        it('should initialize with database schema', async () => {
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('CREATE TABLE IF NOT EXISTS symbol_discussions'));
        });
    });
    ...
    describe('resolveThread', () => {
        it('should resolve a discussion thread', async () => {
            ...
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE discussion_threads'), [threadId, 'bob']);
        });
    });
    describe('addReaction and removeReaction', () => {
        it('should add a reaction to a comment', async () => {
            ...
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO comment_reactions'), [commentId, '👍', 'bob']);
        });
        it('should remove a reaction from a comment', async () => {
            ...
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('DELETE FROM comment_reactions'), [commentId, '👍', 'bob']);
        });
    });
    ...
    describe('updateSymbolFQN', () => {
        it('should update symbol FQN for refactoring', async () => {
            ...
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE symbol_discussions'), [newFQN, oldFQN]);
        });
    });
    describe('deleteThread', () => {
        it('should delete a discussion thread and all associated data', async () => {
            ...
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('DELETE FROM discussion_threads'), [threadId]);
        });
    });
});
//# sourceMappingURL=symbol-discussions.test.js.map