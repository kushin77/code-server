...
function writeMetric(metricName, value, labels) {
    if (!METRICS_FILE) {
        return;
    }
...
test.describe('Collaboration Synthetic Monitor', () => {
    ...
    test('file lock release, list, and cleanup succeed', async ({ request }) => {
        await runScenario('file_lock_release_cleanup', async () => {
            ...
            expect(acquireResponse.status()).toBe(201);
            const lock = await acquireResponse.json();
            const releaseResponse = await request.delete(new URL(`/api/file-locks/${lock.lockId}?userId=synthetic-monitor`, IDE_BASE_URL).toString());
...
//# sourceMappingURL=collaboration-synthetic-monitor.spec.js.map