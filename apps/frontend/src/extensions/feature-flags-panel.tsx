...
interface FeatureFlag {
  ...
  description: string;
  enabled: boolean;
  provider: 'launchdarkly' | 'unleash' | 'local';
  createdAt: string;
  modifiedAt: string;
}
...
export const FeatureFlagsPanel: React.FC = () => {
  ...
  const toggleFlag = async (flag: FeatureFlag): Promise<void> => {
    try {
      ...
      vscode.window.showErrorMessage(
        `Error toggling flag: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  };

  const createFlag = async (): Promise<void> => {
    if (!newFlagName.trim()) {
    ...
    try {
      ...
      vscode.window.showErrorMessage(
        `Error creating flag: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  };
  ...
  const deleteFlag = async (flagKey: string): Promise<void> => {
    ...
    try {
      ...
      vscode.window.showErrorMessage(
        `Error deleting flag: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  };
  ...
  const exportFlags = async (): Promise<void> => {
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to export flags: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  };
  ...
  return (
    <div style={{ padding: '16px', fontFamily: 'var(--vscode-font-family)' }}>
      ...
      {loading ? (
        <div style={{ color: 'var(--vscode-foreground)', opacity: 0.7 }}>Loading flags...</div>
      ) : filteredFlags.length === 0 ? (
        <div style={{ color: 'var(--vscode-foreground)', opacity: 0.7 }}>No flags found</div>
      ) : (
