...
export class SharedPromptLibraryService extends EventEmitter {
    ...
    async createPrompt(teamId, name, content, category, createdBy, options = {}) {
    ...
    async listTeamPrompts(teamId, category) {
        const client = await this.pool.connect();
        try {
    ...
    async updatePrompt(promptId, content, updatedBy, changeDescription) {
        if (content.length > this.config.maxPromptLength) {
            throw new Error(`Prompt exceeds max length of ${this.config.maxPromptLength} characters`);
        }
        const client = await this.pool.connect();
    ...
    async suggestPrompts(teamId, context, category) {
    ...
    async trackUsage(promptId, userId, context) {
        const client = await this.pool.connect();
        try {
            // Record usage
    ...
    calculateMatchScore(context, content, name) {
