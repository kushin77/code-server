...
import { formatDate, isApprovedDataProfile, normalizeDataProfile, normalizeUsername, SESSION_DATA_PROFILES, stateTone, } from './ephemeralSessionsUtils';
const TTL_MIN_SECONDS = 3600;
const TTL_MAX_SECONDS = 86400;
const TTL_DEFAULT_SECONDS = 28800;
const fieldClassName = 'w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-slate-900 shadow-sm outline-none transition focus:border-sky-500 focus:ring-4 focus:ring-sky-100';
/**
...
export function EphemeralSessionsPage() {
    ...
    const { session, status, isLoading, error, launchSession, fetchSessionStatus, cancelSession, destroySession, } = useEphemeralSessions();
    const [ttlSeconds, setTtlSeconds] = useState(String(TTL_DEFAULT_SECONDS));
    const [dataProfile, setDataProfile] = useState('synthetic');
    const [priorityLane, setPriorityLane] = useState('standard');
    ...
    const handleLaunch = async (event) => {
        ...
        try {
            ...
            setAnnouncement(launched.status === 'queued'
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm shadow-slate-200/60">
          ...
          <form className="mt-6 space-y-4" onSubmit={handleLaunch}>
            <label className="block">
              ...
              <select aria-label="Data profile" className={fieldClassName} name="dataProfile" value={dataProfile} onChange={(event) => {
            setDataProfile(event.target.value);
            setLocalError(null);
        }}>
                {SESSION_DATA_PROFILES.map((profile) => (<option key={profile} value={profile}>
                    {profile}
                  </option>))}
              </select>
            </label>
            ...
            <label className="block">
              ...
              <input aria-label="TTL seconds" className={fieldClassName} inputMode="numeric" min={TTL_MIN_SECONDS} max={TTL_MAX_SECONDS} name="ttlSeconds" type="number" value={ttlSeconds} onChange={(event) => {
            ...
            <label className="block">
              ...
              <select aria-label="Queue lane" className={fieldClassName} name="priorityLane" value={priorityLane} onChange={(event) => {
            setPriorityLane(event.target.value);
            setLocalError(null);
        }}>
            ...
            {(localError || error) && (<div role="alert" className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-800">
                {localError || error}
              </div>)}

            <button className="inline-flex w-full items-center justify-center rounded-2xl bg-sky-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-sky-700 disabled:cursor-not-allowed disabled:bg-slate-300" disabled={isLoading || !user} type="submit">
        ...
        <section className="space-y-6">
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm shadow-slate-200/60">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              ...
              <button className="inline-flex items-center justify-center rounded-xl border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-50" disabled={!activeSessionId || isLoading} onClick={handleRefresh} type="button">
                Refresh status
              </button>
            </div>
            ...
            <div className="mt-6 space-y-4">
              {currentSession ? (<>
                  ...
                  <dl className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                      ...
                      <dd className="mt-2 break-all font-medium text-slate-900">
                        {brokerUrl ? (<a className="text-sky-700 underline decoration-sky-300 underline-offset-4 hover:text-sky-900" href={brokerUrl} rel="noreferrer" target="_blank">
                            {brokerUrl}
                          </a>) : ('Waiting for broker URL')}
                      </dd>
                    </div>
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    {queueInfo && (<div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  ...
                  {queueInfo && (<div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                  ...
                  {'active' in currentSession && (<div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
                      <p>
                        Active: <span className="font-semibold text-slate-900">{currentSession.active ? 'yes' : 'no'}</span>
                        {' · '}
                        Terminal: <span className="font-semibold text-slate-900">{currentSession.terminal ? 'yes' : 'no'}</span>
                      </p>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm shadow-slate-200/60">
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row">
              <button className="inline-flex items-center justify-center rounded-2xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-900 transition hover:bg-amber-100 disabled:cursor-not-allowed disabled:opacity-50" disabled={!activeSessionId || isLoading || sessionState === 'destroyed'} onClick={handleCancel} type="button">
                Cancel session
              </button>
              <button className="inline-flex items-center justify-center rounded-2xl border border-rose-300 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-900 transition hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-50" disabled={!activeSessionId || isLoading || sessionState === 'destroyed'} onClick={handleDestroy} type="button">
                Destroy now
              </button>
            </div>
...
//# sourceMappingURL=EphemeralSessions.js.map