...
if [[ $VIOLATION_COUNT -eq 0 ]]; then
    ...
    echo "   - 192.168.168.31 → \${PRIMARY_HOST} or var.primary_host" | tee -a "$AUDIT_LOG"
    echo "   - 192.168.168.42 → \${REPLICA_HOST} or var.replica_host" | tee -a "$AUDIT_LOG"
    echo "   - 192.168.168.56 → \${NAS_HOST} or var.nas_host" | tee -a "$AUDIT_LOG"
    echo "" | tee -a "$AUDIT_LOG"
    echo "2. Replace hardcoded domains with environment variables:" | tee -a "$AUDIT_LOG"
    echo "   - kushnir.cloud → \${APEX_DOMAIN} or var.apex_domain" | tee -a "$AUDIT_LOG"
    echo "   - *.kushnir.cloud → \${SUBDOMAIN}.\${APEX_DOMAIN}" | tee -a "$AUDIT_LOG"
    echo "" | tee -a "$AUDIT_LOG"
    echo "3. Verify templates use environment variable substitution" | tee -a "$AUDIT_LOG"
    echo "4. Re-run this script to verify compliance" | tee -a "$AUDIT_LOG"
