...
describe('Mention System Service', () => {
    ...
    function createTarget(type = 'file') {
        return {
            type,
    ...
    describe('Mention Creation', () => {
        it('should create mention', async () => {
            ...
            const mention = await service.createMention('user-alice', 'user-bob', target, '@user-bob check this code', 'code', 'workspace-1', 'session-1', '192.168.1.1', 'Chrome/95.0', 'normal');
        ...
        it('should create mention with different priorities', async () => {
            const target = createTarget('file');
            const normalMention = await service.createMention('user-alice', 'user-bob', target, 'Check this', 'code', undefined, undefined, undefined, undefined, 'normal');
            const urgentMention = await service.createMention('user-alice', 'user-bob', target, 'Urgent!', 'code', undefined, undefined, undefined, undefined, 'urgent');
        ...
        it('should handle different context types', async () => {
            const target = createTarget();
            const contextTypes = ['code', 'comment', 'commit-message', 'pr-review', 'issue-comment', 'chat'];
            for (const contextType of contextTypes) {
                const mention = await service.createMention('user-alice', 'user-bob', target, 'Check this', contextType);
    ...
    describe('Mention Query', () => {
        beforeEach(async () => {
            ...
            await service.createMention('user-alice', 'user-bob', fileTarget, 'Check code', 'code', 'workspace-1', 'session-1', '192.168.1.1', 'Chrome', 'normal');
            await service.createMention('user-charlie', 'user-bob', functionTarget, 'Review function', 'pr-review', 'workspace-1', 'session-2', '192.168.1.2', 'Firefox', 'high');
            await service.createMention('user-alice', 'user-bob', fileTarget, 'Check again', 'comment', 'workspace-2', 'session-1', '192.168.1.1', 'Chrome', 'low');
        });
    ...
    describe('Audit Logging', () => {
        it('should log mention creation', async () => {
            const target = createTarget();
            await service.createMention('user-alice', 'user-bob', target, 'Check this', 'code', 'workspace-1', 'session-1', '192.168.1.1', 'Chrome');
        ...
        it('should include IP and user agent in audit log', async () => {
            ...
            await service.createMention('user-alice', 'user-bob', target, 'Check this', 'code', undefined, undefined, ipAddress, userAgent);
            const auditLog = await service.getAuditLog('user-bob');
    ...
    describe('Settings Management', () => {
        ...
        it('should block mentions from specific users', async () => {
            ...
            await expect(service.createMention('user-charlie', 'user-bob', target, 'Check this', 'code')).rejects.toThrow('blocked');
        });
    });
    describe('Statistics', () => {
        beforeEach(async () => {
    ...
    describe('Integration', () => {
        it('should handle complete mention workflow', async () => {
            // 1. Create mention
            const target = createTarget('file');
            const mention = await service.createMention('user-alice', 'user-bob', target, '@user-bob check this', 'code', 'workspace-1');
...
//# sourceMappingURL=mention-system-service.test.js.map