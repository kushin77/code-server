...
import {
  GitSignatureVerificationService,
  getGitSignatureVerificationService,
} from '../verification-service.js';
import {
  GitHookSetupService,
  getGitHookSetupService,
} from '../hook-setup-service.js';
import {
  GitsignConfig,
  SigningPolicy,
  CommitSignature,
 ...
 */
const MOCK_SIGNATURES: Record<string, CommitSignature> = {
  'abc1234': {
    commitHash: 'abc1234',
...
class MockGitSignatureVerificationService extends GitSignatureVerificationService {
  async verifyWithGitsignMock(commitHash: string): Promise<CommitSignature> {
...
describe('Git Signature Verification', () => {
  let service: MockGitSignatureVerificationService;
  let hookService: GitHookSetupService;

  const testConfig: GitsignConfig = {
    enabled: true,
    required: true,
  ...
  const testPolicy: SigningPolicy = {
    id: 'policy-1',
    workspaceId: 'ws-test',
    enabled: true,
    enforceAll: true,
  ...
  describe('Hook Setup', () => {
    it('should generate valid hook script', async () => {
      const script = (hookService as any).generateHookScript(testConfig);
  ...
  describe('Integration', () => {
    ...
    it('should handle policy enforcement across batch', async () => {
      ...
      const hashes = ['abc1234', 'def5678', 'unsigned'];
      const compliances = await Promise.all(
        hashes.map((hash) => service.checkCompliance(hash))
      );
