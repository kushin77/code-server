...
import {
  ...
  MergeResolverServiceConfig,
  MergeResolverAuditEntry,
  MergeResolverStatistics,
  DiffStatistics,
  ResolutionStrategy,
} from './types.js';
...
export class MergeResolverService extends EventEmitter {
  private static instance: MergeResolverService;

  private config: Required<MergeResolverServiceConfig>;
  private sessions: Map<string, MergeSession>;
  private auditLog: Map<string, MergeResolverAuditEntry[]>;
  private statistics: MergeResolverStatistics;
  private resolutionHistory: Array<{ sessionId: string; conflictId: string; timestamp: number }>;

  /**
   * Get singleton instance
   */
  static getInstance(config?: Partial<MergeResolverServiceConfig>): MergeResolverService {
    if (!this.instance) {
   ...
   * Constructor
   */
  private constructor(config?: Partial<MergeResolverServiceConfig>) {
    super();
   ...
   * Get merge session
   */
  getMergeSession(sessionId: string): MergeSession | null {
   ...
   * Resolve conflict
   */
  resolveConflict(request: ResolutionRequest, sessionId: string, ipAddress: string, userAgent: string): ResolutionResult {
   ...
   */
  completeMerge(
    request: MergeCompletionRequest,
    ipAddress: string,
    userAgent: string
  ): MergeCompletionResult {
   ...
   * Abort merge
   */
  abortMerge(sessionId: string, userId: string, userEmail: string, ipAddress: string, userAgent: string): boolean {
    const session = this.sessions.get(sessionId);
    if (!session) {
      return false;
    }
   ...
   * Get diff statistics
   */
  getDiffStatistics(sessionId: string): DiffStatistics {
    const session = this.sessions.get(sessionId);
    if (!session) {
      return {
        filesChanged: 0,
   ...
   * Get audit log
   */
  getAuditLog(userId: string): MergeResolverAuditEntry[] {
   ...
   * Get statistics
   */
  getStatistics(): MergeResolverStatistics {
   ...
   */
  updateConfig(config: Partial<MergeResolverServiceConfig>, userId: string, ipAddress: string, userAgent: string): void {
   ...
   */
  private performSmartMerge(conflict: MergeConflict): string {
   ...
   */
  private isSimilar(conflict1: MergeConflict, conflict2: MergeConflict): boolean {
    // Same file and within 10 lines
    return (
      conflict1.filePath === conflict2.filePath &&
   ...
   * Record audit entry
   */
  private recordAudit(entry: MergeResolverAuditEntry): void {
