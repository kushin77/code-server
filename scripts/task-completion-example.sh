...
main() {
    ...
    if safe_task_complete 984; then
        ...
        if [[ "$completion_status" -eq 2 ]]; then
            log_info "🟡 Agent-owned work is complete - task is ready for handoff"
            log_info ""
            ...
            log_info ""
            log_info "ACCEPTANCE (Path C - if manual steps are explicitly accepted):"
            log_info "  safe_task_complete 984 force"
            log_info "  (with comment in issue explaining credential blockers)"
        else
            log_info "⏳ Agent-owned DoD items remain - showing next steps..."
            log_info ""
