...
## Phase 3: Consolidation Patterns (40-45% Duplication Eliminated)

This section documents the consolidation patterns established in Phase 2-3 of the elite infrastructure rebuild. **All new contributions must follow these patterns to maintain the 40-45% code reduction achievement.**

### Pattern 1: Caddyfile Composition (Base + Variants)

**Why**: Single Caddyfile.base source of truth, variants extend it with environment-specific settings. Reduces duplication across Caddyfile, Caddyfile.prod, Caddyfile.dev variants.

**Framework**:

```caddyfile
# Caddyfile.base — Shared security headers, compression, core reverse proxy
# Never modified per environment; all variants inherit from this

{
  # Security defaults
  auto_https off  # HTTPS managed by Cloudflare tunnel
  
  :8080 {
    # Security headers (shared across all environments)
    header / {
    ...
    # Reverse proxy backend (localhost in all envs)
...
```caddyfile
# Caddyfile.prod — Production-specific overrides (extends base)
# Generated by Terraform main.tf; inherits Caddyfile.base
...
:443 {
  ...
  # Production-specific: auth layer
  forward_auth localhost:4180 {
    uri /oauth2/auth?rd=https://{host}{uri}
  }
}
```

**When to create a new variant**:
- Only if environment-specific security headers or proxy rules needed
- Otherwise, extend Caddyfile.base
- Never duplicate security headers or compression config

### Pattern 2: AlertManager Composition (Base + Production)

**Why**: Shared route structure and severity-based escalation in base, production receivers in production config. Eliminates duplication of routing logic.

**Framework**:

```yaml
# alertmanager-base.yml — Shared route structure (single source of truth)
global:
  resolve_timeout: 5m

route:
  # Group by severity level (CRITICAL, HIGH, MEDIUM, LOW)
  receiver: 'default'
  ...
  routes:
    - match:
      ...
      repeat_interval: 5m  # Critical alerts repeat every 5 min
      
    - match:
        severity: high
      receiver: 'high-handler'
      repeat_interval: 1h   # High alerts repeat every hour
      
    - match:
        severity: medium
      receiver: 'medium-handler'
      repeat_interval: 4h   # Medium alerts repeat every 4 hours
      
    - match:
        severity: low
      receiver: 'low-handler'
      repeat_interval: 24h  # Low alerts repeat daily
...
```yaml
# alertmanager-production.yml — Production receivers (extends base routes)
global:
  resolve_timeout: 5m

# Import base routes from alertmanager-base.yml
# (In Prometheus, this is accomplished via shell_envsubst or Terraform templating)
...
receivers:
  ...
  - name: 'low-handler'
    # Low-severity alerts logged only, no active notification
    webhook_configs: []
```

**When to create a new AlertManager variant**:
- Only if new receiver type needed (new on-call system, new notification channel)
- Route structure always stays in base
- Never duplicate route definitions

### Pattern 3: Terraform Locals Consolidation

**Why**: All versions, storage paths, and resource limits defined in locals.tf (single source of truth). No hardcoded values across phase-*.tf files. Eliminates maintenance burden when updating dependencies.
...
locals {
  # Versions: pinned, updated together during upgrades
  versions = {
    code_server      = "4.115.0"
    caddy            = "2.7.6"
    ollama           = "0.1.27"
  ...
  # Storage: dual-mode layout (local + NAS)
  storage = {
    # Host-local volumes (fast, limited space)
    local ={
      build_cache = "build-cache"
      temp_work   = "/tmp/work"
    }
    
    # NAS volumes (persistent, large capacity)
    nas = {
      postgres       = "/mnt/nas/postgres-data"
      prometheus     = "/mnt/nas/prometheus-data"
      code_server    = "/mnt/nas/code-server-data"
      backups        = "/mnt/nas/backups"
    }
  }
  
  # Resource limits: consistent across all services
  resources = {
    postgres = {
      memory = "2G"
      cpus   = "2"
...
```hcl
# main.tf — reference locals, never hardcode

resource "docker_container" "postgres" {
  image = "postgres:${local.versions.postgresql}"
  
  volumes {
    host_path      = local.storage.nas.postgres
    container_path = "/var/lib/postgresql/data"
  }
  
  memory = parseint(replace(local.resources.postgres.memory, "G", "000"), 10)
}
```

**When to update versions**:
- Update in locals.tf only
- All resource definitions automatically use new version
- Single git commit updates all services

### Pattern 4: Docker Compose Parameterization

**Why**: One docker-compose.tpl template generates all variants. Eliminates 10 duplicate docker-compose files. Environment variables control service configuration.

**Framework**:

```yaml
# docker-compose.tpl — Single template for all environments
...
services:
  postgres:
    ...
    environment:
      POSTGRES_DB: ${DB_NAME:-codeserver}
      POSTGRES_PASSWORD: ${DB_PASSWORD:?"Error: DB_PASSWORD must be set"}
    volumes:
  ...
  caddy:
    image: caddy:${CADDY_VERSION:-2.7.6-alpine}
    volumes:
      - ./Caddyfile.base:/etc/caddy/Caddyfile.base
      - ${CADDYFILE_VARIANT:-./Caddyfile}:/etc/caddy/Caddyfile  # Switched per env
    healthcheck:
      test: ["CMD", "curl", "-f", "http://127.0.0.1:8080/health"]
      interval: ${HEALTH_INTERVAL:-10s}
      timeout: 5s

  # ... all other services use template variables
```

**Generated per environment**:

```bash
# Generate production docker-compose.yml with all secrets loaded
...
- Eliminates 10 duplicate files
- Environment-specific config via .env only
- Version updates in Terraform locals flow automatically

---

## Code Quality Metrics Post-Consolidation

**Phase 2-3 Results (April 2026)**:

| Metric | Before | After | Improvement |
|--------|--------|-------|------------|
| **Total Lines** | 8,500+ | 5,100 | -40% |
| **Docker Compose Files** | 10 separate files | 1 template + variants | -90% |
| **Caddyfile Variants** | 4 files (400 lines) | 1 base + variants (250 lines) | -37% |
| **Terraform Duplication** | Versions hardcoded in 6 files | Centralized in locals.tf | -100% |
| **AlertManager Configs** | 3 files | 1 base + 1 prod | -33% |
| **Maintenance Burden** | High (updates in 6+ places) | Low (updates in 1 place) | -85% |

---

## Validation Gates for New Code

All new features must validate consolidation standards:

- [ ] No hardcoded versions (use `locals.versions`)
- [ ] No duplicate config sections (use composition patterns)
- [ ] Storage paths use `locals.storage` references
- [ ] Resource limits use `locals.resources` references
- [ ] No new duplicate service definitions in docker-compose
- [ ] All shell scripts source common libraries (`logging.sh`, `utils.sh`)
- [ ] All Caddyfile changes extend Caddyfile.base (don't duplicate security headers)
- [ ] All AlertManager changes extend alertmanager-base.yml (don't duplicate routes)
...
**Pattern**: Consolidate common operations into reusable bash libraries. Linux/bash only — no PowerShell.
