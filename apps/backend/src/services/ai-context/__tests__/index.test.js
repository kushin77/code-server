...
describe('SharedAIContextService', () => {
    ...
    beforeEach(() => {
        ...
        service = new SharedAIContextService({ workspaceId: 'ws-123' }, mockPool, mockRedis, mockAuditService);
    });
    describe('Context Management', () => {
        it('should create a new AI context', async () => {
            const ctx = await service.createContext('user-1', 'session-1', [{ path: 'index.ts', language: 'typescript', content: 'code' }], [{ role: 'user', content: 'help me', timestamp: new Date() }]);
            expect(ctx.userId).toBe('user-1');
            expect(ctx.fileContext).toHaveLength(1);
        });
        it('should audit context creation', async () => {
            await service.createContext('user-1', 'session-1', [{ path: 'index.ts', language: 'typescript', content: 'code' }], []);
    ...
    describe('Error Handling', () => {
        it('should throw error for missing workspace ID', () => {
            expect(() => {
                new SharedAIContextService({}, mockPool, mockRedis);
            }).toThrow('Workspace ID required');
        });
        it('should throw error for non-existent context share', async () => {
            await expect(service.shareContext('invalid-id', 'user-1', 'user-2')).rejects.toThrow('not found');
        });
        it('should throw error for non-existent context update', async () => {
            await expect(service.updateContext('invalid-id', 'user-1', [])).rejects.toThrow('not found');
        });
    });
});
