...
export class MessageCompressionPipelineService extends EventEmitter {
    constructor(pool) {
        super();
        this.logger = getLogger('MessageCompressionPipelineService');
        this.pool = pool;
    }
    async initialize() {
    ...
    async compressMessage(sessionId, channel, message, metadata = {}) {
    ...
    async decompressMessage(messageId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT id, session_id, channel, compression_method, base_message_id, compressed_payload, metadata FROM compressed_collaboration_messages WHERE id = $1`, [messageId]);
    ...
    async getCompressedMessage(messageId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`
    ...
    async getPipelineMetrics(days = 7) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`
          SELECT
    ...
    async cleanupOldMessages(daysOld = 30) {
        ...
        try {
            ...
            const result = await client.query(`DELETE FROM compressed_collaboration_messages WHERE created_at < $1`, [cutoff]);
            await client.query(`DELETE FROM compression_pipeline_metrics WHERE recorded_at < $1`, [cutoff]);
    ...
    async resolveMessageFromPayload(client, row, payload) {
        ...
        const baseResult = await client.query(`SELECT id, session_id, channel, compression_method, base_message_id, compressed_payload, metadata FROM compressed_collaboration_messages WHERE id = $1`, [payload.baseMessageId]);
    ...
    getCommonPrefixLength(left, right) {
        let index = 0;
...
export async function initializeMessageCompressionPipelineRoutes(service) {
    ...
    router.delete('/api/message-compression/cleanup', async (req, res) => {
        try {
            const daysOld = parseInt(req.query.daysOld, 10) || 30;
