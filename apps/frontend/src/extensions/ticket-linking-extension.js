...
export async function activate(context) {
    ...
    const { default: TicketDetector } = await import('../../../backend/src/services/ticket-linking/ticket-detector');
    ...
    const apiCredentials = new Map([
        ['linear', config.get('linearApiKey') || process.env.LINEAR_API_KEY || ''],
        ['jira', config.get('jiraApiKey') || process.env.JIRA_API_KEY || ''],
        ['github', config.get('githubToken') || process.env.GITHUB_TOKEN || ''],
    ]);
    detector = new TicketDetector(apiCredentials);
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('ticketLinking.showPanel', () => {
    ...
    context.subscriptions.push(vscode.window.onDidChangeActiveTextEditor(async (editor) => {
        if (editor && detector) {
            const tickets = await detector.getResolvedTicketsForFile(editor.document.uri.fsPath, editor.document.getText());
            if (tickets.length > 0) {
                if (!ticketPanel) {
                    ticketPanel = TicketLinkingPanel.createOrShow(context.extensionUri);
                }
    ...
    let changeTimer;
    context.subscriptions.push(vscode.workspace.onDidChangeTextDocument(async (_event) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('ticketLinking.linkTicket', async (_args) => {
...
async function refreshTickets() {
    ...
    try {
        ...
        // Update status bar
        const statusBar = vscode.window.statusBarItems.find((item) => item.text === '📋 Tickets');
...
async function configureCredentials() {
    const selected = await vscode.window.showQuickPick(['Linear', 'Jira', 'GitHub', 'Cancel'], {
    ...
    const apiKey = await vscode.window.showInputBox({
        ...
        validateInput: (value) => {
            if (!value)
                return `${selected} API Key cannot be empty`;
            return null;
        },
    });
    if (apiKey) {
        ...
        const newCredentials = new Map([
            ['linear', config.get('linearApiKey') || ''],
            ['jira', config.get('jiraApiKey') || ''],
            ['github', config.get('githubToken') || ''],
        ]);
        const { default: TicketDetector } = await import('../../../backend/src/services/ticket-linking/ticket-detector');
...
//# sourceMappingURL=ticket-linking-extension.js.map