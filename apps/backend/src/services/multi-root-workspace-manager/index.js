...
export class MultiRootWorkspaceManagerService extends EventEmitter {
    constructor(pool) {
        super();
        this.logger = getLogger('MultiRootWorkspaceManagerService');
        this.pool = pool;
    }
    async initialize() {
    ...
    async createProfile(input) {
        ...
        `, [
            ...
            await client.query(`INSERT INTO workspace_profile_history (profile_id, action, details) VALUES ($1, $2, $3)`, [profile.id, 'created', JSON.stringify({ projectName: profile.projectName, roots: profile.roots.length })]);
    ...
    async getProfile(profileId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT id, user_id, project_name, description, roots, settings, is_active, created_at, updated_at FROM workspace_profiles WHERE id = $1`, [profileId]);
    ...
    async listProfiles(userId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT id, user_id, project_name, description, roots, settings, is_active, created_at, updated_at
         FROM workspace_profiles
         WHERE user_id = $1
         ORDER BY is_active DESC, updated_at DESC`, [userId]);
    ...
    async updateProfile(profileId, updates) {
        ...
        `, [nextProjectName, nextDescription, JSON.stringify(nextRoots), JSON.stringify(nextSettings), nextActive, profileId]);
            ...
            await client.query(`INSERT INTO workspace_profile_history (profile_id, action, details) VALUES ($1, $2, $3)`, [profileId, 'updated', JSON.stringify({ projectName: updated.projectName, roots: updated.roots.length })]);
    ...
    async addRoot(profileId, root) {
        ...
        `, [JSON.stringify(nextRoots), profileId]);
            const updated = this.rowToProfile(result.rows[0]);
            await client.query(`INSERT INTO workspace_profile_history (profile_id, action, details) VALUES ($1, $2, $3)`, [profileId, 'root-added', JSON.stringify({ rootPath: root.path })]);
    ...
    async removeRoot(profileId, rootPath) {
        const client = await this.pool.connect();
        try {
            const profile = await this.getProfile(profileId);
            if (!profile) {
        ...
        `, [JSON.stringify(nextRoots), profileId]);
            const updated = this.rowToProfile(result.rows[0]);
            await client.query(`INSERT INTO workspace_profile_history (profile_id, action, details) VALUES ($1, $2, $3)`, [profileId, 'root-removed', JSON.stringify({ rootPath })]);
    ...
    async activateProfile(profileId) {
        ...
        `, [profileId]);
            ...
            await client.query(`INSERT INTO workspace_profile_history (profile_id, action, details) VALUES ($1, $2, $3)`, [profileId, 'activated', JSON.stringify({ userId: profile.userId })]);
    ...
    async cloneProfile(profileId, newProjectName) {
        const profile = await this.getProfile(profileId);
        if (!profile) {
            throw new Error(`Workspace profile ${profileId} not found`);
        }
    ...
    async deleteProfile(profileId) {
        ...
        try {
            ...
            await client.query(`INSERT INTO workspace_profile_history (profile_id, action, details) VALUES ($1, $2, $3)`, [profileId, 'deleted', JSON.stringify({ userId: result.rows[0].user_id })]);
...
export async function initializeMultiRootWorkspaceManagerRoutes(service) {
