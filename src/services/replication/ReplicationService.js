...
export class ReplicationService extends EventEmitter {
    ...
    async start() {
        ...
        console.log(`[ReplicationService] Started for replica ${this.config.replicaId} in region ${this.config.regionId}`);
    }
    /**
    ...
    async stop() {
        ...
        console.log(`[ReplicationService] Stopped for replica ${this.config.replicaId}`);
    }
    /**
     * Write data to the local replica
     */
    async write(key, value, operationType) {
        const operation = {
            type: operationType,
            key,
            value,
     ...
     */
    async receiveOperation(envelope) {
    ...
    applyOperation(operation) {
        ...
        if (existingData) {
            ...
            if (this.conflictResolver.detectConflict(existingOp, operation)) {
                this.metrics.conflictsDetected++;
                const conflict = this.conflictResolver.resolveConflict(existingOp, operation, existingData.value, operation.value);
     ...
     */
    async broadcastOperation(envelope) {
        for (const [replicaId, peer] of this.peers) {
            if (peer.isHealthy) {
                try {
                    ...
                    console.error(`Failed to send operation to ${replicaId}:`, error);
    ...
    async syncWithPeer(replicaId) {
        ...
        try {
            ...
            // Build sync request
            const syncRequest = this.syncProtocol.buildSyncRequest(replicaId, peer.vectorClock);
