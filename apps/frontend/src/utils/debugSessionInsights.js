...
export function analyzeDebugSession(session) {
    if (!session) {
        return [];
    }
    const insights = [];
    ...
    if (suspiciousVariables.length > 0) {
        insights.push({
            ...
            fixApproach: 'Step back to the branch that produces the value, add a guard for missing payloads, and capture a fresh snapshot before the breakpoint fires again.',
    ...
    if (hotFiles.length > 0) {
        insights.push({
            ...
            fixApproach: 'Treat this as a flow problem instead of a single line bug: instrument the entry and exit points, then compare the captured values across each breakpoint.',
    ...
    if (session.relayTarget && unresolvedRelays.length > 0) {
        insights.push({
            ...
            fixApproach: 'Check the relay endpoint health, confirm the DAP transport contract, and retry the step after a successful relay handshake.',
    ...
    if (pauseEvents.length > 0 && session.variables.length === 0) {
        insights.push({
            ...
            fixApproach: 'Pause again at the failing branch, capture locals and watched expressions, and compare the state before and after the step.',
            relevantDocs: ['docs/README.md', 'docs/status/README.md'],
            confidence: 72,
    ...
    if (insights.length === 0) {
        insights.push({
            ...
            fixApproach: 'Add one breakpoint around the failing branch, record the watched variables, and step once more before asking for an AI diagnosis.',
            relevantDocs: ['docs/README.md', 'docs/status/README.md'],
            confidence: 55,
            evidence: unique([
...
//# sourceMappingURL=debugSessionInsights.js.map