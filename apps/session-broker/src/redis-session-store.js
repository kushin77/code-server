...
export class RedisSessionStore {
    ...
    parseSentinelUrls() {
        return REDIS_SENTINEL_URLS.map((url) => {
    ...
    async storeSession(sessionId, context) {
        if (!this.client)
    ...
    async getSession(sessionId) {
        ...
        try {
            ...
            if (typeof context.createdAt === 'number')
                context.createdAt = new Date(context.createdAt);
            if (typeof context.lastActivity === 'number')
    ...
    // Audit event storage
    async storeAuditEvent(sessionId, event) {
        if (!this.client)
            throw new Error('Redis client not connected');
        try {
    ...
    async storeDeletionManifest(sessionId, manifest) {
        if (!this.client)
            throw new Error('Redis client not connected');
        try {
    ...
    async getDeletionManifest(sessionId) {
        if (!this.client)
            throw new Error('Redis client not connected');
        try {
            const key = `${SESSION_REDIS_NAMESPACE}:deletion:${sessionId}`;
    ...
    async storeShadowReplayArtifact(sessionId, artifact) {
        if (!this.client)
            throw new Error('Redis client not connected');
        try {
    ...
    async getShadowReplayArtifact(sessionId) {
        if (!this.client)
            throw new Error('Redis client not connected');
        try {
            const key = `${SESSION_REDIS_NAMESPACE}:shadow_replay:${sessionId}`;
