...
class MockWorkspaceStateCache {
  ...
  async loadState(workspaceId: string) {
  ...
  async getStats() {
    ...
    const timestamps = states.map((s: any) => s.timestamp).sort((a: number, b: number) => a - b);
    return {
...
class MockWorkspaceSwitcher {
  private cache = new MockWorkspaceStateCache();
  private activeWorkspaceIds = new Set<string>();
  private switchMetrics = new Map<string, any>();
  private onStateChange: any = null;

  async initialize() {
    await this.cache.initialize();
  }

  onWorkspaceStateChange(callback: any) {
    this.onStateChange = callback;
  }

  async pauseWorkspace(workspaceId: string, state: any) {
  ...
  async switchWorkspace(fromId: string, fromState: any, toId: string) {
  ...
  getPerformanceStats() {
    ...
    const times = metrics.map((m: any) => m.totalTime);
    const avgTime = times.reduce((a: number, b: number) => a + b) / times.length;
    const maxTime = Math.max(...times);
    const cacheHits = metrics.filter((m: any) => m.cacheHit).length;
...
const createSampleState = (workspaceId: string, overrides?: any): any => ({
  workspaceId,
...
describe('WorkspaceSwitcher', () => {
  let switcher: MockWorkspaceSwitcher;
  ...
  describe('Workspace Switching Latency (SLA < 200ms)', () => {
    it('should switch workspaces in under 200ms', async () => {
      ...
      console.log(
        `Switch latency: ${metrics.totalTime.toFixed(1)}ms (SLA: < 200ms)`
      );
    });

    it('should return consistent latency across multiple switches', async () => {
      const states = Array.from({ length: 5 }, (_, i) =>
        createSampleState(`ws-${i}`)
      );

      const latencies = [];

      for (let i = 0; i < states.length - 1; i++) {
        const metrics = await switcher.switchWorkspace(
          `ws-${i}`,
          states[i],
          `ws-${i + 1}`
        );
