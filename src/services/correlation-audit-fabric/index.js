...
import { AuditDecisionType, DecisionResult, } from "./types";
/**
 ...
 */
export class CorrelationAuditFabric {
    constructor(config) {
        this.traces = new Map(); // correlationId → trace
        this.events = []; // All events for querying
        this.parentLinks = new Map(); // childId → parentId
        this.stats = {
            totalEvents: 0,
    ...
    async recordDecision(event) {
        ...
        const typeKey = event.decisionType;
     ...
     */
    async startTrace(correlationId, metadata) {
        const trace = {
            correlationId,
            startedAt: Date.now(),
            complete: false,
     ...
     */
    async linkParentTrace(correlationId, parentCorrelationId) {
     ...
     */
    async completeTrace(correlationId, finalResult) {
        const trace = this.traces.get(correlationId);
        if (!trace)
            return;
        trace.complete = true;
     ...
     */
    async checkPrivilegedOperationAudit(op) {
     ...
     */
    async reconstructDecisionChain(correlationId) {
        const trace = this.traces.get(correlationId);
        if (!trace)
            return undefined;
        const decisions = {};
 ...
 */
export function createCorrelationAuditFabric(config) {
