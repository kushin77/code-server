...
describe('DeltaSyncService', () => {
  ...
  describe('Delta Computation (O(changes) not O(doc))', () => {
    beforeEach(() => {
      const ops: DeltaOperation[] = Array.from({ length: 10 }, (_, i) => ({
        clientId: 'client-1',
        clock: i + 1,
    ...
    it('should compute delta as operations not in remote vector', () => {
      const remoteVector: StateVector = { 'client-1': 5 }
    ...
    it('should return empty delta when fully synchronized', () => {
      const remoteVector: StateVector = { 'client-1': 10 }
      const delta = service.computeDelta('doc-1', remoteVector)
    ...
    it('should return all operations when remote is empty', () => {
      const remoteVector: StateVector = {}
      const delta = service.computeDelta('doc-1', remoteVector)
  ...
  describe('Multi-Client Synchronization', () => {
    it('should sync between two clients with different views', () => {
      ...
      const client1Ops: DeltaOperation[] = [
      ...
      const client2Vector: StateVector = { 'client-1': 2 }
  ...
  describe('Statistics & Monitoring', () => {
    beforeEach(() => {
      const ops: DeltaOperation[] = Array.from({ length: 100 }, (_, i) => ({
        clientId: 'client-1',
        clock: i + 1,
        type: 'insert' as const,
        position: i,
    ...
    it('should track sync statistics', () => {
      ...
      service.sync(
        { docId: 'doc-1', clientId: 'client-3', remoteVector: {} },
        'a'.repeat(100),
      )
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle very large operation counts', () => {
      const ops: DeltaOperation[] = Array.from({ length: 10000 }, (_, i) => ({
        clientId: 'client-1',
        clock: i + 1,
        type: 'insert' as const,
        position: i,
