...
export interface CodeLocation {
  ...
  endLine: number;
  functionName?: string;
  functionHash?: string; // Hash of function signature for tracking across refactors
}
...
export interface InlineComment {
  ...
  updatedAt?: Date;
  resolved: boolean;
  mentions: string[]; // @mentions in comment
}

/**
 * Represents a comment thread tied to source code
 */
...
/**
 * Archive entry for historical threads
 */
...
export class InlineCommunicationService extends EventEmitter {
  private threads = new Map<string, InlineCommentThread>();
  private threadsByLocation = new Map<string, Set<string>>();
  private threadArchive: ThreadArchive[] = [];
  private static instance: InlineCommunicationService;

  private constructor() {
    super();
  }

  static getInstance(): InlineCommunicationService {
   ...
   */
  resolveThread(threadId: string, resolvedBy: string): InlineCommentThread | null {
    const thread = this.threads.get(threadId);
    if (!thread) return null;
   ...
   */
  unresolveThread(threadId: string): InlineCommentThread | null {
    const thread = this.threads.get(threadId);
    if (!thread) return null;
   ...
   */
  updateComment(threadId: string, commentId: string, newContent: string): InlineComment | null {
    const thread = this.threads.get(threadId);
    if (!thread) return null;
   ...
   */
  getThreadsForLocation(codeLocation: CodeLocation): InlineCommentThread[] {
    const locationKey = this.getLocationKey(codeLocation);
   ...
   */
  getThreadsInFile(filePath: string): InlineCommentThread[] {
    return Array.from(this.threads.values()).filter(
      (t) => t.codeLocation.filePath === filePath && t.status === 'active'
    );
  }
   ...
   */
  getAllThreads(): InlineCommentThread[] {
   ...
   * Get thread by ID
   */
  getThread(threadId: string): InlineCommentThread | null {
  ...
  getThreadStatistics(): {
    ...
    const averageCommentCount =
      threads.length > 0 ? threads.reduce((sum, t) => sum + t.comments.length, 0) / threads.length : 0;
  ...
  getUserStatistics(userId: string): {
    ...
    threads.forEach((thread) => {
      thread.comments.forEach((comment) => {
        if (comment.authorId === userId) commentsAdded++;
        if (comment.mentions.includes(userId)) mentions++;
      });
    });
   ...
   */
  searchThreads(query: string): InlineCommentThread[] {
    const lowerQuery = query.toLowerCase();
    return Array.from(this.threads.values()).filter((thread) =>
      thread.comments.some((comment) => comment.content.toLowerCase().includes(lowerQuery))
    );
  }
   ...
   */
  handleCodeRefactor(oldLocation: CodeLocation, newLocation: CodeLocation): number {
   ...
   */
  exportThreads(): InlineCommentThread[] {
    return Array.from(this.threads.values());
  }
   ...
   */
  importThreads(threads: InlineCommentThread[]): number {
    threads.forEach((thread) => {
