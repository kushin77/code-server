...
describe('AIReviewerRouterService', () => {
    ...
    beforeEach(() => {
        ...
        service = new AIReviewerRouterService(mockPool);
    });
    afterEach(() => {
        vi.clearAllMocks();
    });
    ...
    describe('registerReviewerExpertise', () => {
        ...
        it('should reject invalid confidence', async () => {
            await expect(service.registerReviewerExpertise('reviewer1', 'src/**', 'expert', 101)).rejects.toThrow(/between 0 and 100/);
            await expect(service.registerReviewerExpertise('reviewer1', 'src/**', 'expert', -1)).rejects.toThrow(/between 0 and 100/);
        });
    ...
    describe('assignReview', () => {
        ...
        it('should reject when no suitable reviewers found', async () => {
            ...
            await expect(service.assignReview('pr-123', ['src/api.ts'], 'team-1')).rejects.toThrow(/No suitable reviewers found/);
        });
    });
    ...
    describe('completeReview', () => {
        it('should complete a review and update workload', async () => {
            ...
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE review_assignments SET completed_at = NOW()'), expect.arrayContaining(['assign-1']));
            expect(eventSpy).toHaveBeenCalled();
        });
...
//# sourceMappingURL=ai-reviewer-router.test.js.map