...
exports.CanaryDeploymentEngine = void 0;
class CanaryDeploymentEngine {
    ...
    async startCanaryDeployment(currentVersion, newVersion, canaryPercentage) {
    ...
    async increaseCanaryTraffic(deploymentId, newPercentage) {
    ...
    async completeCanaryPromotion(deploymentId) {
        const deployment = this.canaryDeployments.get(deploymentId);
        if (!deployment) {
    ...
    async evaluateCanaryHealth(deploymentId) {
        const deployment = this.canaryDeployments.get(deploymentId);
        if (!deployment) {
            throw new Error(`Canary deployment ${deploymentId} not found`);
        }
    ...
    async compareCanaryMetrics(baseline, canary) {
    ...
    async tryAutoProgressCanary(deploymentId) {
    ...
    async abortCanaryDeployment(deploymentId) {
        const deployment = this.canaryDeployments.get(deploymentId);
        if (deployment) {
    ...
    generateCanaryReport(deploymentId) {
        const deployment = this.canaryDeployments.get(deploymentId);
        if (!deployment) {
            throw new Error(`Canary deployment ${deploymentId} not found`);
        }
    ...
    generateHealthRecommendations(violations) {
        if (violations.length === 0)
            return ['Proceed with next canary stage'];
...
exports.CanaryDeploymentEngine = CanaryDeploymentEngine;
//# sourceMappingURL=CanaryDeploymentEngine.js.map