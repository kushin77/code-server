...
export class TeamHealthDashboard extends EventEmitter {
    ...
    async initialize() {
        ...
        try {
            ...
            if (this.config.enableRealTimeUpdates) {
                this.refreshTimer = setInterval(() => this.updateDashboard(), this.config.refreshIntervalSeconds * 1000);
            }
     ...
     */
    async getDashboardSnapshot(teamId) {
        const health = await this.getTeamHealth(teamId);
        const topRecommendations = this.getTopRecommendations(teamId, 5);
        const activeAlerts = Array.from(this.alerts.values()).filter((a) => a.teamId === teamId && !a.resolvedAt);
     ...
     */
    async getTopRecommendations(teamId, limit = 5, options) {
        const recommendations = Array.from(this.recommendations.values())
            ...
            if (options?.minConfidence && r.confidence < options.minConfidence)
                return false;
            if (options?.minImpact && r.estimatedBenefit < options.minImpact)
                return false;
            return true;
        })
            .sort((a, b) => {
     ...
     */
    async getHealthTrends(teamId, options) {
     ...
     */
    async compareTeams(teamId, options) {
     ...
     */
    async configureAlerts(teamId, thresholds) {
     ...
     */
    async evaluateAlerts(teamId) {
        const health = await this.getTeamHealth(teamId);
        const newAlerts = [];
        if (this.config.alertThresholds.highNotificationOverload &&
        ...
        if (this.config.alertThresholds.lowAsyncAdoption &&
        ...
        if (this.config.alertThresholds.highMeetingHeaviness &&
     ...
     * Configure widgets
     */
    async configureWidgets(widgets) {
    ...
    getTopRecommendations(teamId, limit) {
...
//# sourceMappingURL=team-health-dashboard.js.map