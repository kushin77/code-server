...
test.describe('OAuth Login Flow - Comprehensive Validation', () => {
    ...
    test.describe('Happy Path - Basic Login Flow', () => {
        ...
        test('session is accessible after login', async ({ authenticatedPage }) => {
            ...
            const response = await authenticatedPage.waitForResponse(response => response.url().includes(PORTAL_URL) && response.status() === 200);
    ...
    test.describe('Error Handling - Invalid Credentials', () => {
        test('non-whitelisted user gets clear error', async ({ page, vpnConnected }) => {
            ...
            const response = await page.goto(`${process.env.OAUTH2_PROXY_URL || 'http://localhost:4180'}/oauth2/callback?code=fake&state=fake`);
        ...
        test('expired oauth code is rejected', async ({ page }) => {
            ...
            const response = await page.goto(`${process.env.OAUTH2_PROXY_URL}/oauth2/callback?code=expired_code&state=test_state`, { waitUntil: 'networkidle' });
            // Should reject with error
            expect(response?.status()).toBeGreaterThanOrEqual(400);
        });
        test('missing oauth code parameter rejected', async ({ page }) => {
            const response = await page.goto(`${process.env.OAUTH2_PROXY_URL}/oauth2/callback?state=test_state`, { waitUntil: 'networkidle' });
            expect(response?.status()).toBeGreaterThanOrEqual(400);
        });
    });
    /**
    ...
    test.describe('Error Handling - CSRF Protection', () => {
        test('CSRF mismatch is blocked', async ({ page }) => {
            ...
            const response = await page.goto(`${process.env.OAUTH2_PROXY_URL}/oauth2/callback?code=test&state=mismatched_state`, { waitUntil: 'networkidle' });
            // oauth2-proxy should reject state mismatch
            expect(response?.status()).toBeGreaterThanOrEqual(400);
        });
        test('missing state parameter is blocked', async ({ page }) => {
            const response = await page.goto(`${process.env.OAUTH2_PROXY_URL}/oauth2/callback?code=test`, { waitUntil: 'networkidle' });
            expect(response?.status()).toBeGreaterThanOrEqual(400);
        });
        test('duplicate state usage is blocked', async ({ page }) => {
            // oauth2-proxy should use nonce or one-time state
            const state = 'test_state_123';
            await page.goto(`${process.env.OAUTH2_PROXY_URL}/oauth2/callback?code=test1&state=${state}`, { waitUntil: 'networkidle' });
            // Using same state again should fail
            const response2 = await page.goto(`${process.env.OAUTH2_PROXY_URL}/oauth2/callback?code=test2&state=${state}`, { waitUntil: 'networkidle' });
    ...
    test.describe('Error Handling - Session Errors', () => {
        ...
        test('network error during oauth callback handled gracefully', async ({ page }) => {
            ...
            const promise = page.goto(`${process.env.OAUTH2_PROXY_URL}/oauth2/callback?code=test&state=test`, { timeout: 5000 }).catch(() => null);
    ...
    test.describe('Edge Cases - Special Characters & Encoding', () => {
        ...
        test('state parameter with special characters is handled', async ({ page }) => {
            ...
            const response = await page.goto(`${process.env.OAUTH2_PROXY_URL}/oauth2/callback?code=test&state=${specialState}`, { waitUntil: 'networkidle' });
...
//# sourceMappingURL=oauth-login.spec.js.map