...
const hoisted = vi.hoisted(() => {
    ...
    let callback = null;
    const watch = vi.fn((_rootPath, _options, onChange) => {
        callback = onChange;
        return watcher;
    });
    return {
...
import { startRepositoryFileWatcher } from '../indexing';
describe('startRepositoryFileWatcher', () => {
    ...
    it('emits a file-read audit event before indexing a watched file', async () => {
        ...
        expect(hoisted.emitSpy).toHaveBeenCalledWith(expect.objectContaining({
            method: 'READ',
            path: 'src/demo.ts',
            resourceType: 'file',
            fileAction: 'read',
...
//# sourceMappingURL=indexing-watcher.test.js.map