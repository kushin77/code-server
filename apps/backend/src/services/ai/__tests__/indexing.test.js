...
import { RepositoryIndexer, chunkByTokenWindow, evaluateIncrementalIndexingLatency, formatRetrievalQualityPrometheus, inferLanguage, isIndexablePath, semanticBoundaries, } from "../indexing";
...
describe("RepositoryIndexer", () => {
    ...
    it("reindexes changed file and keeps queue depth stable", async () => {
        ...
        const changed = await indexer.processFileChange("backend/src/sample.ts", "export function a(){ return 2 }");
    ...
    it("throws when queue is full", async () => {
        ...
        await expect(indexer.reindexChangedFile({ path: "a.ts", content: "function x(){}" })).rejects.toThrow("Index queue is full");
    });
