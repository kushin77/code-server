...
import type { VaultCredential } from "../../services/ephemeral-credentials/vault-service.js"
  ...
  describe("List Response Formatting", () => {
    it("should format credential list without sensitive data", () => {
      ...
      safeList.forEach((item) => {
        ...
        expect((item as any).accessKey).toBeUndefined()
        expect((item as any).secretKey).toBeUndefined()
      })
    })
  })
    ...
    it("should calculate expiration status correctly", () => {
      ...
      const status = {
        ...
        minutesRemaining: Math.floor(
          (expiresAt.getTime() - issuedAt.getTime()) / 1000 / 60
        ),
      }
