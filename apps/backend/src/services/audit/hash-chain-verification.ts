...
export interface ChainVerificationResult {
  ...
  chainBreaks: Array<{ after: number; reason: string }>;
  errors: string[];
}

export interface TamperDetectionReport {
  timestamp: string;
  status: 'INTACT' | 'COMPROMISED' | 'INCOMPLETE_CHAIN';
...
/** Minimal DB interface for hash chain verification */
export interface HashChainDb {
...
export class HashChainVerificationService {
  private db: HashChainDb;

  constructor(db: HashChainDb) {
    this.db = db;
  }
  ...
  async verifyChainIntegrity(): Promise<ChainVerificationResult> {
    try {
      ...
      const result = await this.db.query(
        'SELECT id, user_id, action, resource, event_hash, previous_hash, created_at FROM audit_logs ORDER BY id ASC'
      );

      const entries: AuditLogEntry[] = result?.rows || [];
   ...
   */
  async generateTamperDetectionReport(): Promise<TamperDetectionReport> {
    const verificationResult = await this.verifyChainIntegrity();

    let status: 'INTACT' | 'COMPROMISED' | 'INCOMPLETE_CHAIN' = 'INTACT';
    const recommendations: string[] = [];

    if (verificationResult.chainBreaks.length > 0) {
      status = 'COMPROMISED';
      recommendations.push(
        'Audit log chain integrity compromised. Investigate immediately.',
        'Review all entries after first tamper point for unauthorized modifications.',
        'Escalate to security team for forensic analysis.',
        `Affected records: ${verificationResult.tamperedEntries.join(', ')}`
      );
    }
    ...
    if (!verificationResult.isValid && verificationResult.errors.length > 0) {
      ...
      recommendations.push(
        'Audit log chain validation encountered errors.',
        'Verify database integrity and availability.',
        'Restore from known-good backup if chain corruption suspected.'
      );
    }
   ...
   */
  async verifySingleEntry(entryId: number): Promise<{ isValid: boolean; reason?: string }> {
    try {
   ...
   */
  private _recalculateHash(entry: AuditLogEntry): string {
...
let _instance: HashChainVerificationService | null = null;

/**
 * Initialize the hash chain verification service.
 */
export function initHashChainVerificationService(db: HashChainDb): HashChainVerificationService {
 ...
 */
export function getHashChainVerificationService(): HashChainVerificationService | null {
  return _instance;
}
 ...
 */
export function resetHashChainVerificationService(): void {
  _instance = null;
}
