...
export interface WorkspaceProfile {
  ...
  debugConfigurations: Array<{
    ...
    args?: string[]
    cwd?: string
    [key: string]: any // Allow additional debug-config-specific properties
  }>
}

export class ProjectDetectionEngine {
  private readonly detectionRules = new Map<ProjectType, DetectionRule>()
  ...
  async detectProjectType(timeout = 2000): Promise<DetectionResult> {
  ...
  private calculateConfidence(rule: DetectionRule, detectedFiles: string[]): number {
    let confidence = 0
    ...
    if (rule.indicatorFiles.length > 0) {
      const indicatorsFound = rule.indicatorFiles.filter((f) =>
        detectedFiles.some((detected) => detected.includes(f.replace(/\/$/, "")))
      ).length
  ...
  getProfileForType(type: ProjectType): WorkspaceProfile {
    const profiles: Record<ProjectType, WorkspaceProfile> = {
...
// Helper interface for detection rules
interface DetectionRule {
  type: ProjectType
  priority: number
  requiredFiles: string[]
  indicatorFiles: string[]
  detectFn: (files: string[]) => boolean
}
