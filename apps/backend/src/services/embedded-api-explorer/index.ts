...
export class EmbeddedAPIExplorerService extends EventEmitter {
  private logger = getLogger('EmbeddedAPIExplorerService');
  private pool: Pool;
  ...
  async createRequest(userId: string, method: string, url: string, headers: Record<string, string>, body?: string, variables?: Record<string, any>): Promise<string> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `INSERT INTO api_requests (user_id, method, url, headers, body, variables)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING id`,
        [userId, method, url, JSON.stringify(headers), body, JSON.stringify(variables)]
      );
  ...
  async getRequest(requestId: string): Promise<APIRequest | null> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `SELECT id, user_id, method, url, headers, body, variables, created_at FROM api_requests WHERE id = $1`,
        [requestId]
      );
  ...
  async executeRequest(requestId: string, injectedVars?: Record<string, string>): Promise<APIResponse> {
    const client = await this.pool.connect();
    try {
      ...
      // Record response
      await client.query(
        `INSERT INTO api_responses (request_id, status_code, headers, body, execution_time_ms)
         VALUES ($1, $2, $3, $4, $5)`,
        [requestId, statusCode, JSON.stringify(finalHeaders), responseBody, executionTime]
      );
      ...
      if (requestData) {
        await client.query(
          `INSERT INTO api_request_history (user_id, request_id) VALUES ($1, $2)`,
  ...
  async importOpenAPI(userId: string, openApiSpec: any): Promise<string> {
    const client = await this.pool.connect();
    try {
      ...
      const result = await client.query(
        `INSERT INTO api_openapi_imports (user_id, openapi_spec, title, version)
         VALUES ($1, $2, $3, $4)
         RETURNING id`,
        [userId, JSON.stringify(openApiSpec), title, version]
      );
  ...
  async getResponses(requestId: string, limit: number = 10): Promise<APIResponse[]> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `SELECT id, request_id, status_code, headers, body, execution_time_ms, timestamp
         FROM api_responses
         WHERE request_id = $1
  ...
  async compareResponses(responseId1: string, responseId2: string): Promise<ResponseDiff> {
    const client = await this.pool.connect();
    try {
      // Fetch both responses
      const result1 = await client.query(
        `SELECT request_id, body FROM api_responses WHERE id = $1`,
        [responseId1]
      );
  ...
  private findKeys(obj: any, exclude: any): Record<string, any> {
  ...
  private findChanged(obj1: any, obj2: any): Record<string, any> {
    const result: Record<string, any> = {};
  ...
  async shareRequest(requestId: string, sharedByUserId: string, sharedWithUserId?: string, teamId?: string): Promise<void> {
    ...
    try {
      await client.query(
        `INSERT INTO api_shared_requests (request_id, shared_by_user_id, shared_with_user_id, team_id)
         VALUES ($1, $2, $3, $4)`,
        [requestId, sharedByUserId, sharedWithUserId, teamId]
      );
  ...
  async getSharedRequests(userId: string, teamId?: string): Promise<any[]> {
    const client = await this.pool.connect();
    try {
  ...
  async getRequestHistory(userId: string, limit: number = 50): Promise<any[]> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `SELECT h.executed_at, r.id, r.method, r.url
  ...
  async cleanupOldResponses(daysOld: number = 30): Promise<number> {
    const client = await this.pool.connect();
    try {
      ...
      const result = await client.query(
        `DELETE FROM api_responses WHERE timestamp < $1`,
        [cutoffDate]
      );
...
export async function initializeEmbeddedAPIExplorerRoutes(service: EmbeddedAPIExplorerService) {
  ...
  router.get('/api/shared-requests', async (req, res) => {
    try {
      ...
      const requests = await service.getSharedRequests(userId as string, teamId as string);
      res.json(requests);
    } catch (error) {
  ...
  router.get('/api/request-history', async (req, res) => {
    try {
      ...
      const history = await service.getRequestHistory(userId as string);
      res.json(history);
    } catch (error) {
