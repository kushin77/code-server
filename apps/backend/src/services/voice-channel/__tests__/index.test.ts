...
import { VoiceChannelConfig } from '../types';

describe('VoiceChannelService', () => {
  let pool: Pool;
  let redis: Redis;
  let voiceService: VoiceChannelService;
  ...
  describe('Session Management', () => {
    ...
    it('should join existing session', async () => {
      ...
      const result = await voiceService.joinSession(
        session.sessionId,
        'user-2',
        'Bob'
      );
    ...
    it('should fail to join non-existent session', async () => {
      await expect(
        voiceService.joinSession('non-existent', 'user-1', 'Alice')
  ...
  describe('Statistics', () => {
    it('should calculate voice statistics', async () => {
      const session1 = await voiceService.createSession(
        'workspace-123',
        'user-1',
        'Alice'
      );
      ...
      const session2 = await voiceService.createSession(
        'workspace-456',
        'user-3',
        'Charlie'
      );
  ...
  describe('Workspace Sessions', () => {
    it('should get all sessions for a workspace', async () => {
      ...
      expect(sessions123.every((s) => s.workspaceId === 'workspace-123')).toBe(
        true
      );
      expect(sessions456.every((s) => s.workspaceId === 'workspace-456')).toBe(
        true
      );
      ...
      expect(
        allSessions.map((s) => s.sessionId).includes(sess1.sessionId)
      ).toBe(true);
      expect(
        allSessions.map((s) => s.sessionId).includes(sess3.sessionId)
      ).toBe(true);
    });
  });
  ...
  describe('Error Handling', () => {
    it('should throw error for incomplete LiveKit config', () => {
      const badConfig: VoiceChannelConfig = {
        liveKitUrl: '',
        liveKitApiKey: '',
        liveKitApiSecret: '',
        noiseCancellationEnabled: true,
      ...
      expect(() => new VoiceChannelService(badConfig, pool, redis)).toThrow(
