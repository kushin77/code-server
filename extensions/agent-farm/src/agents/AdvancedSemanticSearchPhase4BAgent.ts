...
import { QueryUnderstanding, ExpandedQuery } from '../ml/QueryUnderstanding';
import { CrossEncoderReranker } from '../ml/CrossEncoderReranker';
import { MultiModalAnalyzer } from '../ml/MultiModalAnalyzer';

export class AdvancedSemanticSearchPhase4BAgent extends Agent {
    ...
    try {
      ...
      if (codeQuery) {
        ...
        if (codeQuery.constraints.length > 0) {
          recommendations.push(
            `Applied ${codeQuery.constraints.length} search constraints`
          );
        }
      }
      ...
      recommendations.push(
        `  • Test coverage: ${(modalities.tests.testCoverage * 100).toFixed(0)}%`
      );
      recommendations.push(
        `  • Documentation: ${modalities.documentation.commentDensity.toFixed(2)}%`
      );
      ...
      return this.formatOutput(
        `Advanced semantic analysis complete. ${recommendations.length} insights found.`,
        recommendations,
        recommendations.length > 5 ? 'info' : 'warning'
      );
    } catch (error) {
      return this.formatOutput(
        `Advanced search analysis failed: ${error}`,
        ['Check if ML components are properly initialized'],
        'error'
      );
    }
  }
   ...
   */
  private extractCodeQueryFromContext(code: string) {
    ...
    let type: 'search' | 'find' | 'analyze' | 'optimize' | 'refactor' | 'debug' | 'test' = 'analyze';
    if (query.includes('fix') || query.includes('error')) type = 'debug';
    if (query.includes('test')) type = 'test';
    if (query.includes('optimize') || query.includes('speed')) type = 'optimize';
    if (query.includes('refactor') || query.includes('clean')) type = 'refactor';
  ...
  private detectCodePatterns(code: string): string[] {
    ...
    if (
      code.match(/for\s*\(\s*let|for\s*\(\s*const|forEach|map|filter/g)?.length! >
      2
    ) {
    ...
    if (
      code.match(/const\s+\w+\s*=\s*memo|useMemo|useCallback|lru|memoize/g)
        ?.length! > 0
    ) {
   ...
   */
  private suggestOptimizations(modalities: any): string[] {
    ...
    if (
      modalities.patterns.asyncPatterns > 3 &&
    ...
    if (
      modalities.patterns.performanceOptimization === false &&
