...
export class FigmaIntegrationService extends EventEmitter {
    ...
    async fetchDesignTokens(fileKey) {
    ...
    async getCSSVariables(scope = 'global') {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT variable_name, variable_value, fallback_value, scope 
         FROM css_variables 
         WHERE scope = $1 
         ORDER BY variable_name`, [scope]);
    ...
    async setCSSVariable(variable) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO css_variables (variable_name, variable_value, fallback_value, scope)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (variable_name) DO UPDATE SET
           variable_value = $2, fallback_value = $3, scope = $4, updated_at = NOW()`, [variable.name, variable.value, variable.fallback || null, variable.scope]);
    ...
    async mapTokenToCSSVariable(tokenId, tokenName, cssVarName, cssValue) {
        ...
        try {
            ...
            const result = await client.query(`INSERT INTO token_mappings (figma_token_id, figma_token_name, css_variable_name, css_value, mismatch, mismatch_reason)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (figma_token_id, css_variable_name) DO UPDATE SET
           css_value = $4, mismatch = $5, mismatch_reason = $6, last_updated_at = NOW()
         RETURNING *`, [tokenId, tokenName, cssVarName, cssValue, mismatch, mismatchReason]);
    ...
    async getTokenMappings(includeMismatches = false) {
        const client = await this.pool.connect();
        try {
    ...
    async saveFigmaFrame(fileKey, frame) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO figma_frames (file_key, frame_id, frame_name, frame_type, figma_url, node_id, thumbnail_url, description)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
         ON CONFLICT (file_key, frame_id) DO UPDATE SET
           frame_name = $3, figma_url = $5, thumbnail_url = $7, description = $8, updated_at = NOW()`, [fileKey, frame.id, frame.name, frame.type, frame.url, frame.nodeId, frame.thumbnail, frame.description]);
    ...
    async getFigmaFrames(fileKey) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT id, frame_id, frame_name, frame_type, figma_url, node_id, thumbnail_url, description
         FROM figma_frames 
         WHERE file_key = $1 
         ORDER BY frame_name`, [fileKey]);
            return result.rows.map(row => ({
                id: row.id,
                name: row.frame_name,
                type: row.frame_type,
    ...
    async addFigmaComment(fileId, comment) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO figma_comments (file_id, frame_id, comment_id, author_name, author_id, content, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
    ...
    async getFigmaComments(fileId, frameId) {
        const client = await this.pool.connect();
        try {
    ...
    async addIDEComment(frameId, comment) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO ide_comments (frame_id, author, content, created_at, figma_comment_id)
         VALUES ($1, $2, $3, $4, $5)`, [frameId, comment.author, comment.content, comment.createdAt, comment.figmaCommentId || null]);
    ...
    async getIDEComments(frameId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT id, frame_id, author, content, created_at, figma_comment_id, synced_to_figma
         FROM ide_comments 
         WHERE frame_id = $1 
         ORDER BY created_at DESC`, [frameId]);
            return result.rows.map(row => ({
                id: row.id,
                frameId: row.frame_id,
                author: row.author,
    ...
    async syncCommentsFromFigma(fileId, frameId) {
        const client = await this.pool.connect();
        try {
    ...
    async createWebViewSession(userId, config) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`INSERT INTO webview_sessions (user_id, file_key, frame_id, config)
         VALUES ($1, $2, $3, $4)
         RETURNING id`, [userId, config.fileKey, config.frameId, JSON.stringify(config)]);
    ...
    async getWebViewConfig(sessionId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT config FROM webview_sessions WHERE id = $1`, [sessionId]);
    ...
    async generateWebViewHTML(config) {
