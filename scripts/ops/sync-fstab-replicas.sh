...
backup_fstab() {
    ...
    ssh "${DEPLOY_USER}@${host}" "sudo cp /etc/fstab $backup_file" || {
...
get_nas_mounts() {
    ...
    ssh "${DEPLOY_USER}@${host}" "grep -E '(192.168.168.56|mnt-|nas|eiq-shared)' /etc/fstab 2>/dev/null || true"
}
...
add_nas_entries() {
    ...
    while IFS= read -r entry; do
        ...
        if ssh "${DEPLOY_USER}@${host}" "grep -q '$mount_point' /etc/fstab 2>/dev/null"; then
            log_debug "Entry already exists: $mount_point"
        else
            # Append entry to fstab
            log_info "Adding entry: $mount_point"
            ssh "${DEPLOY_USER}@${host}" "echo '$entry' | sudo tee -a /etc/fstab > /dev/null"
        fi
    done <<< "$entries"
...
reload_mounts() {
    ...
    ssh "${DEPLOY_USER}@${host}" "sudo systemctl daemon-reload" || {
...
verify_mounts() {
    ...
    local mounts_output
    mounts_output=$(ssh "${DEPLOY_USER}@${host}" "mount | grep -E '(mnt-|nas|eiq)' | wc -l" 2>/dev/null || echo "0")
    ...
    ssh "${DEPLOY_USER}@${host}" "grep -E '(192.168.168.56|mnt-|nas|eiq-shared)' /etc/fstab 2>/dev/null || echo 'No NAS entries found'" | sed 's/^/  /'
}
...
main() {
    ...
    if ! ssh -o ConnectTimeout=5 "${DEPLOY_USER}@${PRIMARY_HOST}" "echo 'Connected' > /dev/null" 2>/dev/null; then
        log_fatal "Cannot connect to primary host ($PRIMARY_HOST)"
    fi
    log_success "✓ Connected to primary host"
    
    if ! ssh -o ConnectTimeout=5 "${DEPLOY_USER}@${REPLICA_HOST}" "echo 'Connected' > /dev/null" 2>/dev/null; then
