...
describe('SnapshotService', () => {
    ...
    it('should create snapshot', async () => {
        const snap = await service.createSnapshot('user1', 'user1@example.com', 'workspace1', 'session1', createTestSnapshot());
    ...
    it('should emit snapshot-created event', () => {
        return new Promise((resolve) => {
            ...
            service.createSnapshot('user1', 'user1@example.com', 'workspace1', 'session1', createTestSnapshot());
        });
    });
    it('should assign unique snapshot IDs', async () => {
        const snap1 = await service.createSnapshot('user1', 'user1@example.com', 'workspace1', 'session1', createTestSnapshot());
        await new Promise((resolve) => setTimeout(resolve, 1));
        const snap2 = await service.createSnapshot('user1', 'user1@example.com', 'workspace1', 'session1', createTestSnapshot());
    ...
    it('should track file count in metadata', async () => {
        const snap = await service.createSnapshot('user1', 'user1@example.com', 'workspace1', 'session1', {
            ...createTestSnapshot(),
            files: [
    ...
    it('should retrieve snapshot by ID', async () => {
        const created = await service.createSnapshot('user1', 'user1@example.com', 'workspace1', 'session1', createTestSnapshot());
    ...
    it('should emit snapshot-restored event', () => {
        return new Promise((resolve) => {
            service
                .createSnapshot('user1', 'user1@example.com', 'workspace1', 'session1', createTestSnapshot())
                .then((snap) => {
    ...
    it('should list snapshots for user', async () => {
        await service.createSnapshot('user1', 'user1@example.com', 'workspace1', 'session1', createTestSnapshot());
        await service.createSnapshot('user1', 'user1@example.com', 'workspace1', 'session1', createTestSnapshot());
    ...
    it('should list snapshots filtered by workspace', async () => {
        await service.createSnapshot('user1', 'user1@example.com', 'workspace1', 'session1', createTestSnapshot());
        await service.createSnapshot('user1', 'user1@example.com', 'workspace2', 'session1', createTestSnapshot());
    ...
    it('should compare snapshots', async () => {
        const snap1 = await service.createSnapshot('user1', 'user1@example.com', 'workspace1', 'session1', {
            ...createTestSnapshot(),
            files: [
                { path: '/src/a.ts', content: 'a', encoding: 'utf8', isModified: false, isUnsaved: false, lastModified: Date.now() },
            ],
        });
        const snap2 = await service.createSnapshot('user1', 'user1@example.com', 'workspace1', 'session1', {
            ...createTestSnapshot(),
            files: [
                { path: '/src/a.ts', content: 'a', encoding: 'utf8', isModified: false, isUnsaved: false, lastModified: Date.now() },
                { path: '/src/b.ts', content: 'b', encoding: 'utf8', isModified: false, isUnsaved: false, lastModified: Date.now() },
    ...
    it('should log audit entry for snapshot creation', async () => {
        await service.createSnapshot('user1', 'user1@example.com', 'workspace1', 'session1', createTestSnapshot(), '192.168.1.1', 'Mozilla/5.0');
    ...
    it('should track snapshots by user', async () => {
        await service.createSnapshot('user1', 'user1@example.com', 'workspace1', 'session1', createTestSnapshot());
        await service.createSnapshot('user2', 'user2@example.com', 'workspace1', 'session1', createTestSnapshot());
        const stats = await service.getStatistics();
    ...
    it('should limit to max versions', async () => {
        ...
        for (let i = 0; i < 5; i++) {
            await smallService.createSnapshot('user1', 'user1@example.com', 'workspace1', 'session1', createTestSnapshot());
        }
    ...
    it('should throw error if not initialized', async () => {
        const newService = new SnapshotService();
        await expect(newService.createSnapshot('user1', 'user1@example.com', 'workspace1', 'session1', createTestSnapshot())).rejects.toThrow();
    });
    ...
    it('should handle multiple users', async () => {
        for (let i = 1; i <= 5; i++) {
            await service.createSnapshot(`user${i}`, `user${i}@example.com`, 'workspace1', 'session1', createTestSnapshot());
        }
        const stats = await service.getStatistics();
...
//# sourceMappingURL=snapshot-service.test.js.map