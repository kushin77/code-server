...
const computeSessionProvenanceFingerprint = (manifest) => {
    const payload = {
...
const isDigestPinnedImage = (imageDigest) => /^sha256:[a-f0-9]{64}$/i.test(imageDigest);
const parseVerifiedAt = (verifiedAt) => {
...
export const buildSessionProvenanceManifest = (config) => {
    ...
    if (!isDigestPinnedImage(imageDigest)) {
        throw new SessionProvenancePolicyError(422, 'provenance_image_not_pinned', 'CODE_SERVER_IMAGE_ID must be a sha256 digest-pinned image reference');
    }
    if (!attestationRef) {
        throw new SessionProvenancePolicyError(422, 'provenance_attestation_missing', 'SESSION_PROVENANCE_ATTESTATION_REF is required');
    }
    if (!signerIdentity) {
        throw new SessionProvenancePolicyError(422, 'provenance_signer_missing', 'SESSION_PROVENANCE_SIGNER_IDENTITY is required');
    }
    if (!policyVersion) {
        throw new SessionProvenancePolicyError(422, 'provenance_policy_missing', 'SESSION_PROVENANCE_POLICY_VERSION is required');
    }
    if (!Number.isInteger(freshnessHours) || freshnessHours < 1) {
        throw new SessionProvenancePolicyError(422, 'provenance_freshness_invalid', 'SESSION_PROVENANCE_FRESHNESS_HOURS must be a positive integer');
    }
    if (verificationResult !== 'verified') {
        throw new SessionProvenancePolicyError(422, 'provenance_not_verified', 'Launch provenance verification must be verified before session launch');
    }
    ...
    if (expiresAt.getTime() <= Date.now()) {
        throw new SessionProvenancePolicyError(422, 'provenance_stale', 'Launch provenance verification has expired and must be refreshed before session launch');
    }
...
export const normalizeSessionProvenanceManifest = (manifest) => {
    if (!manifest) {
        return null;
    }
...
export const validateSessionProvenanceManifest = (manifest, expectedImageDigest) => {
    const normalized = normalizeSessionProvenanceManifest(manifest);
    if (!normalized) {
        throw new SessionProvenancePolicyError(422, 'provenance_manifest_invalid', 'Session provenance manifest is missing required fields');
    }
    if (normalized.manifestVersion !== 'v1') {
        throw new SessionProvenancePolicyError(422, 'provenance_manifest_version_unsupported', 'Session provenance manifest version is unsupported');
    }
    if (!isDigestPinnedImage(expectedImageDigest.trim())) {
        throw new SessionProvenancePolicyError(422, 'provenance_image_not_pinned', 'Trusted launch image must be pinned to a sha256 digest');
    }
    if (normalized.imageDigest !== expectedImageDigest.trim()) {
        throw new SessionProvenancePolicyError(422, 'provenance_image_mismatch', 'Session provenance manifest image digest does not match the trusted launch image digest');
    }
    ...
    if (providedFingerprint && providedFingerprint !== expectedFingerprint) {
        throw new SessionProvenancePolicyError(422, 'provenance_fingerprint_mismatch', 'Session provenance fingerprint does not match the canonical manifest fingerprint');
    }
...
export const resolveSessionProvenanceManifest = (config, manifest) => {
    if (manifest) {
...
//# sourceMappingURL=session-provenance.js.map