...
COMPOSE_FILE="-f docker-compose.yml -f docker-compose.enterprise.yml"
...
fetch_container_state() {
  ...
  local state=$(ssh $SSH_OPTS "$host" "cd $DEPLOY_DIR && docker ps --format '{{.Names}}\t{{.Image}}\t{{.Status}}' | grep -E '^code-server-|^hermes-|^purebliss-' | sort" 2>&1)
...
validate_compose_file() {
  ...
  if ! ssh $SSH_OPTS "$host" "cd $DEPLOY_DIR && bash -lc 'set -a; source .env; source .env.production 2>/dev/null; set +a; docker-compose $COMPOSE_FILE config >/dev/null'" 2>&1; then
...
cleanup_stale_containers() {
  ...
  stale_containers=$(ssh $SSH_OPTS "$host" "cd $DEPLOY_DIR && docker ps -a --format '{{.Names}}' --filter 'status=exited' | grep -E '^code-server-|^hermes-|^purebliss-' | sort" 2>/dev/null || true)
...
deploy_to_host() {
  ...
  if [[ "$MODE" == "dry-run" ]] || [[ "$MODE" == "validate" ]]; then
    ...
    ssh $SSH_OPTS "$host" "cd $DEPLOY_DIR && bash -lc 'set -a; source .env; source .env.production 2>/dev/null; set +a; docker-compose $COMPOSE_FILE config | head -50'"
  ...
  if [[ "$MODE" == "apply" ]]; then
    ...
    if ssh $SSH_OPTS "$host" "cd $DEPLOY_DIR && bash -lc 'set -a; source .env; source .env.production 2>/dev/null; set +a; docker-compose $COMPOSE_FILE up -d 2>&1'" > /tmp/deploy-output-$host.log 2>&1; then
