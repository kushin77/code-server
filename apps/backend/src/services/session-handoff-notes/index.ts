...
export type SessionState = 'done' | 'in-progress' | 'blocked' | 'next';
...
export class SessionHandOffNotesService extends EventEmitter {
  private logger = getLogger('SessionHandOffNotesService');
  private pool: Pool;
  ...
  async recordNote(sessionId: string, status: SessionState, note: string, nextAction: string | null = null): Promise<SessionHandOffNote> {
    const client = await this.pool.connect();
    try {
      ...
      const record: SessionHandOffNote = {
        id: row.id,
  ...
  async listNotes(sessionId: string): Promise<SessionHandOffNote[]> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `
  ...
  async generateHandOffDraft(sessionId: string): Promise<HandOffDraft> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `
      ...
      for (const row of result.rows) {
        if (row.status === 'done') done.push(row.note);
        if (row.status === 'in-progress') inProgress.push(row.note);
        if (row.status === 'blocked') blocked.push(row.note);
        if (row.status === 'next') next.push(row.next_action || row.note);
      }
  ...
  async getLatestDraft(sessionId: string): Promise<HandOffDraft | null> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `SELECT session_id, summary, draft, created_at FROM session_handoff_drafts WHERE session_id = $1`,
        [sessionId]
      );
  ...
  async markNotePosted(noteId: string): Promise<void> {
    const client = await this.pool.connect();
    try {
      await client.query(
        `UPDATE session_handoff_notes SET posted = TRUE WHERE id = $1`,
        [noteId]
      );
  ...
  async acknowledgeSession(sessionId: string, userId: string): Promise<void> {
    const client = await this.pool.connect();
    try {
      await client.query(
        `INSERT INTO session_handoff_acknowledgements (session_id, user_id) VALUES ($1, $2)`,
        [sessionId, userId]
      );
    } finally {
      client.release();
...
export async function initializeSessionHandOffNotesRoutes(service: SessionHandOffNotesService) {
