...
describe('Help Queue Service', () => {
    ...
    describe('Request Creation', () => {
        it('should create help request', async () => {
            const request = await service.createRequest('user-alice', 'alice@example.com', 'debugging', 'Debugger not stopping at breakpoints', 'My debugger in VSCode is not stopping at breakpoints...', 'high', 'const x = 1;', 'typescript', 'workspace-1', 'workspace-1', 'session-1', '192.168.1.1', 'Chrome/95.0');
        ...
        it('should create request with different types', async () => {
            ...
            for (const type of types) {
                const request = await service.createRequest('user-alice', 'alice@example.com', type, `${type} request`, 'Description', 'normal');
        ...
        it('should emit request-created event', async () => {
            return new Promise((resolve) => {
                ...
                service.createRequest('user-alice', 'alice@example.com', 'debugging', 'Issue', 'Description', 'normal');
            });
        });
        ...
        it('should include code context in request', async () => {
            const request = await service.createRequest('user-alice', 'alice@example.com', 'debugging', 'Code issue', 'Not working', 'normal', 'function test() { throw new Error(); }', 'typescript', 'Line 42 in auth.ts');
    ...
    describe('Expert Registration', () => {
        it('should register expert', async () => {
            const expert = await service.registerExpert('expert-bob', 'bob@example.com', 'Bob Developer', 'expert', ['typescript', 'debugging', 'performance'], '192.168.1.2', 'Firefox/90.0');
        ...
        it('should emit expert-registered event', async () => {
            return new Promise((resolve) => {
                ...
                service.registerExpert('expert-bob', 'bob@example.com', 'Bob', 'expert', ['typescript']);
            });
        });
        it('should register experts with different expertise levels', async () => {
            const levels = ['beginner', 'intermediate', 'expert', 'architect'];
            for (const level of levels) {
                const expert = await service.registerExpert(`expert-${level}`, `${level}@example.com`, `${level} Expert`, level, ['typescript']);
    ...
    describe('Request Retrieval', () => {
        it('should get request by ID', async () => {
            const created = await service.createRequest('user-alice', 'alice@example.com', 'debugging', 'Issue', 'Description');
        ...
        it('should get expert by ID', async () => {
            const created = await service.registerExpert('expert-bob', 'bob@example.com', 'Bob', 'expert', ['typescript']);
    ...
    describe('Request Assignment', () => {
        it('should assign request to expert', async () => {
            const request = await service.createRequest('user-alice', 'alice@example.com', 'debugging', 'Issue', 'Description');
            const expert = await service.registerExpert('expert-bob', 'bob@example.com', 'Bob', 'expert', ['typescript']);
        ...
        it('should emit request-assigned event', async () => {
            return new Promise((resolve) => {
                ...
                service.createRequest('user-alice', 'alice@example.com', 'debugging', 'Issue', 'Desc')
                    .then((req) => service.registerExpert('expert-bob', 'bob@example.com', 'Bob', 'expert', ['ts'])
    ...
    describe('Request Responses', () => {
        it('should add response to request', async () => {
            const request = await service.createRequest('user-alice', 'alice@example.com', 'debugging', 'Issue', 'Desc');
            const response = await service.addResponse(request.id, 'expert-bob', 'bob@example.com', 'expert', 'Here is the fix...', 'const fix = true;', 'typescript', false, '192.168.1.2', 'Chrome/95.0');
        ...
        it('should emit response-added event', async () => {
            return new Promise((resolve) => {
                ...
                service.createRequest('user-alice', 'alice@example.com', 'debugging', 'Issue', 'Desc')
                    .then((req) => service.addResponse(req.id, 'expert-bob', 'bob@example.com', 'expert', 'Fix it'));
            });
        });
    ...
    describe('Request Resolution', () => {
        it('should resolve request', async () => {
            ...
            const expert = await service.registerExpert('expert-bob', 'bob@example.com', 'Bob', 'expert', ['ts']);
            await service.assignRequest(request.id, expert.id, 'admin', 'admin@example.com');
            const resolved = await service.resolveRequest(request.id, expert.id, 'bob@example.com', 'Fixed by updating TypeScript version');
        ...
        it('should emit request-resolved event', async () => {
            return new Promise((resolve) => {
                ...
                service.createRequest('user-alice', 'alice@example.com', 'debugging', 'Issue', 'Desc')
                    .then((req) => service.registerExpert('expert-bob', 'bob@example.com', 'Bob', 'expert', ['ts'])
                    .then((exp) => service.assignRequest(req.id, exp.id, 'admin', 'admin@example.com')
                    .then(() => service.resolveRequest(req.id, exp.id, 'bob@example.com', 'Fixed'))));
            });
        });
    ...
    describe('Request Rating', () => {
        it('should rate resolved request', async () => {
            ...
            const rated = await service.rateResolution(request.id, 5, 'Great help, issue resolved quickly!', '192.168.1.1', 'Chrome/95.0');
        ...
        it('should reject rating for non-resolved request', async () => {
            const request = await service.createRequest('user-alice', 'alice@example.com', 'debugging', 'Issue', 'Desc');
            await expect(service.rateResolution(request.id, 5, 'Good')).rejects.toThrow('Can only rate resolved requests');
        });
    ...
    describe('Audit Logging', () => {
        it('should log request creation', async () => {
            await service.createRequest('user-alice', 'alice@example.com', 'debugging', 'Issue', 'Description', 'high', undefined, undefined, undefined, 'workspace-1', 'session-1', '192.168.1.1', 'Chrome');
        ...
        it('should include IP and user agent in audit log', async () => {
            ...
            await service.createRequest('user-alice', 'alice@example.com', 'debugging', 'Issue', 'Description', 'normal', undefined, undefined, undefined, 'workspace-1', 'session-1', ipAddress, userAgent);
            const auditLog = await service.getAuditLog('user-alice', 1);
    ...
    describe('Integration', () => {
        it('should handle complete help queue workflow', async () => {
            ...
            const request = await service.createRequest('user-alice', 'alice@example.com', 'debugging', 'Debugger issue', 'Not stopping at breakpoints');
            expect(request).toBeDefined();
            // 2. Register expert
            const expert = await service.registerExpert('expert-bob', 'bob@example.com', 'Bob', 'expert', ['debugging', 'typescript']);
            expect(expert).toBeDefined();
            // 3. Assign request
            const assigned = await service.assignRequest(request.id, expert.id, 'admin', 'admin@example.com');
            expect(assigned.status).toBe('assigned');
            // 4. Expert responds
            await service.addResponse(request.id, 'expert-bob', 'bob@example.com', 'expert', 'Try updating TypeScript...');
            // 5. Resolve request
            const resolved = await service.resolveRequest(request.id, expert.id, 'bob@example.com', 'Fixed by upgrading TypeScript');
            expect(resolved.status).toBe('resolved');
            // 6. Rate resolution
...
//# sourceMappingURL=help-queue-service.test.js.map