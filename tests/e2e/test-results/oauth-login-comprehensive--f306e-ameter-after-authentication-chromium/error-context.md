...
- Name: oauth-login-comprehensive.spec.js >> OAuth Login Comprehensive (#986) >> Happy Path - Login Flow >> 2: login preserves rd (redirect) parameter after authentication
- Location: tests\e2e\specs\oauth-login-comprehensive.spec.ts:40:9
    ...
    - generic [ref=e4]:
      ...
      - main [ref=e14]:
        - generic [ref=e15]:
          ...
          - generic [ref=e26]:
            - heading "Welcome" [level=1] [ref=e27]
            - generic [ref=e31]:
              - img [ref=e33]
              - generic [ref=e36]: qa@kushnir.cloud
        - generic [ref=e45]:
          - generic [ref=e53]:
            - textbox "Enter your password" [active] [ref=e54]
            - generic: Enter your password
          - generic [ref=e57]:
            - img [ref=e59]
            - generic [ref=e61]: Enter a password
          - generic [ref=e66]:
            - generic [ref=e69] [cursor=pointer]:
              - checkbox "Show password" [ref=e70]
              - generic:
                - img
            - generic [ref=e73] [cursor=pointer]: Show password
        - generic [ref=e75]:
          - button "Next" [ref=e79] [cursor=pointer]:
            - generic [ref=e82]: Next
          - button "Forgot password?" [ref=e86]:
            - generic [ref=e89]: Forgot password?
    - contentinfo [ref=e93]:
      - combobox "Change language English (United States)" [ref=e97] [cursor=pointer]:
        - generic:
...
> 55  |       await page.waitForURL(new RegExp(`.*/admin`), { timeout: 15000 });
  ...
  121 |       
  122 |       // Navigate within same origin
  123 |       await page.goto(`${PORTAL_URL}/health`);
  124 |       
  125 |       // Session should still be valid
  126 |       const cookies2 = await context.cookies();
  127 |       const oauth2 = cookies2.find(c => c.name === '_oauth2_proxy');
  128 |       expect(oauth2).toBeDefined();
  129 |       expect(oauth2?.value).toBe(oauth1?.value);
  130 |     });
  131 | 
  132 |     test('6: refresh token extends session (if applicable)', async ({ page, context }) => {
  133 |       // This test may not apply if oauth2-proxy uses simple cookie sessions
  134 |       // Skip if no refresh token mechanism
  135 |       
  136 |       await page.goto(PORTAL_URL);
  137 |       await page.waitForURL(/accounts\.google\.com/);
  138 |       await page.fill('input[type="email"]', QA_EMAIL);
  139 |       await page.click('#identifierNext, button:has-text("Next")');
  140 |       await page.waitForSelector('input[type="password"]');
  141 |       await page.fill('input[type="password"]', QA_PASSWORD);
  142 |       await page.click('#passwordNext, button:has-text("Next")');
  143 |       await page.waitForURL(new RegExp(`^${PORTAL_URL}`), { timeout: 15000 });
  144 |       
  145 |       const cookies = await context.cookies();
  146 |       const refreshToken = cookies.find(c => c.name === '_oauth2_refresh');
  147 |       
  148 |       if (refreshToken) {
  149 |         expect(refreshToken).toBeDefined();
  150 |         expect(refreshToken?.value).toBeTruthy();
  151 |       }
  152 |     });
  153 | 
  154 |     test('7: session valid across subdomains (kushnir.cloud and ide.kushnir.cloud)', async ({ page, context }) => {
  155 |       // Login to portal
```