...
describe('CalendarIntegrationService', () => {
    ...
    it('connects a calendar integration', async () => {
        ...
        const integration = await service.connectIntegration('user-1', 'google', 'user@example.com', 'access-token', 'refresh-token', ['calendar.readonly']);
    ...
    it('records a free busy window', async () => {
        mockClient.query.mockResolvedValueOnce({ rowCount: 1 });
        const window = await service.recordFreeBusyWindow('user-1', 'outlook', new Date('2026-04-22T13:00:00Z'), new Date('2026-04-22T14:00:00Z'), 'Design review', true);
    ...
    it('gets free busy windows', async () => {
        ...
        const windows = await service.getFreeBusyWindows('user-1', new Date('2026-04-22T00:00:00Z'), new Date('2026-04-23T00:00:00Z'));
