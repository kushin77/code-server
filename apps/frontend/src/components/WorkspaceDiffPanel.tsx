...
import { fetchLatestWorkspaceDiff, refreshWorkspaceDiff, type WorkspaceDiffSnapshot } from '../utils/workspaceDiff'

export type WorkspaceDiffPanelProps = {
  workspaceId: string
  repoPath: string
  workspaceLabel: string
}

export function WorkspaceDiffPanel({ workspaceId, repoPath, workspaceLabel }: WorkspaceDiffPanelProps) {
  const [snapshot, setSnapshot] = useState<WorkspaceDiffSnapshot | null>(null)
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        ...
        <button
          ...
          disabled={refreshing}
          className="rounded-full border border-cyan-300 px-3 py-2 text-sm font-medium text-cyan-800 transition hover:border-cyan-400 hover:bg-cyan-50 disabled:cursor-not-allowed disabled:opacity-60"
        >
      ...
      <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        {loading ? (
          <p className="text-sm text-slate-600">Loading the latest snapshot…</p>
        ) : snapshot ? (
          <>
            ...
            {snapshot.changedFiles.length > 0 ? (
              <ul className="mt-4 space-y-2">
                {snapshot.changedFiles.map((file) => (
                  <li key={`${file.status}:${file.path}`} className="rounded-xl border border-slate-200 bg-white px-3 py-2">
              ...
              <p className="mt-3 text-sm text-slate-600">No file-level changes were recorded in the latest snapshot.</p>
            )}
          </>
        ) : (
