...
export class SmartNotificationRoutingService extends EventEmitter {
    constructor(pool, config = {}) {
        super();
        this.logger = getLogger('SmartNotificationRoutingService');
    ...
    async updateUserStatus(userId, status, context) {
        const client = await this.pool.connect();
        try {
            const existing = await client.query('SELECT id FROM user_status WHERE user_id = $1', [userId]);
            if (existing.rows.length === 0) {
                await client.query(`INSERT INTO user_status (user_id, current_status, location, calendar_status, current_device)
           VALUES ($1, $2, $3, $4, $5)`, [userId, status, context?.location || null, context?.calendarStatus || null, context?.currentDevice || null]);
            }
            else {
                await client.query(`UPDATE user_status SET current_status = $1, last_status_change = NOW(),
                                  location = COALESCE($2, location),
                                  calendar_status = COALESCE($3, calendar_status),
                                  current_device = COALESCE($4, current_device),
                                  updated_at = NOW()
           WHERE user_id = $5`, [status, context?.location, context?.calendarStatus, context?.currentDevice, userId]);
            }
    ...
    async getUserStatus(userId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query('SELECT * FROM user_status WHERE user_id = $1', [userId]);
    ...
    async setNotificationRoute(userId, priority, channels, conditions) {
        const client = await this.pool.connect();
        try {
            const id = require('crypto').randomUUID();
            await client.query(`INSERT INTO notification_routes (id, user_id, priority, channels, conditions)
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT (user_id, priority) DO UPDATE SET channels = $4, conditions = $5, updated_at = NOW()`, [id, userId, priority, channels, conditions ? JSON.stringify(conditions) : null]);
    ...
    async getNotificationRoute(userId, priority) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT * FROM notification_routes WHERE user_id = $1 AND priority = $2`, [userId, priority]);
            if (result.rows.length === 0) {
                return null;
            }
            const row = result.rows[0];
    ...
    async getUserRoutes(userId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT * FROM notification_routes WHERE user_id = $1 ORDER BY priority`, [userId]);
    ...
    async routeNotification(userId, notificationId, content, priority = 'normal') {
        const client = await this.pool.connect();
        try {
            // Get user status
            const statusResult = await client.query('SELECT current_status FROM user_status WHERE user_id = $1', [userId]);
            const userStatus = statusResult.rows[0]?.current_status || 'offline';
            // Get routing config
            const routeResult = await client.query(`SELECT channels, conditions FROM notification_routes WHERE user_id = $1 AND priority = $2 AND is_active = true`, [userId, priority]);
            let channels = ['ide', 'slack', 'matrix'];
            ...
            const dedupResult = await client.query(`SELECT last_delivery_at FROM notification_dedup_cache WHERE user_id = $1 AND notification_hash = $2`, [userId, contentHash]);
            const deliveries = [];
            if (dedupResult.rows.length === 0 || (Date.now() - new Date(dedupResult.rows[0].last_delivery_at).getTime()) > this.config.deduplicationWindow) {
                ...
                for (const channel of channels) {
                    ...
                    await client.query(`INSERT INTO notification_deliveries (id, user_id, notification_id, content, channel, status, max_attempts)
             VALUES ($1, $2, $3, $4, $5, 'pending', $6)`, [deliveryId, userId, notificationId, content, channel, this.config.maxDeliveryAttempts]);
                    deliveries.push({
                        id: deliveryId,
                        userId,
                        notificationId,
                ...
                // Update dedup cache
                await client.query(`INSERT INTO notification_dedup_cache (user_id, notification_hash, last_delivery_at)
           VALUES ($1, $2, NOW())
           ON CONFLICT (user_id, notification_hash) DO UPDATE SET last_delivery_at = NOW()`, [userId, contentHash]);
    ...
    async recordDelivery(deliveryId, success, failureReason) {
        const client = await this.pool.connect();
        try {
            await client.query('BEGIN');
            const getResult = await client.query('SELECT status, attempt_count, max_attempts FROM notification_deliveries WHERE id = $1', [deliveryId]);
         ...
         WHERE id = $5`, [newStatus, newAttemptCount, success ? new Date() : null, failureReason || null, deliveryId]);
            // Record history
            await client.query(`INSERT INTO notification_delivery_history (delivery_id, status_from, status_to, reason, attempt_number)
         VALUES ($1, $2, $3, $4, $5)`, [deliveryId, delivery.status, newStatus, failureReason, newAttemptCount]);
            await client.query('COMMIT');
    ...
    async markAsDelivered(deliveryId) {
        const client = await this.pool.connect();
        try {
            await client.query(`UPDATE notification_deliveries SET status = 'delivered', delivered_at = NOW(), updated_at = NOW()
    ...
    async markAsRead(deliveryId) {
        const client = await this.pool.connect();
        try {
            await client.query(`UPDATE notification_deliveries SET status = 'read', read_at = NOW(), updated_at = NOW()
         WHERE id = $1`, [deliveryId]);
    ...
    async getPendingDeliveries(limit = 10) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT * FROM notification_deliveries WHERE status = 'pending' AND attempt_count < max_attempts
    ...
    async getDeliveryHistory(deliveryId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT * FROM notification_delivery_history WHERE delivery_id = $1 ORDER BY created_at ASC`, [deliveryId]);
            return result.rows.map(row => ({
                id: row.id,
