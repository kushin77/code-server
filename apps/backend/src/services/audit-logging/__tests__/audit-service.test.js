...
describe('Audit Logging Service', () => {
    ...
    beforeEach(async () => {
        ...
        // Set up user context
        auditService.setUserContext('user-alice', 'alice@example.com', 'user');
        auditService.setUserContext('user-bob', 'bob@example.com', 'expert');
        auditService.setUserContext('user-admin', 'admin@example.com', 'admin');
    });
    ...
    describe('Basic Audit Logging', () => {
        it('should log generic events', async () => {
            const entryId = await auditService.logEvent('ws-test', 'user-alice', 'CREATE', 'MENTION', 'mention-123', { context: 'test' });
        ...
        it('should preserve immutability markers', async () => {
            const entryId = await auditService.logEvent('ws-test', 'user-alice', 'CREATE', 'MENTION', 'mention-123');
            const entry = await auditService.getEntry(entryId);
        ...
        it('should track error results', async () => {
            const entryId = await auditService.logEvent('ws-test', 'user-alice', 'DELETE', 'MENTION', 'mention-456', {}, 'FAILURE', 'Permission denied');
            const entry = await auditService.getEntry(entryId);
    ...
    describe('Mention Audit Events', () => {
        it('should log mention creation', async () => {
            ...
            const entryId = await auditService.logMentionEvent('ws-test', event, 'session-123');
            const entry = await auditService.getEntry(entryId);
            expect(entry?.action).toBe('CREATE');
        ...
        it('should log mention association', async () => {
            ...
            const entryId = await auditService.logMentionEvent('ws-test', event);
            const entry = await auditService.getEntry(entryId);
    ...
    describe('Querying Audit Logs', () => {
        beforeEach(async () => {
            ...
            await auditService.logMentionEvent('ws-1', {
                action: 'CREATE',
                mentionId: 'mention-a',
                mentionText: 'test1',
                userId: 'user-alice',
            ...
            await auditService.logEvent('ws-1', 'user-admin', 'DELETE', 'MENTION', 'mention-c');
        });
    ...
    describe('Compliance Reporting', () => {
        beforeEach(async () => {
            ...
            for (let i = 0; i < 10; i++) {
                await auditService.logEvent('ws-comply', 'user-alice', 'CREATE', 'MENTION', `mention-${i}`, { index: i });
            }
            // Add some failures
            for (let i = 0; i < 3; i++) {
                await auditService.logEvent('ws-comply', 'user-bob', 'DELETE', 'MENTION', `mention-fail-${i}`, {}, 'FAILURE', 'Unauthorized');
            }
        });
        ...
        it('should generate compliance report', async () => {
            ...
            const report = await auditService.generateComplianceReport('ws-comply', startTime, endTime);
        ...
        it('should identify suspicious patterns', async () => {
            ...
            for (let i = 0; i < 50; i++) {
                await auditService.logEvent('ws-suspicious', 'user-alice', 'DELETE', 'MENTION', `mention-${i}`, {}, 'FAILURE', 'Access denied');
            }
            const report = await auditService.generateComplianceReport('ws-suspicious', Date.now() - 86400000, Date.now());
            // Should flag high failure rate
            expect(report.securityEvents.suspiciousPatterns.length).toBeGreaterThan(0);
        });
        it('should track top actions in report', async () => {
            const report = await auditService.generateComplianceReport('ws-comply', Date.now() - 86400000, Date.now());
    ...
    describe('Export Functionality', () => {
        beforeEach(async () => {
            for (let i = 0; i < 5; i++) {
                await auditService.logEvent('ws-export', 'user-alice', 'CREATE', 'MENTION', `mention-${i}`);
            }
        });
    ...
    describe('Integrity Validation', () => {
        beforeEach(async () => {
            for (let i = 0; i < 5; i++) {
                await auditService.logEvent('ws-integrity', 'user-alice', 'CREATE', 'MENTION', `mention-${i}`);
            }
        });
        it('should validate log integrity', async () => {
            const result = await auditService.validateIntegrity('ws-integrity');
    ...
    describe('Event Emission', () => {
        it('should emit audit-logged event', async () => {
            return new Promise((resolve) => {
                ...
                auditService.logEvent('ws-test', 'user-alice', 'CREATE', 'MENTION', 'mention-123');
            });
        });
    });
    ...
    describe('Integration', () => {
        it('should handle complete audit workflow', async () => {
            // Create mention
            const createId = await auditService.logMentionEvent('ws-full', {
                action: 'CREATE',
            ...
            // Associate
            const assocId = await auditService.logMentionEvent('ws-full', {
                action: 'ASSOCIATE',
                mentionId: 'mention-full-1',
                mentionText: '@alice check this',
                userId: 'user-alice',
            ...
            // Generate report
            const report = await auditService.generateComplianceReport('ws-full', Date.now() - 86400000, Date.now());
