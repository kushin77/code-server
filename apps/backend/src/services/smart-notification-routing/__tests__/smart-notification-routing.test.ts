...
describe('SmartNotificationRoutingService', () => {
  let service: SmartNotificationRoutingService;
  let mockClient: any;
  ...
  describe('initialization', () => {
    it('should initialize with database schema', async () => {
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('CREATE TABLE IF NOT EXISTS user_status')
      );
    });
  ...
  describe('updateUserStatus', () => {
    ...
    it('should update existing user status', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('UPDATE user_status'),
        expect.any(Array)
      );
    });
    ...
    it('should include optional context fields', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('UPDATE user_status'),
        expect.arrayContaining(['meeting-room-1', 'busy', 'mobile'])
      );
    });
  });
  ...
  describe('setNotificationRoute', () => {
    it('should create notification route', async () => {
      ...
      const route = await service.setNotificationRoute(
        'user-1',
        'urgent',
        ['ide', 'slack']
      );
    ...
    it('should include routing conditions', async () => {
      ...
      const conditions = [
        { status: 'online', channels: ['ide'] as any[] },
        { status: 'away', channels: ['slack'] as any[] },
      ];
