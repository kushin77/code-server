...
export class ActivityFeedService extends EventEmitter {
    constructor(pool, auditService, config = {}) {
        super();
        this.logger = getLogger('ActivityFeedService');
...
Promise < Activity > {
    ...
    try: {
        ...
        const: now = new Date(),
        await, client, : .query(`INSERT INTO activities (id, type, status, title, description, deep_link, user_id, repository, metadata, tags)
        ...
        await, client, : .query(`INSERT INTO activity_search_index (activity_id, search_text) VALUES ($1, $2)`, [id, searchText]),
...
async;
getActivities(filter ?  : ActivityFilter, limit, number = 50);
Promise < Activity[] > {
    const: client = await this.pool.connect(),
    try: {
...
async;
getStats(filter ?  : ActivityFilter);
...
async;
createSubscription(userId, string, filters, ActivityFilter);
Promise < ActivitySubscription > {
    const: client = await this.pool.connect(),
    try: {
        const: id = require('crypto').randomUUID(),
        await, client, : .query(`INSERT INTO activity_subscriptions (id, user_id, filters)
         VALUES ($1, $2, $3)`, [id, userId, JSON.stringify(filters)]),
...
Promise < ActivitySubscription | null > {
    const: client = await this.pool.connect(),
    try: {
        const: result = await client.query('SELECT * FROM activity_subscriptions WHERE id = $1', [subscriptionId]),
        if(result) { }, : .rows.length === 0
    }
};
{
...
Promise < ActivitySubscription[] > {
    const: client = await this.pool.connect(),
    try: {
        const: result = await client.query('SELECT * FROM activity_subscriptions WHERE user_id = $1 ORDER BY created_at DESC', [userId]),
        return: result.rows.map(row => ({
            id: row.id,
            userId: row.user_id,
            filters: this.normalizeFilters(row.filters),
...
Promise < void  > {
    const: client = await this.pool.connect(),
    try: {
        await, client, : .query('DELETE FROM activity_subscriptions WHERE id = $1', [subscriptionId]),
...
async;
notifySubscribers(activityId, string);
Promise < number > {
    const: client = await this.pool.connect(),
    try: {
        ...
        const: result = await client.query(`SELECT id, filters FROM activity_subscriptions WHERE is_active = true`),
        let, notifiedCount = 0,
        for(, subscription, of, result) { }, : .rows
    }
};
{
    ...
    if (this.matchesFilter(activity, filters)) {
        await client.query(`INSERT INTO activity_notifications (activity_id, subscription_id) VALUES ($1, $2)`, [activityId, subscription.id]);
        notifiedCount++;
    }
}
...
matchesFilter(activity, Activity, filter, ActivityFilter);
boolean;
{
    if (filter.types && !filter.types.includes(activity.type))
        return false;
    if (filter.statuses && !filter.statuses.includes(activity.status))
        return false;
    if (filter.userId && activity.userId !== filter.userId)
        return false;
    if (filter.repository && activity.repository !== filter.repository)
        return false;
    if (filter.tags && (!activity.tags || !activity.tags.some(t => filter.tags?.includes(t))))
        return false;
    return true;
}
normalizeJsonObject(value, unknown);
(Record) | undefined;
{
    if (!value) {
        return undefined;
...
normalizeFilters(value, unknown);
ActivityFilter;
{
    return this.normalizeJsonObject(value) || {};
}
async;
getPendingNotifications(subscriptionId, string, limit, number = 10);
Promise < Activity[] > {
    const: client = await this.pool.connect(),
    try: {
        const: result = await client.query(`SELECT a.* FROM activities a
         INNER JOIN activity_notifications n ON a.id = n.activity_id
         WHERE n.subscription_id = $1 AND n.read_at IS NULL
         ORDER BY a.timestamp DESC LIMIT $2`, [subscriptionId, limit]),
        return: result.rows.map(row => ({
            id: row.id,
            type: row.type,
            status: row.status,
...
async;
markNotificationAsRead(activityId, string, subscriptionId, string);
Promise < void  > {
    const: client = await this.pool.connect(),
    try: {
        await, client, : .query(`UPDATE activity_notifications SET read_at = NOW()
         WHERE activity_id = $1 AND subscription_id = $2`, [activityId, subscriptionId]),
