...
export class DependencyImpactGraphService extends EventEmitter {
    constructor(pool) {
        super();
        this.logger = getLogger('DependencyImpactGraphService');
        this.pool = pool;
    }
    async initialize() {
    ...
    async addModule(moduleId, moduleName, filePath, sizeBytes) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO dependency_modules (module_id, module_name, file_path, size_bytes)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (module_id) DO UPDATE SET module_name = $2, last_updated = CURRENT_TIMESTAMP`, [moduleId, moduleName, filePath, sizeBytes]);
    ...
    async addDependency(sourceModuleId, targetModuleId, type = 'import') {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO module_dependencies (source_module_id, target_module_id, dependency_type)
         VALUES ($1, $2, $3)
         ON CONFLICT (source_module_id, target_module_id) DO UPDATE SET dependency_type = $3`, [sourceModuleId, targetModuleId, type]);
    ...
    async getDependency(moduleId) {
        const client = await this.pool.connect();
        try {
            const moduleResult = await client.query(`SELECT module_id, module_name FROM dependency_modules WHERE module_id = $1`, [moduleId]);
            if (moduleResult.rows.length === 0)
                return null;
            const module = moduleResult.rows[0];
            // Get dependencies (what this module depends on)
            const dependsOnResult = await client.query(`SELECT target_module_id FROM module_dependencies WHERE source_module_id = $1`, [moduleId]);
            // Get dependents (what depends on this module)
            const dependentOfResult = await client.query(`SELECT source_module_id FROM module_dependencies WHERE target_module_id = $1`, [moduleId]);
    ...
    async calculateBlastRadius(moduleId, client) {
        // Check cache
        const cacheResult = await client.query(`SELECT blast_radius_score FROM blast_radius_cache WHERE module_id = $1`, [moduleId]);
        ...
        while (queue.length > 0) {
            ...
            if (!current || visited.has(current))
                continue;
            visited.add(current);
            const result = await client.query(`SELECT source_module_id FROM module_dependencies WHERE target_module_id = $1`, [current]);
        ...
        // Cache the result
        await client.query(`INSERT INTO blast_radius_cache (module_id, blast_radius_score, affected_count)
       VALUES ($1, $2, $3)
       ON CONFLICT (module_id) DO UPDATE SET blast_radius_score = $2, affected_count = $3, calculated_at = CURRENT_TIMESTAMP`, [moduleId, score, affectedCount]);
        return score;
    }
    async hasCircularDependency(moduleId, client) {
        const visited = new Set();
        const recursionStack = new Set();
        const hasCycle = async (node) => {
            visited.add(node);
            recursionStack.add(node);
            const result = await client.query(`SELECT target_module_id FROM module_dependencies WHERE source_module_id = $1`, [node]);
    ...
    async findCircularDependencies(client) {
        const result = await client.query(`SELECT DISTINCT d1.source_module_id, d2.source_module_id
    ...
    async cleanupOldData(daysOld = 30) {
        ...
        try {
            ...
            const result = await client.query(`DELETE FROM module_dependencies
...
export async function initializeDependencyImpactGraphRoutes(service) {
