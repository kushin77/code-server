...
export class DebugAIService extends EventEmitter {
  ...
  private sessionHistory = new Map<string, DebugSessionState[]>();
  private analyses = new Map<string, DebugAIAnalysis>();
  private feedback = new Map<string, AnalysisFeedback>();
  ...
  async updateOnPause(
    sessionId: string,
    reason: DebugSessionState['pauseReason'],
    ...
    console.log(
      `[DebugAIService] Paused session ${sessionId} due to ${reason}`
    );
   ...
   */
  async updateOnResume(sessionId: string): Promise<void> {
    if (!this.isInitialized) throw new Error('Service not initialized');
  ...
  async analyzeDebugState(
    ...
    const context: DebugAnalysisContext = {
      state: session,
      history,
   ...
   * Get analysis result
   */
  async getAnalysis(analysisId: string): Promise<DebugAIAnalysis | null> {
    if (!this.isInitialized) throw new Error('Service not initialized');
   ...
   * Get session state
   */
  async getSession(sessionId: string): Promise<DebugSessionState | null> {
    if (!this.isInitialized) throw new Error('Service not initialized');
   ...
   * Get session history
   */
  async getSessionHistory(sessionId: string): Promise<DebugSessionState[]> {
    if (!this.isInitialized) throw new Error('Service not initialized');
   ...
   * Get statistics
   */
  async getStatistics(workspaceId: string): Promise<DebugStats> {
    const sessions = Array.from(this.sessions.values()).filter(
      (s) => s.workspaceId === workspaceId
    );

    const allAnalyses = Array.from(this.analyses.values()).filter(
      (a) =>
        sessions.some(
    ...
    const stats: DebugStats = {
      ...
      suggestedFixesGenerated: allAnalyses.reduce(
        (sum, a) => sum + a.suggestedFixes.length,
        0
      ),
      fixesApplied: 0,
      applyRate: 0,
    ...
    const sessionFeedback = allFeedback.filter((f) =>
      sessions.some((s) => s.sessionId === f.sessionId)
    );
    if (sessionFeedback.length > 0) {
      stats.fixesApplied = sessionFeedback.reduce(
        (sum, f) => sum + f.appliedSuggestions,
        0
      );
      stats.applyRate =
 ...
 */
let serviceInstance: DebugAIService | null = null;

/**
 * Get global service instance
 */
export async function getDebugAIService(): Promise<DebugAIService> {
  if (!serviceInstance) {
