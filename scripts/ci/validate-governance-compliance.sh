...
compose_files=(
  ...
  "${SCRIPT_DIR}/docker-compose.replica.yml"
  "${SCRIPT_DIR}/docker-compose.replica-port-override.yml"
)
...
for compose_file in "${compose_files[@]}"; do
  ...
  file_unpinned=$(grep 'image:' "$compose_file" | grep -v '@sha256' | grep -v '#' | grep -v 'code-server-enterprise:[0-9]*\.[0-9]*\.[0-9]*' | grep -v 'session-broker:[0-9]*\.[0-9]*\.[0-9]*' | grep -v 'sentry-integration-api:[0-9]*\.[0-9]*\.[0-9]*' | grep -v 'slack-slash-commands-api:[0-9]*\.[0-9]*\.[0-9]*' | wc -l || true)
  unpinned=$((unpinned + file_unpinned))
done
...
for compose_file in "${compose_files[@]}"; do
  ...
  file_hardcoded_config=$(grep -E 'POSTGRES_HOST:|REDIS_HOST:|DATABASE_URL=' "$compose_file" | grep -v '\${' | wc -l || true)
  hardcoded_config=$((hardcoded_config + file_hardcoded_config))
done
...
windows_code=$(find "${SCRIPT_DIR}/scripts" -name "*.sh" ! -name "check-no-*.sh" ! -name "validate-*.sh" -exec grep -lE 'powershell\.exe|ssh\.exe|node\.exe' {} \; 2>/dev/null | wc -l)
