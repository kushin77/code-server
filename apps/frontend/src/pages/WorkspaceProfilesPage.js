...
import { ALL_WORKSPACES } from '../utils/workspaceCatalog';
import { buildWorkspaceProfileSnapshot, getWorkspaceProfile, resolveWorkspaceRootProfile, } from '../utils/workspaceProfilesData';
export function WorkspaceProfilesPage({ workspaceState }) {
    ...
    const profileSnapshot = useMemo(() => buildWorkspaceProfileSnapshot(selectedWorkspaceId, selectedRootPath, workspaceState.projectFiles), [selectedRootPath, selectedWorkspaceId, workspaceState.projectFiles]);
    const activeRoot = useMemo(() => {
        return (resolveWorkspaceRootProfile(selectedWorkspaceId, selectedRootPath) ?? {
    ...
    const handleApplyAutoConfig = async () => {
        try {
            ...
            const updatedExtensions = Array.from(new Set([...currentRoot.enabledExtensions, ...autoConfig.recommendedExtensions]));
            localStorage.setItem(`workspace-auto-config-${selectedWorkspaceId}`, JSON.stringify({
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm shadow-slate-100">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[260px_minmax(0,1fr)]">
          <aside className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            return (<button key={workspace.id} type="button" onClick={() => handleWorkspaceSelect(workspace.id)} className={`flex w-full flex-col rounded-2xl border px-3 py-3 text-left transition ${isSelected ? 'border-violet-500 bg-violet-50 shadow-sm' : 'border-slate-200 bg-white hover:border-violet-300'}`}>
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                ...
                <div className="flex flex-wrap gap-2">
                  <button type="button" onClick={handleCopyProfile} className="rounded-full border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700 transition hover:border-violet-400 hover:text-violet-700">
                    Copy workspace.json
                  </button>
                  {profile?.workspaceId && (<button type="button" onClick={() => selectWorkspace(profile.workspaceId)} className="rounded-full bg-violet-600 px-3 py-2 text-sm font-medium text-white transition hover:bg-violet-700">
                      Activate profile
                    </button>)}
                </div>
              </div>
            ...
            return (<button key={root.path} type="button" onClick={() => setSelectedRootPath(root.path)} className={`rounded-full px-3 py-2 text-sm font-medium transition ${isSelected ? 'bg-slate-900 text-white' : 'border border-slate-300 bg-white text-slate-700 hover:border-violet-400 hover:text-violet-700'}`}>
            ...
            <div className="grid gap-6 xl:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                ...
                <dl className="mt-4 space-y-3">
                  {Object.entries(activeRoot.settings).map(([settingKey, settingValue]) => (<div key={settingKey} className="rounded-xl border border-slate-200 bg-white px-3 py-2">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                ...
                <div className="mt-4 space-y-4">
                  ...
                  <div className="rounded-xl border border-slate-200 bg-white px-3 py-3">
                    ...
                    {activeRoot.terminal.env ? (<p className="mt-2 text-xs text-slate-700">Env: {Object.entries(activeRoot.terminal.env).map(([key, value]) => `${key}=${value}`).join(', ')}</p>) : null}
                  </div>

                  {profileSnapshot.autoConfig ? (<div className="rounded-xl border border-violet-200 bg-violet-50 px-3 py-3">
                      ...
                      <div className="mt-3 flex gap-2">
                        <button type="button" onClick={handleApplyAutoConfig} disabled={applyingAutoConfig} className="rounded-full bg-violet-600 px-3 py-2 text-xs font-medium text-white transition hover:bg-violet-700 disabled:opacity-50 disabled:cursor-not-allowed">
                          {applyingAutoConfig ? 'Applying...' : 'Apply auto-config'}
                        </button>
                        <button type="button" onClick={() => setAutoConfigMessage(null)} className="rounded-full border border-violet-300 px-3 py-2 text-xs font-medium text-violet-700 transition hover:bg-violet-100">
                          Dismiss
                        </button>
                      </div>
                      
                      {autoConfigMessage && (<p className={`mt-2 text-xs font-medium ${autoConfigMessage.type === 'success' ? 'text-emerald-700' : 'text-red-700'}`}>
                          {autoConfigMessage.text}
                        </p>)}
                    </div>) : (<div className="rounded-xl border border-dashed border-slate-200 bg-white px-3 py-3">
                  ...
                  <div className="rounded-xl border border-slate-200 bg-white px-3 py-3">
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {activeRoot.enabledExtensions.map((extensionId) => (<span key={extensionId} className="rounded-full bg-slate-100 px-2 py-1 text-xs font-medium text-slate-700">
                          {extensionId}
                        </span>))}
                    </div>
                  </div>
            ...
            <CollaborativeDebuggingPanel workspaceId={selectedWorkspaceId} actorName={activeWorkspace.label} debuggerName={activeRoot.debugger.name} debuggerProgram={activeRoot.debugger.program} debuggerCwd={activeRoot.debugger.cwd}/>
...
//# sourceMappingURL=WorkspaceProfilesPage.js.map