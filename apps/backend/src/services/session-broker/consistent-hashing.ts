...
import { BrokerInstance, SessionRoutingContext, HashRingLookup } from './types'
...
export class ConsistentHashRing {
  private instances: BrokerInstance[] = []
  private logger: pino.Logger
  private hashCache = new Map<string, BrokerInstance>()

  constructor(instances: BrokerInstance[] = [], logger?: pino.Logger) {
   ...
   */
  updateInstanceStatus(instanceId: string, status: BrokerInstance['status']): void {
  ...
  getInstances(
    ...
    if (!includeUnhealthy) {
      candidateInstances = this.instances.filter(
        (i) => i.status !== 'unhealthy' && i.status !== 'draining'
      )
    }
  ...
  private getReplicas(
    ...
    if (!includeUnhealthy) {
      candidateInstances = candidateInstances.filter(
        (i) => i.status !== 'unhealthy' && i.status !== 'draining'
      )
    }
   ...
   */
  private hash(key: string, nodeId: string): number {
   ...
   * Get all instances.
   */
  getAllInstances(): BrokerInstance[] {
