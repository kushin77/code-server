...
import { createDebugSession, fetchDebugSession, fetchRelayedDebugMessages, joinDebugSession, leaveDebugSession, recordDebugStep, relayDebugProtocolMessage, updateDebugBreakpoints, updateDebugVariables, } from '../utils/debugCollaboration';
import { analyzeDebugSession } from '../utils/debugSessionInsights';
const STEP_ACTIONS = ['continue', 'next', 'stepIn', 'stepOut', 'pause'];
function isDebugSessionRecord(value) {
...
export function CollaborativeDebuggingPanel({ workspaceId, actorName, debuggerName, debuggerProgram, debuggerCwd, }) {
    ...
    const [stepAction, setStepAction] = useState('next');
    const [stepNote, setStepNote] = useState('');
    const insights = useMemo(() => analyzeDebugSession(session), [session]);
    const refreshSession = useCallback(async (nextSessionId) => {
        if (!nextSessionId) {
            return;
        }
        try {
        ...
        <div className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="grid gap-3 md:grid-cols-2">
            <label className="block text-sm">
              ...
              <input value={sessionId} onChange={(event) => setSessionId(event.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="Create or join a session"/>
            </label>
            <label className="block text-sm">
              <span className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Relay target</span>
              <input value={relayTarget} onChange={(event) => setRelayTarget(event.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="https://debug-relay.example.test"/>
            </label>
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <label className="block text-sm">
              ...
              <input value={breakpointPath} onChange={(event) => setBreakpointPath(event.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="src/server.ts"/>
            </label>
            <label className="block text-sm">
              <span className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Breakpoint line</span>
              <input value={breakpointLine} onChange={(event) => setBreakpointLine(event.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" inputMode="numeric" placeholder="1"/>
            </label>
          </div>

          <label className="block text-sm">
            <span className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Breakpoint condition</span>
            <input value={breakpointCondition} onChange={(event) => setBreakpointCondition(event.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="request.userId === actorId"/>
          </label>
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <label className="block text-sm">
              ...
              <input value={variableScope} onChange={(event) => setVariableScope(event.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm"/>
            </label>
            <label className="block text-sm">
              <span className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Variable name</span>
              <input value={variableName} onChange={(event) => setVariableName(event.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm"/>
            </label>
            <label className="block text-sm">
              <span className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Variable value</span>
              <input value={variableValue} onChange={(event) => setVariableValue(event.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm"/>
            </label>
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-[0.7fr_1.3fr]">
            <label className="block text-sm">
              ...
              <select value={stepAction} onChange={(event) => setStepAction(event.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm">
                {STEP_ACTIONS.map((action) => (<option key={action} value={action}>
                    {action}
                  </option>))}
              </select>
            </label>
            <label className="block text-sm">
              ...
              <input value={stepNote} onChange={(event) => setStepNote(event.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="Explain what the shared step should inspect"/>
            </label>
          </div>
        ...
        <div className="space-y-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-3">
            ...
            <div className="mt-2 space-y-2 text-sm text-slate-700">
              {session?.participants?.length ? (session.participants.map((participant) => (<div key={participant.actor} className="rounded-xl bg-slate-50 px-3 py-2">
                  ...
                  </div>))) : (<p className="text-slate-500">No collaborators yet.</p>)}
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-3">
            ...
            <div className="mt-2 space-y-2 text-sm text-slate-700">
              {session?.breakpoints?.length ? (session.breakpoints.map((breakpoint) => (<div key={breakpoint.id || `${breakpoint.filePath}:${breakpoint.line}`} className="rounded-xl bg-slate-50 px-3 py-2">
                  ...
                  </div>))) : (<p className="text-slate-500">No shared breakpoints captured yet.</p>)}
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-3">
            ...
            <div className="mt-2 space-y-2 text-sm text-slate-700">
              {session?.variables?.length ? (session.variables.map((variable) => (<div key={`${variable.scope}.${variable.name}`} className="rounded-xl bg-slate-50 px-3 py-2">
                    <p className="font-medium text-slate-900">{variable.scope}.{variable.name}</p>
                    <p className="text-xs text-slate-500">{variable.value}</p>
                  </div>))) : (<p className="text-slate-500">No variable snapshots captured yet.</p>)}
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-3">
            ...
            <div className="mt-2 space-y-2 text-sm text-slate-700">
              {session?.stepEvents?.length ? (session.stepEvents.map((stepEvent) => (<div key={stepEvent.id} className="rounded-xl bg-slate-50 px-3 py-2">
                  ...
                  </div>))) : (<p className="text-slate-500">No step activity recorded yet.</p>)}
            </div>
          </div>
          <div className="rounded-2xl border border-violet-200 bg-violet-50 p-3">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-violet-700">AI debug insights</p>
            <div className="mt-2 space-y-3 text-sm text-slate-700">
              {insights.length ? (insights.map((insight) => (<div key={insight.title} className="rounded-xl border border-violet-200 bg-white px-3 py-2 shadow-sm">
                  ...
                  </div>))) : (<p className="text-slate-500">Capture or join a session to generate debug insights.</p>)}
            </div>
          </div>
        </div>
      </div>
...
//# sourceMappingURL=CollaborativeDebuggingPanel.js.map