...
import { LLMWikiExtractionService } from '../services/llm-wiki-extraction';

export function initializeLLMWikiExtractionRoutes(service: LLMWikiExtractionService): Router {
  const router = Router();
  ...
  router.put('/knowledge/:entryId', async (req: Request, res: Response) => {
    try {
      const { entryId } = req.params;
  ...
  router.post('/knowledge/:entryId/usage', async (req: Request, res: Response) => {
    try {
      const { entryId } = req.params;
  ...
  router.post('/knowledge/:entryId/feedback', async (req: Request, res: Response) => {
    try {
      const { entryId } = req.params;
  ...
  router.post('/knowledge/extract', async (req: Request, res: Response) => {
    try {
      ...
      const entries = await service.extractFromSession(
        { sessionId, content, context, authorId },
        extractedEntries
      );
  ...
  router.get('/knowledge-base', async (req: Request, res: Response) => {
    try {
      ...
      const entries = await service.getKnowledgeBase({
        ...
        minViews: minViews ? parseInt(minViews as string) : undefined,
      });
