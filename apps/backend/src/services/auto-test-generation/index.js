...
export class AutoTestGenerationService extends EventEmitter {
    ...
    async generateTestsForSession(request, suggestedTests) {
        const client = await this.pool.connect();
        try {
            await client.query('BEGIN');
            // Create batch record
    ...
    async getTestSuggestion(suggestionId) {
        const client = await this.pool.connect();
        try {
    ...
    async acceptTestSuggestion(suggestionId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`
    ...
    async rejectTestSuggestion(suggestionId, reason) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`
        UPDATE test_suggestions 
    ...
    async getSessionTestSuggestions(sessionId, status) {
        const client = await this.pool.connect();
        try {
    ...
    async markTestAsUsed(suggestionId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`
        UPDATE test_suggestions 
    ...
    async recordTestExecution(suggestionId, passed, durationMs, errorMessage) {
        const client = await this.pool.connect();
        try {
            await client.query(`
    ...
    async getTestMetrics(sessionId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`
        SELECT 
    ...
    async addFeedback(suggestionId, userId, feedbackType, comment) {
        const client = await this.pool.connect();
        try {
            await client.query(`
