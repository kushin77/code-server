...
export class MentionSystemService extends EventEmitter {
    ...
    async createMention(data) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`INSERT INTO mentions (
    ...
    async sendMatrixNotification(mention, roomId) {
    ...
    formatMatrixNotification(mention) {
    ...
    async queueEmailDigest(mention, emailAddress) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO email_digest_queue (user_id, mention_id) VALUES ($1, $2)`, [mention.mentionedUser, mention.id]);
    ...
    async createInAppNotification(mention) {
    ...
    async getMentionNotificationPreferences(userId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query('SELECT * FROM mention_notification_preferences WHERE user_id = $1', [userId]);
    ...
    async setMentionNotificationPreferences(userId, preferences) {
        const client = await this.pool.connect();
        try {
            const existing = await this.getMentionNotificationPreferences(userId);
            if (existing) {
                await client.query(`UPDATE mention_notification_preferences SET
           ...
           WHERE user_id = $1`, [
                ...
                await client.query(`INSERT INTO mention_notification_preferences (
