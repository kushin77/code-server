...
collect_incremental_files() {
  ...
  # Deduplicate and keep existing files only.
  if [[ ${#files[@]} -eq 0 ]]; then
    return 0
  fi
...
# 3) Validate key single-source-of-truth inventories.
if command -v yq >/dev/null 2>&1; then
  echo "[gate] Validating inventory YAML"
  yq eval '.' inventory/infrastructure.yaml >/dev/null
  yq eval '.' inventory/dns.yaml >/dev/null
else
