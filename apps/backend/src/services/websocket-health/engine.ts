...
export const DEFAULT_HEALTH_CONFIG: WebSocketHealthConfig = {
  enabled: true,
...
export class WebSocketHealthEngine {
  ...
  constructor(config: Partial<WebSocketHealthConfig> = {}) {
   ...
   */
  recordLatency(connectionId: string, latencyMs: number): LatencyMeasurement {
    const conn = this.connections.get(connectionId);
    if (!conn) {
   ...
   */
  recordMessageLoss(connectionId: string, count: number = 1): void {
    const conn = this.connections.get(connectionId);
    if (!conn) return;
  ...
  getAggregatedMetrics(): AggregatedHealthMetrics {
    ...
    conns.forEach((conn) => {
      ...
      if (conn.latencyMs > 0) latencies.push(conn.latencyMs);
    ...
    return {
      ...
      p95LatencyMs:
        latencies.length > 0 ? latencies[p95Index] : 0,
  ...
  getSessionHealth(sessionId: string): PerSessionHealthStats | null {
    const sessionConns = Array.from(this.connections.values()).filter(
      (c) => c.sessionId === sessionId
    );

    if (sessionConns.length === 0) return null;

    const healthy = sessionConns.filter((c) => c.isHealthy).length;
    const totalLatency = sessionConns.reduce(
      (sum, c) => sum + c.averageLatencyMs,
      0
    );
    const totalDelivery = sessionConns.reduce(
   ...
   */
  getAllConnections(): WebSocketConnectionMetrics[] {
  ...
  private updateLatencyStats(
    conn: WebSocketConnectionMetrics,
  ...
  private checkLatencyHealth(conn: WebSocketConnectionMetrics): void {
    const issues = this.healthIssues.get(conn.connectionId) || [];
    const hasLatencyIssue = issues.some((i) => i.type === 'high_latency');

    if (
      conn.latencyMs > this.config.latencyCriticalMs &&
      !hasLatencyIssue
    ) {
      ...
      conn.latencyMs > this.config.latencyWarningMs &&
      !hasLatencyIssue
    ) {
      this.addHealthIssue(conn.connectionId, {
        type: 'high_latency',
  ...
  private updateDeliveryRate(conn: WebSocketConnectionMetrics): void {
  ...
  private checkMessageLossHealth(conn: WebSocketConnectionMetrics): void {
    if (
      conn.deliverySuccessRate <
  ...
  private addHealthIssue(connectionId: string, issue: HealthIssue): void {
    const issues = this.healthIssues.get(connectionId) || [];
  ...
  private calculateHealthScore(conn: WebSocketConnectionMetrics): void {
    let score = 100;

    // Latency impact
    if (conn.latencyMs > this.config.latencyCriticalMs) score -= 30;
    else if (conn.latencyMs > this.config.latencyWarningMs) score -= 15;

    // Delivery impact
    score -= 100 - conn.deliverySuccessRate;

    // Stability impact
    if (conn.reconnectionAttempts > 3) score -= 20;

    // Connection state impact
    if (conn.state !== 'connected') score -= 25;
