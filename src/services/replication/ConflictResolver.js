...
export class ConflictResolver {
     ...
     */
    resolveConflict(op1, op2, data1, data2) {
        ...
        switch (this.config.strategy) {
            case 'lww':
                ({ winner, resolvedData, resolutionReason } = this.resolveLWW(op1, op2, data1, data2));
                break;
            case 'first-write-wins':
                ({ winner, resolvedData, resolutionReason } = this.resolveFWW(op1, op2, data1, data2));
                break;
            case 'replica-id':
                ({ winner, resolvedData, resolutionReason } = this.resolveByReplicaId(op1, op2, data1, data2));
                break;
            case 'custom':
        ...
        const metadata = {
            ...
            resolvedBy: this.config.strategy,
            resolutionReason,
        };
     ...
     */
    resolveLWW(op1, op2, data1, data2) {
     ...
     */
    resolveFWW(op1, op2, data1, data2) {
     ...
     */
    resolveByReplicaId(op1, op2, data1, data2) {
    ...
    getConflictStats() {
        ...
        for (const conflict of this.conflictHistory) {
            ...
            byStrategy.set(conflict.resolvedBy, (byStrategy.get(conflict.resolvedBy) ?? 0) + 1);
        }
        return {
            total: this.conflictHistory.length,
            byReplica,
            byStrategy,
            lastConflict: this.conflictHistory[this.conflictHistory.length - 1],
        };
    }
    /**
