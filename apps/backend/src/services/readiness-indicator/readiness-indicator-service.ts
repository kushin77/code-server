...
export class ReadinessIndicatorService extends EventEmitter {
  private signals: Map<string, AvailabilitySignal[]> = new Map();
  private userStatus: Map<string, UserReadinessStatus> = new Map();
  private capacityMetrics: Map<string, CollaborativeCapacity> = new Map();
  private subscriptions: Map<string, (update: ReadinessUpdate) => void> = new Map();
  private config: ReadinessIndicatorConfig;
  private cleanupInterval: NodeJS.Timeout | null = null;
  private stats: ReadinessIndicatorStats;

  constructor(config?: Partial<ReadinessIndicatorConfig>) {
    super();
   ...
   */
  async addSignal(signal: AvailabilitySignal): Promise<boolean> {
    try {
  ...
  getTeamReadiness(teamId: string, userIds: string[]): TeamReadinessMetrics {
    const metrics: TeamReadinessMetrics = {
      teamId,
  ...
  predictReadiness(userId: string): ReadinessPrediction | null {
    ...
    try {
      ...
      return {
        ...
        basedOnSignals: Object.keys(factors).map((k) => k.replace('Factor', '') as SignalType),
        factors,
   ...
   */
  findOptimalCollaborationWindow(
    teamId: string,
    userIds: string[],
    windowSizeMs: number = 60 * 60 * 1000, // 1 hour default
  ): CollaborationWindowRecommendation {
    const now = Date.now();
    const searchWindow = now + 24 * 60 * 60 * 1000; // Look ahead 24 hours

    // Simplified: recommend next hour with most availability
    const recommendation: CollaborationWindowRecommendation = {
      teamId,
  ...
  queryReadiness(options: ReadinessQueryOptions): ReadinessQueryResult {
    const startTime = Date.now();
    let results: UserReadinessStatus[] = [];

    for (const [_, status] of this.userStatus) {
      if (options.userId && status.userId !== options.userId) continue;
      if (options.minReadinessScore && status.readinessScore < options.minReadinessScore)
        continue;
      if (options.readinessLevel && status.readinessLevel !== options.readinessLevel) continue;
  ...
  private calculateReadiness(userId: string): UserReadinessStatus {
    const userSignals = this.signals.get(userId) || [];
    const now = Date.now();

    // Get recent signals
    const recentSignals = userSignals.filter(
      (s) => now - s.timestamp < this.config.signalFreshnessMs,
    );
  ...
  private scoreToReadinessLevel(score: number): ReadinessLevel {
    if (score >= this.config.readinessThresholds.available) return ReadinessLevel.AVAILABLE;
    if (score >= this.config.readinessThresholds.busy) return ReadinessLevel.BUSY;
    if (score >= this.config.readinessThresholds.away) return ReadinessLevel.AWAY;
    return ReadinessLevel.OFFLINE;
  }

  private getSignalWeight(signalType: SignalType): number {
  ...
  private calculateSignalFactor(userId: string, signalType: SignalType): number {
    const userSignals = this.signals.get(userId) || [];
  ...
  private generateExplanation(level: ReadinessLevel, signals: AvailabilitySignal[]): string {
 ...
 */
export function createReadinessIndicatorService(
  config?: Partial<ReadinessIndicatorConfig>,
): ReadinessIndicatorService {
  return new ReadinessIndicatorService(config);
}

// Export singleton instance
let instance: ReadinessIndicatorService;

export function getReadinessIndicatorService(): ReadinessIndicatorService {
  if (!instance) {
