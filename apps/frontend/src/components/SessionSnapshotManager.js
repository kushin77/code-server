...
export const SessionSnapshotManager = ({ workspaceId, onSnapshotCreated, onSnapshotRestored, }) => {
    ...
    useEffect(() => {
        const initService = async () => {
            try {
                const { getSessionSnapshotService } = await import('../services/session-snapshot/service.js');
    ...
    return (<div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
          <input type="text" placeholder="Optional label (e.g., 'Before refactor')" value={label} onChange={(e) => setLabel(e.target.value)} disabled={isSaving} style={{
            flex: 1,
            padding: '6px 8px',
            background: '#3e3e42',
          ...
          <button onClick={handleCreateSnapshot} disabled={isSaving || isLoading} style={{
            padding: '6px 12px',
        ...
        {/* Loading state */}
        {isLoading && (<div style={{ fontSize: '12px', color: '#858585' }}>
            Loading snapshots...
          </div>)}
      </div>
      ...
      <div style={{
        ...
        {snapshots.length === 0 ? (<div style={{
          ...
          </div>) : (snapshots.map((snap) => (<div key={snap.id} style={{
                background: selectedSnapshot === snap.id ? '#2d2d30' : 'transparent',
                border: selectedSnapshot === snap.id ? '1px solid #4ec9b0' : '1px solid #3e3e42',
                borderRadius: '2px',
                padding: '10px 12px',
                marginBottom: '8px',
                cursor: 'pointer',
      ...
      {total > 10 && (<div style={{
          ...
          <div style={{ display: 'flex', gap: '4px' }}>
            ...
            <button onClick={() => setPage(page + 1)} disabled={page >= Math.ceil(total / 10)} style={{
                padding: '2px 8px',
      ...
      {selectedSnapshot && (<div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            ...
            ].map(([key, label]) => (<label key={key} style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', cursor: 'pointer' }}>
                <input type="checkbox" checked={restoreOptions[key]} onChange={(e) => setRestoreOptions({ ...restoreOptions, [key]: e.target.checked })} style={{ cursor: 'pointer' }}/>
                {label}
              </label>))}
          </div>
        </div>)}
...
//# sourceMappingURL=SessionSnapshotManager.js.map