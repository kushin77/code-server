...
export class DevcontainerPinningService extends EventEmitter {
     ...
     */
    async scanDevcontainer(workspaceId, devcontainerPath, content) {
        if (!this.isInitialized)
        ...
        try {
            ...
            console.log(`[DevcontainerPinningService] Scanned ${devcontainerPath} in ${result.scanTime.toFixed(2)}ms`);
     ...
     */
    async pinHashes(workspaceId, content, policy) {
        if (!this.isInitialized)
            throw new Error('Service not initialized');
        const pinned = JSON.parse(JSON.stringify(content));
        // Store original
        ...
        console.log(`[DevcontainerPinningService] Pinned hashes for ${workspaceId}`);
     ...
     */
    async verifyReproducibility(pinnedConfig, currentDigests) {
        if (!this.isInitialized)
            throw new Error('Service not initialized');
        const startTime = performance.now();
        const mismatches = [];
        ...
        if (pinnedConfig.featureHashes) {
            for (const [featureId, featureInfo] of Object.entries(pinnedConfig.featureHashes)) {
     ...
     */
    async scanFeature(featureId, version) {
 ...
 */
export async function getDevcontainerPinningService() {
    if (!serviceInstance) {
...
//# sourceMappingURL=pinning-service.js.map