...
vi.mock('../../utils/debugCollaboration', () => ({
    createDebugSession: (...args) => createDebugSession(...args),
    fetchDebugSession: (...args) => fetchDebugSession(...args),
    fetchRelayedDebugMessages: (...args) => fetchRelayedDebugMessages(...args),
    joinDebugSession: (...args) => joinDebugSession(...args),
    leaveDebugSession: (...args) => leaveDebugSession(...args),
    recordDebugStep: (...args) => recordDebugStep(...args),
    relayDebugProtocolMessage: (...args) => relayDebugProtocolMessage(...args),
    updateDebugBreakpoints: (...args) => updateDebugBreakpoints(...args),
    updateDebugVariables: (...args) => updateDebugVariables(...args),
}));
afterEach(() => {
    cleanup();
    vi.clearAllMocks();
...
describe('CollaborativeDebuggingPanel', () => {
    it('refreshes remote session state and polls relay deltas after creating a session', async () => {
        render(<CollaborativeDebuggingPanel workspaceId="portal-main" actorName="Portal main" debuggerName="Portal debugger" debuggerProgram="src/main.ts" debuggerCwd="/workspace/portal"/>);
...
//# sourceMappingURL=CollaborativeDebuggingPanel.test.js.map