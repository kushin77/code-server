...
export function initializeAuditVerificationRoutes(router) {
    /**
     ...
     * - 404: Hash chain verification service not initialized
     */
    router.get('/audit/tamper-detection-report', async (req, res) => {
        const service = getHashChainVerificationService();
        if (!service) {
            return res.status(404).json({
                error: 'Hash chain verification service not initialized',
     ...
     * - 404: Hash chain verification service not initialized
     */
    router.get('/audit/verify/:entryId', async (req, res) => {
        const service = getHashChainVerificationService();
        if (!service) {
            return res.status(404).json({
                error: 'Hash chain verification service not initialized',
...
//# sourceMappingURL=audit-verification.js.map