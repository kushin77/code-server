...
export class SessionSnapshotStorage {
    constructor() {
        ...
        this.MAX_VERSIONS_PER_WORKSPACE = 10;
    }
    /**
    ...
    async initialize() {
        ...
        return new Promise((resolve, reject) => {
            const request = indexedDB.open(SNAPSHOT_DB_CONFIG.name, SNAPSHOT_DB_CONFIG.version);
            request.onerror = () => {
    ...
    async saveSnapshot(snapshot) {
        ...
        return new Promise((resolve, reject) => {
            ...
            putRequest.onsuccess = () => {
                console.log(`[SessionSnapshotStorage] Saved snapshot ${snapshot.id} for workspace ${snapshot.workspaceId}`);
    ...
    async listSnapshots(workspaceId, page = 1, pageSize = 10) {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = () => {
                ...
                console.log(`[SessionSnapshotStorage] Listed ${metadata.length}/${total} snapshots for workspace ${workspaceId}`);
     ...
     */
    async pruneOldVersions(workspaceId) {
        if (!this.db)
            return;
        try {
            const tx = this.db.transaction(['snapshots'], 'readwrite');
            ...
            request.onsuccess = () => {
                ...
                if (all.length > this.MAX_VERSIONS_PER_WORKSPACE) {
                    ...
                    console.log(`[SessionSnapshotStorage] Pruned ${toDelete.length} old snapshots for workspace ${workspaceId}, keeping ${this.MAX_VERSIONS_PER_WORKSPACE}`);
                }
            };
        }
        catch (error) {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = () => {
                ...
                if (cursor) {
                    ...
                    console.log(`[SessionSnapshotStorage] Cleared ${deletedCount.count} snapshots for workspace ${workspaceId}`);
                    resolve();
                }
            };
        });
    ...
    async getStats() {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = () => {
                ...
                const workspaceStats = Array.from(workspaceMap.entries()).map(([workspaceId, { count, sizeBytes }]) => ({
                    workspaceId,
                    count,
                    sizeBytes,
                }));
 ...
 */
export async function getSessionSnapshotStorage() {
    if (!storageInstance) {
