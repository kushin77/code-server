...
describe('SharedPromptLibraryService', () => {
  let service: SharedPromptLibraryService;
  ...
  beforeEach(() => {
    ...
    service = new SharedPromptLibraryService(mockPool as Pool);
  });
  ...
  describe('createPrompt', () => {
    ...
    it('should reject oversized prompts', async () => {
      ...
      await expect(service.createPrompt('team-1', 'Large', largeContent, 'code-review', 'user123')).rejects.toThrow(
        /exceeds max length/
      );
    });
  ...
  describe('listTeamPrompts', () => {
    ...
    it('should filter by category', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        ...
        expect.arrayContaining(['team-1', 'testing'])
      );
    });
  });
  ...
  describe('trackUsage', () => {
    it('should track prompt usage', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO prompt_usage'),
        expect.arrayContaining(['prompt-1', 'user123'])
      );
  ...
  describe('ratePrompt', () => {
    it('should rate a prompt', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO prompt_ratings'),
        expect.arrayContaining(['prompt-1', 'user123', 5])
      );
      expect(eventSpy).toHaveBeenCalled();
    });
