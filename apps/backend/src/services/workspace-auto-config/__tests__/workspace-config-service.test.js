...
describe("WorkspaceConfigurationService", () => {
    ...
    describe("configureWorkspace", () => {
        ...
        it("should write workspace settings to .vscode/settings.json", async () => {
            mockFs.existsSync.mockReturnValue(false);
            await service.configureWorkspace();
            expect(mockFs.writeFileSync).toHaveBeenCalledWith("/test/workspace/.vscode/settings.json", expect.any(String));
        });
        it("should generate tasks.json with build tasks", async () => {
            mockFs.existsSync.mockReturnValue(false);
            await service.configureWorkspace();
            const writeCall = mockFs.writeFileSync.mock.calls.find((call) => call[0].includes("tasks.json"));
        ...
        it("should generate launch.json with debug configurations", async () => {
            mockFs.existsSync.mockReturnValue(false);
            await service.configureWorkspace();
            const writeCall = mockFs.writeFileSync.mock.calls.find((call) => call[0].includes("launch.json"));
            expect(writeCall).toBeDefined();
        ...
        it("should merge with existing settings", async () => {
            ...
            await service.configureWorkspace();
            const writeCall = mockFs.writeFileSync.mock.calls.find((call) => call[0].includes("settings.json"));
        ...
        it("should not duplicate tasks when merging", async () => {
            ...
            const writeCall = mockFs.writeFileSync.mock.calls.find((call) => call[0].includes("tasks.json"));
            const tasksContent = JSON.parse(writeCall[1]);
            const buildTaskCount = tasksContent.tasks.filter((t) => t.label === "npm: build").length;
        ...
        it("should not duplicate debug configurations when merging", async () => {
            ...
            const writeCall = mockFs.writeFileSync.mock.calls.find((call) => call[0].includes("launch.json"));
            const launchContent = JSON.parse(writeCall[1]);
            const launchConfigCount = launchContent.configurations.filter((c) => c.name === "Launch Program").length;
...
//# sourceMappingURL=workspace-config-service.test.js.map