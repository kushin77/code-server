...
import { VSIXManifest } from '../types.js';
...
describe('Extension Registry Service', () => {
  ...
  describe('Extension Publishing', () => {
    ...
    it('should reject missing manifest fields', async () => {
      const manifest = { name: 'test', publisher: 'test' } as any;
      const buffer = Buffer.from('mock vsix content');
    ...
    it('should reject oversized files', async () => {
      ...
      await expect(
        service.publishExtension(manifest, largeBuffer, 'test-user', 'private')
  ...
  describe('Installation', () => {
    ...
    it('should reject non-existent extension', async () => {
      await expect(
        service.installExtension('non-existent.ext', 'user-alice')
  ...
  describe('Version Pinning', () => {
    ...
    it('should pin version', async () => {
      const pin = await service.pinVersion(
        'pub.ext',
        '1.0.0',
        'organization',
        'Stable release',
  ...
  describe('Blocklist', () => {
    ...
    it('should blocklist extension', async () => {
      const entry = await service.blocklistExtension(
        'test-pub.test-ext',
        'security',
        'critical',
        'Contains malware',
    ...
    it('should check if blocklisted', async () => {
      await service.blocklistExtension(
        'test-pub.test-ext',
        'security',
        'critical',
        'Contains malware',
    ...
    it('should reject installation of blocklisted extension', async () => {
      ...
      await expect(
        service.installExtension('test-pub.test-ext', 'user-alice')
