...
export function PagerDutyIncidentsPage() {
    ...
    const fetchIncidents = async (cfg) => {
        if (!cfg?.token) {
            setIncidents([]);
        ...
        try {
            await measureAsyncExtensionProfiler({
                ...
                const { incidents: fetchedIncidents, stats: fetchedStats } = await fetchPagerDutyIncidents(cfg, activeStatus === 'all' ? undefined : activeStatus);
      ...
      <div className="flex flex-col h-full bg-gradient-to-b from-slate-900 via-purple-900 to-slate-900">
        ...
        <div className="bg-gradient-to-r from-purple-600 to-blue-600 text-white px-6 py-6 border-b border-purple-500">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={handleRefresh} disabled={isLoading} className="flex items-center gap-2 bg-white text-purple-600 px-4 py-2 rounded-lg font-medium hover:bg-gray-100 disabled:opacity-50 transition" title="Refresh incidents">
        ...
        <div className="flex gap-2 px-6 py-4 border-b border-gray-700 bg-gray-800 bg-opacity-50">
          {['all', 'triggered', 'acknowledged', 'resolved'].map((status) => {
            ...
            return (<button key={status} onClick={() => handleStatusFilter(status)} className={`px-4 py-2 rounded-lg font-medium transition ${isActive
                    ? status === 'all'
        ...
        {/* Error message */}
        {error && (<div className="mx-6 mt-4 bg-red-900 bg-opacity-50 border border-red-500 text-red-200 p-4 rounded">
            {error}
          </div>)}

        {/* Incidents list */}
        <div className="flex-1 overflow-auto p-6">
          {sortedIncidents.length === 0 ? (<div className="text-center text-gray-400 py-12">
              {isLoading ? 'Loading incidents...' : 'No incidents to display'}
            </div>) : (<div className="space-y-3">
              {sortedIncidents.map((incident) => (<div key={incident.id} className="bg-gray-800 border border-gray-700 rounded-lg p-4 hover:border-gray-600 transition">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        <span className={`px-2 py-1 rounded text-xs font-medium ${STATUS_STYLES[incident.status].badge}`}>
                      ...
                      <div className="flex flex-col gap-1 text-sm text-gray-400">
                        ...
                        {incident.assignees.length > 0 && (<div>
                            Assigned to:{' '}
                    ...
                    {incident.html_url && (<a href={incident.html_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 px-3 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded text-sm font-medium transition" title="Open in PagerDuty">
...
//# sourceMappingURL=PagerDutyIncidentsPage.js.map