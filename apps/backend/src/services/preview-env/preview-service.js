...
export class PreviewEnvironmentService extends EventEmitter {
     ...
     */
    async provisionEnvironment(pullRequestId, branchName, sha, repository, userId, workspaceId, config) {
        if (!this.isInitialized)
        ...
        console.log(`[PreviewEnvironmentService] Provisioned environment ${envId} for PR #${pullRequestId}`);
     ...
     */
    async simulateProvisioning(envId, config) {
     ...
     */
    updateHealthChecks(envId, config) {
        if (config.frontend.enabled) {
     ...
     */
    async getEnvironmentByPullRequest(workspaceId, prId) {
        const env = Array.from(this.environments.values()).find((e) => e.workspaceId === workspaceId &&
     ...
     */
    async listUserEnvironments(workspaceId, userId) {
        if (!this.isInitialized)
            throw new Error('Service not initialized');
        return Array.from(this.environments.values())
            .filter((env) => env.workspaceId === workspaceId &&
     ...
     */
    async markForDestruction(envId, reason) {
        if (!this.isInitialized)
            throw new Error('Service not initialized');
        const env = this.environments.get(envId);
        if (!env)
        ...
        this.addEvent(envId, 'marked-for-destruction', `Marked for destruction (reason: ${reason || 'unknown'})`);
        console.log(`[PreviewEnvironmentService] Marked ${envId} for destruction (${reason})`);
        this.emit('environment-marked-for-destruction', { environmentId: envId, reason });
    }
    /**
     * Cancel destruction (if still in grace period)
     */
    async cancelDestruction(envId) {
        if (!this.isInitialized)
            throw new Error('Service not initialized');
        const env = this.environments.get(envId);
        if (!env)
    ...
    async destroyEnvironment(envId) {
        ...
        for (const resource of ['frontend', 'backend', 'database']) {
     ...
     */
    addEvent(envId, type, message, details) {
     ...
     */
    async getEnvironmentEvents(envId, limit = 100) {
        if (!this.isInitialized)
            throw new Error('Service not initialized');
        const events = this.events.get(envId) || [];
    ...
    async getStatistics(workspaceId) {
        const envs = Array.from(this.environments.values()).filter((e) => e.workspaceId === workspaceId);
        const activeEnvs = envs.filter((e) => e.status === 'ready' || e.status === 'degraded');
        const provisioningEnvs = envs.filter((e) => e.status === 'provisioning' || e.status === 'updating');
    ...
    async performCleanup() {
        ...
        for (const env of Array.from(this.environments.values())) {
            ...
            if (env.markedForDestructionAt) {
                const gracePeriodExpired = now - env.markedForDestructionAt >= env.gracePeriodMs;
     ...
     */
    async updateStatus(envId, status) {
        if (!this.isInitialized)
            throw new Error('Service not initialized');
        const env = this.environments.get(envId);
        if (!env)
 ...
 */
export async function getPreviewEnvironmentService(cleanupPolicy) {
    if (!serviceInstance) {
