...
describe('LLMWikiExtractionService', () => {
    ...
    beforeEach(() => {
        ...
        service = new LLMWikiExtractionService(mockPool);
    });
    afterEach(() => {
        vi.clearAllMocks();
    });
    ...
    describe('searchKnowledge', () => {
        ...
        it('should limit results to specified count', async () => {
            ...
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('LIMIT $2'), expect.arrayContaining([expect.anything(), 5]));
        });
    });
    describe('getKnowledgeEntry', () => {
        it('should retrieve knowledge entry and increment views', async () => {
            ...
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE knowledge_entries SET last_accessed_at = NOW(), views = views + 1'), ['entry-1']);
        });
    ...
    describe('trackUsage', () => {
        it('should record knowledge usage and increment counter', async () => {
            ...
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO knowledge_usage'), expect.arrayContaining(['entry-1', 'user-1', 'session-1', 'Used in debugging']));
            expect(eventSpy).toHaveBeenCalled();
        });
    });
    describe('rateFeedback', () => {
        it('should record feedback with rating', async () => {
            ...
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO knowledge_feedback'), expect.arrayContaining(['entry-1', 'user-1', 5, true, 'Very helpful!']));
            expect(eventSpy).toHaveBeenCalled();
        });
...
//# sourceMappingURL=llm-wiki-extraction.test.js.map