...
export class AnomalyDetectionEngine {
  ...
  private calculateZScore(value: number, stats: DimensionStatistics): number {
    if (stats.stdDev === 0) {
  ...
  calculateDimensionStatistics(
    values: number[],
    dimension: string
  ): DimensionStatistics {
    if (values.length === 0) throw new Error(`No values for dimension: ${dimension}`);
  ...
  scoreSession(event: SessionAccessEvent, profile: UserBehavioralProfile): AnomalyScore {
    const dimensionScores: Record<string, number> = {};
    const topAnomalies: Array<{dimension: string; score: number; description: string}> = [];

    // Login time analysis
    if (this.config.enableLoginTimeAnalysis && profile.loginTimes.stdDev) {
      const stats: DimensionStatistics = {
        dimension: 'loginTime',
  ...
  buildProfile(userId: string, events: SessionAccessEvent[]): UserBehavioralProfile {
    if (events.length < this.config.minSamplesForProfile) {
      throw new Error(
        `Insufficient samples (${events.length} < ${this.config.minSamplesForProfile})`
      );
    }
