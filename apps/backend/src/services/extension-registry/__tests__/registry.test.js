...
class ExtensionRegistryService {
    ...
    async checkBlocklist(extensionId) {
        const result = await this.db.query("SELECT blocklisted FROM registry.extensions WHERE extension_id = $1", [extensionId]);
        return result.rows[0]?.blocklisted ?? false;
    }
    async getAllowedVersions(extensionId) {
        const result = await this.db.query(`SELECT version FROM registry.extension_versions 
       WHERE extension_id = (SELECT id FROM registry.extensions WHERE extension_id = $1)
       ORDER BY published_at DESC`, [extensionId]);
        return result.rows.map((row) => row.version);
    }
    async recordInstallation(extensionId, version, userId) {
        await this.db.query(`INSERT INTO registry.installation_logs (version_id, user_id, install_status, installed_at)
       SELECT id, $2, 'success', NOW()
       FROM registry.extension_versions
       WHERE version = $3 AND extension_id = (SELECT id FROM registry.extensions WHERE extension_id = $1)`, [extensionId, userId, version]);
    }
    async getInstallationStats(extensionId) {
...
describe("ExtensionRegistryService", () => {
    ...
    describe("checkBlocklist", () => {
        it("should return true if extension is blocklisted", async () => {
            ...
            expect(mockDb.query).toHaveBeenCalledWith("SELECT blocklisted FROM registry.extensions WHERE extension_id = $1", ["ms-vscode-remote.remote-ssh"]);
        });
    ...
    describe("recordInstallation", () => {
        it("should record successful installation", async () => {
            ...
            expect(mockDb.query).toHaveBeenCalledWith(expect.stringContaining("INSERT INTO registry.installation_logs"), ["ms-python.python", "user-123", "2024.4.1"]);
        });
    });
