...
terraform {
  ...
  # Uncomment to enable remote state tracking (recommended for production)
  # backend "s3" {
  #   bucket         = "phase-13-terraform-state"
  #   key            = "cloudflare/phase-13.tfstate"
  #   region         = "us-east-1"
  #   encrypt        = true
...
variable "cloudflare_zone_id" {
  description = "Cloudflare Zone ID (set via TF_VAR_cloudflare_zone_id or .tfvars)"
  type        = string
  sensitive   = true
}

variable "cloudflare_account_id" {
  description = "Cloudflare Account ID (set via TF_VAR_cloudflare_account_id or .tfvars)"  
  type        = string
  sensitive   = true
}

variable "domain" {
  description = "Domain for code-server access"
  type        = string
