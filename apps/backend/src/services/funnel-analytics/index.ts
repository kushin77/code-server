...
export interface UserJourney {
  ...
  sessionJoinTime?: Date;
  firstEditTime?: Date;
  sevenDayStreakTime?: Date;
  currentStep: string;
  completedSteps: string[];
}

export class FunnelAnalyticsService extends EventEmitter {
  private logger = getLogger('FunnelAnalyticsService');
  private pool: Pool;
  ...
  async recordFunnelEvent(userId: string, eventType: string, eventName: string, metadata?: Record<string, any>): Promise<void> {
    const client = await this.pool.connect();
    try {
      const occurredAt = new Date();
      
      await client.query(
        `INSERT INTO funnel_events (user_id, event_type, event_name, occurred_at, metadata)
         VALUES ($1, $2, $3, $4, $5)`,
        [userId, eventType, eventName, occurredAt, JSON.stringify(metadata || {})]
      );
  ...
  private async updateUserJourney(client: PoolClient, userId: string, eventType: string): Promise<void> {
    ...
    const journeyResult = await client.query(
      `SELECT completed_steps, current_step FROM user_funnel_journeys
       WHERE user_id = $1 AND funnel_name = $2`,
      [userId, funnelName]
    );
    ...
    await client.query(
      `INSERT INTO user_funnel_journeys (user_id, funnel_name, current_step, completed_steps, last_updated_at)
       VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP)
       ON CONFLICT (user_id, funnel_name) DO UPDATE SET
       current_step = $3, completed_steps = $4, last_updated_at = CURRENT_TIMESTAMP`,
      [userId, funnelName, nextStep, completedSteps]
    );

    // Update conversion tracking
    await client.query(
      `INSERT INTO funnel_conversion_tracking (funnel_name, step_number, step_name, completed_count)
       VALUES ($1, $2, $3, 1)
       ON CONFLICT (funnel_name, step_number) DO UPDATE SET
       completed_count = funnel_conversion_tracking.completed_count + 1`,
      [funnelName, stepIndex, eventType]
    );
  }

  async getUserJourney(userId: string, funnelName: string = 'onboarding'): Promise<UserJourney | null> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `SELECT user_id, current_step, completed_steps FROM user_funnel_journeys
         WHERE user_id = $1 AND funnel_name = $2`,
        [userId, funnelName]
      );
      ...
      const eventResult = await client.query(
        `SELECT event_type, occurred_at FROM funnel_events
         WHERE user_id = $1
  ...
  private normalizeCompletedSteps(value: unknown): string[] {
  ...
  async getFunnelMetrics(funnelName: string = 'onboarding'): Promise<FunnelMetrics> {
    const client = await this.pool.connect();
    try {
      // Get total users
      const totalUsersResult = await client.query(
        `SELECT COUNT(DISTINCT user_id) as count FROM funnel_events
      ...
      const stepsResult = await client.query(
        `SELECT step_number, step_name, completed_count FROM funnel_conversion_tracking
         WHERE funnel_name = $1
  ...
  async getConversionRateByStep(funnelName: string, stepNumber: number): Promise<number> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `SELECT completed_count FROM funnel_conversion_tracking
         WHERE funnel_name = $1 AND step_number = $2`,
        [funnelName, stepNumber]
      );
  ...
  async trackConversionBySegment(userId: string, segmentName: string): Promise<void> {
    const client = await this.pool.connect();
    try {
  ...
  async getDropoffRate(funnelName: string, fromStep: number, toStep: number): Promise<number> {
    const client = await this.pool.connect();
    try {
      ...
      if (fromResult.rows.length === 0) return 0;
  ...
  async cleanupOldFunnelData(daysOld: number = 90): Promise<number> {
    const client = await this.pool.connect();
    try {
...
export async function initializeFunnelAnalyticsRoutes(service: FunnelAnalyticsService) {
  ...
  router.get('/api/funnel/dropoff', async (req, res) => {
    try {
      const funnelName = req.query.funnel as string || 'onboarding';
      const fromStep = parseInt(req.query.from as string) || 0;
      const toStep = parseInt(req.query.to as string) || 1;
