...
- **Without version control** — impossible to audit who changed wha
...
- **Automated testing** — infrastructure validated before deploymen
...
- **Cost** — requires Pulumi service account for state managemen
...
- ✅ **Code reviewed** — infrastructure changes peer-reviewed before deploymen
...
- **Mitigation strategy**:
  - State encryption: GCS bucket encryption at res
...
- **Monitoring**:
  - Terraform apply logs streamed to CI outpu
...
- Access control: IAM role `roles/storage.admin` restricted to service accoun
...
- `terraform plan`: generate plan artifac
