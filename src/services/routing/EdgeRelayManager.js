...
export class EdgeRelayManager extends EventEmitter {
     ...
     */
    updateRelayHealth(relayId, healthy, latencyMs) {
    ...
    selectRelay(request) {
        ...
        return this.buildDecision(selectedRelay, selectedRelay.latencyMs <= this.config.targetLatencyMs
            ? 'target latency'
            : 'best available relay', false, request, candidateRelays.slice(1).map((relay) => relay.relayId));
    }
    /**
     * Migrate a session from one relay to another.
     */
    migrateSession(sessionId, toRelayId, reason = 'relay migration') {
     ...
     */
    recordSessionActivity(sessionId, relayId, latencyMs) {
        const relay = this.relays.get(relayId);
        if (!relay)
            return;
    ...
    recordLatency(relayId, latencyMs) {
        const samples = this.latencySamples.get(relayId) || [];
    ...
    buildDecision(relay, reason, migrated, request, alternateRelays = []) {
        return {
            relayId: relay.relayId,
            regionId: relay.regionId,
...
//# sourceMappingURL=EdgeRelayManager.js.map