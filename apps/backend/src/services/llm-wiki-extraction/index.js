...
export class LLMWikiExtractionService extends EventEmitter {
    ...
    async createKnowledgeEntry(title, content, summary, embedding, authorId, options) {
        const client = await this.pool.connect();
        try {
    ...
    async searchKnowledge(query, queryEmbedding, limit = 10) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`
        SELECT 
    ...
    async trackUsage(entryId, userId, sessionId, context) {
        const client = await this.pool.connect();
        try {
            await client.query(`
    ...
    async rateFeedback(entryId, userId, rating, helpful, comment) {
    ...
    async extractFromSession(request, extractedEntries) {
        const client = await this.pool.connect();
        try {
            await client.query('BEGIN');
