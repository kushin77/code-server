...
export class ActivityFeedService extends EventEmitter {
    ...
    async recordActivity(type, title, status = 'info', options) {
        const client = await this.pool.connect();
        try {
    ...
    async getActivities(filter, limit = 50) {
        const client = await this.pool.connect();
        try {
            let query = 'SELECT * FROM activities WHERE timestamp >= NOW() - INTERVAL \'1 day\' * $1';
            const params = [this.config.maxHistoryDays];
    ...
    async createSubscription(userId, filters) {
        const client = await this.pool.connect();
        try {
            const id = require('crypto').randomUUID();
    ...
    async notifySubscribers(activityId) {
        const client = await this.pool.connect();
        try {
    ...
    async getPendingNotifications(subscriptionId, limit = 10) {
        const client = await this.pool.connect();
        try {
    ...
    async markNotificationAsRead(activityId, subscriptionId) {
        const client = await this.pool.connect();
        try {
