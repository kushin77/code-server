...
import { HelpQueueService } from '../services/help-queue';
import { getLogger } from '../lib/logger';
const logger = getLogger('HelpQueueRoutes');
export function initializeHelpQueueRoutes(pool, auditService) {
    const router = Router();
    ...
    router.post('/', async (req, res) => {
        try {
            ...
            const request = await helpQueueService.createRequest(userId, codeSnippet, question, urgency || 'normal', tags);
            res.status(201).json({
                success: true,
                request,
            });
    ...
    router.get('/user/:userId', async (req, res) => {
        try {
            ...
            const requests = await helpQueueService.getUserRequests(userId, status);
            res.json({
                success: true,
    ...
    router.get('/queue/open', async (req, res) => {
        try {
            ...
            const requests = await helpQueueService.getOpenRequests(parseInt(limit, 10));
            res.json({
                success: true,
                count: requests.length,
                requests,
    ...
    router.get('/metrics/sla', async (req, res) => {
        try {
            ...
            const metrics = await helpQueueService.getSLAMetrics(parseInt(timeframeMs, 10));
            res.json({
                success: true,
                metrics,
            });
    ...
    router.get('/experts/available', async (req, res) => {
        try {
            ...
            const experts = await helpQueueService.getAvailableExperts(tagArray);
            res.json({
                success: true,
...
//# sourceMappingURL=help-queue.js.map