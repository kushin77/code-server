...
import { initPRPreviewService, getPRPreviewService } from "../index.js"

describe("PR Preview Service", () => {
  let service: ReturnType<typeof initPRPreviewService>
  ...
  describe("Billing Calculation", () => {
    it("should calculate billing for repository", (done) => {
      ...
      setTimeout(() => {
        ...
        expect(billing.breakdown.computerCost + billing.breakdown.databaseCost + billing.breakdown.networkCost).toBeCloseTo(
          billing.totalCost,
          1
        )
  ...
  describe("Concurrent Previews", () => {
    it("should handle multiple concurrent previews", (done) => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        if (result.previewId) ids.push(result.previewId)
      }
    ...
    it("should manage unique IDs for concurrent previews", () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (result.previewId) previews.push(result.previewId)
      }
