...
export type ExpertiseLevel = 'expert' | 'intermediate' | 'novice';
...
export interface ReviewerWorkload {
  ...
  completedReviewsLast7Days: number;
...
export interface AIReviewerRouterConfig {
  maxReviewersToScore?: number;
  minExpertiseThreshold?: number;
  workloadWeightPercent?: number;
  expertiseWeightPercent?: number;
...
export class AIReviewerRouterService extends EventEmitter {
  ...
  async registerReviewerExpertise(
    reviewerId: string,
    filePattern: string,
    expertiseLevel: ExpertiseLevel,
    confidence: number
  ): Promise<ReviewerExpertise> {
    ...
    try {
      ...
      await client.query(
        `INSERT INTO reviewer_expertise (id, reviewer_id, file_pattern, expertise_level, confidence, last_worked_at)
         VALUES ($1, $2, $3, $4, $5, NOW())`,
        [id, reviewerId, filePattern, expertiseLevel, confidence]
      );
  ...
  async updateReviewerWorkload(
    ...
    try {
      await client.query(
        `INSERT INTO reviewer_workload (reviewer_id, pending_reviews, completed_reviews_last_7days, average_review_time_minutes)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (reviewer_id) DO UPDATE SET
           ...
           updated_at = NOW()`,
        [reviewerId, pendingReviews, completedReviewsLast7Days, averageReviewTimeMinutes]
      );
  ...
  async updateReviewerAvailability(
    ...
    timezone: string,
    isOnline: boolean,
    preferredWorkHoursStart?: number,
    preferredWorkHoursEnd?: number
  ): Promise<ReviewerAvailability> {
    const client = await this.pool.connect();
    try {
      await client.query(
        `INSERT INTO reviewer_availability (reviewer_id, timezone, is_online, last_active_at, preferred_work_hours_start, preferred_work_hours_end)
  ...
  async assignReview(
    ...
    try {
      ...
      await client.query(
        `INSERT INTO review_assignments (id, pull_request_id, reviewer_id, expertise_score, workload_score, availability_score, total_score, score_explanation)
      ...
      await client.query(
        `UPDATE reviewer_workload SET pending_reviews = pending_reviews + 1 WHERE reviewer_id = $1`,
        [bestReviewer.reviewerId]
      );
  ...
  async scoreReviewers(changedFiles: string[], teamId: string): Promise<ReviewerScore[]> {
    const client = await this.pool.connect();
    try {
      // Get all reviewers with their data
      const result = await client.query(
        `SELECT DISTINCT re.reviewer_id FROM reviewer_expertise re
         WHERE re.confidence >= $1
         LIMIT $2`,
        [this.config.minExpertiseThreshold, this.config.maxReviewersToScore]
      );
      ...
      for (const reviewerId of reviewerIds) {
        ...
        const expertiseResult = await client.query(
          `SELECT AVG(confidence) as avg_confidence FROM reviewer_expertise
           WHERE reviewer_id = $1`,
          [reviewerId]
        );
        ...
        const workloadResult = await client.query(
          `SELECT pending_reviews, completed_reviews_last_7days FROM reviewer_workload
           WHERE reviewer_id = $1`,
          [reviewerId]
        );
        ...
        const availabilityResult = await client.query(
          `SELECT is_online, timezone FROM reviewer_availability
           WHERE reviewer_id = $1`,
          [reviewerId]
        );
        ...
        const totalScore =
          (expertiseScore * this.config.expertiseWeightPercent +
  ...
  async completeReview(assignmentId: string): Promise<void> {
    const client = await this.pool.connect();
    try {
      // Get assignment to find reviewer
      const result = await client.query(
        `SELECT reviewer_id FROM review_assignments WHERE id = $1`,
        [assignmentId]
      );
      ...
      // Mark as completed
      await client.query(
        `UPDATE review_assignments SET completed_at = NOW() WHERE id = $1`,
        [assignmentId]
      );

      // Decrement pending reviews
      await client.query(
        `UPDATE reviewer_workload SET pending_reviews = GREATEST(0, pending_reviews - 1) WHERE reviewer_id = $1`,
        [reviewerId]
      );
