...
const cloneWorkingHours = (workingHours?: TeamHubUser['workingHours']): TeamHubUser['workingHours'] | undefined => {
  if (!workingHours) {
    return undefined;
  }
...
export const cloneUser = (user: TeamHubUser): TeamHubUser => ({
  ...user,
...
export const cloneUsers = (users: TeamHubUser[]): TeamHubUser[] => users.map((user) => cloneUser(user));
