...
export async function registerAuthServiceWorker() {
    ...
    try {
        ...
        console.info('[auth-sw-register] SW registered successfully', `scope: ${swRegistration.scope}`);
 ...
 */
export function getServiceWorkerHealth() {
    return { ...swHealth };
}
/**
 ...
 */
export async function unregisterAuthServiceWorker() {
    if (!swRegistration)
        return;
    try {
...
if (typeof window !== 'undefined') {
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
            registerAuthServiceWorker().catch(err => console.error('[auth-sw-register] Initialization failed:', err));
        });
    }
    else {
        registerAuthServiceWorker().catch(err => console.error('[auth-sw-register] Initialization failed:', err));
...
//# sourceMappingURL=auth-sw-register.js.map