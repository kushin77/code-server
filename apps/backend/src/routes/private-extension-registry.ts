...
import {
  ...
  type PrivateExtensionRegistryDecision,
} from '../services/private-extension-registry';
import { getLogger } from '../lib/logger';

const logger = getLogger('PrivateExtensionRegistryRoutes');

function emitValidationAudit(decision: PrivateExtensionRegistryDecision, actor: string): void {
...
export function initializePrivateExtensionRegistryRoutes(service: PrivateExtensionRegistryService): Router {
  const router = Router();

  router.get('/api/extensions/registry', (_request: Request, response: Response) => {
    try {
  ...
  router.get('/api/extensions/registry/approved', (_request: Request, response: Response) => {
    try {
  ...
  router.get('/api/extensions/registry/blocked', (_request: Request, response: Response) => {
    try {
  ...
  router.get('/api/extensions/registry/validate', (request: Request, response: Response) => {
    try {
  ...
  router.get('/api/extensions/registry/publish-check', (request: Request, response: Response) => {
    try {
      const actor = typeof request.query.actor === 'string' && request.query.actor.trim()
        ? request.query.actor.trim()
        : 'system';
