...
import type { HotStandbyConfig } from './types';
...
export class HotStandbyIntegration {
  private stateMachine: HotStandbyStateMachine | null = null;
  ...
  async enable(options: {
    ...
    try {
      this.stateMachine = new HotStandbyStateMachine(
        options.brokerId,
        options.remoteBrokerId,
        this.redis,
        options.config
      );
   ...
   */
  getStateMachine(): HotStandbyStateMachine | null {
 ...
 */
let instance: HotStandbyIntegration | null = null;

export function createHotStandbyIntegration(redis: Redis): HotStandbyIntegration {
  if (!instance) {
...
export function getHotStandbyIntegration(): HotStandbyIntegration | null {
  return instance;
}
