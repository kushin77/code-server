...
export class HotStandbyStateMachine extends EventEmitter {
    constructor(brokerId, remoteBrokerId, redis, config) {
        super();
        this.role = 'unknown';
        this.state = 'healthy';
        this.remoteBrokerHealth = null;
        ...
        this.failureDetectionStartTime = 0;
        this.promotionStartTime = 0;
     ...
     */
    async initialize(initialRole = 'primary') {
    ...
    async checkHeartbeatHealth() {
        ...
        if (timeSinceLastHeartbeat > this.config.heartbeatTimeout) {
            ...
            if (this.remoteBrokerHealth.missedCount >= this.config.failureThreshold &&
    ...
    async promoteToMaster() {
        ...
        try {
            ...
            const lockAcquired = await this.redis.set(lockKey, this.brokerId, 'EX', this.config.promotionLockTtl / 1000, 'NX');
            if (!lockAcquired) {
    ...
    startRecoveryChecking() {
        if (this.recoveryCheckIntervalId)
            clearInterval(this.recoveryCheckIntervalId);
    ...
    async updateBrokerInfo() {
        try {
            ...
            await this.redis.set(infoKey, JSON.stringify({
                brokerId: this.brokerId,
                role: this.role,
                timestamp: Date.now(),
                sessionCount: this.sessionCount,
     ...
     */
    recordFailoverEvent(type, options = {}) {
        const event = {
            type,
            timestamp: Date.now(),
            brokerId: this.brokerId,
