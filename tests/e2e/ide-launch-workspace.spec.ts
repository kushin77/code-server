...
test.describe('IDE Launch & Workspace Operations - E2E Tests', () => {
  ...
  test.describe('Happy Path - IDE Launch', () => {
    ...
    test('user can open file browser', async ({ authenticatedPage }) => {
      ...
      const fileExplorer = authenticatedPage.locator('[data-testid="file-explorer"]').or(
        authenticatedPage.locator('.codicon-files')
      );
    ...
    test('user can create new file', async ({ authenticatedPage }) => {
      ...
      const newFileBtn = authenticatedPage.locator('button[aria-label*="New File"]').or(
        authenticatedPage.locator('text=New File')
      );
      ...
      if (await newFileBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
        ...
        const filenameInput = authenticatedPage.locator('input[placeholder*="name"]').or(
    ...
    test('user can see file tree structure', async ({ authenticatedPage }) => {
      ...
      const fileTree = authenticatedPage.locator('[data-testid="file-tree"]').or(
        authenticatedPage.locator('.explorer')
      );
  ...
  test.describe('Happy Path - Terminal Operations', () => {
    test('user can open terminal', async ({ authenticatedPage }) => {
      ...
      const terminalBtn = authenticatedPage.locator('[aria-label*="Terminal"]').or(
        authenticatedPage.locator('text=Terminal')
      );
    ...
    test('user can run commands in terminal', async ({ authenticatedPage }) => {
      ...
      const terminal = authenticatedPage.locator('[data-testid="terminal"]').or(
        authenticatedPage.locator('.xterm')
      );
  ...
  test.describe('Happy Path - Extensions & Customization', () => {
    test('user can access extensions panel', async ({ authenticatedPage }) => {
      ...
      const extensionsBtn = authenticatedPage.locator('[aria-label*="Extensions"]').or(
        authenticatedPage.locator('.codicon-extensions')
      );
    ...
    test('user can search for extensions', async ({ authenticatedPage }) => {
      ...
      const searchInput = authenticatedPage.locator('[data-testid="extension-search"]').or(
