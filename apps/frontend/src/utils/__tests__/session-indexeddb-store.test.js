...
import { storeSessionExpiry, getSessionExpiry, clearSessionExpiry, isSessionValid, } from '../session-indexeddb-store';
...
class MockIDBObjectStore {
    ...
    static _makeRequest(result) {
        const req = { result, error: null };
        Object.defineProperty(req, 'onsuccess', {
            set(cb) {
                if (cb)
                    queueMicrotask(() => cb({ target: req }));
            },
            configurable: true,
        });
...
function makeMockOpenRequest(db) {
...
describe('session-indexeddb-store', () => {
    ...
    describe('storeSessionExpiry', () => {
        ...
        it('should handle store error gracefully', async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith('[session-store] Failed to store expiry in IndexedDB:', expect.any(Error));
...
//# sourceMappingURL=session-indexeddb-store.test.js.map