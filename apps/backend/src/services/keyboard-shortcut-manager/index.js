...
export class KeyboardShortcutManagerService extends EventEmitter {
    constructor(pool) {
        super();
        this.logger = getLogger('KeyboardShortcutManagerService');
        this.pool = pool;
    }
    async initialize() {
    ...
    async createProfile(name, description, createdBy, shortcuts, isOrgDefault = false) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`INSERT INTO shortcut_profiles (name, description, created_by, shortcuts, is_org_default)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING id`, [name, description, createdBy, JSON.stringify(shortcuts), isOrgDefault]);
    ...
    async getProfile(profileId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT id, name, description, is_org_default, created_by, created_at, shortcuts FROM shortcut_profiles WHERE id = $1`, [profileId]);
    ...
    async getOrgDefaultProfile() {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT id, name, description, is_org_default, created_by, created_at, shortcuts FROM shortcut_profiles WHERE is_org_default = TRUE LIMIT 1`);
            if (result.rows.length === 0)
                return null;
            const row = result.rows[0];
            return {
    ...
    async setPersonalOverrides(userId, profileId, overrides) {
        const client = await this.pool.connect();
        try {
            // Check if override exists
            const existing = await client.query(`SELECT id FROM personal_shortcut_overrides WHERE user_id = $1 AND profile_id = $2`, [userId, profileId]);
            if (existing.rows.length > 0) {
                // Update existing
                await client.query(`UPDATE personal_shortcut_overrides SET overrides = $1, updated_at = CURRENT_TIMESTAMP WHERE user_id = $2 AND profile_id = $3`, [JSON.stringify(overrides), userId, profileId]);
            }
            else {
                // Insert new
                await client.query(`INSERT INTO personal_shortcut_overrides (user_id, profile_id, overrides) VALUES ($1, $2, $3)`, [userId, profileId, JSON.stringify(overrides)]);
            }
    ...
    async getPersonalOverrides(userId, profileId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT overrides FROM personal_shortcut_overrides WHERE user_id = $1 AND profile_id = $2`, [userId, profileId]);
            if (result.rows.length === 0)
                return null;
    ...
    async getMergedShortcuts(userId, profileId) {
        const client = await this.pool.connect();
        try {
    ...
    async detectClashes(profileId1, profileId2) {
        const client = await this.pool.connect();
        try {
            ...
            for (const [shortcut, command1] of Object.entries(profile1.shortcuts)) {
                if (shortcut in profile2.shortcuts) {
                    ...
                    if (command1 !== command2) {
                        ...
                        await client.query(`INSERT INTO shortcut_clash_detection (shortcut_key, profile_id1, profile_id2, conflict_command1, conflict_command2, severity)
               VALUES ($1, $2, $3, $4, $5, $6)`, [shortcut, profileId1, profileId2, command1, command2, severity]);
                    }
                }
            }
            if (clashes.length > 0) {
    ...
    async trackUsage(userId, shortcutKey, command) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO shortcut_usage_tracking (user_id, shortcut_key, command) VALUES ($1, $2, $3)`, [userId, shortcutKey, command]);
        }
        finally {
            client.release();
        }
    }
    async getTopShortcuts(userId, limit = 10) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT shortcut_key, command, COUNT(*) as usage_count
    ...
    async syncProfileToTeam(profileId, teamId) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO shortcut_team_sync (team_id, profile_id, sync_status) VALUES ($1, $2, 'synced')`, [teamId, profileId]);
    ...
    async getTeamSyncedProfiles(teamId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT p.id, p.name, p.description, p.is_org_default, p.created_by, p.created_at, p.shortcuts
         FROM shortcut_team_sync s
         JOIN shortcut_profiles p ON s.profile_id = p.id
         WHERE s.team_id = $1
         ORDER BY s.synced_at DESC`, [teamId]);
    ...
    async quickSwitch(userId, fromProfileId, toProfileId) {
        const client = await this.pool.connect();
        try {
    ...
    async listProfiles(limit = 20) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT id, name, description, is_org_default, created_by, created_at, shortcuts
    ...
    async updateProfile(profileId, name, description, shortcuts) {
        const client = await this.pool.connect();
        try {
            await client.query(`UPDATE shortcut_profiles SET name = $1, description = $2, shortcuts = $3, updated_at = CURRENT_TIMESTAMP WHERE id = $4`, [name, description, JSON.stringify(shortcuts), profileId]);
    ...
    async cleanupOldUsageData(daysOld = 90) {
        const client = await this.pool.connect();
        try {
            const cutoffDate = new Date();
            cutoffDate.setDate(cutoffDate.getDate() - daysOld);
            const result = await client.query(`DELETE FROM shortcut_usage_tracking WHERE used_at < $1`, [cutoffDate]);
...
export async function initializeKeyboardShortcutManagerRoutes(service) {
