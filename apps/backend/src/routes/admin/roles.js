 ...
 */
router.post('/:userId/assign', requireRole('admin'), async (req, res) => {
    try {
        ...
        const roleManager = getRoleManager();
        await roleManager.assignRoles(req.params.userId, roles, expiresIn);
        logger.info(`Assigned roles ${roles.join(', ')} to user ${req.params.userId}`, {
            admin: req.user?.email,
            userId: req.params.userId,
            roles,
            expiresIn,
 ...
 */
router.delete('/:userId/:roleName', requireRole('admin'), async (req, res) => {
    try {
        const roleManager = getRoleManager();
        await roleManager.removeRole(req.params.userId, req.params.roleName);
        logger.info(`Removed role ${req.params.roleName} from user ${req.params.userId}`, {
            admin: req.user?.email,
            userId: req.params.userId,
 ...
 */
router.post('/:userId/clear', requireRole('admin'), async (req, res) => {
    try {
        const roleManager = getRoleManager();
        await roleManager.clearRoles(req.params.userId);
        logger.warn(`Cleared all roles for user ${req.params.userId}`, {
            admin: req.user?.email,
            userId: req.params.userId,
        });
        res.json({
 ...
 */
router.get('/list/all', requireRole('admin'), async (req, res) => {
    try {
        const roleManager = getRoleManager();
 ...
 */
router.post('/audit/export', requireRole('admin'), async (req, res) => {
    try {
        const roleManager = getRoleManager();
