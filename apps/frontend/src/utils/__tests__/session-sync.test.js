...
import { initSessionSync, destroySessionSync, broadcastSessionRefresh, broadcastSessionExpiry, acquireRefreshLock, releaseRefreshLock, isLeader, getTabId, getKnownTabs, getMetrics, resetMetrics, } from "../session-sync";
...
describe("Session Sync - Multi-tab Synchronization", () => {
    ...
    describe("Session Broadcasts", () => {
        test("broadcastSessionRefresh() sends SESSION_REFRESHED message", () => {
            initSessionSync();
            const channel = MockBroadcastChannel.instances.find((c) => c.name === "code-server-session");
        ...
        test("BroadcastChannel messages are received by other tabs", () => {
            ...
            anotherTab.addEventListener("message", (event) => {
    ...
    describe("Configuration", () => {
        test("initSessionSync() with custom config", () => {
            ...
            const customChannel = MockBroadcastChannel.instances.find((c) => c.name === "custom-channel");
...
//# sourceMappingURL=session-sync.test.js.map