...
export const LoginPage = () => {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-sky-50 to-blue-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <>
              ...
              <form onSubmit={handleLoginSubmit} className="space-y-4">
                {/* Email Input */}
                <Input label="Email" type="email" placeholder="admin@example.com" value={formData.email} onChange={(e) => {
                setFormData({ ...formData, email: e.target.value });
                setLocalError(null);
            }} disabled={isLoading}/>

                {/* Password Input */}
                <Input label="Password" type="password" placeholder="••••••••" value={formData.password} onChange={(e) => {
                setFormData({ ...formData, password: e.target.value });
                setLocalError(null);
            }} disabled={isLoading}/>

                {/* Organization Input */}
                <Input label="Organization" type="text" placeholder="acme-corp" value={formData.org_slug} onChange={(e) => {
                setFormData({ ...formData, org_slug: e.target.value });
                setLocalError(null);
            }} disabled={isLoading}/>

                {/* Submit Button */}
                <Button type="submit" label={isLoading ? 'Signing in...' : 'Sign In'} variant="primary" fullWidth loading={isLoading} disabled={isLoading}/>
              </form>
        ...
        <>
              ...
              <form onSubmit={handleMFASubmit} className="space-y-4">
                {/* TOTP Code Input */}
                <Input label="Authenticator Code" type="text" placeholder="000000" maxLength={6} value={mfaState.totpCode} onChange={(e) => {
                // Only allow digits
                const value = e.target.value.replace(/\D/g, '').slice(0, 6);
                setMfaState({ ...mfaState, totpCode: value });
                setLocalError(null);
            }} disabled={isLoading} inputMode="numeric"/>

                {/* Submit Button */}
                <Button type="submit" label={isLoading ? 'Verifying...' : 'Verify'} variant="primary" fullWidth loading={isLoading} disabled={isLoading || mfaState.totpCode.length !== 6}/>

                {/* Back Button */}
                <Button type="button" label="Back" variant="secondary" fullWidth onClick={handleBackFromMFA} disabled={isLoading}/>
              </form>
