...
export interface RankingFeatures {
  ...
  syntacticSimilarity: number; // 0-1
  tokenOverlap: number; // 0-1
  fileRelevance: number; // 0-1
  recency: number; // 0-1
...
export class CrossEncoderReranker {
  ...
  extractFeatures(
    ...
    const recency =
      maxAge > 0 ? 1 - (result.lineNumber || 0) / maxAge : 0.5;
  ...
  private calculateFileRelevance(
    ...
    const dirBoosts: Record<string, number> = {
      'test': 0.2,
      'spec': 0.2,
      'mock': 0.15,
      'utils': 0.15,
    ...
    const intentFileTypes: Record<string, string[]> = {
   ...
   */
  private estimateCodeQuality(result: SearchResult): number {
   ...
   */
  private estimateTestCoverage(result: SearchResult): number {
    const filePath = result.filePath.toLowerCase();
    const text = result.text.toLowerCase();
   ...
   */
  private generateReasoning(features: RankingFeatures, score: number): string {
  ...
  adjustWeightsForIntent(intent: QueryIntent): void {
    ...
    for (const key in this.weights) {
      (this.weights as Record<string, number>)[key] /= total;
    }
  }
