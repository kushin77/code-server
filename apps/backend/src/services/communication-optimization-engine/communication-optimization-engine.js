...
import { CommunicationMode, CommunicationChannel, CommunicationUrgency, DecisionReason, } from './types';
/**
...
export class CommunicationOptimizationEngine extends EventEmitter {
     ...
     */
    async recommendCommunication(context) {
        ...
        if (!readinessCheck.allAvailable && context.urgency !== CommunicationUrgency.CRITICAL) {
            ...
            if (readinessCheck.focusTimeActive)
                deferralReasons.push(DecisionReason.FOCUS_TIME);
            if (readinessCheck.dndActive)
                deferralReasons.push(DecisionReason.DND_ACTIVE);
            if (readinessCheck.unavailable)
                deferralReasons.push(DecisionReason.USER_UNAVAILABLE);
        ...
        const shouldEscalate = this.config.enableEscalationLogic &&
     ...
     */
    async generateDigest(teamId, userId, periodStart, periodEnd) {
        const decisions = this.decisions.get(teamId) || [];
        // Filter decisions for digest candidates (low urgency, batch eligible)
        const digestItems = decisions
            .filter((d) => d.createdAt >= periodStart &&
     ...
     */
    async queryOptimization(context) {
        const startTime = performance.now();
        const result = {
            teamId: context.teamId,
            queryTime: 0,
     ...
     */
    async updateReadinessSignal(signal) {
        this.readinessSignals.set(signal.userId, signal);
    }
    /**
     * Record collaboration pattern
     */
    async recordCollaborationPattern(pattern) {
     ...
     */
    calculateDeferralTime(context, readiness) {
    ...
    async scoreChannels(context) {
        ...
        scores.push({
            ...
            rationale: context.urgency === CommunicationUrgency.LOW
        ...
        scores.push({
            ...
            rationale: context.urgency === CommunicationUrgency.CRITICAL
     ...
     */
    calculateModeScore(context, _channelScores) {
        let score = 50; // Neutral starting point
        // Async favoring factors
        if (context.urgency === CommunicationUrgency.LOW)
            score += 20;
        if (context.communicationType === 'update')
            score += 15;
        if (context.threadDepth && context.threadDepth > 3)
            score += 10;
        ...
        if (context.blockerPresent)
            score -= 20;
        if (context.communicationType === 'decision')
            score -= 15;
     ...
     */
    async recommendTiming(_context) {
        return {
     ...
     */
    estimateResponseTime(context, mode) {
 ...
 */
export function createCommunicationOptimizationEngine(config) {
