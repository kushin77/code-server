...
interface OnboardingWizardPanelProps {
  sessionId: string
  userId: string
  workspaceId: string
  teamId: string
 ...
 */
export const OnboardingWizardPanel: React.FC<OnboardingWizardPanelProps> = ({
  sessionId,
  userId,
  workspaceId,
  teamId,
  ...
  const updateProgress = (newSteps: Step[]) => {
              ...
              <div className="step-details">
                {currentStep.status === 'pending' && (
                  <div className="step-pending">
                    ...
                    {currentStep.autoRunnable && (
                      <button className="btn-primary" onClick={handleAutoRunAll}>
                        Start All Steps
                      </button>
                    )}
                  </div>
                ...
                {currentStep.status === 'completed' && (
                  <div className="step-success">
                    ...
                    {currentStep.result && (
                      <pre className="result-details">{JSON.stringify(currentStep.result, null, 2)}</pre>
                    )}
                  </div>
                )}
                ...
                {currentStep.status === 'failed' && (
                  <div className="step-error">
                    ...
                    {currentStep.error && (
                      <pre className="error-details">{currentStep.error}</pre>
                    )}
                    {currentStep.manualFallback && (
                      <p className="fallback-info">Please complete this step manually</p>
                    )}
                  </div>
                )}
              ...
              <div className="step-actions">
                ...
                {currentStep.status !== 'pending' && (
                  <button
                    ...
                    disabled={currentStepIndex === steps.length - 1 && !currentStep.completed}
                  >
