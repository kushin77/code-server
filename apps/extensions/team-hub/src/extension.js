...
export async function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('teamHub.sidebar', sidebarProvider), vscode.commands.registerCommand('teamHub.mentionUser', (userId) => actions.mentionUser(userId)), vscode.commands.registerCommand('teamHub.startMeet', (userIds) => actions.startMeet(userIds ?? [])), vscode.commands.registerCommand('teamHub.goToUserFile', (userId) => actions.goToUserFile(userId)), vscode.commands.registerCommand('teamHub.refreshPresence', () => actions.refreshPresence()), vscode.commands.registerCommand('teamHub.settings', () => actions.openSettings()), vscode.commands.registerCommand('teamHub.shareWorkspace', () => actions.shareWorkspace()), vscode.commands.registerCommand('teamHub.toggleDLP', () => terminalMonitor.toggleDLP()), new vscode.Disposable(() => presenceService.dispose()), new vscode.Disposable(() => terminalMonitor.dispose()));
...
class TerminalDLPMonitor {
    ...
    logDLPEvent(terminal, result) {
        ...
        if (result.severity === 'critical') {
            vscode.window.showWarningMessage(`Terminal DLP: Critical sensitive data detected in ${terminal.name}`, 'View Details').then((selection) => {
