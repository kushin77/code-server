...
describe('DORAMetricsService', () => {
  ...
  describe('initialization', () => {
    it('should initialize with database schema', async () => {
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('CREATE TABLE IF NOT EXISTS deployment_events')
      );
    });
  ...
  describe('recordDeployment', () => {
    it('should record a successful deployment', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO deployment_events'),
        expect.any(Array)
      );
    });
  ...
  describe('recordCommitMetric', () => {
    it('should record commit metric with lead time', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO commit_metrics'),
        expect.any(Array)
      );
    });
    ...
    it('should calculate lead time in milliseconds', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO commit_metrics'),
        expect.arrayContaining([3600000])
      );
    });
  });
  ...
  describe('recordFailureRecovery', () => {
    it('should record failure recovery with MTTR', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('UPDATE deployment_failures'),
        expect.any(Array)
      );
    });
    ...
    it('should calculate MTTR', async () => {
      ...
      await service.recordFailureRecovery(
        'deploy-1',
        new Date('2026-04-21T10:15:00Z'),
        'Database timeout'
      );

      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('UPDATE deployment_failures'),
        expect.arrayContaining([expectedMttr])
      );
    });
    ...
    it('should handle missing failure', async () => {
      ...
      await expect(
        service.recordFailureRecovery('unknown', new Date(), 'Test')
      ).rejects.toThrow();
    });
  });
  ...
  describe('getMetrics', () => {
    it('should calculate deployment frequency', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('deployment_events'),
        expect.any(Array)
      );
    });
  ...
  describe('snapshottMetrics', () => {
    it('should create weekly snapshot', async () => {
      ...
      await service.snapshottMetrics(
        1,
        new Date('2026-04-01'),
        new Date('2026-04-07')
      );
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO dora_weekly_snapshots'),
        expect.any(Array)
      );
    });
  ...
  describe('getTierBenchmarks', () => {
    ...
    it('should have increasing thresholds from elite to low', () => {
      ...
      expect(benchmarks.elite.deploymentFrequency.min).toBeGreaterThan(
        benchmarks.high.deploymentFrequency.min
      );
      expect(benchmarks.high.deploymentFrequency.min).toBeGreaterThan(
        benchmarks.medium.deploymentFrequency.min
      );
    });
  });
