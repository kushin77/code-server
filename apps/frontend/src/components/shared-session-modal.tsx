          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <Box>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <Chip
                  size="small"
                  label={getTimeRemaining(session.expiresAt)}
                  color={Date.now() > session.expiresAt ? 'error' : 'primary'}
                />
              </Box>
            </Box>
      ...
      <DialogActions>
        {onRevoke && session && (
          <Button
            ...
            disabled={revoking}
            startIcon={revoking ? <CircularProgress size={20} /> : <Delete />}
            color="error"
          >
        ...
        {session?.shareUrl && (
          <Button
            onClick={() => window.open(session.shareUrl, '_blank')}
            startIcon={<OpenInNew />}
            color="primary"
          >
            Join Session
