...
describe('AutoTestGenerationService', () => {
  let service: AutoTestGenerationService;
  ...
  beforeEach(() => {
    ...
    service = new AutoTestGenerationService(mockPool as Pool);
  });
    ...
    it('should initialize tables on first call', async () => {
      ...
      expect(
        mockClient.query.mock.calls.some(
          call => typeof call[0] === 'string' && call[0].includes('acceptance_at')
        )
      ).toBe(false);
  ...
  describe('generateTestsForSession', () => {
    ...
    it('should generate and store test suggestions', async () => {
      ...
      const tests = [{ id: 'test-1', sessionId: 'sess-1', fileName: 'api.ts', functionName: 'fetchUser', testCode: 'test code', coverage: 85, confidence: 92, aiContext: 'context', status: 'pending' as const, createdAt: new Date() }];
    ...
    it('should emit tests-generated event', async () => {
      ...
      const tests = [{ id: 'test-1', sessionId: 'sess-1', fileName: 'api.ts', functionName: 'fn', testCode: 'code', coverage: 80, confidence: 90, aiContext: 'ctx', status: 'pending' as const, createdAt: new Date() }];
  ...
  describe('recordTestExecution', () => {
    it('should record test execution result', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO test_execution_results'),
        expect.arrayContaining(['test-1', true, 150])
      );
      expect(eventSpy).toHaveBeenCalled();
    });
    ...
    it('should record test execution with error message', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO test_execution_results'),
        expect.arrayContaining(['test-1', false, 200, 'Expected 5 to be 10'])
      );
    });
  });
  ...
  describe('addFeedback', () => {
    it('should record feedback on test suggestion', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO test_feedback'),
        expect.arrayContaining(['test-1', 'user-1', 'helpful', 'Great test!'])
      );
      expect(eventSpy).toHaveBeenCalled();
    });
    ...
    it('should record feedback without comment', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO test_feedback'),
        expect.arrayContaining(['test-1', 'user-1', 'incorrect'])
      );
    });
  });
});
