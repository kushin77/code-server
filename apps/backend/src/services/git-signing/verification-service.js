...
export class GitSignatureVerificationService extends EventEmitter {
    constructor() {
        ...
        this.cacheTtlMs = 3600000; // 1 hour
    }
    /**
     * Initialize with configuration
     */
    async initialize(config, policy) {
        if (this.isInitialized)
            return;
        this.config = config;
        this.policy = policy || null;
        this.isInitialized = true;
        console.log(`[GitSignatureVerificationService] Initialized with provider: ${config.provider}`);
    ...
    async verifyCommit(commitHash) {
        ...
        try {
            ...
            if (this.policy?.allowedIdentities) {
                signature.trustLevel = this.policy.allowedIdentities.includes(signature.signerIdentity || '')
                    ? 'trusted'
                    : 'untrusted';
            }
            else {
            ...
            console.log(`[GitSignatureVerificationService] Verified ${commitHash} in ${signature.verificationTime.toFixed(2)}ms`);
            return signature;
        }
        catch (error) {
     ...
     */
    async verifyBatch(commitHashes) {
        if (!this.config)
            throw new Error('Service not initialized');
        const startTime = performance.now();
        const results = [];
        ...
        for (let i = 0; i < commitHashes.length; i += concurrency) {
            ...
            const verifications = await Promise.allSettled(batch.map((hash) => this.verifyCommit(hash)));
     ...
     */
    async checkCompliance(commitHash) {
        ...
        if (this.policy.allowedIdentities.length > 0 &&
            signature.signed &&
     ...
     */
    async getStatistics(commitHashes) {
 ...
 */
export async function getGitSignatureVerificationService(config, policy) {
    if (!serviceInstance) {
