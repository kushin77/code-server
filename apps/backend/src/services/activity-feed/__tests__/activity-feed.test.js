...
describe('ActivityFeedService', () => {
    ...
    describe('initialization', () => {
        it('should initialize with database schema', async () => {
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('CREATE TABLE IF NOT EXISTS activities'));
        });
    ...
    describe('recordActivity', () => {
        it('should record a commit activity', async () => {
            ...
            const activity = await service.recordActivity('commit', 'Fix: resolve memory leak in WebSocket handler', 'success', { userId: 'user-1', repository: 'code-server', tags: ['memory', 'websocket'] });
        ...
        it('should record a deployment activity', async () => {
            mockClient.query.mockResolvedValueOnce({ rows: [] }); // INSERT activity
            mockClient.query.mockResolvedValueOnce({ rows: [] }); // INSERT search index
            const activity = await service.recordActivity('deploy', 'Deploy v1.5.0 to production', 'success', {
        ...
        it('should record a test flake', async () => {
            mockClient.query.mockResolvedValueOnce({ rows: [] }); // INSERT activity
            mockClient.query.mockResolvedValueOnce({ rows: [] }); // INSERT search index
            const activity = await service.recordActivity('test-flake', 'Intermittent timeout in auth tests', 'warning', {
                repository: 'code-server',
                tags: ['testing', 'auth'],
        ...
        it('should create search index for activity', async () => {
            mockClient.query.mockResolvedValueOnce({ rows: [] }); // INSERT activity
            mockClient.query.mockResolvedValueOnce({ rows: [] }); // INSERT search index
            await service.recordActivity('pr', 'Add support for async/await in resolver', 'success', { description: 'Implement async patterns for better performance' });
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO activity_search_index'), expect.any(Array));
        });
    });
    ...
    describe('getActivities', () => {
        ...
        it('should filter by activity type', async () => {
            ...
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('type = ANY'), expect.any(Array));
        });
        it('should filter by status', async () => {
            ...
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('status = ANY'), expect.any(Array));
        });
        it('should filter by user ID', async () => {
            mockClient.query.mockResolvedValueOnce({ rows: [] });
            await service.getActivities({ userId: 'user-1' });
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('user_id = '), expect.arrayContaining(['user-1']));
        });
        it('should filter by repository', async () => {
            mockClient.query.mockResolvedValueOnce({ rows: [] });
            await service.getActivities({ repository: 'code-server' });
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('repository = '), expect.arrayContaining(['code-server']));
        });
        it('should filter by tags', async () => {
            mockClient.query.mockResolvedValueOnce({ rows: [] });
            await service.getActivities({ tags: ['memory', 'performance'] });
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('tags && '), expect.any(Array));
        });
        it('should search by text', async () => {
            mockClient.query.mockResolvedValueOnce({ rows: [] });
            await service.getActivities({ search: 'memory leak' });
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('activity_search_index'), expect.any(Array));
        });
        it('should respect limit parameter', async () => {
            mockClient.query.mockResolvedValueOnce({ rows: [] });
            await service.getActivities({}, 25);
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('LIMIT'), expect.arrayContaining([25]));
        });
    });
    describe('getStats', () => {
    ...
    describe('deleteSubscription', () => {
        it('should delete subscription', async () => {
            ...
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('DELETE FROM activity_subscriptions'), expect.any(Array));
        });
    });
    describe('notifySubscribers', () => {
        it('should notify matching subscribers', async () => {
            ...
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO activity_notifications'), expect.any(Array));
        });
    });
    ...
    describe('markNotificationAsRead', () => {
        it('should mark notification as read', async () => {
            ...
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE activity_notifications'), expect.arrayContaining(['activity-1', 'sub-1']));
        });
    });
});
//# sourceMappingURL=activity-feed.test.js.map