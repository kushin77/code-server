...
type OnboardingState = {
  ...
  checklist: Record<WizardChecklistKey, boolean>
}
...
function createDefaultOnboardingState(suggestedWorkspaceId: string, mfaEnabled: boolean): OnboardingState {
  return {
    stepIndex: 0,
...
function readOnboardingState(suggestedWorkspaceId: string, mfaEnabled: boolean): OnboardingState {
  ...
  try {
    ...
    const selectedWorkspaceId = getWorkspaceById(parsedState.selectedWorkspaceId ?? '')
      ? (parsedState.selectedWorkspaceId as string)
      : suggestedWorkspaceId
    const checklist = (parsedState.checklist ?? {}) as Partial<Record<WizardChecklistKey, boolean>>
...
export function WorkspaceOnboardingWizard() {
  ...
  const [state, setState] = useState<OnboardingState>(() =>
    readOnboardingState(suggestedWorkspaceId, user?.mfaEnabled ?? false)
  )
  ...
  const selectedWorkspace = useMemo<WorkspaceTab>(() => {
  ...
  const handleFinish = () => {
    ...
    const nextRecentRepoIds = buildRecentWorkspaceIds(
      ...
      currentTabs.recentRepoIds,
      currentTabs.recentRepoIds.length || DEFAULT_RECENT_WORKSPACES.length,
    )
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.9fr)]">
        <article className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm shadow-slate-200/70">
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {STEPS.map((step, index) => (
              <button
                key={step.key}
                type="button"
                onClick={() => setState((current) => ({ ...current, stepIndex: index }))}
          ...
          {!state.completed ? (
                ...
                <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
                  <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      {roleIds.length > 0 ? (
                        roleIds.map((roleId) => (
                          <span key={roleId} className="rounded-full bg-white px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-slate-600">
                            {formatRole(roleId)}
                          </span>
                        ))
                      ) : (
                  ...
                  <aside className="rounded-3xl border border-emerald-200 bg-emerald-50 p-5">
                    ...
                    <button
                      type="button"
                      onClick={() => handleWorkspaceSelect(suggestedWorkspaceId)}
                          ...
                          <div className="flex items-start justify-between gap-4">
                            ...
                            <span
                              className={`rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] ${
                                workspace.pinned ? 'bg-white text-emerald-700' : 'bg-white text-slate-500'
                              }`}
                            >
                  ...
                  {CHECKLIST_ITEMS.map((item) => (
                    <label
                      ...
                      className={`flex cursor-pointer gap-4 rounded-3xl border p-4 transition ${
                        state.checklist[item.key] ? 'border-emerald-300 bg-emerald-50' : 'border-slate-200 bg-slate-50 hover:border-slate-300'
                      }`}
                    >
                      <input
            ...
            <section className="mt-6 rounded-[1.75rem] border border-emerald-200 bg-emerald-50 p-6">
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <Link
                    className="rounded-full border border-emerald-300 bg-white px-4 py-2 text-sm font-medium text-emerald-800 transition hover:border-emerald-400"
                    to="/sessions"
                  >
                    Review sessions
                  </Link>
            ...
            <div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-5">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className="rounded-full border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-emerald-400 hover:text-emerald-700"
                >
                  Restart
                </button>
                <button
                  ...
                  disabled={checklistCompletedCount < CHECKLIST_ITEMS.length}
        ...
        <aside className="space-y-4">
          ...
          <section className="rounded-[1.75rem] border border-slate-200 bg-slate-50 p-5">
            ...
            <div className="mt-4 flex flex-col gap-3">
              <Link
                className="rounded-full bg-slate-900 px-4 py-2 text-center text-sm font-medium text-white transition hover:bg-slate-700"
                to="/"
              >
                Go to dashboard
              </Link>
              <Link
                className="rounded-full border border-slate-300 px-4 py-2 text-center text-sm font-medium text-slate-700 transition hover:border-slate-400 hover:bg-white"
                to="/sessions"
              >
