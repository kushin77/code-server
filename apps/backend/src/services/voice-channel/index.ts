...
export class VoiceChannelService extends EventEmitter {
  ...
  private sessionParticipants: Map<string, VoiceParticipant[]> = new Map();

  constructor(config: VoiceChannelConfig, pool: Pool, redis: Redis) {
    super();
    this.config = config;
    this.pool = pool;
    this.redis = redis;
  ...
  async joinSession(
    ...
    userId: string,
    userName: string
  ): Promise<{ token: string; session: VoiceSession }> {
   ...
   */
  getWorkspaceSessions(workspaceId: string): VoiceSession[] {
   ...
   */
  private generateLiveKitToken(identity: string, room: string, userId: string): string {
    try {
 ...
 */
export async function initializeVoiceChannelService(
