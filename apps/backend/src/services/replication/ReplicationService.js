...
export class ReplicationService extends EventEmitter {
    ...
    createOperation(operationType, resourceType, resourceId, payload) {
        const operation = this.crdtProtocol.createOperation(operationType, resourceType, resourceId, payload);
        this.health.pendingOperations++;
        this.metrics.queuedOperations++;
        // Queue for all peers
        const message = this.crdtProtocol.createReplicationMessage(this.nodeId, // Placeholder, will be set per peer
        [operation]);
     ...
     */
    async handleReplicationMessage(message) {
     ...
     */
    sendAcknowledgment(peerId, messageId) {
        const ackMessage = {
            id: `ack:${messageId}`,
    ...
    async syncWithPeer(peerId) {
        ...
        if (pending.length === 0)
            return;
        const message = this.crdtProtocol.createReplicationMessage(peerId, pending);
        ...
        this.syncInterval = setInterval(async () => {
            ...
            const event = {
                ...
                syncState: {
                    ...
                    pendingOperations: Array.from(this.messageQueue.values()).flatMap((msgs) => msgs.flatMap((msg) => msg.operations)),
                    appliedOperations: Array.from(this.peers.values()).flatMap((p) => p.appliedOperations),
                },
                timestamp: Date.now(),
            };
    ...
    updateMetrics(processingTime) {
        ...
        this.metrics.replicationLagMs = Math.max(0, timeSinceLastSync - this.policy.synchronizationBound);
     ...
     */
    getHealth() {
        this.health.activeConnections = Array.from(this.health.nodeAvailability.values()).filter((n) => n.available).length;
        this.health.pendingOperations = Array.from(this.messageQueue.values()).flatMap((msgs) => msgs).flatMap((msg) => msg.operations).length;
        return { ...this.health };
    }
    /**
...
//# sourceMappingURL=ReplicationService.js.map