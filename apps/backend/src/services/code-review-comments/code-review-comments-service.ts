...
import {
  ...
  ArchiveThreadResult,
  CommentThreadAuditEntry,
  CommentThreadServiceStatistics,
 ...
 */
export class CodeReviewCommentsService
  extends EventEmitter
  implements ICommentThreadService {
  private static instance: CodeReviewCommentsService;

  private threads: Map<string, CommentThread>;
  private threadsByReview: Map<string, Set<string>>;
  private commentNotifications: Map<string, UserNotification[]>;
  private auditLogs: Map<string, CommentThreadAuditEntry[]>;
   ...
   */
  static getInstance(config?: Partial<CommentThreadConfig>): CodeReviewCommentsService {
    if (!CodeReviewCommentsService.instance) {
      CodeReviewCommentsService.instance = new CodeReviewCommentsService();
    }
    if (config) {
      CodeReviewCommentsService.instance.updateConfig(
        config,
        'system',
        '127.0.0.1',
        'node'
  ...
  static reset(): void {
    ...
    CodeReviewCommentsService.instance = null as any;
  }
  ...
  createCommentThread(
    ...
    try {
      ...
      this.logAudit(
        ...
        threadId,
        { location: request.location, priority: request.priority }
      );
  ...
  addThreadComment(
    ...
    try {
      ...
      this.logAudit(
        ...
        request.author.userEmail,
        ipAddress,
        userAgent,
        'comment-added',
        thread.reviewRequestId,
        request.threadId,
        { commentId, mentionedCount: request.mentionedUserIds?.length || 0 }
      );
  ...
  editThreadComment(
    ...
    try {
      ...
      // Audit log
      this.logAudit(
        request.editedBy.userId,
        request.editedBy.userEmail,
        ipAddress,
        userAgent,
        'comment-edited',
        thread.reviewRequestId,
        request.threadId,
        { commentId: request.commentId, editCount: comment.editHistory.length }
      );
  ...
  deleteThreadComment(
    ...
    try {
      ...
      this.logAudit(
        ...
        request.deletedBy.userEmail,
        ipAddress,
        userAgent,
        'comment-deleted',
        thread.reviewRequestId,
        request.threadId,
        { commentId: request.commentId, reason: request.reason }
      );
  ...
  reopenThread(
    ...
    try {
      ...
      // Audit log
      this.logAudit(
        request.reopenedBy.userId,
        request.reopenedBy.userEmail,
        ipAddress,
        userAgent,
        'thread-reopened',
        thread.reviewRequestId,
  ...
  addReaction(
    ...
    try {
      ...
      this.logAudit(
        ...
        thread.reviewRequestId,
        request.threadId,
        { commentId: request.commentId, reactionType: request.reactionType }
      );
  ...
  mentionUser(
    ...
    try {
      ...
      // Audit log
      this.logAudit(
        request.mentionerEmail.split('@')[0],
        request.mentionerEmail,
        ipAddress,
        userAgent,
        'user-mentioned',
        '',
        request.threadId,
        { mentionedUser: request.userId, mentionedEmail: request.userEmail }
      );
  ...
  listCommentThreads(request: ListThreadsRequest): ListThreadsResult {
    try {
      ...
      if (request.filter) {
        ...
        if (request.filter.tags && request.filter.tags.length > 0) {
          threads = threads.filter((t) =>
            t.tags.some((tag) => request.filter!.tags!.includes(tag))
          );
        }
      }
      ...
      return { success: false, threads: [], count: 0, total: 0, error: (error as Error).message };
    }
  }
  ...
  archiveThread(
    ...
    try {
      ...
      // Audit log
      this.logAudit(
        request.archivedBy.userId,
        request.archivedBy.userEmail,
        ipAddress,
        userAgent,
        'thread-archived',
        thread.reviewRequestId,
        request.threadId,
        { reason: request.reason, commentCount: thread.comments.size }
      );
  ...
  getStatistics(): CommentThreadServiceStatistics {
    ...
    const averageThreadResolutionTime =
      resolutionTimes.length > 0
        ? resolutionTimes.reduce((a, b) => a + b, 0) / resolutionTimes.length
        : 0;

    const threadsWithMentions = threads.filter((t) =>
      Array.from(t.comments.values()).some((c) => c.mentionedUsers.length > 0)
    ).length;

    const threadsWithReactions = threads.filter((t) =>
      Array.from(t.comments.values()).some((c) => c.reactionCounts.size > 0)
    ).length;
  ...
  private logAudit(
    ...
    const entry: CommentThreadAuditEntry = {
      timestamp: Date.now(),
      userId,
      userEmail,
      ipAddress,
