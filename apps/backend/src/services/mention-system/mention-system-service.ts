...
export class MentionSystemService extends EventEmitter {
  ...
  private mentions: Map<string, MentionNotification> = new Map();
  private auditLog: Map<string, MentionAuditEntry[]> = new Map(); // Per-user audit trails
  private settings: Map<string, MentionSettings> = new Map();
  private auditService?: AuditService;
  private stats: MentionStats = {
    totalMentions: 0,
    mentionsByType: {},
    mentionsByUser: {},
    mentionedByUser: {},
  ...
  private config: MentionSystemConfig;

  constructor(config?: Partial<MentionSystemConfig>, auditService?: AuditService) {
    super();
  ...
  async createMention(
    ...
    ipAddress?: string,
    userAgent?: string,
    priority: MentionNotification['priority'] = 'normal'
    ...
    if (mentionedSettings?.privacyLevel === 'private' && createdBy !== mentionedUserId) {
      ...
      if (mentionedSettings.blockedUsers?.includes(createdBy)) {
        const auditEntry: MentionAuditEntry = {
   ...
   * Get mention
   */
  async getMention(mentionId: string): Promise<MentionNotification | undefined> {
   ...
   * Query mentions
   */
  async queryMentions(query: MentionQuery): Promise<MentionNotification[]> {
    let results: MentionNotification[] = Array.from(this.mentions.values());
  ...
  async readMention(
    ...
    if (!mention) throw new Error(`Mention not found: ${mentionId}`);
   ...
   */
  async getUserMentions(userId: string, unreadOnly: boolean = false): Promise<MentionNotification[]> {
   ...
   */
  async getMentionsByUser(createdBy: string): Promise<MentionNotification[]> {
   ...
   */
  async getAuditLog(userId: string, limit?: number): Promise<MentionAuditEntry[]> {
   ...
   * Get user settings
   */
  async getSettings(userId: string): Promise<MentionSettings | undefined> {
   ...
   */
  async updateSettings(userId: string, settings: Partial<MentionSettings>): Promise<MentionSettings> {
   ...
   * Get statistics
   */
  async getStatistics(): Promise<MentionStats> {
    return { ...this.stats };
  }
   ...
   * Get all mentions
   */
  async getAllMentions(): Promise<MentionNotification[]> {
   ...
   */
  private async logAudit(userId: string, entry: MentionAuditEntry): Promise<void> {
   ...
   */
  private static instance: MentionSystemService;

  static getInstance(config?: Partial<MentionSystemConfig>): MentionSystemService {
