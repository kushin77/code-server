...
/**
 * Configuration for partition recovery service
 */
export interface PartitionRecoveryConfig {
  enabled: boolean;
  primaryHost: string;
  replicaHost: string;
  checkIntervalMs: number; // How often to check connectivity (default 30s)
  failureThreshold: number; // Consecutive failures before declaring partition (default 3)
  recoveryCheckIntervalMs: number; // How often to check for partition healing (default 5s)
  quorumSize: number; // Minimum nodes for quorum (2 or 3)
  readOnlyMode: boolean; // Enable read-only during partition
}

/**
 * Network partition status
 */
export type PartitionStatus = 'healthy' | 'degraded' | 'partitioned' | 'recovering';

/**
 * Partition event details
 */
...
export class NetworkPartitionRecoveryService extends EventEmitter {
  private static instance: NetworkPartitionRecoveryService;
  private config: PartitionRecoveryConfig;
  private partitionStatus: PartitionStatus = 'healthy';
  ...
  private recoveryCheckInterval: NodeJS.Timer | null = null;

  private constructor(config?: Partial<PartitionRecoveryConfig>) {
    super();
    this.config = {
      enabled: true,
   ...
   */
  static getInstance(config?: Partial<PartitionRecoveryConfig>): NetworkPartitionRecoveryService {
  ...
  private async checkConnectivity(): Promise<void> {
    ...
    try {
      // Check each node
      const statusPromises = Array.from(this.nodeStatuses.values()).map(node =>
        this.checkNodeReachability(node)
      );
   ...
   */
  private async checkNodeReachability(node: NodeStatus): Promise<void> {
   ...
   */
  private async isNodeReachable(host: string): Promise<boolean> {
    try {
   ...
   */
  private evaluatePartitionStatus(): void {
   ...
   */
  private handlePartitionDetected(): void {
   ...
   */
  private handlePartitionRecovering(): void {
   ...
   */
  private handlePartitionHealed(): void {
    if (this.recoveryCheckInterval) {
      clearInterval(this.recoveryCheckInterval);
      this.recoveryCheckInterval = null;
    }
   ...
   */
  getPartitionHistory(limit: number = 100): PartitionEvent[] {
   ...
   */
  getConfig(): PartitionRecoveryConfig {
    return { ...this.config };
  }
