...
import {
  ...
  getWebSocketHealthService,
} from '../service';

describe('WebSocketHealthEngine', () => {
  let engine: WebSocketHealthEngine;
  ...
  describe('Connection State Management', () => {
    ...
    it('should detect frequent reconnections as critical', () => {
      ...
      const health = engine.getConnectionHealth('conn1');
      expect(health?.isCritical).toBe(true);
      expect(health?.issues.some((i) => i.type === 'frequent_reconnects')).toBe(
        true
      );
    });
  });
  ...
  describe('Stale Connection Detection', () => {
    it('should detect stale connections', (done) => {
      ...
      setTimeout(() => {
        ...
        expect(health?.issues.some((i) => i.type === 'stale_connection')).toBe(
          true
        );
        testEngine.destroy();
        done();
  ...
  describe('Aggregated Metrics', () => {
    ...
    it('should calculate health percentage', () => {
      ...
      engine.recordLatency(
        'conn1',
        DEFAULT_HEALTH_CONFIG.latencyCriticalMs + 100
      );
