...
export class SigstoreSigningService {
  private logger = getLogger("SigstoreSigningService")
  private gitsignPath = process.env.GITSIGN_PATH || "gitsign"
  private oidcIssuer = process.env.OIDC_ISSUER || "https://oauth2.googleapis.com"
  private sigstoreRoot = process.env.SIGSTORE_ROOT || path.join(process.env.HOME || "/home", ".sigstore")
  private gitBinary = process.env.GIT_BINARY || "git"
  ...
  verifyCommitSignature(
    ...
    try {
      ...
      const signatureData = execSync(
        `${this.gitBinary} show --format=%G? --no-patch ${commitHash}`,
        { cwd: workspacePath, encoding: "utf-8" }
      ).trim()

      // Map git verification status to ours
      const statusMap: Record<string, "valid" | "invalid" | "unknown"> = {
        G: "valid",
        B: "invalid",
        U: "unknown",
        X: "unknown",
      ...
      // Get signer name
      const signerName = execSync(
        `${this.gitBinary} show --format=%GS --no-patch ${commitHash}`,
        { cwd: workspacePath, encoding: "utf-8" }
      ).trim()
   ...
   */
  checkCertificateExpiration(key: SigningKey): {
    isExpiring: boolean
  ...
  getSignedCommits(
    ...
    try {
      ...
      const logOutput = execSync(
        `${this.gitBinary} log --format=%H%n%GS%n%G? -${limit}`,
        { cwd: workspacePath, encoding: "utf-8" }
      )

      const lines = logOutput.trim().split("\n")
      const commits: CommitSignature[] = []
   ...
   */
  exportPublicCertificate(userId: string): string | null {
    try {
  ...
  private ensureSigstoreRoot(): void {
  ...
  private generateFingerprint(userId: string, userEmail: string): string {
  ...
  private getCommitTime(commitHash: string, workspacePath: string): Date {
    try {
      const timestamp = execSync(
        `${this.gitBinary} show --format=%cI --no-patch ${commitHash}`,
        { cwd: workspacePath, encoding: "utf-8" }
      ).trim()
...
// Singleton instance
let instance: SigstoreSigningService | null = null

export function getSigstoreSigningService(): SigstoreSigningService {
  if (!instance) {
