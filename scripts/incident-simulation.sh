...
# ============================================================================
# GitHub issue routing for simulation findings
# ============================================================================
route_simulation_finding() {
  local scenario="$1"
  local response_time_ms="$2"
  local report_file="$3"

  if [[ "$ROUTE_TO_GITHUB_ISSUES" != "1" ]]; then
    echo "🧭 GitHub issue routing disabled; set ROUTE_TO_GITHUB_ISSUES=1 to route simulation findings."
    return 0
  fi

  if [[ -z "$GH_BIN" ]]; then
    log_fatal "GitHub CLI is required for incident issue routing"
  fi

  local fingerprint short_fp marker title body summary_block existing_issue tracker_state route_mode
  fingerprint="$(printf '%s' "incident-simulation:${scenario}" | sha256sum | awk '{print $1}')"
  short_fp="${fingerprint:0:8}"
  marker="<!-- incident-simulation-fingerprint:${fingerprint} -->"
  title="[incident-sim:${short_fp}] ${scenario} simulation findings"
  summary_block="$(tail -n 12 "$report_file" 2>/dev/null || true)"
  route_mode="create"

  body=$(cat <<EOF
${marker}
## Incident Simulation Findings
...
### Summary
The simulation completed and produced a report for attack-surface review and follow-up routing.
...
### Triage
- [ ] Convert the simulated finding into tracked remediation work
- [ ] Keep the attack surface review and runbook coverage current
- [ ] Comment back here when the follow-up is closed

Auto-generated by scripts/incident-simulation.sh
EOF
)

  if [[ -n "$TRACKER_ISSUE_NUMBER" ]]; then
    tracker_state="$("$GH_BIN" issue view "$TRACKER_ISSUE_NUMBER" --repo "$GH_REPO" --json state --jq '.state' 2>/dev/null || true)"
    if [[ "$tracker_state" == "OPEN" ]]; then
      existing_issue="$TRACKER_ISSUE_NUMBER"
      route_mode="comment"
    fi
  fi

  if [[ -z "${existing_issue:-}" ]]; then
    existing_issue="$("$GH_BIN" issue list --repo "$GH_REPO" --state open --limit 200 --json number,body,title --jq ".[] | select((.body // \"\") | contains(\"${marker}\")) | .number" | head -1)"
  ...
  if [[ -n "${existing_issue:-}" ]]; then
    "$GH_BIN" issue comment "$existing_issue" --repo "$GH_REPO" --body "$body" >/dev/null
    echo "📝 Routed simulation findings to issue #${existing_issue} (${route_mode})"
  else
    "$GH_BIN" issue create --repo "$GH_REPO" --title "$title" --body "$body" --label "P1" >/dev/null
    echo "📝 Created a new GitHub issue for simulation findings"
  fi
}
...
# Route the simulation result into GitHub Issues when requested.
route_simulation_finding "$SCENARIO" "$RESPONSE_TIME_MS" "$REPORT_FILE"
