...
export type CalendarStatus = 'active' | 'needs_refresh' | 'disconnected';
export type PresenceState = 'available' | 'busy' | 'in-meeting';
...
export class CalendarIntegrationService extends EventEmitter {
  private logger = getLogger('CalendarIntegrationService');
  private pool: Pool;
  ...
  async connectIntegration(
    ...
    try {
      ...
      await client.query(
        `INSERT INTO calendar_sync_audit (user_id, provider, action) VALUES ($1, $2, $3)`,
        [userId, provider, 'connect']
      );
  ...
  async listIntegrations(userId: string): Promise<CalendarIntegrationRecord[]> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `
  ...
  async getFreeBusyWindows(userId: string, startTime: Date, endTime: Date): Promise<FreeBusyWindow[]> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `
  ...
  async syncPresenceCard(userId: string): Promise<PresenceCard> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `
      ...
      const state: PresenceState = busyUntil ? 'in-meeting' : 'available';
      ...
      await client.query(
        `INSERT INTO calendar_sync_audit (user_id, provider, action) VALUES ($1, $2, $3)`,
        [userId, row?.provider || 'google', 'sync-presence']
      );
  ...
  async getPresenceCard(userId: string): Promise<PresenceCard | null> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `SELECT user_id, state, message, busy_until, updated_at FROM calendar_presence_cards WHERE user_id = $1`,
        [userId]
      );
  ...
  async disconnectIntegration(userId: string, provider: CalendarProvider): Promise<void> {
    const client = await this.pool.connect();
    try {
      await client.query(
        `UPDATE calendar_integrations SET status = 'disconnected', updated_at = CURRENT_TIMESTAMP WHERE user_id = $1 AND provider = $2`,
        [userId, provider]
      );

      await client.query(
        `INSERT INTO calendar_sync_audit (user_id, provider, action) VALUES ($1, $2, $3)`,
        [userId, provider, 'disconnect']
      );
...
export async function initializeCalendarIntegrationRoutes(service: CalendarIntegrationService) {
  ...
  router.get('/api/calendar/free-busy/:userId', async (req, res) => {
    try {
      const startTime = new Date((req.query.startTime as string) || Date.now());
      const endTime = new Date((req.query.endTime as string) || Date.now() + 86400000);
  ...
  router.delete('/api/calendar/integrations/:userId/:provider', async (req, res) => {
    try {
      await service.disconnectIntegration(req.params.userId, req.params.provider as CalendarProvider);
