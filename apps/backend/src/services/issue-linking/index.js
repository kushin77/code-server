...
export class IssueLinkingService extends EventEmitter {
    ...
    async searchTickets(query, provider) {
        const providers = provider ? [provider] : ['linear', 'jira'];
        const results = [];
    ...
    async searchLinearTickets(query) {
    ...
    async saveTicket(ticket) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO issue_tickets (provider, external_id, ticket_key, title, description, status, assignee, priority, labels, url)
    ...
    async linkIssue(ticketId, githubIssueNumber, provider) {
        ...
        try {
            ...
            const ticketResult = await client.query('SELECT id FROM issue_tickets WHERE external_id = $1 AND provider = $2', [ticketId, provider]);
            if (ticketResult.rows.length === 0) {
                throw new Error(`Ticket not found: ${ticketId}`);
            }
            const dbTicketId = ticketResult.rows[0].id;
            await client.query(`INSERT INTO linked_issues (ticket_id, github_issue_number, provider)
         VALUES ($1, $2, $3)`, [dbTicketId, githubIssueNumber, provider]);
    ...
    async getIssueContext(ticketKey) {
        const client = await this.pool.connect();
        try {
            // Check cache first
            const cached = await client.query('SELECT * FROM issue_context WHERE ticket_key = $1', [ticketKey]);
    ...
    async saveIssueContext(context) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO issue_context (ticket_key, title, acceptance_criteria, related_issues, linked_prs, estimated_effort)
         VALUES ($1, $2, $3, $4, $5, $6)
    ...
    async saveBranchName(ticketKey, suggestedName, actualName, createdBy) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO branch_names (ticket_key, suggested_name, actual_name, created_by)
         VALUES ($1, $2, $3, $4)`, [ticketKey, suggestedName, actualName || null, createdBy || null]);
    ...
    async getBranchNames(ticketKey) {
        const client = await this.pool.connect();
        try {
            const result = await client.query('SELECT suggested_name, actual_name FROM branch_names WHERE ticket_key = $1 ORDER BY created_at DESC', [ticketKey]);
    ...
    async getSearchCache(query, provider) {
        const client = await this.pool.connect();
        try {
            const result = await client.query('SELECT results FROM issue_search_cache WHERE query = $1 AND provider = $2 AND expires_at > NOW()', [query, provider]);
    ...
    async setSearchCache(query, provider, tickets) {
        const client = await this.pool.connect();
        try {
            // Cache for 1 hour
            const expiresAt = new Date(Date.now() + 60 * 60 * 1000);
            await client.query(`INSERT INTO issue_search_cache (query, provider, results, expires_at)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (query, provider) DO UPDATE SET
           results = $3, expires_at = $4, cached_at = NOW()`, [query, provider, JSON.stringify(tickets), expiresAt]);
    ...
    formatContextForCopilot(context) {
        const lines = [
