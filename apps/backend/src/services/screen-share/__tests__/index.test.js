...
describe('ScreenShareService', () => {
    ...
    beforeEach(() => {
        ...
        service = new ScreenShareService({ workspaceId: 'workspace-123' }, mockPool, mockRedis, mockAuditService);
    });
    ...
    describe('Error Handling', () => {
        it('should throw error for missing workspace ID', () => {
            expect(() => {
                new ScreenShareService({}, mockPool, mockRedis);
        ...
        it('should throw error for non-existent session annotation', async () => {
            await expect(service.addAnnotation('invalid-id', 'user-1', 'drawing', 100, 200)).rejects.toThrow();
        });
    });
});
