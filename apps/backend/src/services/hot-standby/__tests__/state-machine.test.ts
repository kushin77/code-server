...
describe('HotStandbyStateMachine', () => {
  let stateMachine: HotStandbyStateMachine;
  ...
  beforeEach(() => {
    ...
    stateMachine = new HotStandbyStateMachine(
      'broker-1',
      'broker-2',
      redisMock as Redis,
      {
  ...
  describe('Initialization', () => {
    ...
    it('should set up broker info in Redis', async () => {
      ...
      expect(setSpy).toHaveBeenCalledWith(
        expect.stringContaining('broker:broker-1'),
        expect.any(String),
        'EX',
        60
      );
  ...
  describe('Heartbeat Mechanism', () => {
    it('should send heartbeat periodically', async () => {
      ...
      expect(publishSpy).toHaveBeenCalledWith(
        expect.stringContaining('heartbeat:broker-2'),
        expect.stringContaining('broker-1')
      );
    });
  ...
  describe('Failure Detection', () => {
    ...
    it('should trigger failover after threshold exceeded', async () => {
      const promoteSpy = vi.spyOn(stateMachine as any, 'promoteToMaster');
  ...
  describe('Promotion Logic', () => {
    it('should promote replica to primary on failure', async () => {
      ...
      expect(setSpy).toHaveBeenCalledWith(
        expect.stringContaining('promotion:lock'),
        'broker-1',
        'EX',
        expect.any(Number),
        'NX'
    ...
    it('should set primary ID in Redis on promotion', async () => {
      ...
      expect(setSpy).toHaveBeenCalledWith(
        expect.stringContaining('primary:id'),
        'broker-1'
      );
    });
    ...
    it('should update promotion metrics', async () => {
      ...
      expect(status.metrics.promotionTime).toBeLessThan(
        afterPromotion - beforePromotion + 100
      );
    });
  });
  ...
  describe('Session Management', () => {
    it('should update session count', async () => {
      ...
      expect(setSpy).toHaveBeenCalledWith(
        expect.stringContaining('broker:broker-1'),
        expect.stringContaining('"sessionCount":10'),
        'EX',
        60
      );
    });
