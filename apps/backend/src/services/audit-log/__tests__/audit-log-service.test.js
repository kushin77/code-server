...
describe('Immutable Audit Log Service', () => {
    ...
    describe('Event Recording', () => {
        it('should record file write event', async () => {
            const event = await service.recordEvent('user-alice', 'session-123', 'write', 'file', 'file-1', 'success', { fileName: 'config.json', size: 1024 });
        ...
        it('should set timestamp on event', async () => {
            ...
            const event = await service.recordEvent('user-alice', 'session-123', 'write', 'file', 'file-1');
        ...
        it('should reject recording when not initialized', async () => {
            const svc = new AuditLogService();
            await expect(svc.recordEvent('user-alice', 'session-123', 'write', 'file', 'file-1')).rejects.toThrow('Service not initialized');
        });
        it('should accept optional context (ipAddress, userAgent)', async () => {
            const event = await service.recordEvent('user-alice', 'session-123', 'write', 'file', 'file-1', 'success', {}, {
    ...
    describe('Event Retrieval', () => {
        it('should get event by ID', async () => {
            const recorded = await service.recordEvent('user-alice', 'session-123', 'write', 'file', 'file-1');
            await service.flushBatch();
    ...
    describe('Integration', () => {
        it('should handle complete audit workflow', async () => {
            ...
            const event1 = await service.recordEvent('user-alice', 'session-123', 'write', 'file', 'file-1', 'success', { fileName: 'config.json' });
            const event2 = await service.recordEvent('user-alice', 'session-123', 'delete', 'file', 'file-1', 'success', { fileName: 'config.json' });
...
//# sourceMappingURL=audit-log-service.test.js.map