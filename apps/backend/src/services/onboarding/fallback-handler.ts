...
/**
 * Fallback instruction for a step
 */
export interface FallbackInstruction {
  stepId: string
  stepTitle: string
  reason: string
  manualSteps: string[]
...
export class OnboardingFallbackHandler {
   ...
   */
  static getGitConfigFallback(error?: string): FallbackInstruction {
    return {
      stepId: 'git-config',
   ...
   */
  static getSSHSetupFallback(error?: string): FallbackInstruction {
    return {
      stepId: 'ssh-setup',
   ...
   */
  static getCloudLoginFallback(error?: string): FallbackInstruction {
    return {
      stepId: 'cloud-login',
   ...
   */
  static getRepoCloneFallback(error?: string): FallbackInstruction {
    return {
      stepId: 'repo-clone',
   ...
   */
  static getBuildConfigFallback(error?: string): FallbackInstruction {
    return {
      stepId: 'build-config',
   ...
   */
  static getVerifyFallback(error?: string): FallbackInstruction {
    return {
      stepId: 'verify',
   ...
   */
  static getFallback(stepId: string, error?: string): FallbackInstruction {
    switch (stepId) {
      case 'git-config':
   ...
   */
  static logFallback(instruction: FallbackInstruction): void {
   ...
   */
  static formatMarkdown(instruction: FallbackInstruction): string {
    const lines: string[] = [
   ...
   */
  static formatHTML(instruction: FallbackInstruction): string {
    return `
