...
import {
  fetchSymbolDiscussionsByLocation,
  type SymbolDiscussion,
  type SymbolDiscussionLocationResult,
} from '../utils/symbolDiscussions';

export interface SymbolDiscussionsPanelProps {
  initialFilePath?: string;
  initialLineNumber?: number;
}

function clampLineNumber(value: string): number | undefined {
  if (!value.trim()) {
    return undefined;
  }
...
function describeLocation(filePath: string, lineNumber?: number): string {
  return lineNumber ? `${filePath}:${lineNumber}` : filePath;
}

function summarizeComment(discussion: SymbolDiscussion): string {
...
export const SymbolDiscussionsPanel: React.FC<SymbolDiscussionsPanelProps> = ({
  initialFilePath = '',
  initialLineNumber,
}) => {
  const [filePath, setFilePath] = useState(initialFilePath);
  const [lineNumberText, setLineNumberText] = useState(initialLineNumber ? String(initialLineNumber) : '');
  const [result, setResult] = useState<SymbolDiscussionLocationResult | null>(null);
  ...
  return (
    <section className="mx-auto flex min-h-screen max-w-6xl flex-col gap-5 bg-[radial-gradient(circle_at_top,_rgba(15,23,42,0.94),_rgba(15,23,42,0.76)_58%,_rgba(30,41,59,0.94))] px-4 py-6 text-slate-100">
      ...
      <div className="rounded-[28px] border border-slate-700/60 bg-slate-900/75 p-5 shadow-xl shadow-slate-950/30 backdrop-blur">
        <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_220px_auto] md:items-end">
          <label className="block">
            ...
            <input
              ...
              placeholder="src/features/userService.ts"
              className="w-full rounded-2xl border border-slate-700 bg-slate-950/70 px-4 py-3 text-sm text-slate-100 outline-none transition placeholder:text-slate-500 focus:border-cyan-400"
            />
          </label>
          ...
          <label className="block">
            ...
            <input
              ...
              onChange={(event) => setLineNumberText(event.target.value)}
              placeholder="42"
              className="w-full rounded-2xl border border-slate-700 bg-slate-950/70 px-4 py-3 text-sm text-slate-100 outline-none transition placeholder:text-slate-500 focus:border-cyan-400"
            />
          </label>
        ...
        {!loading && !error && result && result.discussions.length === 0 ? (
        ...
        <div className="mt-5 grid gap-4">
          {result?.discussions.map((discussion) => {
            ...
            return (
              <article
                key={discussion.id}
                className="rounded-[24px] border border-slate-700 bg-slate-950/80 p-4 shadow-lg shadow-slate-950/20 transition hover:border-cyan-400/50"
              >
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2 text-xs font-semibold uppercase tracking-[0.18em]">
                    <span
                      className={`rounded-full px-3 py-1 ${discussion.thread.isResolved ? 'bg-emerald-500/15 text-emerald-300' : 'bg-amber-500/15 text-amber-200'}`}
                    >
