...
if [[ ! -f "${MANIFEST}" ]]; then
  log_fatal "MANIFEST not found: ${MANIFEST}"
  exit 1
fi
...
while IFS=$'\t' read -r rel_path purpose; do
  ...
  if [[ ! -f "${script_path}" ]]; then
    log_error "active MANIFEST script missing from disk: scripts/${rel_path}"
    failures=$((failures + 1))
    continue
  fi
  ...
  if ! grep -qE '^# @file[[:space:]]+scripts/' <<<"${header}"; then
    log_error "missing @file header: scripts/${rel_path}"
    failures=$((failures + 1))
  fi

  if ! grep -qE '^# @module[[:space:]]+[^[:space:]].*$' <<<"${header}"; then
    log_error "missing @module header: scripts/${rel_path}"
    failures=$((failures + 1))
  fi

  if ! grep -qE '^# @description[[:space:]]+[^[:space:]].*$' <<<"${header}"; then
    log_error "missing @description header: scripts/${rel_path}"
    failures=$((failures + 1))
    continue
  fi
  ...
  if [[ -n "${purpose}" && "${purpose}" != "TODO: add purpose" ]]; then
    ...
    if [[ "${desc_norm}" != *"${manifest_norm}"* ]]; then
      log_error "purpose/header drift: scripts/${rel_path}"
      log_error "  MANIFEST purpose: ${purpose}"
      log_error "  Header desc:      ${desc_line}"
      failures=$((failures + 1))
    fi
  else
    warnings=$((warnings + 1))
    log_warn "TODO purpose in MANIFEST (cross-ref skipped): scripts/${rel_path}"
  fi
done < <(
  awk -F '"' '
...
log_info ""
log_info "Metadata header audit complete"
log_info "Active scripts scanned: ${total}"
...
log_info "Failures: 0"
log_info "PASS: active scripts satisfy GOV-002 metadata header requirements"
