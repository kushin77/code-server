...
export class SessionBrokerService extends EventEmitter {
  private static instances = new Map<string, SessionBrokerService>()

  private hashRing: ConsistentHashRing
  private config: Required<SessionBrokerConfig>
  ...
  constructor(config: SessionBrokerConfig, logger?: pino.Logger) {
    super()
   ...
   */
  static getInstance(config?: SessionBrokerConfig, logger?: pino.Logger): SessionBrokerService {
    const key = 'default'
   ...
   */
  routeSession(context: SessionRoutingContext): HashRingLookup {
    try {
   ...
   */
  getBackupInstances(context: SessionRoutingContext, count: number = 2): BrokerInstance[] {
    try {
   ...
   */
  addInstance(instance: BrokerInstance): void {
   ...
   */
  drainInstance(instanceId: string): void {
  ...
  private async performHealthCheck(): Promise<void> {
    const checks = this.config.instances.map((instance) =>
   ...
   */
  private async healthCheckInstance(instance: BrokerInstance): Promise<InstanceHealthCheckResult> {
    ...
    try {
      ...
      const response = await fetch(
        `http://${instance.host}:${instance.port}/healthz`,
   ...
   */
  private recordRequest(instanceId: string, latency = 0, isError = false): void {
   ...
   */
  recordLatency(instanceId: string, latencyMs: number): void {
   ...
   * Get instance by ID.
   */
  getInstance(instanceId: string): BrokerInstance | undefined {
   ...
   */
  getHealthyInstances(): BrokerInstance[] {
