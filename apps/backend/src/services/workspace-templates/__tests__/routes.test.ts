...
class MockWorkspaceTemplateCatalogService implements Partial<WorkspaceTemplateCatalogService> {
  getSnapshot() {
    return {
      templates: [
        {
...
describe('Workspace Template Routes', () => {
  ...
  it('devcontainer endpoint builds payload correctly', async () => {
    ...
    expect(devcontainer.customizations.vscode.extensions).toContain(
      'ms-vscode.vscode-typescript-next@5.3.3'
    )
  })
