 ...
 */
const vpnConnected = base.extend({
    vpnConnected: async ({}, use) => {
        ...
        try {
            ...
            throw new Error(`VPN connectivity check failed: ${error instanceof Error ? error.message : String(error)}\n` +
...
const authContext = base.extend({
    authContext: async ({}, use) => {
        ...
        if (!context.password) {
            throw new Error('E2E_USER_PASSWORD is required for authentication. ' +
...
export const helpers = {
     ...
     */
    async waitForApiResponse(page, urlPattern, timeout = 10000) {
