...
export class PreviewService extends EventEmitter {
  ...
  private environments: Map<string, PreviewEnvironmentInstance> = new Map();
  private pullRequests: Map<string, PullRequest> = new Map();
  private scalingEvents: Map<string, ScalingEvent[]> = new Map();
  private statistics: Map<string, PreviewEnvironmentStatistics> = new Map();
  private auditLog: Map<string, PreviewAuditEntry[]> = new Map();
  private healthChecks: Map<string, HealthCheckResult[]> = new Map();
  private config: PreviewServiceConfig;

  private constructor(config?: Partial<PreviewServiceConfig>) {
    super();
    this.config = {
   ...
   */
  static getInstance(config?: Partial<PreviewServiceConfig>): PreviewService {
  ...
  async provisionEnvironment(
    ...
    try {
      ...
      const config: PreviewEnvironmentConfig = {
      ...
      const environment: PreviewEnvironmentInstance = {
        id: environmentId,
   ...
   */
  getEnvironment(environmentId: string): PreviewEnvironmentInstance | null {
   ...
   */
  getEnvironmentsByPullRequest(pullRequestNumber: number): PreviewEnvironmentInstance[] {
   ...
   */
  performHealthCheck(environmentId: string): HealthCheckResult {
    const environment = this.environments.get(environmentId);
  ...
  startGracePeriod(
    ...
    const affected: PreviewEnvironmentInstance[] = [];
   ...
   */
  getStatistics(pullRequestNumber: number): PreviewEnvironmentStatistics | null {
   ...
   */
  getAuditLog(userId: string): PreviewAuditEntry[] {
   ...
   */
  getHealthCheckHistory(environmentId: string, limit?: number): HealthCheckResult[] {
    const checks = this.healthChecks.get(environmentId) || [];
    if (limit) {
   ...
   */
  updateConfig(config: Partial<PreviewServiceConfig>, userId: string, ipAddress: string, userAgent: string): void {
  ...
  static reset(): void {
    ...
    PreviewService.instance = undefined as any;
  }
  ...
  private recordAudit(entry: {
    ...
    const auditEntry: PreviewAuditEntry = {
   ...
   */
  private updateStatistics(pullRequestId: string, pullRequestNumber: number): void {
    const stats: PreviewEnvironmentStatistics = {
      pullRequestId,
      pullRequestNumber,
      totalProvisions: 1,
