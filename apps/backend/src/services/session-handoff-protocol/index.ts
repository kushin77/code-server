...
export type HandoffState = 'pending' | 'accepted' | 'rejected' | 'expired' | 'completed';

export interface SessionHandoffProtocol {
  id: string;
  sessionId: string;
...
export class SessionHandoffProtocolService extends EventEmitter {
  private logger = getLogger('SessionHandoffProtocolService');
  private pool: Pool;
  ...
  async startHandoff(input: StartHandoffInput): Promise<SessionHandoffProtocol> {
    const client = await this.pool.connect();
    try {
  ...
  async acceptHandoff(protocolId: string, acceptedBy: string): Promise<SessionHandoffProtocol> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `
  ...
  async rejectHandoff(protocolId: string, rejectedBy: string): Promise<SessionHandoffProtocol> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `
  ...
  async completeHandoff(protocolId: string): Promise<SessionHandoffProtocol> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `
  ...
  async getHandoff(protocolId: string): Promise<SessionHandoffProtocol | null> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `SELECT id, session_id, current_owner_id, target_owner_id, state, reason, expires_at, accepted_at, rejected_at, completed_at, created_at, updated_at FROM session_handoff_protocols WHERE id = $1`,
        [protocolId]
      );
  ...
  async listHandoffs(sessionId?: string): Promise<SessionHandoffProtocol[]> {
    const client = await this.pool.connect();
    try {
      const params: string[] = [];
  ...
  async expireStaleHandoffs(): Promise<number> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `
  ...
  private rowToProtocol(row: any): SessionHandoffProtocol {
    return {
      id: row.id,
...
export async function initializeSessionHandoffProtocolRoutes(service: SessionHandoffProtocolService) {
