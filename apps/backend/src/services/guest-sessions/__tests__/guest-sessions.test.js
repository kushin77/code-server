...
describe('GuestSessionService', () => {
    ...
    beforeEach(() => {
        ...
        service = new GuestSessionService(mockPool);
    });
    afterEach(() => {
        vi.clearAllMocks();
    });
    ...
    describe('createGuestSession', () => {
        ...
        it('should reject when max active sessions reached', async () => {
            ...
            await expect(service.createGuestSession('user123', '/home/user')).rejects.toThrow(/max active guest sessions/);
        });
    ...
    describe('trackActivity', () => {
        it('should track guest activity', async () => {
            ...
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO guest_activity_log'), expect.arrayContaining(['read', '/home/user/file.txt', '192.168.1.1']));
    ...
    describe('revokeSession', () => {
        it('should revoke a session', async () => {
            ...
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE guest_sessions SET is_active = false'), expect.arrayContaining(['session-123']));
            expect(eventSpy).toHaveBeenCalled();
        });
    });
    describe('getSessionActivity', () => {
        ...
        it('should respect limit parameter', async () => {
            ...
            expect(mockClient.query).toHaveBeenCalledWith(expect.any(String), expect.arrayContaining(['session-123', 25]));
        });
    });
    describe('cleanupExpiredSessions', () => {
        it('should mark expired sessions as inactive', async () => {
            ...
            expect(count).toBe(2);
            expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE guest_sessions SET is_active = false WHERE expires_at <= NOW()'));
    ...
    describe('onSessionEnded callback wiring', () => {
        it('should store onSessionEnded callback from config', async () => {
            ...
            service = new GuestSessionService(mockPool, { onSessionEnded });
        ...
        it('should continue cleanup despite callback errors', async () => {
            let callCount = 0;
            const onSessionEnded = vi.fn(async (id) => {
                callCount++;
                if (callCount === 1) {
...
//# sourceMappingURL=guest-sessions.test.js.map