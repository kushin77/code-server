...
describe('SharedAICopilotContextService', () => {
  let service: SharedAICopilotContextService;

  beforeEach(() => {
    (SharedAICopilotContextService as any).reset();
  ...
  describe('Resolving Conversation Turns', () => {
    it('should resolve turn', () => {
      ...
      const turn = Array.from(conv.conversation!.turns.values()).find(
        (t) => t.id === turnResult.turnId
      );
