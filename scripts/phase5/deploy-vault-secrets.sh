...
# @description Phase 5 - Deploy Vault secrets management
# @summary Wraps the existing Vault deployment helper and validates secret SSOT.
...
PHASE_TAG="phase5-deploy-vault-secrets"
...
log_info "=== Phase 5 - Deploy Vault secrets management ==="
log_info "Dry-run: ${DRY_RUN}"

steps=("Deploy security vault::scripts/phase5/deploy-security-vault.sh" "Validate config SSOT::scripts/ci/validate-config-ssot.sh")
results=()
ok=0; fail=0; skip=0
...
{
    printf '# Phase 5 - Deploy Vault secrets management\n\n'
    printf '%s\n\n' "Wraps the existing Vault deployment helper and validates secret SSOT."
