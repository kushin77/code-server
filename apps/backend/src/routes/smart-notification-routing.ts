...
export function initializeSmartNotificationRoutingRoutes(pool: Pool, auditService?: AuditService): Router {
  ...
  router.post('/routes', async (req, res) => {
    try {
      ...
      const route = await notificationService.setNotificationRoute(
        userId,
  ...
  router.get('/routes/:userId/:priority', async (req, res) => {
    try {
      ...
      const route = await notificationService.getNotificationRoute(userId, priority as NotificationPriority);
  ...
  router.post('/send', async (req, res) => {
    try {
      ...
      const deliveries = await notificationService.routeNotification(
        userId,
        notificationId,
        content,
        priority as NotificationPriority
  ...
  router.get('/pending', async (req, res) => {
    try {
      ...
      const deliveries = await notificationService.getPendingDeliveries(parseInt(limit as string, 10));
