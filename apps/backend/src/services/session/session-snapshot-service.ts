...
export class SessionSnapshotService extends EventEmitter {
  private logger = getLogger("SessionSnapshot")
  private snapshots = new Map<string, SessionSnapshot>()
  private versionHistory = new Map<string, SessionSnapshot[]>()
  ...
  restoreSnapshot(
    ...
    success: boolean
    restoredState?: Partial<SessionSnapshot>
    restoreTime: number
  } {
    try {
   ...
   */
  tagSnapshot(snapshotId: string, tags: string[]): {
    success: boolean
    tags: string[]
  } {
    try {
  ...
  compareSnapshots(
    ...
    differences: {
      ...
      terminalsChanged: number
      debugChanged: boolean
      extensionsChanged: number
    }
  } {
    try {
      ...
      const filesChanged = Math.max(
        Math.abs(snap1.fileState.length - snap2.fileState.length),
        snap1.fileState.filter((f) => !snap2.fileState.find((f2) => f2.path === f.path)).length
      )
...
// Singleton instance
let instance: SessionSnapshotService | null = null

export function getSessionSnapshotService(maxVersions?: number): SessionSnapshotService {
  if (!instance) {
