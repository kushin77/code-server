...
export class MultiModalAnalyzer {
   ...
   */
  private analyzeCode_CodeMetrics(code: string): CodeAnalysis {
    ...
    const coverage = Math.min(
      100,
      ((code.match(testRegex) || []).length / Math.max(1, functions)) * 100
    );
   ...
   */
  private analyzeCode_Tests(code: string): TestAnalysis {
   ...
   */
  private analyzeCode_Documentation(code: string): DocumentationAnalysis {
    ...
    const apiDocumented = apiDocPatterns.some((pattern) =>
   ...
   */
  private analyzeCode_Patterns(code: string): PatternAnalysis {
    // Count async patterns
    const asyncCount = (code.match(/async\s+|await\s+/g) || []).length;

    // Check error handling
    const errorHandling =
      /try\s*{|catch\s*\(|throw\s+/.test(code) ||
      /\.catch\(|\.error|Error|Exception/.test(code);

    // Check performance optimization
    const performanceOptimization =
      /cache|memoiz|lazy|defer|async|Promise/.test(code);

    // Check security
    const securityChecks =
      /encrypt|decrypt|hash|token|auth|password|secret|sanitize|validate/.test(code);

    // Detect code smells
    const codeSmells: string[] = [];

    if (/var\s+\w+/.test(code)) codeSmells.push('using var');
    if (/callback.*callback/.test(code)) codeSmells.push('callback hell');
    if (/console\.(log|warn|error)/.test(code))
      codeSmells.push('debug logging');
    if (/TODO|FIXME|HACK|XXX/.test(code)) codeSmells.push('unfinished work');
  ...
  scoreMultiModal(modalities: CodeModalities): number {
    ...
    if (code.coverage > 80) score += 0.15;
    if (code.complexity < 20) score += 0.1;
    if (code.lines > 50 && code.lines < 500) score += 0.05;

    // Test metrics
    const { tests } = modalities;
    if (tests.hasTests) score += 0.15;
    if (tests.testCoverage > 80) score += 0.1;

    // Documentation
    const { documentation } = modalities;
    if (documentation.hasDocstring) score += 0.1;
    if (documentation.commentDensity > 10) score += 0.05;
    if (documentation.apiDocumented) score += 0.1;

    // Patterns
    const { patterns } = modalities;
    if (patterns.errorHandling) score += 0.1;
    if (patterns.securityChecks) score += 0.1;
    if (patterns.performanceOptimization) score += 0.05;
    if (patterns.codeSmells.length === 0) score += 0.05;
    else if (patterns.codeSmells.length > 3) score -= 0.1;
