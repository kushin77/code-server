...
import { getWorkspaceSwitcher, } from '../services/workspace-switcher.js';
/**
 * WorkspaceSwitcher Panel Component
 * Displays workspace list, current active workspace, and switch metrics
 */
export const WorkspaceSwitcherPanel = ({ workspaces, currentWorkspaceId, onSwitch, onGetCurrentState, }) => {
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                setError(`Failed to initialize switcher: ${err instanceof Error ? err.message : String(err)}`);
            }
        })();
    }, []);
    ...
    const handleWorkspaceSwitch = async (targetId) => {
        if (!switcher || !onGetCurrentState)
            return;
        ...
        try {
            ...
            const switchMetrics = await switcher.switchWorkspace(currentWorkspaceId, currentState, targetId);
      ...
      <div style={{ marginBottom: '12px' }}>
        ...
        <div style={{
          ...
          {workspaces
            ...
            .map((workspace) => (<button key={workspace.id} onClick={() => handleWorkspaceSwitch(workspace.id)} disabled={isSwitching} style={{
                padding: '6px 10px',
      ...
      {metrics && (<div style={{
          ...
          <div style={{ fontSize: '12px', color: '#cccccc' }}>
            ...
            <div>
              ...
              {metrics.totalTime < 200 ? (<span style={{ color: '#4ec9b0' }}>✓ under SLA</span>) : (<span style={{ color: '#f44747' }}>✗ over SLA</span>)}
            </div>
            <div>
