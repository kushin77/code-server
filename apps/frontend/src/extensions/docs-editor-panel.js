...
export const DocsEditorPanel = () => {
    ...
    const loadDocs = async () => {
        try {
            ...
            vscode.window.showErrorMessage(`Failed to load docs: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    };
    const generateDocs = async () => {
        try {
            ...
            vscode.window.showErrorMessage(`Failed to generate: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    };
    const saveDoc = async () => {
        if (!selectedDoc)
    ...
    const exportDoc = async (format) => {
        if (!selectedDoc)
            return;
        try {
            const response = await fetch(`http://localhost:3100/api/docs/${selectedDoc.id}/export?format=${format}`);
            if (response.ok) {
                const data = await response.json();
    ...
    return (<div style={{
      ...
      <div style={{ marginBottom: '12px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
          <button onClick={generateDocs} style={{
            padding: '6px 12px',
          ...
          {['edit', 'preview', 'split'].map((mode) => (<button key={mode} onClick={() => setViewMode(mode)} style={{
                padding: '6px 12px',
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
          <button onClick={() => exportDoc('html')} style={{
            padding: '4px 8px',
            fontSize: '12px',
          ...
          <button onClick={() => exportDoc('pdf')} style={{
            padding: '4px 8px',
            fontSize: '12px',
            background: 'var(--vscode-button-secondaryBackground)',
            color: 'var(--vscode-button-secondaryForeground)',
      ...
      <div style={{ display: 'flex', gap: '12px', flex: 1, minHeight: 0 }}>
        ...
        <div style={{
          ...
          {docs.map((doc) => (<div key={doc.id} onClick={() => {
                setSelectedDoc(doc);
        ...
        <div style={{ flex: 1, display: 'flex', gap: '12px', minWidth: 0 }}>
          {/* Editor */}
          {(viewMode === 'edit' || viewMode === 'split') && (<div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
          ...
          {/* Preview */}
          {(viewMode === 'preview' || viewMode === 'split') && (<div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
...
//# sourceMappingURL=docs-editor-panel.js.map