...
import service, { DebugSessionAIService } from '../debug-session-ai-service';
describe('DebugSessionAIService', () => {
    ...
    describe('Breakpoint Management', () => {
        ...
        it('should handle multiple breakpoints', () => {
            const session = service.startSession('user1', 'ws-123', 'sess-123', 'node');
            const bp1 = { id: 'bp-1', file: '/src/app.ts', line: 10, verified: true };
            const bp2 = { id: 'bp-2', file: '/src/utils.ts', line: 20, verified: true };
            const bp3 = { id: 'bp-3', file: '/src/app.ts', line: 30, verified: true };
        ...
        it('should update breakpoint with hit count', () => {
            ...
            service.setBreakpoint(session.id, breakpoint);
            const updated = { ...breakpoint, hitCount: 5 };
    ...
    describe('Variable Capture', () => {
        it('should capture variables at breakpoint', () => {
            const session = service.startSession('user1', 'ws-123', 'sess-123', 'node');
            const breakpoint = { id: 'bp-1', file: '/src/app.ts', line: 42, verified: true };
            service.setBreakpoint(session.id, breakpoint);
            const variables = [
        ...
        it('should store current breakpoint when capturing variables', () => {
            ...
            const breakpoint = { id: 'bp-1', file: '/src/app.ts', line: 42, verified: true };
            service.setBreakpoint(session.id, breakpoint);
            const variables = [{ name: 'x', value: '10', type: 'number' }];
        ...
        it('should maintain issue history', () => {
            const session = service.startSession('user1', 'ws-123', 'sess-123', 'node');
            const bp = { id: 'bp-1', file: '/src/app.ts', line: 42, verified: true };
            service.setBreakpoint(session.id, bp);
            const vars1 = [{ name: 'x', value: '1', type: 'number' }];
            const vars2 = [{ name: 'x', value: '2', type: 'number' }];
        ...
        it('should limit history to 100 items', () => {
            ...
            service.setBreakpoint(session.id, bp);
            for (let i = 0; i < 150; i++) {
                const vars = [{ name: 'x', value: String(i), type: 'number' }];
    ...
    describe('Root Cause Analysis', () => {
        it('should analyze root cause', () => {
            ...
            const bp = { id: 'bp-1', file: '/src/app.ts', line: 42, verified: true };
            service.setBreakpoint(session.id, bp);
            const variables = [{ name: 'x', value: 'null', type: 'null' }];
            service.captureVariables(session.id, 'bp-1', variables);
...
//# sourceMappingURL=debug-session-ai-service.test.js.map