...
describe('symbolDiscussions', () => {
    it('fetches inline discussion candidates for a file location', async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith('/api/symbol-discussions/location?filePath=src%2Fservices%2FuserService.ts&lineNumber=42', expect.objectContaining({
    ...
    it('omits the line number when not provided', async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith('/api/symbol-discussions/location?filePath=src%2Fservices%2FuserService.ts', expect.objectContaining({
            headers: expect.objectContaining({
                'Content-Type': 'application/json',
            }),
        }));
...
//# sourceMappingURL=symbolDiscussions.test.js.map