...
export class ExpertiseHeatmapService extends EventEmitter {
    constructor(pool) {
        super();
        this.logger = getLogger('ExpertiseHeatmapService');
        this.pool = pool;
    }
    async initialize() {
    ...
    async recordContribution(userId, filePath, functionName, expertiseScore, commitCount = 1) {
        const client = await this.pool.connect();
        try {
    ...
    async generateHeatmap(target, targetType = 'file') {
        const client = await this.pool.connect();
        try {
    ...
    async findExpert(target, targetType = 'file') {
        const client = await this.pool.connect();
        try {
            const query = targetType === 'function'
            ...
            const totalRows = await client.query(targetType === 'function'
                ? `SELECT SUM(expertise_score) AS total FROM expertise_contributions WHERE function_name = $1`
                : `SELECT SUM(expertise_score) AS total FROM expertise_contributions WHERE file_path = $1`, [target]);
    ...
    async listContributions(userId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT id, user_id, file_path, function_name, expertise_score, commit_count, created_at FROM expertise_contributions WHERE user_id = $1 ORDER BY created_at DESC`, [userId]);
    ...
    async getLatestHeatmap(target, targetType = 'file') {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT heatmap FROM expertise_heatmap_snapshots WHERE target = $1 AND target_type = $2 ORDER BY created_at DESC LIMIT 1`, [target, targetType]);
            if (result.rows.length === 0)
                return null;
...
export async function initializeExpertiseHeatmapRoutes(service) {
    ...
    router.get('/api/expertise/heatmap/:target', async (req, res) => {
        try {
            const targetType = req.query.targetType || 'file';
