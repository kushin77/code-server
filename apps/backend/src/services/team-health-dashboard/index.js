...
export class TeamHealthDashboardService extends EventEmitter {
    constructor(pool) {
        super();
        this.logger = getLogger('TeamHealthDashboardService');
        this.pool = pool;
    }
    async initialize() {
    ...
    async recordFlowTime(teamId, userId, activityType, durationMins) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO flow_time_tracking (team_id, user_id, activity_type, duration_mins) VALUES ($1, $2, $3, $4)`, [teamId, userId, activityType, durationMins]);
    ...
    async recordPairingSession(teamId, userId1, userId2, durationMins) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO team_pairing_sessions (team_id, user_id_1, user_id_2, session_duration_mins, started_at, ended_at)
         VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP + INTERVAL '1 minute' * $4)`, [teamId, userId1, userId2, durationMins]);
    ...
    async recordCodeReviewMetrics(teamId, pullRequestId, reviewerId, timeToFirstReviewMins, totalReviewTimeMins) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO code_review_metrics (team_id, pull_request_id, reviewer_id, time_to_first_review_mins, total_review_time_mins)
         VALUES ($1, $2, $3, $4, $5)`, [teamId, pullRequestId, reviewerId, timeToFirstReviewMins, totalReviewTimeMins]);
    ...
    async recordAIUtilization(teamId, userId, aiCommand) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO ai_utilization_tracking (team_id, user_id, ai_command) VALUES ($1, $2, $3)`, [teamId, userId, aiCommand]);
        }
        finally {
            client.release();
        }
    }
    async calculateTeamHealthMetrics(teamId) {
        const client = await this.pool.connect();
        try {
            // Calculate average flow time
            const flowResult = await client.query(`SELECT AVG(duration_mins) as avg_flow_time FROM flow_time_tracking WHERE team_id = $1 AND recorded_at > NOW() - INTERVAL '7 days'`, [teamId]);
            const avgFlowTime = flowResult.rows[0]?.avg_flow_time || 0;
            // Calculate pairing frequency
            const pairingResult = await client.query(`SELECT COUNT(*) as pair_count FROM team_pairing_sessions WHERE team_id = $1 AND started_at > NOW() - INTERVAL '7 days'`, [teamId]);
            const pairingFreq = pairingResult.rows[0]?.pair_count || 0;
            // Calculate review latency
            const reviewResult = await client.query(`SELECT AVG(time_to_first_review_mins) as avg_latency FROM code_review_metrics WHERE team_id = $1 AND reviewed_at > NOW() - INTERVAL '7 days'`, [teamId]);
            const reviewLatency = (reviewResult.rows[0]?.avg_latency || 0) / 60; // Convert to hours
            // Calculate AI utilization
            const aiResult = await client.query(`SELECT COUNT(*) as ai_uses FROM ai_utilization_tracking WHERE team_id = $1 AND used_at > NOW() - INTERVAL '7 days'`, [teamId]);
            ...
            const healthScore = Math.min(100, (100 - Math.min(avgFlowTime, 100)) * 0.3 +
            ...
            // Store metrics
            await client.query(`INSERT INTO team_health_metrics (team_id, average_flow_time_mins, pairing_frequency, review_latency_hours, ai_utilization_percent, collaboration_index, health_score)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`, [teamId, avgFlowTime, pairingFreq, reviewLatency, metrics.aiUtilization, collaborationIndex, healthScore]);
    ...
    async getLatestMetrics(teamId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT team_id, average_flow_time_mins, pairing_frequency, review_latency_hours, ai_utilization_percent, collaboration_index, health_score, generated_at
    ...
    async generateWeeklyDigest(teamId) {
        const client = await this.pool.connect();
        try {
            const metrics = await this.calculateTeamHealthMetrics(teamId);
            // Get top pairing combinations
            const pairingResult = await client.query(`SELECT user_id_1, user_id_2, COUNT(*) as sessions FROM team_pairing_sessions
         ...
         LIMIT 5`, [teamId]);
            ...
            // Get slow reviews
            const reviewResult = await client.query(`SELECT pull_request_id, time_to_first_review_mins FROM code_review_metrics
         ...
         LIMIT 5`, [teamId]);
            ...
            // Store digest
            await client.query(`INSERT INTO weekly_digests (team_id, week_start_date, metrics, top_pairs, slow_reviews, ai_trends, summary)
         VALUES ($1, CURRENT_DATE, $2, $3, $4, $5, $6)`, [teamId, JSON.stringify(metrics), JSON.stringify(topPairs), JSON.stringify(slowReviews), JSON.stringify(aiTrends), summary]);
    ...
    generateSummaryText(metrics, topPairs, slowReviews) {
    ...
    async getWeeklyDigest(teamId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT team_id, week_start_date, metrics, top_pairs, slow_reviews, ai_trends, summary, generated_at
         FROM weekly_digests
         WHERE team_id = $1
         ORDER BY generated_at DESC
         LIMIT 1`, [teamId]);
    ...
    async getTeamHistory(teamId, days = 30) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT team_id, average_flow_time_mins, pairing_frequency, review_latency_hours, ai_utilization_percent, collaboration_index, health_score, generated_at
         FROM team_health_metrics
         WHERE team_id = $1 AND generated_at > NOW() - INTERVAL '1 day' * $2
         ORDER BY generated_at DESC`, [teamId, days]);
...
export async function initializeTeamHealthDashboardRoutes(service) {
