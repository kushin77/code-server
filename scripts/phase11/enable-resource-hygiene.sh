#!/usr/bin/env bash
# @file scripts/phase11/enable-resource-hygiene.sh
# @description Phase 11 - Enable storage & resource hygiene
# @summary Drives resource-limit enforcement and uncommitted artifact cleanup.
...
PHASE_TAG="phase11-enable-resource-hygiene"
...
log_info "=== Phase 11 - Enable storage & resource hygiene ==="
log_info "Dry-run: ${DRY_RUN}"

steps=("Enforce resource limits::scripts/ops/enforce-resource-limits.sh" "Cleanup uncommitted::scripts/ops/cleanup-uncommitted.sh")
results=()
ok=0; fail=0; skip=0
...
{
    printf '# Phase 11 - Enable storage & resource hygiene\n\n'
    printf '%s\n\n' "Drives resource-limit enforcement and uncommitted artifact cleanup."
