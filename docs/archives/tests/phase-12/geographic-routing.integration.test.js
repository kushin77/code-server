...
import { GeoRouter, LoadBalancingStrategy } from '../src/services/routing/GeoRouter';
import { FailoverManager, CircuitBreakerState, } from '../src/services/routing/FailoverManager';
import { LoadBalancer, } from '../src/services/routing/LoadBalancer';
...
describe('Phase 12.3: Geographic Routing Integration Tests', () => {
    ...
    describe('FailoverManager Tests', () => {
        it('should initialize circuit breakers', () => {
            ...
            for (const regionId of TEST_REGIONS) {
                expect(metrics.regionMetrics[regionId].state).toBe(CircuitBreakerState.CLOSED);
            }
        });
        it('should open circuit breaker on consecutive failures', () => {
            ...
            const metrics = failoverManager.getMetrics();
            expect(metrics.regionMetrics[regionId].state).toBe(CircuitBreakerState.OPEN);
        });
        ...
        it('should prevent excessive failover chains', async () => {
            ...
            await expect(failoverManager.executeFailover(TEST_REGIONS[maxChain % TEST_REGIONS.length])).rejects.toThrow('Max failover chain exceeded');
        });
    });
    describe('LoadBalancer Tests', () => {
        ...
        it('should track active connections', () => {
            const regionId = TEST_REGIONS[0];
            expect(loadBalancer['regionLoad'].get(regionId)?.activeConnections).toBe(0);
            loadBalancer.recordConnectionOpened(regionId);
            expect(loadBalancer['regionLoad'].get(regionId)?.activeConnections).toBe(1);
            loadBalancer.recordConnectionClosed(regionId);
            expect(loadBalancer['regionLoad'].get(regionId)?.activeConnections).toBe(0);
        });
        it('should respect capacity limits', async () => {
            const regionId = TEST_REGIONS[0];
            const capacity = loadBalancer['regionLoad'].get(regionId)?.capacity || 1000;
            // Fill to capacity
        ...
        it('should update region load metrics', () => {
            ...
            const errorRate = 0.5;
            const healthy = true;
            loadBalancer.updateRegionLoad(regionId, latency, errorRate, healthy, 0.8);
    ...
    describe('Integration Scenarios', () => {
        it('should handle regional failure and recovery', async () => {
            ...
            expect(recoveryMetrics.regionMetrics[failedRegion].state).toBe(CircuitBreakerState.CLOSED);
        });
        ...
        it('should balance load across healthy regions', async () => {
            ...
            for (let i = 0; i < clientCount; i++) {
                const decision = await loadBalancer.selectRegion(`client-${i}`);
...
//# sourceMappingURL=geographic-routing.integration.test.js.map