...
- Name: kushnir-cloud-appsmith-login.spec.ts >> kushnir.cloud Appsmith login path >> oauth2 start endpoint emits redirect with rd parameter preserved
- Location: specs\kushnir-cloud-appsmith-login.spec.ts:97:7

# Error details

```
Error: apiRequestContext.get: write EPROTO 24190000:error:0A000438:SSL routines:ssl3_read_bytes:tlsv1 alert internal error:openssl\ssl\record\rec_layer_s3.c:916:SSL alert number 80

Call log:
  - → GET https://kushnir.cloud/oauth2/start?rd=%2Fuser%2Flogin
...
```ts
  ...
  2   | 
  3   | const PORTAL_BASE_URL = process.env.PORTAL_BASE_URL || 'https://kushnir.cloud';
  4   | const IDE_BASE_URL = process.env.IDE_BASE_URL || 'https://ide.kushnir.cloud';
  5   | const AUTH_RESET_PATH = process.env.AUTH_RESET_PATH || '/auth/reset';
  6   | const AUTH_START_PATH = process.env.AUTH_START_PATH || '/oauth2/start';
  7   | const AUTH_LOGOUT_PATH = process.env.AUTH_LOGOUT_PATH || '/logout';
