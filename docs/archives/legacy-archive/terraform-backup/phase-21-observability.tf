...
resource "docker_image" "prometheus" {
  ...
  pull_triggers = [split(":", local.docker_images["prometheus"])[1]]
}
...
resource "docker_image" "grafana" {
  ...
  name          = local.docker_images["grafana"]
  pull_triggers = [split(":", local.docker_images["grafana"])[1]]
}
...
resource "docker_image" "alertmanager" {
  ...
  name          = local.docker_images["alertmanager"]
  pull_triggers = [split(":", local.docker_images["alertmanager"])[1]]
}
