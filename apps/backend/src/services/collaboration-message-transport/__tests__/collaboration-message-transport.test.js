...
function createFakeMatrixClient() {
    return {
        createRoom: vi.fn(async (request) => ({ roomId: '!room:example.org', request })),
...
describe('MatrixCollaborationTransportService', () => {
    it('bootstraps encrypted rooms with Megolm-compatible state', async () => {
        ...
        const service = new MatrixCollaborationTransportService(client, new CollaborationMessageEncryptionService({ keyMaterial: encryptionKey }));
    ...
    it('sends ciphertext-only payloads through the Matrix client', async () => {
        ...
        expect(client.sendEvent).toHaveBeenCalledWith('!room:example.org', 'com.code-server.collaboration.message', expect.objectContaining({
            encrypted: true,
    ...
    it('restores the transport encryption path from a vault backup', async () => {
        const client = createFakeMatrixClient();
        const original = new MatrixCollaborationTransportService(client, new CollaborationMessageEncryptionService({ keyMaterial: encryptionKey }));
...
//# sourceMappingURL=collaboration-message-transport.test.js.map