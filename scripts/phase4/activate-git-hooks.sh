#!/usr/bin/env bash
# @file scripts/phase4/activate-git-hooks.sh
# @description Phase 4 - Activate Git hooks for governance
# @summary Installs pre-commit hooks that enforce SSOT and prevent secret leakage.
...
PHASE_TAG="phase4-activate-git-hooks"
...
log_info "=== Phase 4 - Activate Git hooks for governance ==="
log_info "Dry-run: ${DRY_RUN}"

steps=("Install pre-commit SSOT hook::scripts/ci/install-pre-commit-hook.sh")
results=()
ok=0; fail=0; skip=0
...
{
    printf '# Phase 4 - Activate Git hooks for governance\n\n'
    printf '%s\n\n' "Installs pre-commit hooks that enforce SSOT and prevent secret leakage."
