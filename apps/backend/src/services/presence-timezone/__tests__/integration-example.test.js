...
describe('Presence Timezone Integration Example', () => {
    ...
    describe('Presence with Timezone', () => {
        it('gets presence with timezone info', async () => {
            ...
            const res = await request(app)
                .get(`/api/presence-timezone/presence/info?userId=user-1&teamId=team-1&timezone=America/New_York&presence=online&lastActive=${lastActive.toISOString()}`)
                .send();
            expect(res.status).toBe(200);
            expect(res.body).toMatchObject({
                userId: 'user-1',
    ...
    describe('Meeting Time Suggestions', () => {
        it('suggests meeting times for team', async () => {
            ...
            res.body.suggestions.forEach((suggestion) => {
