...
const MLEmbeddingEngine_1 = require("./MLEmbeddingEngine");
const SimilarityScorer_1 = require("./SimilarityScorer");
const RelevanceRanker_1 = require("./RelevanceRanker");
describe('Phase 4A: ML Semantic Search Foundation', () => {
    describe('MLEmbeddingEngine', () => {
        let engine;
        beforeEach(() => {
            engine = new MLEmbeddingEngine_1.MLEmbeddingEngine('http://localhost:11434');
        });
    ...
    describe('SimilarityScorer', () => {
        test('cosineSimilarity should return 1 for identical vectors', () => {
            const v1 = [1, 0, 0];
            const v2 = [1, 0, 0];
            expect(SimilarityScorer_1.SimilarityScorer.cosineSimilarity(v1, v2)).toBeCloseTo(1, 5);
        });
        test('cosineSimilarity should return 0 for orthogonal vectors', () => {
            const v1 = [1, 0, 0];
            const v2 = [0, 1, 0];
            expect(SimilarityScorer_1.SimilarityScorer.cosineSimilarity(v1, v2)).toBeCloseTo(0, 5);
        });
        test('euclideanDistance should return 0 for identical vectors', () => {
            const v1 = [0, 0, 0];
            const v2 = [0, 0, 0];
            expect(SimilarityScorer_1.SimilarityScorer.euclideanDistance(v1, v2)).toBeCloseTo(0, 5);
        });
        test('euclideanDistance should calculate correct distance', () => {
            const v1 = [0, 0, 0];
            const v2 = [3, 4, 0];
            expect(SimilarityScorer_1.SimilarityScorer.euclideanDistance(v1, v2)).toBeCloseTo(5, 5);
        });
        test('jaroWinklerSimilarity should recognize identical strings', () => {
            const similarity = SimilarityScorer_1.SimilarityScorer.jaroWinklerSimilarity('function', 'function');
            expect(similarity).toBeCloseTo(1, 5);
        });
        test('jaroWinklerSimilarity should handle similar strings', () => {
            const similarity = SimilarityScorer_1.SimilarityScorer.jaroWinklerSimilarity('test', 'text');
            expect(similarity).toBeGreaterThan(0.7);
        });
        test('levenshteinDistance should calculate edit distance', () => {
            const distance = SimilarityScorer_1.SimilarityScorer.levenshteinDistance('kitten', 'sitting');
            expect(distance).toBe(3);
        });
        test('levenshteinSimilarity should return normalized score', () => {
            const similarity = SimilarityScorer_1.SimilarityScorer.levenshteinSimilarity('cat', 'cat');
            expect(similarity).toBeCloseTo(1, 5);
        });
        test('tokenOverlapSimilarity should measure word overlap', () => {
            const similarity1 = SimilarityScorer_1.SimilarityScorer.tokenOverlapSimilarity('function test code', 'function test code');
            expect(similarity1).toBeCloseTo(1, 5);
            const similarity2 = SimilarityScorer_1.SimilarityScorer.tokenOverlapSimilarity('function test', 'function test code other');
        ...
        test('hybridSimilarity should combine multiple metrics', () => {
            ...
            const similarity = SimilarityScorer_1.SimilarityScorer.hybridSimilarity(v1, v2, text1, text2);
        ...
        test('should throw on mismatched vector dimensions', () => {
            const v1 = [1, 0, 0];
            const v2 = [1, 0];
            expect(() => SimilarityScorer_1.SimilarityScorer.cosineSimilarity(v1, v2)).toThrow();
        });
    });
    describe('RelevanceRanker', () => {
        let ranker;
        beforeEach(() => {
            ranker = new RelevanceRanker_1.RelevanceRanker();
        });
        ...
        test('should explain relevance score', () => {
            ...
            const queryText = 'function test';
            const explanation = ranker.explainScore(result, queryVector, queryText);
        ...
        test('should apply boost rules', () => {
            ...
            expect(boosted[0].relevanceScore).toBeGreaterThan(boosted[1].relevanceScore);
        });
    });
    describe('Integration Tests', () => {
        test('should integrate embedding engine with similarity scoring', async () => {
            const engine = new MLEmbeddingEngine_1.MLEmbeddingEngine();
            const scorer = SimilarityScorer_1.SimilarityScorer;
            const ranker = new RelevanceRanker_1.RelevanceRanker();
            // Simulate embeddings
...
//# sourceMappingURL=ml-semantic-search.test.js.map