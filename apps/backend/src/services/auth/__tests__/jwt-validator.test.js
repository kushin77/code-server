...
describe('JwtValidator', () => {
    ...
    beforeEach(() => {
        ...
        verifySignatureSpy = vi.spyOn(validator, 'verifySignature');
    ...
    describe('validateToken', () => {
        it('should reject malformed tokens', async () => {
            ...
            await expect(validator.validateToken(malformedToken, 'test-audience')).rejects.toThrow('Invalid JWT format');
        });
        it('should reject tokens without kid', async () => {
            ...
            await expect(validator.validateToken(token, 'test-audience')).rejects.toThrow('missing key ID');
        });
        it('should reject unsupported algorithms', async () => {
            ...
            const signature = 'invalid';
            const token = `${header}.${payload}.${signature}`;
            await expect(validator.validateToken(token, 'test-audience')).rejects.toThrow('Unsupported algorithm');
        });
        ...
        it('should reject tokens with future iat', async () => {
            ...
            const signature = 'invalid';
            const token = `${header}.${payload}.${signature}`;
            await expect(validator.validateToken(token, 'test-audience')).rejects.toThrow('not yet valid');
        });
        it('should reject tokens with wrong audience', async () => {
            ...
            const signature = 'invalid';
            const token = `${header}.${payload}.${signature}`;
            await expect(validator.validateToken(token, 'test-audience')).rejects.toThrow('Audience mismatch');
        });
    });
...
//# sourceMappingURL=jwt-validator.test.js.map