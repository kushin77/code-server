...
export class SymbolDiscussionsService extends EventEmitter {
    constructor(pool, auditService) {
        super();
        this.logger = getLogger('SymbolDiscussionsService');
    ...
    async createThread(request) {
        ...
        try {
            ...
            let discussionResult = await client.query('SELECT id FROM symbol_discussions WHERE fqn = $1', [request.fqn]);
            let discussionId;
            if (discussionResult.rows.length === 0) {
                // Create new symbol discussion
                const insertResult = await client.query(`INSERT INTO symbol_discussions (fqn, file_path, symbol_name, symbol_type, line_number)
           VALUES ($1, $2, $3, $4, $5)
           RETURNING id`, [request.fqn, request.filePath, request.symbolName, request.symbolType, request.lineNumber]);
                ...
                const threadResult = await client.query(`INSERT INTO discussion_threads (discussion_id, title, created_by)
           VALUES ($1, $2, $3)
           RETURNING id`, [discussionId, request.title, request.author]);
                // Add the initial comment
                await client.query(`INSERT INTO thread_comments (thread_id, author, content)
           VALUES ($1, $2, $3)`, [threadResult.rows[0].id, request.author, request.initialComment]);
            }
            else {
                // For thread-per-function behavior, reuse the existing thread for this symbol.
                discussionId = discussionResult.rows[0].id;
                const threadResult = await client.query('SELECT id FROM discussion_threads WHERE discussion_id = $1 ORDER BY created_at ASC LIMIT 1', [discussionId]);
                if (threadResult.rows.length === 0) {
                    const insertThreadResult = await client.query(`INSERT INTO discussion_threads (discussion_id, title, created_by)
             VALUES ($1, $2, $3)
             RETURNING id`, [discussionId, request.title, request.author]);
                    await client.query(`INSERT INTO thread_comments (thread_id, author, content)
             VALUES ($1, $2, $3)`, [insertThreadResult.rows[0].id, request.author, request.initialComment]);
                }
                else {
                    await client.query(`INSERT INTO thread_comments (thread_id, author, content)
             VALUES ($1, $2, $3)`, [threadResult.rows[0].id, request.author, request.initialComment]);
                    await client.query('UPDATE discussion_threads SET updated_at = NOW() WHERE id = $1', [threadResult.rows[0].id]);
                }
                await client.query('UPDATE symbol_discussions SET updated_at = NOW() WHERE id = $1', [discussionId]);
            }
            await client.query('COMMIT');
    ...
    async addComment(request) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`INSERT INTO thread_comments (thread_id, author, content, parent_comment_id)
         VALUES ($1, $2, $3, $4)
         RETURNING id, created_at, updated_at`, [request.threadId, request.author, request.content, request.parentCommentId || null]);
            // Update thread's updated_at
            await client.query('UPDATE discussion_threads SET updated_at = NOW() WHERE id = $1', [request.threadId]);
            // Update discussion's updated_at
            await client.query(`UPDATE symbol_discussions SET updated_at = NOW()
         WHERE id = (SELECT discussion_id FROM discussion_threads WHERE id = $1)`, [request.threadId]);
    ...
    async updateComment(request) {
        const client = await this.pool.connect();
        try {
            await client.query(`UPDATE thread_comments
         SET content = $1, updated_at = NOW(), is_edited = TRUE
         WHERE id = $2 AND author = $3`, [request.content, request.commentId, request.author]);
    ...
    async resolveThread(threadId, resolvedBy) {
        const client = await this.pool.connect();
        try {
            await client.query(`UPDATE discussion_threads
         SET is_resolved = TRUE, resolved_at = NOW(), resolved_by = $2, updated_at = NOW()
         WHERE id = $1`, [threadId, resolvedBy]);
    ...
    async addReaction(commentId, emoji, user) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO comment_reactions (comment_id, emoji, user)
         VALUES ($1, $2, $3)
         ON CONFLICT (comment_id, emoji, user) DO NOTHING`, [commentId, emoji, user]);
        }
        catch (error) {
    ...
    async removeReaction(commentId, emoji, user) {
        const client = await this.pool.connect();
        try {
            await client.query('DELETE FROM comment_reactions WHERE comment_id = $1 AND emoji = $2 AND user = $3', [commentId, emoji, user]);
        }
        catch (error) {
    ...
    async searchDiscussions(request) {
        const client = await this.pool.connect();
        try {
            let query = `
        SELECT
    ...
    async getDiscussionByFQN(fqn) {
        const client = await this.pool.connect();
        try {
            const result = await client.query('SELECT id FROM symbol_discussions WHERE fqn = $1', [fqn]);
    ...
    async getDiscussionById(discussionId) {
        const client = await this.pool.connect();
        try {
            const discussionResult = await client.query('SELECT * FROM symbol_discussions WHERE id = $1', [discussionId]);
            if (discussionResult.rows.length === 0) {
                throw new Error(`Discussion not found: ${discussionId}`);
            }
            const row = discussionResult.rows[0];
            // Get all threads for this discussion
            const threadsResult = await client.query('SELECT * FROM discussion_threads WHERE discussion_id = $1 ORDER BY created_at ASC', [discussionId]);
    ...
    async getCommentsForThread(threadId) {
        const client = await this.pool.connect();
        try {
            const commentsResult = await client.query(`SELECT c.*, array_agg(json_build_object('emoji', r.emoji, 'user', r.user, 'timestamp', r.timestamp)) as reactions
         FROM thread_comments c
         LEFT JOIN comment_reactions r ON r.comment_id = c.id
         WHERE c.thread_id = $1
         GROUP BY c.id
         ORDER BY c.created_at ASC`, [threadId]);
            return commentsResult.rows.map(row => ({
                ...
                reactions: (row.reactions || []).filter((r) => r && r.emoji !== null) || []
            }));
        }
        catch (error) {
    ...
    async getCommentById(commentId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT c.*, array_agg(json_build_object('emoji', r.emoji, 'user', r.user, 'timestamp', r.timestamp)) as reactions
         FROM thread_comments c
         LEFT JOIN comment_reactions r ON r.comment_id = c.id
         WHERE c.id = $1
         GROUP BY c.id`, [commentId]);
            if (result.rows.length === 0) {
    ...
    async updateSymbolFQN(oldFQN, newFQN) {
        const client = await this.pool.connect();
        try {
            await client.query('UPDATE symbol_discussions SET fqn = $1, updated_at = NOW() WHERE fqn = $2', [newFQN, oldFQN]);
    ...
    async deleteThread(threadId) {
        ...
        try {
            ...
            const discussionIdResult = await client.query('SELECT discussion_id FROM discussion_threads WHERE id = $1', [threadId]);
            await client.query('DELETE FROM discussion_threads WHERE id = $1', [threadId]);
            if (discussionIdResult.rows.length > 0) {
                await client.query(`DELETE FROM symbol_discussions
           WHERE id = $1
             AND NOT EXISTS (SELECT 1 FROM discussion_threads WHERE discussion_id = $1)`, [discussionIdResult.rows[0].discussion_id]);
            }
            await client.query('COMMIT');
