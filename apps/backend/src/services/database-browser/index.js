...
export class DatabaseBrowserService extends EventEmitter {
    constructor(pool) {
        super();
        this.logger = getLogger('DatabaseBrowserService');
        this.pool = pool;
    }
    async initialize() {
    ...
    async getPostgresSchema(databaseName) {
        const client = await this.pool.connect();
        try {
            // Get tables
            ...
            for (const tableRow of tablesResult.rows) {
                ...
                // Get columns
                const columnsResult = await client.query(`SELECT column_name, data_type, is_nullable, column_default
           FROM information_schema.columns
           WHERE table_name = $1
           ORDER BY ordinal_position`, [tableName]);
                // Check for primary key
                const pkResult = await client.query(`SELECT a.attname
           FROM pg_index i
           JOIN pg_attribute a ON a.attrelid = i.indrelid AND a.attnum = ANY(i.indkey)
           WHERE i.indrelname = $1`, [`${tableName}_pkey`]);
            ...
            // Get views
            const viewsResult = await client.query(`SELECT table_name, view_definition FROM information_schema.views WHERE table_schema = 'public'`);
    ...
    async executeQuery(query, databaseType, databaseName, userId) {
        const client = await this.pool.connect();
        try {
            ...
            // Record in history
            await client.query(`INSERT INTO query_history (query_text, database_type, database_name, execution_time_ms, row_count, error_message, user_id)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`, [query, databaseType, databaseName, executionTime, result?.rowCount || 0, error?.message, userId]);
            if (error) {
                throw error;
            }
    ...
    async saveQuery(queryName, queryText, databaseType, userId) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO saved_queries (query_name, query_text, database_type, user_id)
         VALUES ($1, $2, $3, $4)`, [queryName, queryText, databaseType, userId]);
    ...
    async exportQueryResults(query, format, databaseType) {
        const client = await this.pool.connect();
        try {
            ...
            // Record export
            await client.query(`INSERT INTO query_export_history (query_id, export_format, export_size_bytes)
         VALUES ($1, $2, $3)`, [Math.random().toString(36), format, Buffer.byteLength(exportData)]);
    ...
    async getQueryHistory(databaseType, limit = 50) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT id, query_text, execution_time_ms, row_count, error_message, executed_at
         FROM query_history
         WHERE database_type = $1
         ORDER BY executed_at DESC
         LIMIT $2`, [databaseType, limit]);
            return result.rows;
        }
        finally {
            client.release();
    ...
    async autocompleteTableNames(databaseType, prefix) {
        const client = await this.pool.connect();
        try {
    ...
    async autocompleteColumnNames(databaseType, tableName, prefix) {
        const client = await this.pool.connect();
        try {
    ...
    async cleanupOldQueryHistory(daysOld = 30) {
        const client = await this.pool.connect();
        try {
            const cutoffDate = new Date();
            cutoffDate.setDate(cutoffDate.getDate() - daysOld);
            const result = await client.query(`DELETE FROM query_history WHERE executed_at < $1`, [cutoffDate]);
...
export async function initializeDatabaseBrowserRoutes(service) {
    ...
    router.get('/api/database/autocomplete/tables', async (req, res) => {
        try {
            ...
            const tables = await service.autocompleteTableNames(databaseType, prefix);
            res.json(tables);
        }
        catch (error) {
    ...
    router.get('/api/database/autocomplete/columns', async (req, res) => {
        try {
            ...
            const columns = await service.autocompleteColumnNames(databaseType, tableName, prefix);
            res.json(columns);
        }
        catch (error) {
