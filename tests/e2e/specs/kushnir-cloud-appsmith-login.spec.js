...
function isAppsmithLoginUrl(url) {
    return url.includes('/user/login') || url.includes('/login/oauth2/code/github');
}
function assertHttpsAndDomain(url, expectedHost) {
...
async function fetchAppsmithLoginResponse(request, requestUrl) {
    try {
...
async function resolveRedirectUriFromAuthStart(requestUrl, requestGet) {
...
async function getRedirectLocation(requestUrl, request) {
...
test.describe('kushnir.cloud Appsmith login path', () => {
    ...
    test('portal oauth start resolves to expected google redirect_uri contract', async ({ request }) => {
        const redirectUri = await resolveRedirectUriFromAuthStart(`${PORTAL_BASE_URL}${AUTH_START_PATH}?rd=%2F`, async (url) => {
    ...
    test('ide oauth start resolves to expected google redirect_uri contract', async ({ request }) => {
        const redirectUri = await resolveRedirectUriFromAuthStart(`${IDE_BASE_URL}${AUTH_START_PATH}?rd=%2F`, async (url) => {
            const response = await request.get(url, { maxRedirects: 0 });
            return {
                status: response.status(),
                location: response.headers()['location'] || '',
    ...
    test('appsmith GitHub button emits redirect_uri for kushnir.cloud host', async ({ request }) => {
        ...
        if (location) {
            ...
            if (location.includes('redirect_uri=')) {
                ...
                expect(normalizeUrl(redirectUri)).toBe(normalizeUrl(APPSMITH_EXPECTED_REDIRECT_URI));
            }
        }
...
//# sourceMappingURL=kushnir-cloud-appsmith-login.spec.js.map