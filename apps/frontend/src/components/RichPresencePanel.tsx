...
import { fetchTeamRichPresence, upsertRichPresence, type RichPresenceRecord, type RichPresenceStatus } from '../utils/richPresence'

export type RichPresencePanelProps = {
  teamId: string
...
const STATUS_OPTIONS: RichPresenceStatus[] = ['online', 'away', 'dnd', 'offline']

function statusLabel(status: RichPresenceStatus): string {
  switch (status) {
    case 'online':
      return 'Online'
    case 'away':
...
export function RichPresencePanel({ teamId, currentUserId, currentDisplayName, teamName }: RichPresencePanelProps) {
  const [snapshot, setSnapshot] = useState<RichPresenceRecord[]>([])
  const [loading, setLoading] = useState(true)
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [status, setStatus] = useState<RichPresenceStatus>('online')
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              ...
              <button
                ...
                className="rounded-full border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 transition hover:border-sky-400 hover:text-sky-700"
              >
                Refresh
              </button>
            </div>
            ...
            <div className="mt-4 space-y-3">
              {snapshot.length > 0 ? (
                snapshot.map((entry) => (
                  <div key={`${entry.teamId}:${entry.userId}`} className="rounded-2xl border border-slate-200 bg-white px-4 py-3">
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="mt-4 space-y-3">
            <label className="block">
              ...
              <select
                value={status}
                onChange={(event) => setStatus(event.target.value as RichPresenceStatus)}
            ...
            <label className="block">
              ...
              <input
                ...
                placeholder="src/components/RichPresencePanel.tsx"
                className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900"
              />
            </label>
            ...
            <label className="block">
              ...
              <input
                ...
                placeholder="renderPresenceList"
                className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900"
              />
            </label>
            ...
            <label className="block">
              ...
              <input
                ...
                placeholder="Polish presence panel"
                className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900"
              />
            </label>
            ...
            <label className="block">
              ...
              <input
                ...
                placeholder="Reviewing collaboration issues"
                className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900"
              />
            </label>
