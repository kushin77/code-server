...
describe('SessionReplayTimelineService', () => {
    ...
    it('should end a session with duration calculation', async () => {
        ...
        expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE session_replay_sessions'), expect.any(Array));
    });
    ...
    it('should handle session creation with user id', async () => {
        ...
        expect(result).toBe(sessionId);
        expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO session_replay_sessions'), expect.any(Array));
    });
...
//# sourceMappingURL=session-replay-timeline.test.js.map