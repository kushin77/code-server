export type RepoCardCiStatus = 'passing' | 'running' | 'failing' | 'blocked'

export type RepoCardErrorCode = 'offline' | 'auth' | 'ci_unavailable'

export type RepoCardActionId = 'open' | 'switch' | 'pull' | 'pullRequests' | 'issues' | 'runbook'

export type RepoCardErrorHint = {
  code: RepoCardErrorCode
  title: string
  remediation: string
}

export type RepoCardStatus = {
  branch: string
  dirty: boolean
  ciStatus: RepoCardCiStatus
...
export type RepoHomeCard = {
  ...
  errorHint?: RepoCardErrorHint
  links: RepoCardLinks
}

export type RepoHomeSnapshot = {
  cards: RepoHomeCard[]
  fetchedAt: number
  refreshIntervalMs: number
}

export type RepoHomeActionPolicy = {
  label: string
...
export type RepoCardActionState = {
  id: RepoCardActionId
  label: string
  disabled: boolean
  reason?: string
...
export function resolveRepoStatusRefreshIntervalMs(rawValue?: string): number {
...
function buildRepoLinks(repoSlug: string): RepoCardLinks {
  return {
    workspace: '/',
...
export function buildSeedRepoHomeCards(now: number = Date.now()): RepoHomeCard[] {
  return [
    {
      id: 'portal-main',
      label: 'Portal main',
...
export function sortRepoHomeCards(cards: RepoHomeCard[]): RepoHomeCard[] {
...
export function createDefaultRepoHomeSnapshot(now: number = Date.now()): RepoHomeSnapshot {
  return {
...
function isValidRepoCard(value: unknown): value is RepoHomeCard {
  if (!value || typeof value !== 'object') {
    return false
  }

  const candidate = value as Partial<RepoHomeCard>
  return (
...
export function readRepoHomeSnapshot(storage: StorageLike | undefined): RepoHomeSnapshot | null {
  if (!storage) {
    return null
  }
  ...
  try {
    ...
    const parsedValue = JSON.parse(rawValue) as Partial<RepoHomeSnapshot>
    if (
      !Array.isArray(parsedValue.cards) ||
...
export function writeRepoHomeSnapshot(storage: StorageLike | undefined, snapshot: RepoHomeSnapshot): void {
  if (!storage) {
    return
  }
...
export function refreshRepoHomeSnapshot(snapshot: RepoHomeSnapshot, now: number = Date.now()): RepoHomeSnapshot {
  return {
    ...snapshot,
    cards: sortRepoHomeCards(snapshot.cards),
    fetchedAt: now,
...
export function formatRepoHomeRefreshLabel(fetchedAt: number, now: number = Date.now()): string {
...
export function formatRepoLastActivity(lastActivityAt: number, now: number = Date.now()): string {
...
export function getRepoCardTone(ciStatus: RepoCardCiStatus): string {
  switch (ciStatus) {
    case 'passing':
      return 'emerald'
    case 'running':
...
export function buildRepoCardActions(
  card: RepoHomeCard,
  policy: RepoHomeActionPolicy,
  activeRepoId: string
): RepoCardActionState[] {
