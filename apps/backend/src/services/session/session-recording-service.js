...
export class SessionRecordingService extends EventEmitter {
    constructor() {
        ...
        this.shareTokens = new Map(); // token -> recordingId
     ...
     */
    startRecording(sessionId, userId, workspaceId) {
     ...
     * Record file change
     */
    recordFileChange(recordingId, change) {
     ...
     */
    recordTerminalEvent(recordingId, event) {
        const recording = this.recordings.get(recordingId);
        if (!recording || !recording.isActive) {
            return false;
        }
     ...
     * Record debug event
     */
    recordDebugEvent(recordingId, event) {
        const recording = this.recordings.get(recordingId);
        if (!recording || !recording.isActive) {
            return false;
        }
     ...
     * Record chat message
     */
    recordChatMessage(recordingId, message) {
        const recording = this.recordings.get(recordingId);
        if (!recording || !recording.isActive) {
            return false;
        }
     ...
     */
    listRecordingsForSession(sessionId) {
     ...
     */
    setPlaybackSpeed(recordingId, speed) {
        const state = this.playback.get(recordingId);
     ...
     */
    generateShareToken(recordingId) {
        const recording = this.recordings.get(recordingId);
        if (!recording) {
            return null;
        }
     ...
     */
    getRecordingByShareToken(token) {
     ...
     */
    async exportRecording(recordingId, options) {
        const recording = this.recordings.get(recordingId);
        if (!recording) {
            return null;
        }
     ...
     */
    getFrameAtTime(recordingId, timeMs) {
        const recording = this.recordings.get(recordingId);
        if (!recording) {
            return null;
        }
    ...
    getStatistics() {
        ...
        const recordingsByUser = {};
        for (const recording of recordings) {
            recordingsByUser[recording.userId] = (recordingsByUser[recording.userId] || 0) + 1;
        }
        const totalEvents = recordings.reduce((sum, r) => sum + r.fileCount + r.terminalCount + r.debugEventCount + r.chatCount, 0);
        return {
...
//# sourceMappingURL=session-recording-service.js.map