...
import { createVoiceSession, fetchVoiceStats, fetchWorkspaceVoiceSessions, joinVoiceSession, leaveVoiceSession, } from '../utils/voiceChannel';
import { fetchTeamRichPresence, upsertRichPresence, } from '../utils/richPresence';
function formatLatency(value) {
    return `${Math.round(value)}ms`;
}
export function VoiceChannelPanel({ workspaceId, workspaceLabel, teamId, currentUserId, currentDisplayName, authToken, }) {
    ...
    const previousPresenceStatusRef = useRef(null);
    const activeSession = useMemo(() => sessions.find((session) => session.sessionId === activeSessionId) ?? null, [activeSessionId, sessions]);
    const meetingModeActive = Boolean(activeSessionId && teamId && currentUserId);
    const updateMeetingPresence = async (nextStatus) => {
    ...
    const restoreMeetingPresence = async () => {
        if (!teamId || !currentUserId) {
            return;
        }
    ...
    return (<section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-100">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2">
            ...
            {meetingModeActive ? (<span className="inline-flex items-center rounded-full border border-violet-200 bg-violet-50 px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-violet-800">
                📞 Meeting mode active
              </span>) : null}
          </div>
      ...
      <div className="mt-5 grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <button type="button" onClick={() => void loadVoiceData()} className="rounded-full border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 transition hover:border-violet-400 hover:text-violet-700">
                Refresh
              </button>
            </div>

            <div className="mt-4 space-y-3">
              {sessions.length > 0 ? (sessions.map((session) => (<div key={session.sessionId} className="rounded-2xl border border-slate-200 bg-white px-4 py-3">
                    <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                      ...
                      <button type="button" onClick={() => void handleJoinSession(session.sessionId)} className="rounded-full border border-violet-300 bg-violet-50 px-3 py-2 text-sm font-medium text-violet-800 transition hover:border-violet-400 hover:bg-violet-100">
                        Join session
                      </button>
                    </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="mt-4 space-y-3">
            <button type="button" onClick={() => void handleCreateSession()} disabled={!currentUserId || Boolean(operation)} className="w-full rounded-full bg-violet-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-violet-700 disabled:cursor-not-allowed disabled:opacity-60">
              Start voice session
            </button>

            <label className="block">
              <span className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Join by session ID</span>
              <input value={joinSessionId} onChange={(event) => setJoinSessionId(event.target.value)} placeholder="session-123" className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900"/>
            </label>

            <button type="button" onClick={() => void handleJoinSession()} disabled={!joinSessionId.trim() || Boolean(operation)} className="w-full rounded-full border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-violet-400 hover:text-violet-700 disabled:cursor-not-allowed disabled:opacity-60">
              Join voice session
            </button>

            <button type="button" onClick={() => void handleLeaveSession()} disabled={!activeSessionId || Boolean(operation)} className="w-full rounded-full border border-rose-300 bg-white px-4 py-2 text-sm font-semibold text-rose-700 transition hover:border-rose-400 hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-60">
          ...
          <div className="mt-4 rounded-2xl border border-slate-200 bg-white px-4 py-3">
            ...
            {activeSession ? (<div className="mt-2 space-y-1 text-sm text-slate-700">
              ...
              </div>) : (<p className="mt-2 text-sm text-slate-600">No active session selected.</p>)}
          </div>
...
//# sourceMappingURL=VoiceChannelPanel.js.map