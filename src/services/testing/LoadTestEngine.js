...
export var LoadTestPhase;
(function (LoadTestPhase) {
    LoadTestPhase["IDLE"] = "IDLE";
    LoadTestPhase["RAMP_UP"] = "RAMP_UP";
    LoadTestPhase["STEADY_STATE"] = "STEADY_STATE";
    LoadTestPhase["RAMP_DOWN"] = "RAMP_DOWN";
    LoadTestPhase["COOLDOWN"] = "COOLDOWN";
})(LoadTestPhase || (LoadTestPhase = {}));
export class LoadTestEngine extends EventEmitter {
     ...
     */
    selectRegion() {
        return this.config.regions[Math.floor(Math.random() * this.config.regions.length)];
    }
    /**
     * Send single request to region
     */
    async sendRequest(regionId) {
        const startTime = Date.now();
        const requestId = `req-${Date.now()}-${Math.random()}`;
        try {
            // Simulate request with realistic latency
            const regionLatency = this.simulateLatency();
            await new Promise((resolve) => setTimeout(resolve, regionLatency));
    ...
    simulateLatency() {
        ...
        const randomValue = Math.random() + Math.random() - 1; // Approximate normal distribution
     ...
     */
    recordSuccess(regionId, latency) {
     ...
     */
    recordFailure(regionId, latency) {
        this.latencies.push(latency);
        const regionLatencies = this.regionLatencies.get(regionId) || [];
        regionLatencies.push(latency);
        this.regionLatencies.set(regionId, regionLatencies);
     ...
     */
    calculateLatencyBucket(latencies) {
    ...
    generateResult() {
        ...
        const duration = endTime.getTime() - (this.testStartTime?.getTime() || 0);
        ...
        for (const regionId of this.config.regions) {
            ...
            const throughput = (requests.total / duration) * 1000; // requests/sec
            ...
            if (latency.p99 > globalLatency.p99 * 1.5) {
                bottlenecks.push(`Region ${regionId}: P99 latency ${Math.round(latency.p99)}ms exceeds threshold`);
            }
            if (errorRate > 1) {
                bottlenecks.push(`Region ${regionId}: Error rate ${Math.round(errorRate)}% exceeds threshold`);
            }
        }
...
//# sourceMappingURL=LoadTestEngine.js.map