...
export interface PerformanceProfile {
  ...
  slowExtensions: ExtensionMetrics[];
}

export class IDEPerformanceProfilerService extends EventEmitter {
  private logger = getLogger('IDEPerformanceProfilerService');
  private pool: Pool;
  ...
  async recordExtensionMetrics(extensionId: string, extensionName: string, startupTime: number, activationTime: number, latency: number): Promise<ExtensionMetrics> {
    const client = await this.pool.connect();
    try {
      ...
      const result = await client.query(
        `INSERT INTO extension_metrics (extension_id, extension_name, startup_time_ms, activation_time_ms, latency_ms, health_score)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING id, extension_id, extension_name, startup_time_ms, activation_time_ms, latency_ms, health_score, is_disabled, disable_reason, measured_at`,
        [extensionId, extensionName, startupTime, activationTime, latency, healthScore]
      );
  ...
  async getExtensionMetrics(extensionId: string, limit: number = 10): Promise<ExtensionMetrics[]> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `SELECT id, extension_id, extension_name, startup_time_ms, activation_time_ms, latency_ms, health_score, is_disabled, disable_reason, measured_at
  ...
  async getPerformanceProfile(sessionId: string): Promise<PerformanceProfile | null> {
    const client = await this.pool.connect();
    try {
      // Get session data
      const sessionResult = await client.query(
        `SELECT id, session_id, overall_health_score, slow_extension_count, recorded_at
      ...
      const extensionsResult = await client.query(
        `SELECT extension_id, extension_name, startup_time_ms, activation_time_ms, latency_ms, health_score, is_disabled, disable_reason, measured_at
         FROM extension_metrics
  ...
  async disableExtension(extensionId: string, reason: string): Promise<void> {
    const client = await this.pool.connect();
    try {
      await client.query(
        `INSERT INTO disabled_extensions (extension_id, reason)
         VALUES ($1, $2)
         ON CONFLICT (extension_id) DO UPDATE SET reason = $2, disabled_at = CURRENT_TIMESTAMP`,
        [extensionId, reason]
      );
  ...
  async enableExtension(extensionId: string): Promise<void> {
    ...
    try {
      await client.query(
        `DELETE FROM disabled_extensions WHERE extension_id = $1`,
        [extensionId]
      );
  ...
  async getSlowExtensions(threshold: number = 50): Promise<ExtensionMetrics[]> {
    const client = await this.pool.connect();
    try {
      const result = await client.query(
        `SELECT extension_id, extension_name, startup_time_ms, activation_time_ms, latency_ms, health_score, is_disabled, disable_reason, measured_at
...
export async function initializeIDEPerformanceProfilerRoutes(service: IDEPerformanceProfilerService) {
