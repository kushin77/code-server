...
export class ScreenShareService extends EventEmitter {
    ...
    constructor(config) {
        ...
        this.crdtClocks = new Map();
        this.cursorTracking = new Map();
        this.config = {
    ...
    startScreenShare(request, ipAddress, userAgent) {
        try {
    ...
    joinScreenShare(request, ipAddress, userAgent) {
        try {
    ...
    addDrawing(request, ipAddress, userAgent) {
        try {
            const session = this.sessions.get(request.sessionId);
            if (!session) {
                throw new Error(`Session ${request.sessionId} not found`);
    ...
    clearAnnotation(request, ipAddress, userAgent) {
        try {
            const session = this.sessions.get(request.sessionId);
            if (!session) {
                throw new Error(`Session ${request.sessionId} not found`);
    ...
    updateCursor(request, ipAddress, userAgent) {
        try {
            const session = this.sessions.get(request.sessionId);
            if (!session) {
                throw new Error(`Session ${request.sessionId} not found`);
    ...
    leaveScreenShare(request, ipAddress, userAgent) {
        try {
            const session = this.sessions.get(request.sessionId);
            if (!session) {
                throw new Error(`Session ${request.sessionId} not found`);
    ...
    pauseShare(request, ipAddress, userAgent) {
        try {
            const session = this.sessions.get(request.sessionId);
            if (!session) {
                throw new Error(`Session ${request.sessionId} not found`);
    ...
    resumeShare(request, ipAddress, userAgent) {
        try {
            const session = this.sessions.get(request.sessionId);
            if (!session) {
                throw new Error(`Session ${request.sessionId} not found`);
    ...
    getStatistics() {
        const activeSessions = Array.from(this.sessions.values()).filter((s) => s.state === 'streaming' || s.state === 'paused');
        ...
        return {
            ...
            totalParticipants: Array.from(this.sessions.values()).reduce((sum, s) => sum + s.participants.size, 0),
            totalAnnotations,
            ...
            totalViewerHours,
            averageAnnotationsPerSession: this.sessions.size > 0 ? totalAnnotations / this.sessions.size : 0,
        };
    }
...
//# sourceMappingURL=screenshare-service.js.map