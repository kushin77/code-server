...
export class CodeOwnershipGraphService extends EventEmitter {
    constructor(pool) {
        super();
        this.logger = getLogger('CodeOwnershipGraphService');
        this.pool = pool;
    }
    async initialize() {
    ...
    async recordCommit(filePath, userId, commitSha) {
        const client = await this.pool.connect();
        try {
            // Increment commit count
            const existingResult = await client.query(`SELECT id, commit_count FROM ownership_commits WHERE file_path = $1 AND user_id = $2 LIMIT 1`, [filePath, userId]);
            if (existingResult.rows.length > 0) {
                await client.query(`UPDATE ownership_commits SET commit_count = commit_count + 1 WHERE file_path = $1 AND user_id = $2`, [filePath, userId]);
            }
            else {
                await client.query(`INSERT INTO ownership_commits (file_path, user_id, commit_count, commit_sha) VALUES ($1, $2, 1, $3)`, [filePath, userId, commitSha]);
            }
            // Update file ownership
            const topContributorResult = await client.query(`SELECT user_id FROM ownership_commits WHERE file_path = $1 ORDER BY commit_count DESC LIMIT 1`, [filePath]);
            if (topContributorResult.rows.length > 0) {
                const topContributor = topContributorResult.rows[0].user_id;
                const contributorsResult = await client.query(`SELECT user_id, commit_count FROM ownership_commits WHERE file_path = $1 ORDER BY commit_count DESC`, [filePath]);
                ...
                const existingOwnershipResult = await client.query(`SELECT id FROM file_ownership WHERE file_path = $1`, [filePath]);
                if (existingOwnershipResult.rows.length > 0) {
                    await client.query(`UPDATE file_ownership SET primary_owner = $1, contributors = $2, last_modified = CURRENT_TIMESTAMP WHERE file_path = $3`, [topContributor, JSON.stringify(contributors), filePath]);
                }
                else {
                    await client.query(`INSERT INTO file_ownership (file_path, primary_owner, contributors) VALUES ($1, $2, $3)`, [filePath, topContributor, JSON.stringify(contributors)]);
                }
            }
    ...
    async getFileOwnership(filePath) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT file_path, primary_owner, contributors, bus_factor, last_modified, created_at
         FROM file_ownership WHERE file_path = $1`, [filePath]);
    ...
    async analyzeBusFactor(filePath) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT contributors FROM file_ownership WHERE file_path = $1`, [filePath]);
            if (result.rows.length === 0)
                return 0;
            ...
            // Update bus factor
            await client.query(`UPDATE file_ownership SET bus_factor = $1 WHERE file_path = $2`, [busFactor, filePath]);
    ...
    async getOwnershipGraph() {
        const client = await this.pool.connect();
        try {
            const filesResult = await client.query(`SELECT file_path, primary_owner, contributors, bus_factor FROM file_ownership ORDER BY last_modified DESC LIMIT 100`);
            const nodes = [];
            const edges = [];
            const busFactorCritical = [];
            const busFactorMedium = [];
    ...
    async generateContributorHeatmap(userId) {
        const client = await this.pool.connect();
        try {
            // Get total commits
            const commitsResult = await client.query(`SELECT COUNT(*) as total_commits FROM ownership_commits WHERE user_id = $1`, [userId]);
            const totalCommits = parseInt(commitsResult.rows[0]?.total_commits || 0);
            // Get files owned
            const filesResult = await client.query(`SELECT COUNT(*) as files_owned FROM file_ownership WHERE primary_owner = $1`, [userId]);
            const filesOwned = parseInt(filesResult.rows[0]?.files_owned || 0);
            // Get bus factor 1 count
            const busFactor1Result = await client.query(`SELECT COUNT(*) as bus_factor_1 FROM file_ownership WHERE primary_owner = $1 AND bus_factor = 1`, [userId]);
            const busFactor1Count = parseInt(busFactor1Result.rows[0]?.bus_factor_1 || 0);
            // Get recent activity (last 7 days)
            const recentResult = await client.query(`SELECT COUNT(*) as recent_activity FROM ownership_commits WHERE user_id = $1 AND committed_at > NOW() - INTERVAL '7 days'`, [userId]);
            const recentActivity = parseInt(recentResult.rows[0]?.recent_activity || 0);
            // Get dominant files
            const dominantFilesResult = await client.query(`SELECT file_path FROM ownership_commits WHERE user_id = $1 ORDER BY commit_count DESC LIMIT 5`, [userId]);
            ...
            // Store heatmap
            const existingResult = await client.query(`SELECT id FROM contributor_heatmap WHERE user_id = $1`, [userId]);
            if (existingResult.rows.length > 0) {
                await client.query(`UPDATE contributor_heatmap SET total_commits = $1, files_owned = $2, bus_factor_1_count = $3, recent_activity = $4, dominant_files = $5
           WHERE user_id = $6`, [totalCommits, filesOwned, busFactor1Count, recentActivity, JSON.stringify(dominantFiles), userId]);
            }
            else {
                await client.query(`INSERT INTO contributor_heatmap (user_id, total_commits, files_owned, bus_factor_1_count, recent_activity, dominant_files)
           VALUES ($1, $2, $3, $4, $5, $6)`, [userId, totalCommits, filesOwned, busFactor1Count, recentActivity, JSON.stringify(dominantFiles)]);
            }
    ...
    async getContributorHeatmap(userId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT user_id, total_commits, files_owned, bus_factor_1_count, recent_activity, dominant_files
         FROM contributor_heatmap WHERE user_id = $1`, [userId]);
            if (result.rows.length === 0)
                return null;
            const row = result.rows[0];
            return {
    ...
    async cacheCodeowners(filePath, content) {
        const client = await this.pool.connect();
        try {
            const owners = await this.parseCodeowners(content);
            const existingResult = await client.query(`SELECT id FROM codeowners_file_cache WHERE file_path = $1`, [filePath]);
            if (existingResult.rows.length > 0) {
                await client.query(`UPDATE codeowners_file_cache SET owners = $1, parsed_at = CURRENT_TIMESTAMP WHERE file_path = $2`, [JSON.stringify(owners), filePath]);
            }
            else {
                await client.query(`INSERT INTO codeowners_file_cache (file_path, owners) VALUES ($1, $2)`, [filePath, JSON.stringify(owners)]);
            }
    ...
    async getCriticalFiles() {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT file_path FROM file_ownership WHERE bus_factor = 1 ORDER BY last_modified DESC`);
    ...
    async getTopContributors(limit = 10) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT user_id, COUNT(*) as commits FROM ownership_commits GROUP BY user_id ORDER BY commits DESC LIMIT $1`, [limit]);
    ...
    async getFileChangeHistory(filePath, days = 30) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT user_id, commit_count, committed_at FROM ownership_commits
         WHERE file_path = $1 AND committed_at > NOW() - INTERVAL '1 day' * $2
         ORDER BY committed_at DESC`, [filePath, days]);
            return result.rows;
        }
        finally {
            client.release();
...
export async function initializeCodeOwnershipGraphRoutes(service) {
