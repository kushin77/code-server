...
export function initializeWorkspaceTemplateRoutes(service: WorkspaceTemplateCatalogService): Router {
  const router = Router();

  router.get('/api/workspace-templates/snapshot', async (_req: Request, res: Response) => {
    try {
      const snapshot = service.getSnapshot();
      res.json(snapshot);
    } catch (error) {
      console.error('[WorkspaceTemplateRoutes] Failed to get snapshot:', error);
      res.status(500).json({ error: 'Failed to get workspace template snapshot' });
    }
  });

  router.get('/api/workspace-templates/:templateId', async (req: Request, res: Response) => {
    try {
      const template = service.getTemplate(req.params.templateId);
      if (!template) {
      ...
      console.error('[WorkspaceTemplateRoutes] Failed to get template:', error);
      res.status(500).json({ error: 'Failed to get workspace template' });
    }
  });

  router.get('/api/workspace-templates/:templateId/devcontainer', async (req: Request, res: Response) => {
    try {
      const devcontainer = service.buildDevcontainer(req.params.templateId);
      res.json(devcontainer);
    } catch (error) {
      console.error('[WorkspaceTemplateRoutes] Failed to build devcontainer:', error);
      const message = error instanceof Error ? error.message : 'Failed to build devcontainer';
