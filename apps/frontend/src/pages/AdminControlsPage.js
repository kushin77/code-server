...
function describeRemoteAuditLog(log) {
...
function formatCostMetricLabel(metric) {
    switch (metric) {
        case 'cpuHours':
            return 'CPU-h';
        case 'memoryGbHours':
...
function formatCostScopeLabel(report) {
...
function sumCostTotals(report) {
    return report.cpuHours + report.memoryGbHours + report.storageGbDays + report.gpuHours;
}
// --- Sub-components ---
const RestrictedAccessPanel = ({ userRoles }) => (<div className="min-h-screen bg-slate-950 text-slate-100">
...
const ComplianceScoreHeader = ({ complianceScore, postureLabel, remoteSignals, isRefreshing, onRefresh, panelError, pendingApprovalsCount, lockdownArmed, criticalControlsCount, lastSyncedAt, }) => (<>
    <header className="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        ...
        : remoteSignals.health === 'degraded'
            ...
            <button type="button" onClick={onRefresh} className="mt-4 inline-flex items-center rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-white transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-60" disabled={isRefreshing}>
    ...
    {panelError ? (<div className="mt-6 rounded-2xl border border-rose-400/30 bg-rose-500/10 p-4 text-sm text-rose-100">
        {panelError}
      </div>) : null}
...
const PolicyControlsGrid = ({ controls, pendingApprovals, draftReasons, onDraftReasonChange, onControlToggle, }) => (<div className="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur">
    ...
    <div className="mt-6 grid gap-4 lg:grid-cols-2">
      {controls.map((control) => {
        ...
        return (<article key={control.id} className={`rounded-3xl border ${style.border} bg-gradient-to-br ${style.accent} p-5`}>
            <div className="flex flex-col gap-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {control.critical ? (<span className="rounded-full border border-rose-400/30 bg-rose-500/15 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.28em] text-rose-100">
                        Critical
                      </span>) : null}
                  </div>
              ...
              {control.critical ? (<div className="space-y-3 rounded-2xl border border-white/10 bg-slate-950/50 p-4">
                  <label className="block text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">
                    Approval reason
                  </label>
                  <textarea value={draftReasons[control.id] ?? ''} onChange={(event) => {
                    onDraftReasonChange(control.id, event.target.value);
                }} rows={3} placeholder={`Why should ${control.label.toLowerCase()} change?`} className="w-full rounded-2xl border border-white/10 bg-slate-900/80 px-4 py-3 text-sm text-white outline-none transition placeholder:text-slate-500 focus:border-sky-400/60"/>
                  {pendingRequest ? (<div className="rounded-2xl border border-amber-400/30 bg-amber-500/10 p-4 text-sm text-amber-50">
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <button type="button" onClick={() => { onControlToggle(control); }} className="inline-flex items-center justify-center rounded-full bg-white px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-slate-200">
                ...
                </div>) : (<div className="flex flex-col gap-3 rounded-2xl border border-white/10 bg-slate-950/50 p-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                  ...
                  <button type="button" onClick={() => { onControlToggle(control); }} className={`inline-flex items-center justify-center rounded-full px-4 py-2 text-sm font-semibold transition ${control.value
...
const ApprovalWorkflowPanel = ({ pendingApprovals, controls, onUpdateApprover, onApprove, onReject, }) => (<div className="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur">
    ...
    <div className="mt-6 space-y-4">
      {pendingApprovals.length === 0 ? (<div className="rounded-2xl border border-white/10 bg-slate-950/50 p-4 text-sm text-slate-300">
          No pending critical changes.
        </div>) : null}

      {pendingApprovals.map((request) => {
        const control = controls.find((item) => item.id === request.controlId);
        return (<div key={request.id} className="rounded-2xl border border-white/10 bg-slate-950/60 p-4">
            ...
            <select value={request.approver} onChange={(event) => { onUpdateApprover(request.id, event.target.value); }} className="mt-2 w-full rounded-2xl border border-white/10 bg-slate-900/80 px-4 py-3 text-sm text-white outline-none transition focus:border-sky-400/60">
              {APPROVER_OPTIONS.map((approver) => (<option key={approver} value={approver}>{approver}</option>))}
            </select>

            <div className="mt-4 flex flex-col gap-3 sm:flex-row">
              <button type="button" onClick={() => { onApprove(request.id); }} className="inline-flex flex-1 items-center justify-center rounded-full bg-emerald-400 px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-emerald-300">
                Approve and apply
              </button>
              <button type="button" onClick={() => { onReject(request.id); }} className="inline-flex flex-1 items-center justify-center rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-white transition hover:bg-white/10">
                Reject
              </button>
            </div>
          </div>);
...
const AuditTrailPanel = ({ auditTrail, latestAuditEntry }) => (<div className="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur">
    ...
    <div className="mt-6 space-y-3">
      {auditTrail.slice(0, 8).map((entry) => (<div key={entry.id} className="rounded-2xl border border-white/10 bg-slate-950/60 p-4">
          <div className="flex items-start justify-between gap-3">
            <div>
            ...
            <span className={`rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.24em] ${entry.status === 'success'
    ...
    {latestAuditEntry ? (<div className="mt-6 rounded-2xl border border-white/10 bg-slate-900/70 p-4 text-sm text-slate-300">
        <p className="font-semibold text-white">Latest event</p>
        <p className="mt-1">{latestAuditEntry.diff}</p>
      </div>) : null}
  </div>);
const RemoteSignalsPanel = ({ remoteSignals }) => (<div className="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur">
...
const CostInsightsPanel = ({ monthlyReport, budgetAlerts, isLoading, error, lastLoadedAt, onRefresh, }) => {
    const rankedQuotas = useMemo(() => [...(monthlyReport?.quotas ?? [])].sort((left, right) => sumCostTotals(right) - sumCostTotals(left)), [monthlyReport]);
    ...
    return (<div className="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
        ...
        <button type="button" onClick={onRefresh} className="inline-flex items-center justify-center rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-white transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-60" disabled={isLoading}>
          {isLoading ? 'Refreshing...' : 'Refresh cost data'}
        </button>
      </div>

      {error ? (<div className="mt-4 rounded-2xl border border-rose-400/30 bg-rose-500/10 p-4 text-sm text-rose-100">
          {error}
        </div>) : null}
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.4fr_1fr]">
        <div>
          ...
          <div className="mt-3 space-y-3">
            {rankedQuotas.length === 0 ? (<div className="rounded-2xl border border-white/10 bg-slate-950/50 p-4 text-sm text-slate-300">
                No monthly cost samples have been recorded yet.
              </div>) : null}

            {rankedQuotas.slice(0, 5).map((quota) => (<div key={quota.quotaId} className="rounded-2xl border border-white/10 bg-slate-950/60 p-4">
                ...
                <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
                  {['cpuHours', 'memoryGbHours', 'storageGbDays', 'gpuHours'].map((metric) => (<div key={metric} className="rounded-2xl border border-white/10 bg-slate-900/70 p-3">
        ...
        <div>
          ...
          <div className="mt-3 space-y-3">
            {budgetAlerts.length === 0 ? (<div className="rounded-2xl border border-white/10 bg-slate-950/50 p-4 text-sm text-slate-300">
                No budget alerts are currently active.
              </div>) : null}

            {budgetAlerts.map((alert) => (<div key={alert.alertId} className="rounded-2xl border border-white/10 bg-slate-950/60 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                  ...
                  <span className={`rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.24em] ${alert.severity === 'critical'
                ? 'bg-rose-500/15 text-rose-100'
                : 'bg-amber-500/15 text-amber-100'}`}>
                    {alert.severity}
                  </span>
...
export const AdminControlsPage = () => {
...
const AdminControlsPageOrchestratorComponent = ({ userEmail }) => {
    ...
    const pendingApprovals = useMemo(() => storageSnapshot.approvals.filter((request) => request.status === 'pending'), [storageSnapshot.approvals]);
    ...
    const criticalControls = useMemo(() => storageSnapshot.controls.filter((control) => control.critical), [storageSnapshot.controls]);
    const lockdownArmed = storageSnapshot.controls.find((control) => control.id === 'emergencyLockdown')?.value ?? false;
    ...
    const handleApproverUpdate = (requestId, approver) => {
    ...
    return (<div className="relative min-h-screen overflow-hidden bg-slate-950 text-slate-100">
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <ComplianceScoreHeader complianceScore={complianceScore} postureLabel={postureLabel} remoteSignals={remoteSignals} isRefreshing={isRefreshing} onRefresh={() => { void refreshSignals(() => rbacAPI.healthCheck(), () => rbacAPI.getAuditLogs({ limit: 5 })); }} panelError={panelError} pendingApprovalsCount={pendingApprovals.length} lockdownArmed={lockdownArmed} criticalControlsCount={criticalControls.length} lastSyncedAt={storageSnapshot.lastSyncedAt}/>

        <div className="mt-6 grid gap-6 xl:grid-cols-[1.45fr_1fr]">
          <section className="space-y-6">
            <PolicyControlsGrid controls={storageSnapshot.controls} pendingApprovals={pendingApprovals} draftReasons={draftReasons} onDraftReasonChange={(id, value) => { setDraftReasons((current) => ({ ...current, [id]: value })); }} onControlToggle={handleControlToggle}/>
          </section>

          <aside className="space-y-6">
            <CostInsightsPanel monthlyReport={monthlyReport} budgetAlerts={budgetAlerts} isLoading={isCostLoading} error={costError} lastLoadedAt={lastLoadedAt} onRefresh={() => { void refreshCostInsights(); }}/>
            <ApprovalWorkflowPanel pendingApprovals={pendingApprovals} controls={storageSnapshot.controls} onUpdateApprover={handleApproverUpdate} onApprove={handleApproveRequest} onReject={handleRejectRequest}/>
            <AuditTrailPanel auditTrail={storageSnapshot.auditTrail} latestAuditEntry={latestAuditEntry}/>
...
function useWorkspaceStorageManager(initialSnapshot) {
...
function useRemoteSignalsManager() {
    ...
    const refreshSignals = useCallback(async (healthCheck, getAuditLogs) => {
        ...
        try {
            ...
            if (auditResult.status === 'fulfilled') {
                ...
                nextRemoteSignals.auditSummary = latest ? describeRemoteAuditLog(latest) : 'No recent audit events';
            }
...
function useComplianceScore(snapshot, pendingApprovals, remoteSignals) {
    return useMemo(() => {
...
function usePostureLabel(complianceScore, pendingApprovals, remoteSignals) {
    return useMemo(() => {
...
function toggleControl(control, snapshot, setSnapshot, draftReasons, setDraftReasons, userEmail, setPanelError) {
    if (control.critical) {
...
function requestCriticalApproval(control, snapshot, setSnapshot, draftReasons, setDraftReasons, userEmail) {
    const reason = draftReasons[control.id]?.trim() ?? '';
    const nextValue = !control.value;
    setSnapshot({
        ...snapshot,
...
function updateControlDirect(controlId, nextValue, snapshot, setSnapshot, userEmail) {
    ...
    setSnapshot({
        ...snapshot,
        controls: snapshot.controls.map((item) => item.id === controlId
            ? {
                ...item,
                value: nextValue,
...
function updateApproverInSnapshot(requestId, approver, setSnapshot) {
    setSnapshot((current) => ({
        ...current,
        approvals: current.approvals.map((request) => request.id === requestId ? { ...request, approver } : request),
    }));
}
function approveRequestInSnapshot(requestId, userEmail, snapshot, setSnapshot) {
    ...
    setSnapshot({
        ...snapshot,
        controls: snapshot.controls.map((item) => item.id === request.controlId
            ? {
                ...item,
            ...
            : item),
        approvals: snapshot.approvals.map((item) => item.id === requestId ? { ...item, status: 'approved', decidedAt: nowIso() } : item),
        auditTrail: [
            createControlAuditEntry({
                action: 'approve',
                actor: userEmail,
...
function rejectRequestInSnapshot(requestId, userEmail, snapshot, setSnapshot) {
    const request = snapshot.approvals.find((item) => item.id === requestId);
    if (!request) {
        return;
    }
    ...
    setSnapshot({
        ...snapshot,
        approvals: snapshot.approvals.map((item) => item.id === requestId ? { ...item, status: 'rejected', decidedAt: nowIso() } : item),
        auditTrail: [
            createControlAuditEntry({
                action: 'reject',
                actor: userEmail,
...
//# sourceMappingURL=AdminControlsPage.js.map