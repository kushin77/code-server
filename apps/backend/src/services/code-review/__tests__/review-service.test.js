...
import { CodeReviewService, getCodeReviewService, } from '../review-service.js';
describe('Code Review Service', () => {
    ...
    describe('Creating Review Requests', () => {
        it('should create review request', async () => {
            ...
            const requestId = await reviewService.createRequest('ws-test', 'user-alice', 'Fix type annotations', 'Need to add proper types', targets, ['user-bob', 'user-charlie'], 'high');
        ...
        it('should set default priority', async () => {
            ...
            const requestId = await reviewService.createRequest('ws-test', 'user-alice', 'Review', 'Please review', targets, ['user-bob']);
            const request = await reviewService.getRequest(requestId);
        ...
        it('should track request metadata', async () => {
            ...
            const requestId = await reviewService.createRequest('ws-test', 'user-alice', 'Code review', 'Please review app.ts', targets, ['user-bob'], 'urgent');
            const request = await reviewService.getRequest(requestId);
    ...
    describe('Submitting Reviews', () => {
        ...
        beforeEach(async () => {
            ...
            requestId = await reviewService.createRequest('ws-test', 'user-alice', 'Review needed', 'Description', targets, ['user-bob', 'user-charlie'], 'high');
        });
        it('should submit approval review', async () => {
            const reviewId = await reviewService.submitReview(requestId, 'user-bob', 'approved', 'Looks good!');
        ...
        it('should submit requested-changes review', async () => {
            await reviewService.submitReview(requestId, 'user-bob', 'requested-changes', 'Please refactor this function');
            const request = await reviewService.getRequest(requestId);
            expect(request?.status).toBe('requested-changes');
        });
        it('should submit comment review', async () => {
            await reviewService.submitReview(requestId, 'user-bob', 'commented', 'What about error handling?');
            const reviews = await reviewService.getReviews(requestId);
            expect(reviews[0].status).toBe('commented');
        });
        it('should track review count', async () => {
            await reviewService.submitReview(requestId, 'user-bob', 'approved', 'Great!');
            const request = await reviewService.getRequest(requestId);
            expect(request?.reviewsCompleted).toBe(1);
            await reviewService.submitReview(requestId, 'user-charlie', 'approved', 'Good!');
        ...
        it('should handle multiple reviews from same reviewer', async () => {
            await reviewService.submitReview(requestId, 'user-bob', 'commented', 'First comment');
            await reviewService.submitReview(requestId, 'user-bob', 'approved', 'Now approved');
            const reviews = await reviewService.getReviews(requestId);
    ...
    describe('Listing Reviews', () => {
        beforeEach(async () => {
            ...
            for (let i = 0; i < 3; i++) {
                await reviewService.createRequest('ws-pending', 'user-alice', `Review ${i}`, `Description ${i}`, targets, ['user-bob', 'user-charlie'], i === 0 ? 'urgent' : i === 1 ? 'high' : 'medium');
            }
        });
        ...
        it('should not list completed requests', async () => {
            ...
            await reviewService.submitReview(firstRequest.id, 'user-bob', 'approved', 'Good');
            await reviewService.submitReview(firstRequest.id, 'user-charlie', 'approved', 'Good');
            const pending = await reviewService.listPendingForReviewer('user-bob');
    ...
    describe('Updating Requests', () => {
        ...
        beforeEach(async () => {
            ...
            requestId = await reviewService.createRequest('ws-test', 'user-alice', 'Original title', 'Original description', targets, ['user-bob'], 'medium');
        });
    ...
    describe('Closing Requests', () => {
        ...
        beforeEach(async () => {
            ...
            requestId = await reviewService.createRequest('ws-test', 'user-alice', 'Review', 'Description', targets, ['user-bob']);
        });
    ...
    describe('Reviewer Badges', () => {
        ...
        beforeEach(async () => {
            ...
            requestId = await reviewService.createRequest('ws-test', 'user-alice', 'Review', 'Description', targets, ['user-bob', 'user-charlie']);
        });
        ...
        it('should update badge approval status', async () => {
            ...
            await reviewService.submitReview(requestId, 'user-bob', 'approved', 'Good');
            const request = await reviewService.getRequest(requestId);
    ...
    describe('Statistics', () => {
        beforeEach(async () => {
            ...
            for (let i = 0; i < 5; i++) {
                const priority = ['low', 'medium', 'high', 'urgent', 'high'][i];
                const requestId = await reviewService.createRequest('ws-stats', 'user-alice', `Review ${i}`, 'Description', targets, ['user-bob', 'user-charlie'], priority);
                // Complete some
                if (i < 2) {
                    await reviewService.submitReview(requestId, 'user-bob', 'approved', 'Good');
                    await reviewService.submitReview(requestId, 'user-charlie', 'approved', 'Good');
    ...
    describe('Search', () => {
        beforeEach(async () => {
            ...
            await reviewService.createRequest('ws-search', 'user-alice', 'TypeScript fixes needed', 'Fix type annotations in core module', targets, ['user-bob']);
            await reviewService.createRequest('ws-search', 'user-alice', 'React refactor', 'Refactor component hooks', targets, ['user-bob']);
        });
        it('should search by title', async () => {
            const results = await reviewService.searchRequests('ws-search', 'TypeScript');
            expect(results.length).toBe(1);
            expect(results[0].title).toContain('TypeScript');
        });
        it('should search by description', async () => {
            const results = await reviewService.searchRequests('ws-search', 'component hooks');
            expect(results.length).toBe(1);
    ...
    describe('Get By Priority', () => {
        beforeEach(async () => {
            ...
            for (let i = 0; i < 3; i++) {
                await reviewService.createRequest('ws-priority', 'user-alice', `Urgent ${i}`, 'Description', targets, ['user-bob'], 'urgent');
            }
            for (let i = 0; i < 2; i++) {
                await reviewService.createRequest('ws-priority', 'user-alice', `High ${i}`, 'Description', targets, ['user-bob'], 'high');
            }
        });
        it('should get requests by priority', async () => {
            const urgent = await reviewService.getRequestsByPriority('ws-priority', 'urgent');
    ...
    describe('Event Emission', () => {
        it('should emit request-created event', async () => {
            return new Promise((resolve) => {
                ...
                reviewService.createRequest('ws-test', 'user-alice', 'New review', 'Description', targets, ['user-bob']);
            });
        });
    });
    ...
    describe('Integration', () => {
        it('should handle complete review workflow', async () => {
            ...
            // 1. Create request
            const requestId = await reviewService.createRequest('ws-workflow', 'user-alice', 'Critical refactor', 'Please review the new architecture', targets, ['user-bob', 'user-charlie'], 'urgent');
            ...
            // 4. Collect reviews
            await reviewService.submitReview(requestId, 'user-bob', 'approved', 'Great work!');
            await reviewService.submitReview(requestId, 'user-charlie', 'approved', 'Approved');
