...
export function initializeAIReviewerRouterRoutes(service: AIReviewerRouterService): Router {
  const router = Router();

  // Register reviewer expertise
  router.post('/api/reviewers/:reviewerId/expertise', async (req: Request, res: Response) => {
    try {
      ...
      const expertise = await service.registerReviewerExpertise(
        reviewerId,
        filePattern,
        expertiseLevel as ExpertiseLevel,
        confidence
      );
  ...
  router.post('/api/reviewers/:reviewerId/workload', async (req: Request, res: Response) => {
    try {
      const { reviewerId } = req.params;
      ...
      const workload = await service.updateReviewerWorkload(
        reviewerId,
        pendingReviews,
  ...
  router.post('/api/reviewers/:reviewerId/availability', async (req: Request, res: Response) => {
    try {
      const { reviewerId } = req.params;
      ...
      const availability = await service.updateReviewerAvailability(
        reviewerId,
        timezone,
        isOnline,
        preferredWorkHoursStart,
        preferredWorkHoursEnd
      );
  ...
  // Assign review
  router.post('/api/reviews/assign', async (req: Request, res: Response) => {
    try {
  ...
  // Score reviewers
  router.post('/api/reviews/score', async (req: Request, res: Response) => {
    try {
  ...
  // Complete review
  router.post('/api/reviews/:assignmentId/complete', async (req: Request, res: Response) => {
    try {
  ...
  // Get assignment
  router.get('/api/reviews/:assignmentId', async (req: Request, res: Response) => {
    try {
      const { assignmentId } = req.params;
