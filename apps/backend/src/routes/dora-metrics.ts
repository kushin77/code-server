...
export function initializeDORAMetricsRoutes(pool: Pool): Router {
  ...
  router.get('/dora/trend', async (req, res) => {
    try {
      ...
      const trendWeeks = parseInt(weeks as string, 10);
  ...
  router.post('/failure-recovery', async (req, res) => {
    try {
      ...
      await doraMetricsService.recordFailureRecovery(
        deploymentId,
  ...
  router.post('/dora/snapshot-create', async (req, res) => {
    try {
      ...
      await doraMetricsService.snapshottMetrics(
        weekNumber,
        new Date(weekStart),
        new Date(weekEnd)
      );
