...
import { createDebugSession, fetchDebugSession, fetchRelayedDebugMessages, joinDebugSession, leaveDebugSession, recordDebugStep, relayDebugProtocolMessage, updateDebugBreakpoints, updateDebugVariables, } from '../debugCollaboration';
...
function buildJsonResponse(payload, ok = true, status = 200) {
    return {
        ok,
        status,
        json: async () => payload,
...
describe('debugCollaboration', () => {
    it('creates and loads debug sessions through the API helpers', async () => {
        ...
        await expect(createDebugSession({
    ...
    it('routes join, breakpoint, variable, step, leave, and relay calls to the correct endpoints', async () => {
        ...
        expect(fetchMock).toHaveBeenNthCalledWith(1, '/api/debug-sessions/session-1/join', expect.objectContaining({ method: 'POST' }));
        expect(fetchMock).toHaveBeenNthCalledWith(2, '/api/debug-sessions/session-1/breakpoints', expect.objectContaining({ method: 'PUT' }));
        expect(fetchMock).toHaveBeenNthCalledWith(3, '/api/debug-sessions/session-1/variables', expect.objectContaining({ method: 'PUT' }));
        expect(fetchMock).toHaveBeenNthCalledWith(4, '/api/debug-sessions/session-1/step', expect.objectContaining({ method: 'POST' }));
        expect(fetchMock).toHaveBeenNthCalledWith(5, '/api/debug-sessions/session-1/leave', expect.objectContaining({ method: 'POST' }));
        expect(fetchMock).toHaveBeenNthCalledWith(6, '/api/debug-sessions/session-1/relay', expect.objectContaining({ method: 'POST' }));
    });
    it('fetches relay deltas with actor and sequence cursor', async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith('/api/debug-sessions/session-1/relay/messages?actor=Portal+main&since=2', expect.objectContaining({
            headers: expect.objectContaining({ 'Content-Type': 'application/json' }),
        }));
    });
});
//# sourceMappingURL=debugCollaboration.test.js.map