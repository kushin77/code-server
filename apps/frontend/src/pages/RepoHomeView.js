...
import { buildRepoCardActions, formatRepoHomeRefreshLabel, formatRepoLastActivity, getRepoCardTone, } from '@/utils/repoHomeData';
export const RepoHomeView = ({ workspaceState }) => {
    const { activeWorkspace, activeRepoCard, actionNotice, performRepoAction, repoHomeSnapshot, sessionSnapshot, restoreNotice, restoreSavedSession, forgetSavedSession, restorePreferences, setRestorePreference, workspacePolicy, policyReport, rolloutDecision, multiRepoNavigationEnabled, multiRepoTabsEnabled, multiRepoSwitcherEnabled, multiRepoPersistenceEnabled, } = workspaceState;
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm shadow-slate-100">
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-3">
          {allWorkspaceCards.map((card) => {
            ...
            return (<article key={card.id} data-testid={`repo-card-${card.id}`} className={`rounded-2xl border p-4 transition ${isActive ? 'border-sky-500 bg-sky-50 shadow-sm' : 'border-slate-200 bg-slate-50 hover:border-slate-300'}`}>
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex flex-wrap justify-end gap-2">
                    {card.favorite ? (<span className="rounded-full bg-white px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">
                        Favorite
                      </span>) : null}
                    {card.sharedSet ? (<span className="rounded-full bg-white px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-violet-600">
                        {card.sharedSet}
                      </span>) : null}
                  </div>
                </div>
                ...
                <div className="mt-3 space-y-2 text-sm text-slate-600">
                  <div className="flex flex-wrap gap-2">
                    ...
                    <span className={`rounded-full px-2 py-1 text-xs font-medium ${card.status.dirty ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800'}`}>
                ...
                {card.errorHint ? (<div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 px-3 py-3 text-sm text-amber-900">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {actions.map((action) => {
                    const shouldDisableForRollout = (action.id === 'open' || action.id === 'switch' || action.id === 'pull') && !multiRepoTabsEnabled;
                    return (<button key={action.id} type="button" onClick={() => performRepoAction(card.id, action.id)} disabled={action.disabled || shouldDisableForRollout} title={action.reason} className={`rounded-full px-3 py-2 text-sm font-medium transition ${action.id === 'switch' || action.id === 'open'
                            ? 'bg-slate-900 text-white hover:bg-slate-700'
                            : 'border border-slate-300 text-slate-700 hover:border-sky-400 hover:text-sky-700'} disabled:cursor-not-allowed disabled:opacity-50`}>
                        {action.label}
                      </button>);
                })}
                </div>
        ...
        <div className="mt-6 rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        ].map(([moduleName, label]) => (<label key={moduleName} className="inline-flex items-center gap-2 text-xs text-slate-600">
                    <input type="checkbox" checked={restorePreferences[moduleName]} onChange={(event) => setRestorePreference(moduleName, event.target.checked)}/>
                    {label}
                  </label>))}
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              <button type="button" onClick={restoreSavedSession} disabled={!workspacePolicy.canRestoreSession || !multiRepoPersistenceEnabled} className="rounded-full bg-emerald-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-50">
                Restore session
              </button>
              <button type="button" onClick={forgetSavedSession} className="rounded-full border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-slate-400 hover:text-slate-900">
                Clear snapshot
              </button>
            </div>
          </div>
...
//# sourceMappingURL=RepoHomeView.js.map