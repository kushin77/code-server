...
export async function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('teamHub.mentionUser', 
      (userId: string) => commandHandler.mentionUser(userId)
    ),
    vscode.commands.registerCommand('teamHub.startMeet',
      (userIds: string[]) => commandHandler.startMeet(userIds)
    ),
    vscode.commands.registerCommand('teamHub.goToUserFile',
      (userId: string) => commandHandler.goToUserFile(userId)
    ),
    vscode.commands.registerCommand('teamHub.refreshPresence',
      () => commandHandler.refreshPresence()
    ),
    vscode.commands.registerCommand('teamHub.settings',
      () => vscode.commands.executeCommand('workbench.action.openSettings', 'teamHub')
    )
  );

  // Subscribe to editor changes for auto-presence updates
  const activeEditorChangeSubscription = vscode.window.onDidChangeActiveTextEditor(
    async (editor) => {
      if (editor) {
  ...
  const refreshInterval = setInterval(
    ...
    vscode.workspace.getConfiguration('teamHub').get('presenceUpdateInterval', 5000)
  );
