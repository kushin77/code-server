...
-----END RSA PRIVATE KEY-----`;
  ...
  describe('Event Emission', () => {
    it('should emit dlp-detection event on match', async () => {
      const detectionPromise = new Promise<{ matchCount: number; severity: string }>(resolve => {
        dlp.on('dlp-detection', (event) => resolve(event));
      });
      dlp.scan('test@example.com');
    ...
    it('should emit audit-log event when enabled', async () => {
      const auditDLP = new TerminalOutputDLP({ auditLog: true });
      const auditPromise = new Promise<{ timestamp: string; matchCount: number }>(resolve => {
        auditDLP.on('audit-log', (event) => resolve(event));
      });
      auditDLP.scan('test@example.com');
      const event = await auditPromise;
      expect(event.timestamp).toBeDefined();
      expect(event.matchCount).toBeGreaterThan(0);
    });

    it('should not emit events when DLP disabled', async () => {
  ...
  describe('Configuration', () => {
    it('should disable DLP via setEnabled', () => {
      dlp.setEnabled(false);
      const result = dlp.scan('ghp_' + 'AbCdEfGhIjKlMnOpQrSsT1234567890123');
    ...
    it('should switch modes via setMode', () => {
      dlp.setMode('block');
      const result = dlp.scan('ghp_' + 'AbCdEfGhIjKlMnOpQrSsT1234567890123456');
      expect(result.action).toBe('blocked');
    });
  ...
  describe('Performance', () => {
    ...
    it('should scan 10KB of content under 500ms', () => {
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle very long content', { timeout: 30000 }, () => {
