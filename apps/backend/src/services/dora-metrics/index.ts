...
export interface DORAMetrics {
  ...
  meanLeadTime: number; // milliseconds
...
export interface TierBenchmark {
  tier: TierLevel;
  deploymentFrequency: { min: number; max?: number }; // per week
  leadTime: { min?: number; max?: number }; // milliseconds
  changeFailureRate: { min?: number; max?: number }; // 0-1
  mttr: { min?: number; max?: number }; // milliseconds
}
...
export class DORAMetricsService extends EventEmitter {
  ...
  async recordDeployment(event: DeploymentEvent): Promise<void> {
    const client = await this.pool.connect();
    try {
      await client.query(
        `INSERT INTO deployment_events (timestamp, commit_sha, deployment_id, environment, success, duration)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (deployment_id) DO UPDATE SET success = $5, updated_at = NOW()`,
        [event.timestamp, event.commitSha, event.deploymentId, event.environment, event.success, event.duration]
      );
  ...
  async recordCommitMetric(metric: CommitMetric): Promise<void> {
    const client = await this.pool.connect();
    try {
      ...
      await client.query(
        `INSERT INTO commit_metrics (commit_sha, committed_at, deployed_at, lead_time_ms, environment, deployment_success)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (commit_sha) DO UPDATE SET
           deployed_at = $3, lead_time_ms = $4, environment = $5, deployment_success = $6, updated_at = NOW()`,
        [metric.sha, metric.committedAt, metric.deployedAt || null, leadTime, metric.environment || null, metric.success || null]
      );
  ...
  async recordFailureRecovery(deploymentId: string, recoveredAt: Date, rootCause?: string): Promise<void> {
    const client = await this.pool.connect();
    try {
      // Get failure timestamp
      const failureResult = await client.query(
        'SELECT failure_timestamp FROM deployment_failures WHERE deployment_id = $1',
        [deploymentId]
      );
      ...
      await client.query(
        `UPDATE deployment_failures SET recovery_timestamp = $1, mttr_ms = $2, root_cause = $3, updated_at = NOW()
         WHERE deployment_id = $4`,
        [recoveredAt, mttrMs, rootCause || null, deploymentId]
      );
  ...
  async getMetrics(periodDays: number = 28): Promise<DORAMetrics> {
    const client = await this.pool.connect();
    try {
      ...
      const leadTimeResult = await client.query(
        `SELECT AVG(lead_time_ms) as avg_lead_time FROM commit_metrics 
         WHERE committed_at >= $1 AND lead_time_ms IS NOT NULL`,
        [startDate]
      );
      ...
      const mttrResult = await client.query(
        `SELECT AVG(mttr_ms) as avg_mttr FROM deployment_failures 
         WHERE failure_timestamp >= $1 AND mttr_ms IS NOT NULL`,
        [startDate]
      );
  ...
  async getTrendData(weeks: number = 12): Promise<TrendDataPoint[]> {
    const client = await this.pool.connect();
    try {
  ...
  async calculateTier(metrics: DORAMetrics): Promise<TierLevel> {
    // DORA benchmarks (elite, high, medium, low)
    const benchmarks: Record<TierLevel, TierBenchmark> = {
      elite: {
        tier: 'elite',
    ...
    for (const tier of ['elite', 'high', 'medium', 'low'] as TierLevel[]) {
  ...
  async snapshottMetrics(weekNumber: number, weekStart: Date, weekEnd: Date): Promise<void> {
    const client = await this.pool.connect();
    try {
      ...
      await client.query(
        `INSERT INTO dora_weekly_snapshots 
  ...
  async getMetricsSnapshot(periodDays: number = 28): Promise<MetricsSnapshot> {
  ...
  getTierBenchmarks(): Record<TierLevel, TierBenchmark> {
    return {
      elite: {
        tier: 'elite',
  ...
  formatMetricsForDisplay(metrics: DORAMetrics): string {
    const formatTime = (ms: number): string => {
      if (ms < 1000 * 60) return `${Math.round(ms / 1000)}s`;
      if (ms < 1000 * 60 * 60) return `${Math.round(ms / (1000 * 60))}m`;
      if (ms < 1000 * 60 * 60 * 24) return `${Math.round(ms / (1000 * 60 * 60))}h`;
