...
export class TemplateService extends EventEmitter {
  ...
  private metadata: Map<string, TemplateMetadata[]> = new Map(); // Per-user templates
  private auditLog: Map<string, TemplateAuditEntry[]> = new Map(); // Per-user audit trail
  private stats: TemplateStatistics = {
    totalTemplates: 0,
    templatesByType: {},
  ...
  async createTemplate(
    ...
    const fullTemplate: WorkspaceTemplate = {
      ...template,
      id: templateId,
      createdAt: now,
      updatedAt: now,
   ...
   * Get template
   */
  async getTemplate(templateId: string): Promise<WorkspaceTemplate | undefined> {
  ...
  async updateTemplate(
    ...
    if (userMeta) {
      ...
      if (meta) {
        ...
        if (updates.version) meta.version = updates.version;
      }
    }
   ...
   */
  private async logAudit(userId: string, entry: TemplateAuditEntry): Promise<void> {
  ...
  static getInstance(config?: Partial<TemplateServiceConfig>): TemplateService {
