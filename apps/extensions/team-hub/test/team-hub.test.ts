...
describe('team hub helpers', () => {
  ...
  it('builds a team hub snapshot', () => {
    const snapshot = buildTeamHubSnapshot(
      { id: 'you', displayName: 'You', status: 'online', currentFile: 'api/auth.ts', currentLine: 18, currentFunction: 'seedCurrentUser', currentTask: 'Triaging presence sync', customStatus: 'Available for review', lastSeen: Date.now() },
      users,
      'api/auth.ts'
    );
  ...
  it('renders the sidebar html with key sections', () => {
    const snapshot = buildTeamHubSnapshot(
      { id: 'you', displayName: 'You', status: 'online', currentFile: 'api/auth.ts', currentLine: 18, currentFunction: 'seedCurrentUser', currentTask: 'Triaging presence sync', customStatus: 'Available for review', timezone: 'UTC', workingHours: { startHour: 9, endHour: 17 }, lastSeen: Date.now() },
      users,
      'api/auth.ts'
    );
