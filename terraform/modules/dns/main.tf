...
# cloudflare provider v4: consolidated into cloudflare_zone_settings_override
...
# cloudflare provider v4: updated attribute names
...
# cloudflare provider v4: rules must be blocks not list literals
...
# cloudflare provider v4: action and match use block syntax, not map literals
resource "cloudflare_rate_limit" "api" {
  ...
  match {
    request {
      ...
      schemes     = ["HTTP", "HTTPS"]
      url_pattern = "${var.apex_domain}/api/*"
    }
  }
}
...
resource "cloudflare_rate_limit" "auth" {
  ...
  match {
    request {
      ...
      schemes     = ["HTTP", "HTTPS"]
      url_pattern = "${var.apex_domain}/auth/*"
    }
  }
}

# cloudflare provider v4: rules use block syntax; action_parameters headers use blocks
