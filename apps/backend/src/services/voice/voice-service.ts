...
import type {
  ...
  DeafenParticipantResult,
  GetVoiceChannelRequest,
  GetVoiceChannelResult,
  GetParticipantsRequest,
  GetParticipantsResult,
  UpdateAudioLevelRequest,
  UpdateConnectionStatsRequest,
  ToggleLocalAudioRequest,
  ToggleLocalAudioResult,
...
export class VoiceChannelService extends EventEmitter {
  ...
  private recordings: Map<string, { channelId: string; startedAt: number; recordingId: string }> = new Map();
  private auditLogs: Map<string, VoiceAuditEntry[]> = new Map();
  private connectionStats: Map<string, VoiceConnectionStats> = new Map();
  private audioLevels: Map<string, AudioLevel> = new Map();

  static getInstance(config?: Partial<VoiceServiceConfig>): VoiceChannelService {
    if (!this.instance) {
  ...
  private constructor(config?: Partial<VoiceServiceConfig>) {
    super();
    this.config = {
  ...
  createVoiceChannel(request: CreateVoiceChannelRequest, ipAddress: string, userAgent: string): CreateVoiceChannelResult {
    try {
      ...
      return { success: false, channelId: '', livekitToken: '', livekitUrl: '', channel: null as any, error: errorMsg };
    }
  }

  joinVoiceChannel(request: JoinVoiceChannelRequest, ipAddress: string, userAgent: string): JoinVoiceChannelResult {
    try {
      ...
      return { success: false, participantId: '', livekitToken: '', livekitUrl: '', livekitRoomName: '', channel: null as any, error: errorMsg };
    }
  }

  leaveVoiceChannel(request: LeaveVoiceChannelRequest, ipAddress: string, userAgent: string): LeaveVoiceChannelResult {
    try {
      const channel = this.channels.get(request.channelId);
      if (!channel) {
        throw new Error(`Channel ${request.channelId} not found`);
  ...
  muteParticipant(request: MuteParticipantRequest, ipAddress: string, userAgent: string): MuteParticipantResult {
    try {
      const participant = this.participants.get(request.participantId);
      if (!participant) {
        throw new Error(`Participant ${request.participantId} not found`);
      ...
      const errorMsg = error instanceof Error ? error.message : String(error);
      return { success: false, participant: null as any, error: errorMsg };
    }
  }

  deafenParticipant(request: DeafenParticipantRequest, ipAddress: string, userAgent: string): DeafenParticipantResult {
    try {
      const participant = this.participants.get(request.participantId);
      if (!participant) {
        throw new Error(`Participant ${request.participantId} not found`);
  ...
  getVoiceChannel(request: GetVoiceChannelRequest): GetVoiceChannelResult {
    try {
  ...
  getParticipants(request: GetParticipantsRequest): GetParticipantsResult {
    try {
      const channel = this.channels.get(request.channelId);
      if (!channel) {
        throw new Error(`Channel ${request.channelId} not found`);
  ...
  updateAudioLevel(request: UpdateAudioLevelRequest): void {
    const participant = this.participants.get(request.participantId);
    if (participant) {
  ...
  updateConnectionStats(request: UpdateConnectionStatsRequest): void {
    const participant = this.participants.get(request.participantId);
    if (participant) {
      ...
      const stats: VoiceConnectionStats = {
  ...
  toggleLocalAudio(request: ToggleLocalAudioRequest, ipAddress: string, userAgent: string): ToggleLocalAudioResult {
    try {
      const participant = this.participants.get(request.participantId);
      if (!participant) {
        throw new Error(`Participant ${request.participantId} not found`);
  ...
  getAuditLog(userId: string, limit: number = 100): VoiceAuditEntry[] {
  ...
  getStatistics(): VoiceStatistics {
    const activeChannels = Array.from(this.channels.values()).filter(
      (c) => c.connectionState === 'connected' || c.connectionState === 'connecting',
    );
    ...
    this.connectionStats.forEach((stats) => {
      ...
      latencyCount += 1;
      if (stats.noiseSuppressionEnabled) noiseSuppressionCount += 1;
      if (stats.echoCancellationEnabled) echoCancellationCount += 1;
    });
  ...
  updateConfig(config: Partial<VoiceServiceConfig>, userId: string, ipAddress: string, userAgent: string): void {
  ...
  private recordAudit(entry: VoiceAuditEntry): void {
  ...
  private generateLiveKitToken(roomName: string, participantId: string): string {
