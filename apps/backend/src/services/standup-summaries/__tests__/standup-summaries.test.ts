...
describe('StandupSummariesService', () => {
  let service: StandupSummariesService;
  ...
  describe('initialization', () => {
    it('should initialize with default config', async () => {
      ...
      expect(mockPool.query).toHaveBeenCalledWith(
        expect.stringContaining('CREATE TABLE IF NOT EXISTS standup_summaries')
      );
    });
  ...
  describe('generateSummary', () => {
    it('should generate a summary from activity', async () => {
      const activity = {
        ...
        reviews: [],
        comments: [],
        issues: [{ number: 1, title: 'test issue', author: 'test', state: 'open' as const, timestamp: new Date() }],
      };
  ...
  describe('saveDraftSummary', () => {
    it('should save a draft summary', async () => {
      ...
      expect(mockPool.query).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO standup_summaries'),
        [date, summaryText]
      );
  ...
  describe('approveSummary', () => {
    it('should approve a draft summary', async () => {
      ...
      expect(mockPool.query).toHaveBeenCalledWith(
        expect.stringContaining('UPDATE standup_summaries'),
        [date, approvedBy]
      );
  ...
  describe('getSummary', () => {
    it('should get a summary by date', async () => {
      ...
      expect(mockPool.query).toHaveBeenCalledWith(
        'SELECT * FROM standup_summaries WHERE date = $1',
        [date]
      );
  ...
  describe('generateForDate', () => {
    it('should generate summary for a specific date', async () => {
      ...
      const collectSpy = vi.spyOn(service as any, 'collectDailyActivity').mockResolvedValue({
        date: '2024-01-01',
        commits: [],
        reviews: [],
        comments: [],
      ...
      const generateSpy = vi.spyOn(service as any, 'generateSummary').mockResolvedValue('Generated summary');
