...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help) echo "Usage: $0 [--host <ip>] [--user <user>] [--key <path>] [--local]"; exit 0 ;;
        *) shift ;;
    esac
done
...
deploy_remote() {
    ...
    log INFO "Preparing remote deployment to ${user}@${host}:${port}"
    ...
    # Copy to remote
    log INFO "Uploading to remote host..."
    scp -i "${key}" -P "${port}" -r . "${user}@${host}:/home/${user}/code-server-deploy/" || {
        log ERROR "Failed to upload deployment package"
        return 1
    }
    ...
    log SUCCESS "✓ Remote deployment successful"
    return 0
}
