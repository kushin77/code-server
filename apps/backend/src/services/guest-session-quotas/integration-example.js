 ...
 */
export function initializeGuestSessionQuotasRoutes(router, quotas) {
    /**
     * Create new guest session
     * POST /api/quotas/guest-sessions
     */
    router.post('/guest-sessions', (req, res) => {
        try {
            const { guestId, tier } = req.body;
            if (!guestId) {
                return res.status(400).json({ error: 'guestId required' });
            }
            const validTier = Object.values(QuotaTier).includes(tier)
                ? tier
                : QuotaTier.FREE;
     ...
     */
    router.get('/guest-sessions/:guestId', (req, res) => {
        try {
     ...
     */
    router.post('/guest-sessions/:guestId/can-start-concurrent', (req, res) => {
        try {
            const { guestId } = req.params;
     ...
     */
    router.post('/guest-sessions/:guestId/start-concurrent', (req, res) => {
        try {
            const { guestId } = req.params;
     ...
     */
    router.post('/guest-sessions/:guestId/end-concurrent', (req, res) => {
        try {
            const { guestId } = req.params;
     ...
     */
    router.get('/guest-sessions/:guestId/remaining-time', (req, res) => {
        try {
            const { guestId } = req.params;
     ...
     */
    router.post('/guest-sessions/:guestId/storage', (req, res) => {
        try {
            const { guestId } = req.params;
     ...
     */
    router.delete('/guest-sessions/:guestId', (req, res) => {
        try {
            const { guestId } = req.params;
     ...
     */
    router.get('/guest-sessions', (req, res) => {
        try {
 ...
 */
export function setupGuestSessionQuotasIntegration(app) {
 ...
 */
export async function createGuestSessionQuotasExampleApp() {
    const app = express();
    // Middleware
