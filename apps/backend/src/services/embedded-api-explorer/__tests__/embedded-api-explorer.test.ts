...
describe('EmbeddedAPIExplorerService', () => {
  let service: EmbeddedAPIExplorerService;
  let mockPool: any;
  let mockClient: any;
  ...
  it('should create a request', async () => {
    ...
    const requestId = await service.createRequest(
      'user-1',
      'GET',
  ...
  it('should share request', async () => {
    ...
    expect(mockClient.query).toHaveBeenCalledWith(
      expect.stringContaining('INSERT INTO api_shared_requests'),
      expect.any(Array)
    );
  });
  ...
  it('should delete request', async () => {
    ...
    expect(mockClient.query).toHaveBeenCalledWith(
      expect.stringContaining('DELETE FROM api_requests'),
      expect.any(Array)
    );
  });
