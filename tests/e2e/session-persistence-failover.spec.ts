...
test.describe('Session Persistence & Failover Scenarios - E2E Tests', () => {
  ...
  test.describe('Session Timeout Handling', () => {
    ...
    test('warning appears before session timeout', async ({ authenticatedPage }) => {
      ...
      const warningBtn = authenticatedPage.locator('[data-testid="session-warning"]').or(
        authenticatedPage.locator('text=Session Expiring')
      );
    ...
    test('session can be extended if warning is dismissed', async ({ authenticatedPage }) => {
      ...
      const extendBtn = authenticatedPage.locator('[data-testid="extend-session"]').or(
        authenticatedPage.locator('text=Continue Session')
      );
  ...
  test.describe('Multi-Device Sessions', () => {
    ...
    test('session list shows all active sessions', async ({ authenticatedPage }) => {
      ...
      const sessionsBtn = authenticatedPage.locator('[data-testid="active-sessions"]').or(
        authenticatedPage.locator('text=Active Sessions')
      );
      ...
      if (await sessionsBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
        ...
        const sessionList = authenticatedPage.locator('[data-testid="session-item"]').or(
          authenticatedPage.locator('.session-item')
        );
