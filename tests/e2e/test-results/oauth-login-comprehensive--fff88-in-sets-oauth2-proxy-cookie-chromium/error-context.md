...
- Name: oauth-login-comprehensive.spec.ts >> OAuth Login Comprehensive (#986) >> Happy Path - Login Flow >> 3: login sets _oauth2_proxy cookie
- Location: tests\e2e\specs\oauth-login-comprehensive.spec.ts:59:9
...
> 61  |       await page.waitForURL(/accounts\.google\.com/);
  ...
  155 |       // Login to portal
  156 |       await page.goto(PORTAL_URL);
  157 |       await page.waitForURL(/accounts\.google\.com/);
  158 |       await page.fill('input[type="email"]', QA_EMAIL);
  159 |       await page.click('#identifierNext, button:has-text("Next")');
  160 |       await page.waitForSelector('input[type="password"]');
  161 |       await page.fill('input[type="password"]', QA_PASSWORD);
```