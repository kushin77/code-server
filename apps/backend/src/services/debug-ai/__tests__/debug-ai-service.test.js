...
import { DebugAIService, getDebugAIService, } from '../debug-ai-service.js';
describe('Debug AI Service', () => {
    ...
    describe('Session Management', () => {
        it('should start debug session', async () => {
            await debugService.startSession('session-001', 'ws-test', 'typescript', 'node');
    ...
    describe('Output Tracking', () => {
        ...
        it('should add stderr output', async () => {
            await debugService.addOutput(sessionId, 'stderr', 'Error: undefined variable');
            const session = await debugService.getSession(sessionId);
            expect(session?.output.length).toBe(1);
    ...
    describe('AI Analysis', () => {
        ...
        it('should identify suspicious variables', async () => {
            ...
            expect(analysis?.variableAnalysis.suspiciousVariables.some((v) => v.name === 'result')).toBe(true);
        });
        it('should suggest fixes', async () => {
            ...
            expect(analysis?.suggestedFixes[0].difficulty).toMatch(/easy|medium|hard/);
        });
    ...
    describe('Feedback', () => {
        ...
        it('should submit feedback', async () => {
            return new Promise((resolve) => {
                ...
                debugService.submitFeedback(analysisId, sessionId, 'user-123', 5, 4, true, 'Very helpful');
            });
        });
        it('should mark need for further help if accuracy low', async () => {
            return new Promise((resolve) => {
                ...
                debugService.submitFeedback(analysisId, sessionId, 'user-123', 2, 2, false);
            });
        });
    });
    ...
    describe('Statistics', () => {
        beforeEach(async () => {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                await debugService.startSession(`session-${i}`, 'ws-stats', lang);
                const stack = [
                    {
                        id: 1,
                        name: 'test',
                ...
                const reason = ['breakpoint', 'exception', 'step'][i];
...
//# sourceMappingURL=debug-ai-service.test.js.map