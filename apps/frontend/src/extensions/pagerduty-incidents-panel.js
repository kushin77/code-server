 ...
 * - Incident creation
 */
export const PagerDutyIncidentsPanel = () => {
    const [incidents, setIncidents] = useState([]);
    const [stats, setStats] = useState({ triggered: 0, acknowledged: 0, resolved: 0 });
    const [loading, setLoading] = useState(true);
    const [filterStatus, setFilterStatus] = useState('triggered');
    ...
    const loadIncidents = async () => {
        try {
            setLoading(true);
            const response = await fetch(`http://localhost:3100/api/incidents?status=${filterStatus}&limit=20`);
            const data = await response.json();
            setIncidents(data.incidents || []);
            setStats(data.stats || { triggered: 0, acknowledged: 0, resolved: 0 });
        }
        catch (error) {
            vscode.window.showErrorMessage(`Failed to load incidents: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
        finally {
            setLoading(false);
        }
    ...
    const acknowledgeIncident = async (incidentId) => {
        try {
            const response = await fetch(`http://localhost:3100/api/incidents/${incidentId}/acknowledge`, { method: 'POST' });
            if (response.ok) {
                await loadIncidents();
            ...
            vscode.window.showErrorMessage(`Failed to acknowledge: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    };
    const resolveIncident = async (incidentId) => {
        try {
            const response = await fetch(`http://localhost:3100/api/incidents/${incidentId}/resolve`, { method: 'POST' });
            if (response.ok) {
                await loadIncidents();
            ...
            vscode.window.showErrorMessage(`Failed to resolve: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    };
    ...
    return (<div style={{ padding: '16px', fontFamily: 'var(--vscode-font-family)' }}>
      <div style={{ marginBottom: '16px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
          {['triggered', 'acknowledged', 'resolved'].map((status) => (<button key={status} onClick={() => setFilterStatus(status)} style={{
                padding: '6px 12px',
      ...
      {loading ? (<div style={{ color: 'var(--vscode-foreground)', opacity: 0.7 }}>Loading incidents...</div>) : incidents.length === 0 ? (<div style={{ color: 'var(--vscode-foreground)', opacity: 0.7 }}>No incidents</div>) : (<div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {incidents.map((incident) => (<div key={incident.id} style={{
                    padding: '12px',
                    background: 'var(--vscode-editor-background)',
              ...
              {filterStatus === 'triggered' && (<div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
                  <button onClick={(e) => {
                        e.stopPropagation();
              ...
              {filterStatus === 'acknowledged' && (<div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
                  <button onClick={(e) => {
                        e.stopPropagation();
...
//# sourceMappingURL=pagerduty-incidents-panel.js.map