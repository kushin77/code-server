...
describe('Workspace Map Routes', () => {
    ...
    describe('POST /:workspaceId/sessions/:userId/idle', () => {
        ...
        it('should mark user as idle', async () => {
            const res = await request(app).post(`/api/workspace-map/${workspaceId}/sessions/user-1/idle`);
            expect(res.status).toBe(200);
            expect(res.body.success).toBe(true);
    ...
    describe('POST /:workspaceId/sessions/:userId/unregister', () => {
        ...
        it('should unregister session', async () => {
            const res = await request(app).post(`/api/workspace-map/${workspaceId}/sessions/user-1/unregister`);
            expect(res.status).toBe(200);
            expect(res.body.success).toBe(true);
        });
    });
    ...
    describe('GET /:workspaceId/files/:filePath/users', () => {
        ...
        it('should get users on file', async () => {
            const res = await request(app).get(`/api/workspace-map/${workspaceId}/files/user-1/users?filePath=src/app.ts`);
            expect(res.status).toBe(200);
            expect(res.body.users).toHaveLength(1);
            expect(res.body.totalUsers).toBe(1);
        });
        it('should return 400 if filePath is missing', async () => {
            const res = await request(app).get(`/api/workspace-map/${workspaceId}/files/user-1/users`);
            expect(res.status).toBe(400);
        });
    });
...
//# sourceMappingURL=workspace-map.test.js.map