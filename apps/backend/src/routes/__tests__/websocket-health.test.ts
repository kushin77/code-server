...
describe('WebSocket Health Routes', () => {
  beforeEach(() => {
    ...
    app.use(express.json((err: any, req: any, res: any, next: any) => {
  ...
  describe('GET /user/:userId/connections', () => {
    ...
    it('should get all connections for a user', async () => {
      ...
      expect(res.body.data.map((c: any) => c.connectionId)).toContain('conn1');
      expect(res.body.data.map((c: any) => c.connectionId)).toContain('conn2');
    });
  ...
  describe('GET /type/:type', () => {
    ...
    it('should get connections by type', async () => {
      ...
      expect(res.body.success).toBe(true);
      expect(res.body.data).toHaveLength(2);
      expect(res.body.data.every((c: any) => c.type === 'collaboration')).toBe(true);
    });
  ...
  describe('GET /degraded', () => {
    ...
    it('should filter degraded connections by workspace', async () => {
      ...
      expect(res.body.data.every((c: any) => c.workspaceId === 'ws1')).toBe(true);
    });
  });
