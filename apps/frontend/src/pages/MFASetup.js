...
${state.backupCodes.map((code, i) => `${i + 1}. ${code}`).join('\n')}`;
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-sky-50 to-blue-50 py-12 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex justify-center items-center gap-2 mb-8">
          {['Setup', 'Scan', 'Verify', 'Done'].map((label, idx) => (<React.Fragment key={label}>
              <div className={`w-10 h-10 rounded-full flex items-center justify-center font-semibold transition-all ${state.step === ['initial', 'generating', 'scanned', 'verified'][idx]
        ...
        <div className="bg-white rounded-lg shadow-lg p-8">
          ...
          {state.step === 'initial' && (<>
              ...
              <div className="flex gap-4">
                <Button label="Start Setup" variant="primary" fullWidth onClick={handleStartSetup} loading={state.isLoading} disabled={state.isLoading}/>
                <Button label="Cancel" variant="secondary" fullWidth onClick={handleCancel} disabled={state.isLoading}/>
              </div>
            </>)}

          {/* STEP 2: QR Code & Secret */}
          {(state.step === 'generating' || state.step === 'scanned') && (<>
              {state.isLoading ? (<div className="flex flex-col items-center justify-center py-12">
                  <Spinner size="lg"/>
                  <p className="text-gray-600 mt-4">Generating QR code...</p>
                </div>) : state.qrCode && state.secret ? (<>
                  ...
                  <div className="space-y-6">
                    ...
                    <Card>
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button label={secretCopied ? '✓' : 'Copy'} variant="secondary" size="sm" onClick={copySecretToClipboard}/>
                      </div>
                    </Card>
                    ...
                    {/* Next Button */}
                    <Button label="I've scanned the code" variant="primary" fullWidth onClick={() => setState((prev) => ({ ...prev, step: 'verified' }))}/>
                  </div>
                </>) : null}
            </>)}
          ...
          {(state.step === 'verified' || state.step === 'scanned') && (<>
              ...
              <form onSubmit={handleVerifyTOTP} className="space-y-4">
                <Input label="6-Digit Code" type="text" placeholder="000000" maxLength={6} value={state.totpCode} onChange={(e) => {
                const value = e.target.value.replace(/\D/g, '').slice(0, 6);
                setState((prev) => ({ ...prev, totpCode: value, error: null }));
            }} disabled={state.isLoading} inputMode="numeric"/>

                <Button type="submit" label={state.isLoading ? 'Verifying...' : 'Verify & Enable MFA'} variant="primary" fullWidth loading={state.isLoading} disabled={state.isLoading || state.totpCode.length !== 6}/>
              </form>
            </>)}
          ...
          {state.step === 'verified' && state.success && (<>
              ...
              <Card>
                <div className="bg-gray-50 p-4 rounded-lg font-mono text-sm space-y-1 mb-4">
                  {state.backupCodes.map((code, idx) => (<div key={idx} className="text-gray-700">
                      {idx + 1}. {code}
                    </div>))}
                </div>

                <div className="flex gap-2">
                  <Button label={codesCopied ? '✓ Copied' : 'Copy All'} variant="secondary" flex onClick={copyBackupCodes}/>
                  <Button label="Download" variant="secondary" flex onClick={downloadBackupCodes}/>
                </div>
              </Card>
              ...
              <Button label="Done - Go to Dashboard" variant="primary" fullWidth className="mt-6" onClick={() => navigate('/')}/>
            </>)}
        </div>
...
//# sourceMappingURL=MFASetup.js.map