...
import { RevocationStatus, RevocationScope, RevocationReason, UnknownRevocationBehavior, } from "./types";
/**
 ...
 */
export class RevocationBroker {
    constructor(config) {
        this.revocations = new Map(); // revocationId → entry
        this.cache = new Map(); // targetId:scope → result
        this.cacheTimestamps = new Map(); // targetId:scope → cachedAt
        this.auditLog = [];
        this.stats = {
            totalRevocations: 0,
            activeRevocations: 0,
        ...
        this.propagationLatencies = []; // For p95/p99 calculation
    ...
    async checkRevocation(options) {
        ...
        const isUnknownDenied = status === RevocationStatus.UNKNOWN &&
            behavior !== undefined &&
     ...
     */
    async checkPrivilegedOperation(context) {
     ...
     */
    async restoreRevocation(revocationId, actor, correlationId) {
     ...
     */
    async startRevocationDrill(options) {
    ...
    // ============ Private helpers ============
    async propagateRevocation(revocationId, isHighPriority) {
        const entry = this.revocations.get(revocationId);
        if (!entry) {
    ...
    async simulatePropagation(revocationId, isHighPriority) {
        // Simulate network propagation delay
        const delay = isHighPriority ? 10 : 20; // ms
        return new Promise((resolve) => setTimeout(resolve, delay));
    }
    invalidateCacheForTarget(targetId, scope) {
 ...
 */
export function createRevocationBroker(config) {
