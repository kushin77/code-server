...
describe('GitOpsOrchestrator', () => {
    ...
    describe('Error Handling', () => {
        it('should handle Git connection errors gracefully', async () => {
            mockGitClient.getLatestCommit.mockRejectedValue(new Error('Connection timeout'));
            expect(async () => {
    ...
    describe('Performance', () => {
        ...
        it('should handle large manifests efficiently', async () => {
            const largeManifest = {
                apiVersion: 'v1',
                kind: 'ConfigMap',
                data: Object.fromEntries(Array.from({ length: 1000 }).map((_, i) => [`key-${i}`, `value-${i}`])),
            };
            const start = Date.now();
...
//# sourceMappingURL=GitOpsOrchestrator.test.js.map