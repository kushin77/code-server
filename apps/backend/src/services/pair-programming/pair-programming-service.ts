...
import {
  ...
  SwitchDriverRequest,
  SwitchDriverResult,
  EndPairSessionRequest,
  EndPairSessionResult,
  GetPairSessionRequest,
  GetPairSessionResult,
  ListPairSessionsRequest,
  ListPairSessionsResult,
  PairProgrammingAuditEntry,
  PairProgrammingServiceStatistics,
  SyncMessage,
  ConversationTurn,
} from './types.js';
...
export class PairProgrammingService extends EventEmitter {
  private static instance: PairProgrammingService | null = null;

  private sessions: Map<string, PairProgrammingSession> = new Map();
  private auditLogs: Map<string, PairProgrammingAuditEntry[]> = new Map();
  private subscribers: Map<string, Set<string>> = new Map();
  private config: PairProgrammingConfig = {
  ...
  joinPairSession(
    ...
    try {
      ...
      const participant: PairProgrammingParticipant = {
        userId: request.userId,
        userEmail: request.userEmail,
        userName: request.userName,
        role: request.role,
  ...
  getAISuggestion(
    ...
    try {
      ...
      const suggestion: AICodeSuggestion = {
        id: suggestionId,
        sessionId: request.sessionId,
      ...
      this.recordAudit(
        ...
        request.sessionId,
        'success',
        { suggestionId, type: request.suggestionType }
      );
  ...
  switchDriver(
    ...
    try {
      ...
      const newDriver = session.participants.find(
        (p) => p.userId === request.newDriverId
      );
      if (newDriver) {
  ...
  endPairSession(
    ...
    try {
      ...
      const statistics: PairProgrammingServiceStatistics = {
        ...
        acceptedSuggestions: Array.from(session.suggestions.values()).filter(
          (s) => s.status === 'accepted'
        ).length,
  ...
  listPairSessions(request: ListPairSessionsRequest): ListPairSessionsResult {
    try {
  ...
  getStatistics(): PairProgrammingServiceStatistics {
    ...
    const totalSuggestions = sessions.reduce(
      (sum, s) => sum + s.suggestions.size,
      0
    );
    const acceptedSuggestions = sessions.reduce(
      (sum, s) =>
        sum +
  ...
  private recordAudit(
    ...
    details: Map<string, unknown> | Record<string, unknown>
  ): void {
    try {
      const detailsMap = details instanceof Map ? details : new Map(Object.entries(details));
      const entry: PairProgrammingAuditEntry = {
        timestamp: Date.now(),
        userId,
        userEmail,
        ipAddress,
