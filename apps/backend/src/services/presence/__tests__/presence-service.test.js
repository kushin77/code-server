...
describe('PresenceService', () => {
    ...
    it('should update user presence', async () => {
        const presence = await service.updatePresence('user1', 'user1@example.com', { status: 'online' });
    ...
    it('should emit status-changed event when status changes', () => {
        return new Promise((resolve) => {
            service
                ...
                .then(() => {
                service.once('status-changed', (event) => {
    ...
    it('should get workspace presence snapshot', async () => {
        await service.updatePresence('user1', 'user1@example.com', {
            status: 'online',
            currentActivity: { context: 'file', description: 'Editing' },
        }, undefined, undefined, 'workspace1');
        await service.updatePresence('user2', 'user2@example.com', {
            status: 'idle',
    ...
    it('should set custom status', async () => {
        const presence = await service.setCustomStatus('user1', 'user1@example.com', 'In a meeting', '📞');
    ...
    it('should set custom status with expiration', async () => {
        ...
        const presence = await service.setCustomStatus('user1', 'user1@example.com', 'Busy', '🔴', expiresAt);
        expect(presence.customStatus?.expiresAt).toBe(expiresAt);
    });
    // ==================== Activity Tests ====================
    it('should update activity', async () => {
        const presence = await service.updateActivity('user1', 'user1@example.com', 'file', 'Editing src/main.ts', { line: 42 });
        expect(presence.currentActivity?.context).toBe('file');
    ...
    it('should log audit entry', async () => {
        await service.updatePresence('user1', 'user1@example.com', { status: 'online' }, '192.168.1.1', 'Mozilla/5.0');
    ...
    it('should track IP and user agent in audit', async () => {
        await service.updatePresence('user1', 'user1@example.com', { status: 'online' }, '10.0.0.1', 'Chrome/90');
        const log = await service.getAuditLog('user1');
...
//# sourceMappingURL=presence-service.test.js.map