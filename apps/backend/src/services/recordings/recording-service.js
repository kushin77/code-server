...
export class RecordingService extends EventEmitter {
    constructor(config) {
        ...
        this.events = new Map(); // Per-recording events
     ...
     */
    async startRecording(userId, userEmail, workspaceId, sessionId, ipAddress, userAgent) {
        if (!this.isInitialized)
     ...
     */
    async stopRecording(userId, userEmail, recordingId, ipAddress, userAgent) {
     ...
     * Record event
     */
    async recordEvent(recordingId, eventType, userId, workspaceId, sessionId, data, metadata) {
        const recording = this.recordings.get(recordingId);
        if (!recording)
            throw new Error('Recording not found');
        const now = Date.now();
     ...
     */
    async getRecordingEvents(recordingId) {
     ...
     * Pause playback
     */
    async pausePlayback(recordingId) {
     ...
     * Resume playback
     */
    async resumePlayback(recordingId) {
        const state = this.playbackStates.get(recordingId);
        if (state) {
     ...
     * Set playback speed
     */
    async setPlaybackSpeed(recordingId, speed) {
        const state = this.playbackStates.get(recordingId);
        if (state) {
     ...
     * Seek to event
     */
    async seekToEvent(recordingId, eventIndex) {
        const state = this.playbackStates.get(recordingId);
        if (!state)
     ...
     * Export to video
     */
    async exportToVideo(request, ipAddress, userAgent) {
     ...
     */
    async createShareableLink(userId, userEmail, recordingId, ipAddress, userAgent) {
        const recording = this.recordings.get(recordingId);
        if (!recording)
            throw new Error('Recording not found');
     ...
     * Delete recording
     */
    async deleteRecording(userId, userEmail, recordingId, ipAddress, userAgent) {
